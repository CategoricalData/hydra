...
or Maven) must NOT also source-dir the matching package's LOCAL HEAD authoring
tree under packages/<pkg>/src/main/haskell. The two are different kinds of the
same thing — published generated code vs. HEAD DSL source — and mixing them in
one compile breaks the moment HEAD adds a kernel type the published package
lacks.

This is a first-line, toolchain-free (python3 + git only) CI gate.

Background (#608): the retired Haskell cold-seeder (heads/haskell/json-driver,
deleted by #703) linked the published hydra-kernel-0.17.1 while ALSO
source-dirring packages/hydra-kernel/src/main/haskell (for Hydra.Sources.All,
its JSON decode-universe context). That authoring tree contains Term-printers
phantom-annotated with generated kernel types (e.g. MissingCaseBranchesError,
added on main by #598) that do not exist in the published 0.17.1 kernel — so
the driver failed to compile ("Not in scope") on every new-kernel-type PR,
reddening main.
...
What is and is NOT a violation:
  ...
  - OK: source-dirring a BUILD-TIME COPY that lives under the driver's own
    directory. Such copies are not under packages/, so they are not flagged.
...
Exit code is nonzero iff a violation is found.
"""
