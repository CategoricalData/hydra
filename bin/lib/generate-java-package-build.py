...
def render_root_build_gradle(packages: list[str]) -> str:
    ...
    # HYDRA_JAVA_PUBLISH_HOLD=1 uploads as USER_MANAGED (validate + HOLD for review)
    # instead of AUTOMATIC (validate + auto-publish). Mirrors HYDRA_SCALA_PUBLISH_HOLD.
    # A held deployment can be dropped from the Portal; an AUTOMATIC one cannot.
    publishing_type = "USER_MANAGED" if os.environ.get("HYDRA_JAVA_PUBLISH_HOLD") == "1" else "AUTOMATIC"
    return f"""// Generated file. Do not edit.
// Root aggregator build for Sonatype Central Portal publishing (#591).
// Regenerated by bin/lib/generate-java-package-build.py --root-aggregator.
//
...
nmcpAggregation {{
    centralPortal {{
        ...
        // USER_MANAGED instead when HYDRA_JAVA_PUBLISH_HOLD=1: validate and
        // HOLD at VALIDATED for a review checkpoint before releasing from the
        // Portal UI. Mirrors HYDRA_SCALA_PUBLISH_HOLD in publish-sbt.sh — an
        // upload is reversible while held, but AUTOMATIC is not.
