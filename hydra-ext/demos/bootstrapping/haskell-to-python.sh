...
#
# Usage: ./haskell-to-python.sh [--types-only] [--kernel-only]
...
echo "Step 3: Mapping JSON to Python (via Haskell host)..."
STEP_START=$(date +%s)
"$SCRIPT_DIR/haskell-bootstrap.sh" --target python --output "$OUTPUT_BASE" --include-tests $EXTRA_FLAGS 2>&1
STEP_END=$(date +%s)
