...
#
# Usage: ./python-to-java.sh [--types-only] [--kernel-only]
...
echo "=========================================="
echo "Bootstrap: Python -> JSON -> Java"
echo "=========================================="
echo ""
...
echo "Step 3: Generating Java code from JSON (via Python host)..."
STEP_START=$(date +%s)
"$SCRIPT_DIR/python-bootstrap.sh" --target java --output "$OUTPUT_BASE" $EXTRA_FLAGS 2>&1
STEP_END=$(date +%s)
