#!/bin/bash
# Bootstrap Hydra to Java from JSON modules (via Python host).
#
# Usage: ./python-to-java.sh [--output DIR] [--types-only] [--kernel-only]
...
OUTPUT_DIR="$OUTPUT_BASE/python-to-java"

echo "=========================================="
echo "Python to Java bootstrapping demo"
echo "=========================================="
echo ""
...
echo "Step 3: Generating Java code (via Python host)..."
STEP_START=$(date +%s)
"$SCRIPT_DIR/invoke-python-host.sh" --target java --output "$OUTPUT_BASE" --include-tests $EXTRA_FLAGS 2>&1
STEP_END=$(date +%s)
echo "  Time: $((STEP_END - STEP_START))s"
echo ""

# Step 4: Build and test
"$SCRIPT_DIR/test-java-target.sh" "$OUTPUT_DIR"
TEST_EXIT=$?
...
if [ $TEST_EXIT -eq 0 ]; then
    echo "Bootstrap Python-to-Java: COMPLETE (tests PASSED)"
else
    echo "Bootstrap Python-to-Java: COMPLETE (tests FAILED)"
fi
echo "  Output:     $OUTPUT_DIR"
