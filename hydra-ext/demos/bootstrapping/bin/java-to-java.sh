...
#
# Usage: ./java-to-java.sh [--output DIR] [--types-only] [--kernel-only]
...
echo "Step 3: Generating Java code (via Java host)..."
STEP_START=$(date +%s)
"$SCRIPT_DIR/invoke-java-host.sh" --target java --output "$OUTPUT_BASE" --include-tests $EXTRA_FLAGS 2>&1
STEP_END=$(date +%s)
