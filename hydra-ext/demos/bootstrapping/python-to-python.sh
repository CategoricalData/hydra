...
#
# Usage: ./python-to-python.sh [--types-only] [--kernel-only]
...
echo "Step 3: Mapping JSON to Python (via Python host)..."
STEP_START=$(date +%s)
"$SCRIPT_DIR/python-bootstrap.sh" --target python --output "$OUTPUT_BASE" --include-tests $EXTRA_FLAGS 2>&1
STEP_END=$(date +%s)
