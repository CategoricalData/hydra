...
-- | Generate Java kernel source modules.
--
-- This executable creates "source-level" Java modules that contain
-- term-encoded Module objects. Unlike the regular kernel modules which
-- contain application-level code (Java methods), these modules
-- contain the Module AST representation itself.
--
-- For example, hydra.formatting becomes:
--   - Regular: hydra/formatting/Formatting.java with Java methods
--   - Sources: hydra/sources/formatting/Formatting.java with a Module object as data
--
-- This is needed for Java tests that need to access kernel module
...
main = do
  ...
  putStrLn "=== Generate Java kernel sources ==="
  putStrLn ""
  ...
  putStrLn $ "Generating " ++ show (length sourcesModules) ++ " source modules to ../hydra-java/src/gen-main/java..."
  putStrLn ""

  -- Universe includes both kernel modules (for type resolution) and sources modules
  -- We only generate the sources modules
  writeJava "../hydra-java/src/gen-main/java" (kernelModules ++ sourcesModules) sourcesModules

  putStrLn ""
  putStrLn "=== Done! ==="
  putStrLn ""
  putStrLn "Generated modules:"
  mapM_ (\m -> putStrLn $ "  hydra/sources/" ++ dropHydraPrefix (unNamespace (moduleNamespace m)) ++ ".java")
        kernelModules
  where
    dropHydraPrefix ns = case stripPrefix "hydra." ns of
      Just rest -> rest
