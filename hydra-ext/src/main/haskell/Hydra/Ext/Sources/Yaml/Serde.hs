...
module Hydra.Ext.Sources.Yaml.Serde where
...
needsQuoting = define "needsQuoting" $
  ...
  Logic.ifElse (Lists.elem (var "s") (var "hydra.ext.org.yaml.serde.yamlReservedWords" :: TTerm [String])) true $
  ...
  "specials" <~ Strings.toList (var "hydra.ext.org.yaml.serde.yamlSpecialChars" :: TTerm String) $
