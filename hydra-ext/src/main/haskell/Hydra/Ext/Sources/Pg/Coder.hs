...
module_ = Module ns elements
  ...
  where
    elements = [
      toBinding check,
      toBinding checkRecordName,
      toBinding constructEdgeCoder,
      toBinding constructVertexCoder,
      toBinding edgeCoder,
      toBinding edgeIdAdapter,
      toBinding elementCoder,
      toBinding elementTreeEdge,
      toBinding elementTreeVertex,
      toBinding elementTypeTreeEdge,
      toBinding elementTypeTreeVertex,
      toBinding encodeProperties,
      toBinding encodeProperty,
      toBinding extractString,
      toBinding findAdjacenEdgeAdapters,
      toBinding findIdProjectionSpec,
      toBinding findIncidentVertexAdapter,
      toBinding findLabelString,
      toBinding findProjectionSpec,
      toBinding findPropertySpecs,
      toBinding findSingleFieldWithAnnotationKey,
      toBinding hasVertexAdapters,
      toBinding projectionAdapter,
      toBinding propertyAdapter,
      toBinding propertyTypes,
      toBinding selectEdgeId,
      toBinding selectVertexId,
      toBinding traverseToSingleTerm,
      toBinding vertexCoder,
...
constructEdgeCoder :: TBinding (Context -> Graph -> PG.VertexLabel -> PGM.Schema Graph t v -> Type -> t -> t -> PG.Direction -> Name -> [FieldType]
...
edgeCoder :: TBinding (Graph -> PG.Direction -> PGM.Schema Graph t v -> Type -> t -> Name
...
edgeCoder = define "edgeCoder" $
  ...
  "mIdAdapter" ~> "outAdapter" ~> "inAdapter" ~> "propAdapters" ~> "vertexAdapters" ~> lets [
    ...
    Compute.adapter true (var "source")
...
elementTreeVertex :: TBinding (PG.Vertex v -> [PG.ElementTree v] -> PG.ElementTree v)
...
elementTypeTreeEdge :: TBinding (PG.EdgeType t -> [PG.ElementTypeTree t] -> PG.ElementTypeTree t)
...
elementTypeTreeVertex :: TBinding (PG.VertexType t -> [PG.ElementTypeTree t] -> PG.ElementTypeTree t)
...
hasVertexAdapters :: TBinding (PG.Direction -> Y.Maybe a -> Y.Maybe b -> Bool)
  ...
  "cx" ~> "g" ~> "idtype" ~> "coder" ~> "spec" ~> "key" ~> lets [
    ...
    Eithers.bind (TermsToElements.parseValueSpec @@ var "cx" @@ var "g" @@ var "values")
      ("traversal" ~> right (pair
        ...
        (Compute.adapter true (Core.fieldTypeType $ var "field") (var "idtype")
          (Compute.coder
            ("cx'" ~> "typ" ~>
...
propertyAdapter = define "propertyAdapter" $
  ...
  "cx" ~> "g" ~> "schema" ~> "spec" ~> lets [
    ...
    Eithers.bind (Compute.coderEncode (project PGM._Schema PGM._Schema_propertyTypes @@ var "schema") @@ var "cx" @@ (Core.fieldTypeType $ var "tfield"))
      ("pt" ~> Eithers.bind (TermsToElements.parseValueSpec @@ var "cx" @@ var "g" @@ var "values")
        ("traversal" ~> right
          (Compute.adapter true (var "tfield")
            ...
            (Compute.coder
              ("cx'" ~> "dfield" ~>
                Eithers.bind (traverseToSingleTerm @@ var "cx'" @@ string "property traversal" @@ (var "traversal" @@ var "cx'") @@ (Core.fieldTerm $ var "dfield"))
                  ("result" ~> Eithers.bind (Compute.coderEncode (project PGM._Schema PGM._Schema_propertyValues @@ var "schema") @@ var "cx'" @@ var "result")
  ...
  "propAdapters" ~>
    Lists.map
      ("a" ~> record PG._PropertyType [
        PG._PropertyType_key>>: project PG._PropertyType PG._PropertyType_key @@ (Compute.adapterTarget $ var "a"),
        PG._PropertyType_value>>: project PG._PropertyType PG._PropertyType_value @@ (Compute.adapterTarget $ var "a"),
        PG._PropertyType_required>>: true])
      (var "propAdapters")
...
selectEdgeId = define "selectEdgeId" $
  ...
  "cx" ~> "fields" ~> "ad" ~> lets [
    ...
    Maybes.maybe
      ...
      ("t" ~> Compute.coderEncode (Compute.adapterCoder $ var "adapter") @@ var "cx" @@ var "t")
      (Maps.lookup (var "fname") (var "fields"))
...
vertexCoder :: TBinding (Graph -> PGM.Schema Graph t v -> Type -> t -> Name -> PG.VertexLabel
...
vertexCoder = define "vertexCoder" $
  ...
  "idAdapter" ~> "propAdapters" ~> "edgeAdapters" ~> lets [
    ...
    "depTypes">: Lists.map
      ("ea" ~> Compute.adapterTarget $ (Pairs.second $ Pairs.second $ Pairs.second $ var "ea"))
      (var "edgeAdapters"),
