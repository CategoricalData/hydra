...
encodeNamespace nsVal = (Strings.intercalate "::" (Lists.map (Formatting.convertCase Util.CaseConventionCamel Util.CaseConventionLowerSnake) (Strings.splitOn "." (Module.unNamespace nsVal))))
...
fwdHeaderName nsVal = (Names.unqualifyName (Module.QualifiedName {
...
namespaceDecl nsVal decls = (Syntax.DeclarationNamespace (Syntax.NamespaceDeclaration {
...
typeVariableReference env name = (Syntax.TypeExpressionBasic (Syntax.BasicTypeNamed (encodeName True Util.CaseConventionPascal env name)))
