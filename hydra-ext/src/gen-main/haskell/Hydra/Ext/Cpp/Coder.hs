...
createConstructorBody params = (Logic.ifElse (Lists.null params) Syntax.FunctionBodyDefault (Syntax.FunctionBodyCompound (Syntax.CompoundStatement [])))
...
encodeNamespace ns = (Strings.intercalate "::" (Lists.map (\seg -> Formatting.convertCaseCamelToLowerSnake seg) (Strings.splitOn "." (Module.unNamespace ns))))
...
fwdHeaderName ns = (Names.unqualifyName (Module.QualifiedName {
...
namespaceDecl ns decls = (Syntax.DeclarationNamespace (Syntax.NamespaceDeclaration {
...
createTypeReference isPointer name = (Logic.ifElse isPointer (createTemplateType "std::shared_ptr" [
...
encodeLiteralType lt = (Syntax.TypeExpressionBasic ((\x -> case x of
...
createPartialVisitorInterface tname variants = (Syntax.DeclarationTemplate (Syntax.TemplateDeclaration {
  Syntax.templateDeclarationInline = False,
  Syntax.templateDeclarationParameters = [
    "typename R"],
...
createUnionBaseClass name variants = (cppClassDeclaration (className name) [] (Just (Syntax.ClassBody [
...
createVariantClass cx g tname parentClass ft =  
  let fname = (Core.fieldTypeName ft)
  in  
    let variantType = (Core.fieldTypeType ft)
    in  
      let hasValue = (Logic.not (Schemas.isUnitType variantType))
      in  
        let valueField = (Logic.ifElse hasValue (Eithers.map (\cppType -> [
        ...
        in  
          let constructorParams = (Logic.ifElse hasValue (Eithers.map (\paramType -> [
                  Syntax.Parameter {
          ...
          in (Eithers.bind valueField (\vFields -> Eithers.bind constructorParams (\vParams ->  
            let initList = (Logic.ifElse hasValue [
...
createAcceptImplementation tname variants = (Syntax.DeclarationTemplate (Syntax.TemplateDeclaration {
  Syntax.templateDeclarationInline = False,
  Syntax.templateDeclarationParameters = [
    "typename R"],
  Syntax.templateDeclarationDeclaration = (Syntax.DeclarationFunction (Syntax.FunctionDeclaration {
...
generateForwardDeclarations tname fields = (Lists.map (\ft -> cppClassDeclaration (variantName tname (Core.fieldTypeName ft)) [] Nothing) fields)

createLessThanOperator :: (Core.Name -> t0 -> Syntax.Declaration)
createLessThanOperator typeName fields = (Syntax.DeclarationFunction (Syntax.FunctionDeclaration {
  Syntax.functionDeclarationPrefixSpecifiers = [
...
generateTypeFile ns def_ cx g =  
  ...
  in  
    let typ = (Module.typeDefinitionType def_)
