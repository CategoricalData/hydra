...
labelAttr lab = Dot.EqualityPair {
...
labelAttrs style lab =  
  let styleAttrs = (Logic.ifElse (Equality.equal style nodeStyleSimple) [] (Logic.ifElse (Equality.equal style nodeStyleElement) [
          Dot.EqualityPair {
...
toEdgeStmt i1 i2 attrs = (Dot.StmtEdge (Dot.EdgeStmt {
...
termLabel compact namespaces term =  
  ...
  in ((\x -> case x of
    ...
    Core.TermLiteral v0 -> (simpleLabel ((\x -> case x of
...
termToDotStmts namespaces term =  
  let encode = (\mlabstyle -> \isElement -> \ids -> \mparent -> \stmtsVisited -> \accessorTerm ->  
          let accessor = (Pairs.first accessorTerm) 
              ...
              toAccessorEdgeStmt = (\acc -> \sty -> \i1 -> \i2 -> toEdgeStmt i1 i2 (Maybes.map (\s -> labelAttrs sty s) (Accessors_.termAccessor acc)))
              edgeAttrs = (\lab -> Dot.AttrList [
                      [
                        Dot.EqualityPair {
                          Dot.equalityPairLeft = (Dot.Id "label"),
                          Dot.equalityPairRight = (Dot.Id lab)}]])
              ...
              selfStmts = (Lists.concat [
                      stmts,
                      [
                        nodeStmt],
                      parentStmt])
              dflt = (Lists.foldl (encode Nothing False ids (Just selfId)) (selfStmts, selfVisited) (Rewriting.subtermsWithAccessors currentTerm))
          in ((\x -> case x of
            Core.TermFunction v0 -> ((\x -> case x of
              Core.FunctionLambda v1 ->  
                let v = (Core.lambdaParameter v1) 
                    ...
                    varEdgeStmt = (Dot.StmtEdge (Dot.EdgeStmt {
