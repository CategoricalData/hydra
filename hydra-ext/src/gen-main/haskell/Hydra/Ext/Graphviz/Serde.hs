...
writeAttrList al = (Serialization.spaceSep (Lists.map (\alist -> Serialization.brackets Serialization.squareBrackets Serialization.inlineStyle (Serialization.commaSep Serialization.inlineStyle (Lists.map writeEqualityPair alist))) (Dot.unAttrList al)))
...
writeCompassPt p = ((\x -> case x of
  ...
  Dot.CompassPtNone -> (Serialization.cst "none")) p)
...
writeGraph g =  
  let strict = (Dot.graphStrict g) 
      ...
      graphExpr = (Logic.ifElse strict (Serialization.spaceSep [
              Serialization.cst "strict",
              (Serialization.cst graphKeyword)]) (Serialization.cst graphKeyword))
      body = (Serialization.brackets Serialization.curlyBraces Serialization.fullBlockStyle (Serialization.symbolSep ";" Serialization.fullBlockStyle (Lists.map (writeStmt directed) stmts)))
  in (Serialization.spaceSep [
    graphExpr,
    body])
...
writeId i = (Serialization.cst (Strings.cat [
  "\"",
  (Dot.unId i),
  "\""]))
...
writeNodeOrSubgraph directed ns = ((\x -> case x of
  Dot.NodeOrSubgraphNode v0 -> (writeNodeId v0)
  Dot.NodeOrSubgraphSubgraph v0 -> (writeSubgraph directed v0)) ns)
...
writeStmt directed s = ((\x -> case x of
  ...
  Dot.StmtSubgraph v0 -> (writeSubgraph directed v0)) s)
...
writeSubgraph directed sg =  
  let mid = (Dot.subgraphSubgraphId sg) 
      ...
      body = (Serialization.brackets Serialization.curlyBraces Serialization.inlineStyle (Serialization.spaceSep (Lists.map (writeStmt directed) stmts)))
  in (Serialization.spaceSep (Maybes.cat [
...
writeSubgraphId sid = (Serialization.spaceSep (Maybes.cat [
