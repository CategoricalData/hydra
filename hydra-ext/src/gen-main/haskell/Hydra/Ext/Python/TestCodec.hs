...
emptyPythonModuleMetadata ns_ = Helpers.PythonModuleMetadata {
...
pythonTestCodec namespaces_ = Testing.TestCodec {
...
pythonTestCodecWithContext namespaces_ tcontext = Testing.TestCodec {
  Testing.testCodecLanguage = (Coders.LanguageName "python"),
  Testing.testCodecFileExtension = (Module.FileExtension "py"),
...
formatPythonTestName name = (Strings.cat2 "test_" (Strings.fromList (Lists.map (\c -> Logic.ifElse (Chars.isAlphaNum c) (Chars.toLower c) 95) (Strings.toList name))))
...
pythonTestCaseTemplate = (Strings.intercalate "\n" [
  "def {name}():",
  "    assert ({input}) == ({output})"])
...
pythonModuleTemplate = (Strings.intercalate "\n" [
...
namespacesForPythonModule mod graph_ =  
  let bindings = (Lexical.graphToBindings graph_) 
      defs = (Maybes.mapMaybe (\b -> Maybes.map (\ts -> Module.DefinitionTerm (Module.TermDefinition {
...
generateTestFileWithPythonCodec codec testModule testGroup namespaces_ g = (Eithers.map (\testBody ->  
