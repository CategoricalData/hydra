...
emptyPythonModuleMetadata :: Module.Namespace -> Environment.PythonModuleMetadata
emptyPythonModuleMetadata ns_ =
    Environment.PythonModuleMetadata {
      Environment.pythonModuleMetadataNamespaces = Module.Namespaces {
...
termToPythonWithContext namespaces_ graph0 skipCasts term _g =
    Eithers.bimap (\ic -> Errors.error (Context.inContextObject ic)) (\arg_ -> Serialization.printExpr (Serde.encodeExpression arg_)) (Coder.encodeTermInline Lexical.emptyContext (Environment.PythonEnvironment {
      Environment.pythonEnvironmentNamespaces = namespaces_,
      Environment.pythonEnvironmentBoundTypeVariables = ([], Maps.empty),
      Environment.pythonEnvironmentGraph = graph0,
      Environment.pythonEnvironmentNullaryBindings = Sets.empty,
      Environment.pythonEnvironmentVersion = Utils.targetPythonVersion,
      Environment.pythonEnvironmentSkipCasts = skipCasts,
      Environment.pythonEnvironmentInlineVariables = Sets.empty}) skipCasts term)
...
typeToPython namespaces_ typ g =
    Eithers.bimap (\ic -> Errors.error (Context.inContextObject ic)) (\arg_ -> Serialization.printExpr (Serde.encodeExpression arg_)) (Coder.encodeType (Environment.PythonEnvironment {
      Environment.pythonEnvironmentNamespaces = namespaces_,
      Environment.pythonEnvironmentBoundTypeVariables = ([], Maps.empty),
      Environment.pythonEnvironmentGraph = g,
      Environment.pythonEnvironmentNullaryBindings = Sets.empty,
      Environment.pythonEnvironmentVersion = Utils.targetPythonVersion,
      Environment.pythonEnvironmentSkipCasts = False,
      Environment.pythonEnvironmentInlineVariables = Sets.empty}) typ)
