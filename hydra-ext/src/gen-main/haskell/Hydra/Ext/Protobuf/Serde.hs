...
excludeInternalOptions opts = (Lists.filter (\opt -> Logic.not (Equality.equal (Strings.charAt 0 (Proto3.optionName opt)) 95)) opts)
...
protoBlock exprs = (Serialization.brackets Serialization.curlyBraces Serialization.fullBlockStyle (Serialization.doubleNewlineSep exprs))

-- | Append a semicolon to an expression
semi :: (Ast.Expr -> Ast.Expr)
semi e = (Serialization.noSep [
  e,
  (Serialization.cst ";")])

-- | Convert a definition to an expression
writeDefinition :: (Proto3.Definition -> Ast.Expr)
writeDefinition def = ((\x -> case x of
  Proto3.DefinitionEnum v0 -> (writeEnumDefinition v0)
  Proto3.DefinitionMessage v0 -> (writeMessageDefinition v0)) def)
...
writeField f =  
  ...
  in (optDesc False options ((\x -> case x of
...
writeFieldType ftyp = ((\x -> case x of
  Proto3.FieldTypeMap v0 ->  
    let kt = (Proto3.mapTypeKeys v0) 
        vt = (Proto3.mapTypeValues v0)
  ...
  Proto3.FieldTypeOneof _ -> (Serialization.cst "oneof")) ftyp)
...
writeImport ref = (semi (Serialization.spaceSep [
...
writeProtoFile pf =  
  let pkg = (Proto3.protoFilePackage pf) 
      ...
      headerSec = (Maybes.pure (Serialization.newlineSep [
...
writeScalarType sct = (Serialization.cst ((\x -> case x of
