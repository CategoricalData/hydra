...
trueExpr d = ((\x -> case x of
  ...
  Syntax.DialectScheme -> (Serialization.cst "#t")) d)

falseExpr :: (Syntax.Dialect -> Ast.Expr)
falseExpr d = ((\x -> case x of
  ...
  Syntax.DialectScheme -> (Serialization.cst "#f")) d)

nilExpr :: (Syntax.Dialect -> Ast.Expr)
nilExpr d = ((\x -> case x of
  Syntax.DialectClojure -> (Serialization.cst "nil")
  Syntax.DialectEmacsLisp -> (Serialization.cst "nil")
  Syntax.DialectCommonLisp -> (Serialization.cst "cl:nil")
  Syntax.DialectScheme -> (Serialization.cst "'()")) d)
  ...
  in ((\x -> case x of
    Syntax.DialectClojure -> (Maybes.maybe (Serialization.doubleNewlineSep formPart) (\m ->  
      let nameStr = (Syntax.unNamespaceName (Syntax.moduleDeclarationName m)) 
          requireClauses = (Lists.map (\imp -> Serialization.brackets Serialization.squareBrackets Serialization.inlineStyle (Serialization.spaceSep [
                  Serialization.cst imp,
                  (Serialization.cst ":refer"),
                  (Serialization.cst ":all")])) importNames)
          nsForm = (Logic.ifElse (Lists.null requireClauses) (Serialization.parens (Serialization.spaceSep [
                  ...
                  (Serialization.cst ")")])))
      in  
        let varNames = (Lists.concat (Lists.map (\fwc ->  
                ...
                in ((\x -> case x of
                  ...
                  _ -> []) form)) forms))
        in  
          let declareForm = (Logic.ifElse (Lists.null varNames) [] [
    ...
    Syntax.DialectEmacsLisp -> (Maybes.maybe (Serialization.doubleNewlineSep formPart) (\m ->  
      let nameStr = (Syntax.unNamespaceName (Syntax.moduleDeclarationName m)) 
          requireClLib = (Serialization.parens (Serialization.spaceSep [
                  Serialization.cst "require",
                  (Serialization.noSep [
                    Serialization.cst "'",
                    (Serialization.cst "cl-lib")])]))
          requireImports = (Lists.map (\imp -> Serialization.parens (Serialization.spaceSep [
                  Serialization.cst "require",
                  (Serialization.noSep [
                    Serialization.cst "'",
                    (Serialization.cst imp)])])) importNames)
          provideForm = (Serialization.parens (Serialization.spaceSep [
    ...
    Syntax.DialectCommonLisp -> (Maybes.maybe (Serialization.doubleNewlineSep formPart) (\m ->  
      let nameStr = (Syntax.unNamespaceName (Syntax.moduleDeclarationName m)) 
          ...
          exportClause = (Logic.ifElse (Lists.null exportSyms) [] [
                  Serialization.parens (Serialization.spaceSep (Lists.concat2 [
                    Serialization.cst ":export"] (Lists.map (\s -> Serialization.noSep [
                    Serialization.cst ":",
                    s]) exportSyms)))])
          defpkgForm = (Serialization.parens (Serialization.newlineSep (Lists.concat [
                  [
                    Serialization.spaceSep [
                  ...
                  [
                    useClause],
                  exportClause])))
          inpkgForm = (Serialization.parens (Serialization.spaceSep [
    ...
    Syntax.DialectScheme -> (Maybes.maybe (Serialization.doubleNewlineSep formPart) (\m ->  
      let nameStr = (Syntax.unNamespaceName (Syntax.moduleDeclarationName m)) 
          ...
          domainImportExprs = (Lists.map (\idecl ->  
          ...
          schemeBaseExpr = (Serialization.parens (Serialization.spaceSep [
...
topLevelFormToExpr d form = ((\x -> case x of
  ...
  Syntax.TopLevelFormExpression v0 -> (expressionToExpr d v0)) form)
  ...
  in ((\x -> case x of
    Syntax.DialectClojure ->  
      let nameStr = (Syntax.unSymbol (Syntax.recordTypeDefinitionName rdef)) 
          ...
          defrecordForm = (Serialization.parens (Serialization.spaceSep [
          ...
          makeAlias = (Serialization.parens (Serialization.spaceSep (Lists.concat [
                  [
                    Serialization.cst "defn",
    ...
    Syntax.DialectScheme ->  
      let nameStr = (Syntax.unSymbol (Syntax.recordTypeDefinitionName rdef)) 
          ...
          accessors = (Lists.map (\fn -> Serialization.parens (Serialization.spaceSep [
...
expressionToExpr d expr = ((\x -> case x of
  ...
  Syntax.ExpressionUnquote v0 -> ((\x -> case x of
    ...
    Syntax.DialectScheme -> (Serialization.noSep [
      Serialization.cst ",",
      (expressionToExpr d (Syntax.unquoteExpressionBody v0))])) d)
  Syntax.ExpressionSplicingUnquote v0 -> ((\x -> case x of
    Syntax.DialectClojure -> (Serialization.noSep [
      Serialization.cst "~@",
    ...
    Syntax.DialectScheme -> (Serialization.noSep [
      Serialization.cst ",@",
      (expressionToExpr d (Syntax.splicingUnquoteExpressionBody v0))])) d)
  Syntax.ExpressionSExpression v0 -> (sExpressionToExpr v0)) expr)

applicationToExpr :: (Syntax.Dialect -> Syntax.Application -> Ast.Expr)
applicationToExpr d app =  
  let fun = (expressionToExpr d (Syntax.applicationFunction app)) 
...
letExpressionToExpr d letExpr =  
  let kind = (Syntax.letExpressionKind letExpr) 
      ...
      bindingPairs = (Lists.map (\b -> (\x -> case x of
              Syntax.LetBindingSimple v0 -> (symbolToExpr (Syntax.simpleBindingName v0), (expressionToExpr d (Syntax.simpleBindingValue v0)))
              Syntax.LetBindingDestructuring _ -> (Serialization.cst "<destructuring>", (Serialization.cst "<destructuring>"))) b) bindings)
  in ((\x -> case x of
    Syntax.DialectClojure -> ((\x -> case x of
    ...
    Syntax.DialectEmacsLisp ->  
      let kw = ((\x -> case x of
              ...
              Syntax.LetKindRecursive -> "letrec") kind) 
          bindingExprs = (Lists.map (\p -> Serialization.parens (Serialization.spaceSep [
                  Pairs.first p,
                  (Pairs.second p)])) bindingPairs)
      in (Serialization.parens (Serialization.spaceSep (Lists.concat [
        [
  ...
  in ((\x -> case x of
    Syntax.DialectClojure ->  
      ...
      in (Serialization.parens (Serialization.spaceSep (Lists.concat [
        ...
        defaultPart])))
    Syntax.DialectEmacsLisp ->  
      let clauseExprs = (Lists.map (\c -> Serialization.parens (Serialization.spaceSep [
              expressionToExpr d (Syntax.condClauseCondition c),
              (expressionToExpr d (Syntax.condClauseBody c))])) clauses) 
          defaultPart = (Maybes.maybe [] (\e -> [
...
caseExpressionToExpr d caseExpr =  
  let scrutinee = (expressionToExpr d (Syntax.caseExpressionScrutinee caseExpr)) 
      ...
      clauseExprs = (Lists.map (\c -> Serialization.parens (Serialization.spaceSep [
...
andExpressionToExpr d andExpr = (Serialization.parens (Serialization.spaceSep (Lists.concat2 [
...
orExpressionToExpr d orExpr = (Serialization.parens (Serialization.spaceSep (Lists.concat2 [
...
notExpressionToExpr d notExpr = (Serialization.parens (Serialization.spaceSep [
...
variableReferenceToExpr d vref =  
  ...
  in (Logic.ifElse isFnNs ((\x -> case x of
    Syntax.DialectCommonLisp -> (Serialization.noSep [
      Serialization.cst "#'",
      name])
    Syntax.DialectClojure -> name
    Syntax.DialectEmacsLisp -> name
    Syntax.DialectScheme -> name) d) name)
...
literalToExpr d lit = ((\x -> case x of
    ...
    in ((\x -> case x of
      ...
      Syntax.DialectClojure ->  
        let e2 = (Strings.intercalate "\\n" (Strings.splitOn (Strings.fromList [
                10]) e1))
        in  
          let e3 = (Strings.intercalate "\\r" (Strings.splitOn (Strings.fromList [
                  13]) e2))
          in  
            let e4 = (Strings.intercalate "\\t" (Strings.splitOn (Strings.fromList [
                    9]) e3))
            in  
              let escaped = (Strings.intercalate "\\\"" (Strings.splitOn "\"" e4))
              in (Serialization.cst (Strings.cat [
                "\"",
                escaped,
                "\""]))
  ...
  Syntax.LiteralCharacter v0 ->  
    ...
    in ((\x -> case x of
      ...
      Syntax.DialectScheme -> (Serialization.cst (Strings.cat2 "#\\" ch))) d)
...
vectorLiteralToExpr d vl =  
  ...
  in ((\x -> case x of
    ...
    Syntax.DialectScheme -> (Serialization.noSep [
      Serialization.cst "#",
      (Serialization.parens (Serialization.spaceSep elems))])) d)
...
mapLiteralToExpr d ml =  
  ...
  in ((\x -> case x of
    ...
    Syntax.DialectScheme -> (Serialization.parens (Serialization.spaceSep (Lists.concat2 [
      ...
      Serialization.cst "cons",
      (expressionToExpr d (Syntax.mapEntryKey e)),
      (expressionToExpr d (Syntax.mapEntryValue e))])) entries))))) d)
...
setLiteralToExpr d sl =  
  ...
  in ((\x -> case x of
    ...
    Syntax.DialectScheme -> (Serialization.parens (Serialization.spaceSep (Lists.concat2 [
      Serialization.cst "list"] elems)))) d)
...
moduleDeclarationToExpr d mdecl =  
  ...
  in ((\x -> case x of
    ...
    Syntax.DialectScheme -> (Serialization.parens (Serialization.spaceSep [
      Serialization.cst "define-library",
      (Serialization.parens (Serialization.cst name))]))) d)
...
importDeclarationToExpr d idecl =  
  ...
  in ((\x -> case x of
    ...
    Syntax.DialectScheme -> (Serialization.parens (Serialization.spaceSep [
      ...
      (Serialization.parens (Serialization.cst modName))]))) d)
...
exportDeclarationToExpr d edecl =  
  ...
  in ((\x -> case x of
    ...
    Syntax.DialectCommonLisp -> (Serialization.parens (Serialization.spaceSep (Lists.concat2 [
      ...
      s]) syms))))
    Syntax.DialectScheme -> (Serialization.parens (Serialization.spaceSep (Lists.concat2 [
      Serialization.cst "export"] syms)))) d)

sExpressionToExpr :: (Syntax.SExpression -> Ast.Expr)
sExpressionToExpr sexpr = ((\x -> case x of
  Syntax.SExpressionAtom v0 -> (Serialization.cst v0)
  Syntax.SExpressionList v0 -> (Serialization.parens (Serialization.spaceSep (Lists.map sExpressionToExpr v0)))) sexpr)
...
docstringToExpr ds = (Serialization.cst (Strings.cat [
  ";; ",
