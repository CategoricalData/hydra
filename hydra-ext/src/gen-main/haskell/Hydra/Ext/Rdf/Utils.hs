...
emptyDescription node = Syntax.Description {
...
encodeLiteral lit = ((\x -> case x of
  ...
  Core.LiteralFloat v0 -> ((\x -> case x of
    ...
    Core.FloatValueFloat64 v1 -> Syntax.Literal {
      ...
      Syntax.literalLanguageTag = Nothing}) v0)
  ...
  Core.LiteralString v0 -> Syntax.Literal {
    ...
    Syntax.literalLanguageTag = Nothing}) lit)
...
forObjects subj pred objs = (Lists.map (\obj -> Syntax.Triple {
...
resourceToNode r = ((\x -> case x of
  Syntax.ResourceIri v0 -> (Syntax.NodeIri v0)
  Syntax.ResourceBnode v0 -> (Syntax.NodeBnode v0)) r)
