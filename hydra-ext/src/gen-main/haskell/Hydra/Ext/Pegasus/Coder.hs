...
moduleToPdl :: (Module.Module -> [Module.Definition] -> Context.Context -> Graph.Graph -> Either (Context.InContext Error.OtherError) (M.Map String String))
...
constructModule :: (Context.Context -> Graph.Graph -> M.Map Module.Namespace String -> Module.Module -> [Module.TypeDefinition] -> Either (Context.InContext Error.OtherError) (M.Map String Pdl.SchemaFile))
constructModule cx g aliases mod typeDefs =  
  ...
  in (Maybes.cases (Lists.find (\grp -> Equality.gt (Lists.length grp) 1) groups) ( 
    ...
    Context.inContextObject = (Error.OtherError (Strings.cat2 "types form a cycle (unsupported in PDL): [" (Strings.cat2 (Strings.intercalate ", " (Lists.map (\td -> Core.unName (Module.typeDefinitionName td)) cycle)) "]"))),
    Context.inContextContext = cx})))

typeToSchema :: (Context.Context -> Graph.Graph -> M.Map Module.Namespace String -> t0 -> Module.TypeDefinition -> Either (Context.InContext Error.OtherError) (Pdl.NamedSchema, [t1]))
...
moduleToPegasusSchemas :: (Context.Context -> Graph.Graph -> Module.Module -> [Module.Definition] -> Either (Context.InContext Error.OtherError) (M.Map String Pdl.SchemaFile))
...
encodeType :: (Context.Context -> Graph.Graph -> M.Map Module.Namespace String -> Core.Type -> Either (Context.InContext Error.OtherError) (Either Pdl.Schema Pdl.NamedSchemaType))
encodeType cx g aliases typ = ((\x -> case x of
  ...
  Core.TypeLiteral v0 -> ((\x -> case x of
    ...
    Core.LiteralTypeFloat v1 -> ((\x -> case x of
      ...
      _ -> (Left (Context.InContext {
        Context.inContextObject = (Error.OtherError (Strings.cat2 "Expected " (Strings.cat2 "float32 or float64" (Strings.cat2 ", found: " (Core_.type_ typ))))),
    ...
    Core.LiteralTypeInteger v1 -> ((\x -> case x of
      ...
      _ -> (Left (Context.InContext {
        Context.inContextObject = (Error.OtherError (Strings.cat2 "Expected " (Strings.cat2 "int32 or int64" (Strings.cat2 ", found: " (Core_.type_ typ))))),
        Context.inContextContext = cx}))) v1)
    Core.LiteralTypeString -> (Right (Left (Pdl.SchemaPrimitive Pdl.PrimitiveTypeString)))
    _ -> (Left (Context.InContext {
      Context.inContextObject = (Error.OtherError (Strings.cat2 "Expected " (Strings.cat2 "PDL-supported literal type" (Strings.cat2 ", found: " (Core_.type_ typ))))),
  ...
  Core.TypeMaybe _ -> (Left (Context.InContext {
    Context.inContextObject = (Error.OtherError "optionals unexpected at top level"),
    Context.inContextContext = cx}))
  ...
  _ -> (Left (Context.InContext {
    Context.inContextObject = (Error.OtherError (Strings.cat2 "Expected " (Strings.cat2 "PDL-supported type" (Strings.cat2 ", found: " (Core_.type_ typ))))),
    Context.inContextContext = cx}))) typ)

encode :: (Context.Context -> Graph.Graph -> M.Map Module.Namespace String -> Core.Type -> Either (Context.InContext Error.OtherError) Pdl.Schema)
encode cx g aliases t = ((\x -> case x of
  Core.TypeRecord v0 -> (Logic.ifElse (Lists.null (Core.rowTypeFields v0)) (encode cx g aliases (Core.TypeLiteral (Core.LiteralTypeInteger Core.IntegerTypeInt32))) (Eithers.bind (encodeType cx g aliases t) (\res -> Eithers.either (\schema -> Right schema) (\_ -> Left (Context.InContext {
    Context.inContextObject = (Error.OtherError (Strings.cat2 "type resolved to an unsupported nested named schema: " (Core_.type_ t))),
...
encodeRecordField :: (Context.Context -> Graph.Graph -> M.Map Module.Namespace String -> Core.FieldType -> Either (Context.InContext Error.OtherError) Pdl.RecordField)
...
encodeUnionField :: (Context.Context -> Graph.Graph -> M.Map Module.Namespace String -> Core.FieldType -> Either (Context.InContext Error.OtherError) Pdl.UnionMember)
...
encodeEnumField :: (Context.Context -> Graph.Graph -> Core.FieldType -> Either (Context.InContext Error.OtherError) Pdl.EnumField)
...
encodePossiblyOptionalType :: (Context.Context -> Graph.Graph -> M.Map Module.Namespace String -> Core.Type -> Either (Context.InContext Error.OtherError) (Pdl.Schema, Bool))
...
getAnns :: (Context.Context -> Graph.Graph -> Core.Type -> Either (Context.InContext Error.OtherError) Pdl.Annotations)
...
importAliasesForModule :: (Context.Context -> Graph.Graph -> Module.Module -> Either (Context.InContext Error.OtherError) (M.Map Module.Namespace String))
