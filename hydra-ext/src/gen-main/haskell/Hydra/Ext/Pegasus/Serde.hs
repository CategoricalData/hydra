...
exprImport qn = (Serialization.spaceSep [
...
exprNamedSchema ns =  
  ...
  in (withAnnotations anns ((\x -> case x of
...
exprPrimitiveType pt = (Serialization.cst ((\x -> case x of
  ...
  Pdl.PrimitiveTypeString -> "string") pt))
...
exprSchema schema = ((\x -> case x of
  ...
  Pdl.SchemaUnion v0 -> (Serialization.noSep [
    ...
    (Serialization.bracketList Serialization.fullBlockStyle (Lists.map exprUnionMember (Pdl.unUnionSchema v0)))])) schema)
...
exprSchemaFile sf =  
  let ns = (Pdl.unNamespace (Pdl.schemaFileNamespace sf)) 
      ...
      namespaceSec = (Maybes.pure (Serialization.spaceSep [
              Serialization.cst "namespace",
              (Serialization.cst ns)]))
      packageSec = (Maybes.map (\p -> Serialization.spaceSep [
...
withAnnotations anns expr = (Serialization.newlineSep (Maybes.cat [
  exprAnnotations anns,
  (Maybes.pure expr)]))
