...
writeDefn def = ((\x -> case x of
  Meta.DefnDef v0 ->  
    let name = (Meta.defn_DefName v0) 
        ...
        tparamsExpr = (Logic.ifElse (Lists.null tparams) Nothing (Maybes.pure (Serialization.bracketList Serialization.inlineStyle (Lists.map writeType_Param tparams))))
        scodExpr = (Maybes.map (\t -> Serialization.spaceSep [
                Serialization.cst ":",
                (writeType t)]) scod)
        nameAndParams = (Serialization.noSep (Maybes.cat [
  ...
  Meta.DefnVal v0 ->  
    let pats = (Meta.defn_ValPats v0) 
        ...
        patName = ((\x -> case x of
                Meta.PatVar v1 -> (Meta.pat_VarName v1)) firstPat)
        nameStr = (Meta.unPredefString (Meta.data_NameValue patName))
        nameAndType = (Maybes.maybe (Serialization.cst nameStr) (\t -> Serialization.spaceSep [
  ...
  Meta.DefnClass v0 ->  
    let mods = (Meta.defn_ClassMods v0) 
        ...
        tparamsExpr = (Logic.ifElse (Lists.null tparams) Nothing (Maybes.pure (Serialization.bracketList Serialization.inlineStyle (Lists.map writeType_Param tparams))))
        paramsExpr = (Logic.ifElse (Lists.null paramss) Nothing (Maybes.pure (Serialization.parenList False (Lists.map writeData_Param (Lists.concat paramss)))))
        nameAndParams = (Serialization.noSep (Maybes.cat [
                Maybes.pure (writeType_Name name),
                tparamsExpr,
                paramsExpr]))
  ...
  Meta.DefnEnum v0 ->  
    let name = (Meta.defn_EnumName v0) 
        ...
        enumHeader = (Serialization.spaceSep [
                Serialization.cst "enum",
                (Serialization.noSep (Maybes.cat [
                  Maybes.pure (writeType_Name name),
                  (Logic.ifElse (Lists.null tparams) Nothing (Maybes.pure (Serialization.bracketList Serialization.inlineStyle (Lists.map writeType_Param tparams))))])),
                (Serialization.cst ":")])
        enumCases = (Lists.map (\s -> Serialization.spaceSep [
                Serialization.cst "  ",
                (writeStat s)]) stats)
  ...
  Meta.DefnEnumCase v0 ->  
    let name = (Meta.defn_EnumCaseName v0) 
        ...
        params = (Logic.ifElse (Lists.null allParams) (Serialization.cst "") (Serialization.parenList False (Lists.map writeData_Param allParams)))
        extendsClause = (Logic.ifElse (Lists.null inits) (Serialization.cst "") (Serialization.spaceSep [
...
writeImportExportStat ie = ((\x -> case x of
  Meta.ImportExportStatImport v0 ->  
    let importers = (Meta.importImporters v0)
    in (Serialization.newlineSep (Lists.map writeImporter importers))) ie)
...
writeImporter imp =  
  let ref = (Meta.importerRef imp) 
      ...
      refName = ((\x -> case x of
              Meta.Data_RefName v0 -> (Meta.unPredefString (Meta.data_NameValue v0))) ref)
      forImportees = (Logic.ifElse (Lists.null importees) (Serialization.cst "") (Logic.ifElse (Equality.equal (Lists.length importees) 1) (Serialization.noSep [
              Serialization.cst ".",
              ((\x -> case x of
                Meta.ImporteeWildcard -> (Serialization.cst "*")
                Meta.ImporteeName v0 -> (Serialization.cst ((\x -> case x of
                  Meta.NameValue v1 -> v1) (Meta.importee_NameName v0)))) (Lists.head importees))]) (Serialization.noSep [
              Serialization.cst ".",
              (Serialization.curlyBracesList Nothing Serialization.inlineStyle (Lists.map (\it -> (\x -> case x of
                Meta.ImporteeWildcard -> (Serialization.cst "*")
                Meta.ImporteeName v0 -> (Serialization.cst ((\x -> case x of
                  Meta.NameValue v1 -> v1) (Meta.importee_NameName v0)))) it) importees))])))
  in (Serialization.spaceSep [
...
writeLit lit = ((\x -> case x of
  ...
  _ -> (Serialization.cst "TODO:literal")) lit)

-- | Convert a name to an expression
writeName :: (Meta.Name -> Ast.Expr)
writeName name = ((\x -> case x of
  Meta.NameValue v0 -> (Serialization.cst v0)) name)

-- | Convert a pattern to an expression
writePat :: (Meta.Pat -> Ast.Expr)
writePat pat = ((\x -> case x of
  Meta.PatExtract v0 ->  
  ...
  Meta.PatVar v0 -> (writeData_Name (Meta.pat_VarName v0))) pat)
...
writePkg pkg =  
  let name = (Meta.pkgName pkg) 
      ...
      package = (Serialization.spaceSep [
...
writeStat stat = ((\x -> case x of
  Meta.StatDefn v0 -> (writeDefn v0)
  Meta.StatImportExport v0 -> (writeImportExportStat v0)) stat)

-- | Convert a term to an expression
writeTerm :: (Meta.Data -> Ast.Expr)
writeTerm term = ((\x -> case x of
  ...
  Meta.DataFunctionData v0 -> (writeData_FunctionData v0)) term)

-- | Convert function data to an expression
writeData_FunctionData :: (Meta.Data_FunctionData -> Ast.Expr)
writeData_FunctionData ft = ((\x -> case x of
...
writeData_Ref ref = ((\x -> case x of
  Meta.Data_RefName v0 -> (writeData_Name v0)
  Meta.Data_RefSelect v0 -> (writeData_Select v0)) ref)
...
writeType typ = ((\x -> case x of
  Meta.TypeRef v0 -> ((\x -> case x of
    Meta.Type_RefName v1 -> (writeType_Name v1)) v0)
  Meta.TypeApply v0 ->  
  ...
  Meta.TypeVar v0 -> (writeType_Name (Meta.type_VarName v0))) typ)
...
writeMod m = ((\x -> case x of
  ...
  Meta.ModProtected _ -> (Serialization.cst "protected")) m)
