...
prepareLiteralType at = ((\x -> case x of
  Core.LiteralTypeBinary -> (Core.LiteralTypeString, ((\v -> (\x -> case x of
  ...
  Core.LiteralTypeFloat v0 ->  
    ...
    in (Core.LiteralTypeFloat rtyp, ((\v -> (\x -> case x of
  ...
  Core.LiteralTypeInteger v0 ->  
    let result = (prepareIntegerType v0) 
        ...
        rep = (Pairs.first (Pairs.second result))
        msgs = (Pairs.second (Pairs.second result))
    in (Core.LiteralTypeInteger rtyp, ((\v -> (\x -> case x of
...
prepareFloatType ft = ((\x -> case x of
  Core.FloatTypeBigfloat -> (Core.FloatTypeFloat64, ((\v -> (\x -> case x of
...
prepareIntegerType it = ((\x -> case x of
  Core.IntegerTypeBigint -> (Core.IntegerTypeInt64, ((\v -> (\x -> case x of
  ...
  Core.IntegerTypeUint8 -> (Core.IntegerTypeInt8, ((\v -> (\x -> case x of
  ...
  Core.IntegerTypeUint32 -> (Core.IntegerTypeInt32, ((\v -> (\x -> case x of
  ...
  Core.IntegerTypeUint64 -> (Core.IntegerTypeInt64, ((\v -> (\x -> case x of
...
prepareType cx typ = ((\x -> case x of
  Core.TypeLiteral v0 ->  
    let result = (prepareLiteralType v0) 
        rtyp = (Pairs.first result)
        rep = (Pairs.first (Pairs.second result))
        msgs = (Pairs.second (Pairs.second result))
    in (Core.TypeLiteral rtyp, ((\v -> (\x -> case x of
      Core.TermLiteral v1 -> (Core.TermLiteral (rep v1))
      _ -> v) v), msgs))
  _ -> (same typ)) (Rewriting.deannotateType typ))
