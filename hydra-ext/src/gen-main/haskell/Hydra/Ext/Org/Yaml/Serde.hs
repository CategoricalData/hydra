...
module Hydra.Ext.Org.Yaml.Serde where
...
writeScalar s = ((\x -> case x of
  ...
  Model.ScalarStr v0 -> (writeString v0)) s)

-- | Write a string value, quoting if necessary
writeString :: (String -> String)
writeString s = (Logic.ifElse (needsQuoting s) (Strings.cat [
  "'",
...
writeSequenceItem node = ((\x -> case x of
  ...
  Model.NodeMapping v0 -> (Logic.ifElse (Equality.equal (Maps.size v0) 0) "- {}\n" ( 
    ...
    in  
      let firstEntry = (Lists.head entries)
      in  
        let restEntries = (Lists.tail entries)
        in  
          let firstStr = (writeMappingEntryInline firstEntry)
          in  
            let restStr = (Strings.cat (Lists.map (\e -> writeMappingEntry e) restEntries))
            in (Strings.cat [
              "- ",
              firstStr,
              (indentString restStr)])))) node)
  ...
  in  
    ...
    in ((\x -> case x of
      ...
      Model.NodeMapping v0 -> (Logic.ifElse (Equality.equal (Maps.size v0) 0) (Strings.cat [
        ...
        writeNodeInline key,
        ":\n",
        (indentString (writeNode value))]))) value)
...
writeNodeInline node = ((\x -> case x of
  ...
  Model.NodeMapping v0 ->  
    let writeFlowEntry = (\e -> Strings.cat [
...
indentString s = (Strings.cat (Lists.map (\line -> Logic.ifElse (Strings.null line) "" (Strings.cat [
  "  ",
  line,
  "\n"])) (Strings.lines s)))
...
needsQuoting s = (Logic.ifElse (Strings.null s) True (Logic.ifElse (Lists.elem s yamlReservedWords) True (Logic.ifElse (looksLikeNumber s) True ( 
  let chars = (Strings.toList s)
  in  
    let specials = (Strings.toList yamlSpecialChars)
    in  
      let hasSpecial = (Logic.not (Lists.null (Lists.filter (\c -> Lists.elem c specials) chars)))
...
looksLikeNumber s =  
  ...
  in (Logic.ifElse (Lists.null chars) False ( 
    ...
    in (Logic.ifElse (Lists.null rest) False ( 
      ...
      in  
        let allDigits = (Lists.null (Lists.filter (\c -> Logic.not (isDigitFn c)) rest))
...
isDecimalString chars =  
  let dotCode = 46
  in  
    let parts = (Lists.span (\c -> Logic.not (Equality.equal c dotCode)) chars)
    in  
      let before = (Pairs.first parts)
      in  
        let afterWithDot = (Pairs.second parts)
