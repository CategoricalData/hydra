...
exprDefinition def = ((\x -> case x of
  Syntax.DefinitionExecutable _ -> (Serialization.cst "Unsupported: executable definition")
  Syntax.DefinitionTypeSystem v0 -> (exprTypeSystemDefinitionOrExtension v0)) def)
...
exprListType lt = (Serialization.noSep [
  Serialization.cst "[",
...
exprNonNullType nnt =  
  let typeExpr = ((\x -> case x of
          Syntax.NonNullTypeNamed v0 -> (exprNamedType v0)
          Syntax.NonNullTypeList v0 -> (exprListType v0)) nnt)
...
exprTypeDefinition def = ((\x -> case x of
  ...
  Syntax.TypeDefinitionInputObject _ -> (Serialization.cst "Unsupported: input object type definition")) def)

-- | Convert a GraphQL type system definition to an expression
exprTypeSystemDefinition :: (Syntax.TypeSystemDefinition -> Ast.Expr)
exprTypeSystemDefinition def = ((\x -> case x of
  ...
  Syntax.TypeSystemDefinitionDirective _ -> (Serialization.cst "Unsupported: directive definition")) def)
...
exprTypeSystemDefinitionOrExtension de = ((\x -> case x of
  Syntax.TypeSystemDefinitionOrExtensionDefinition v0 -> (exprTypeSystemDefinition v0)
  Syntax.TypeSystemDefinitionOrExtensionExtension _ -> (Serialization.cst "Unsupported: type system extension")) de)

-- | Prepend an optional description to an expression
withDescription :: (Maybe Syntax.Description -> Ast.Expr -> Ast.Expr)
withDescription mdesc expr = (Serialization.newlineSep (Maybes.cat [
