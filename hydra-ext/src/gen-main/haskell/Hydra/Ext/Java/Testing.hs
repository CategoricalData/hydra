...
module Hydra.Ext.Java.Testing where
...
termToJava term g =
    Eithers.bimap (\ic -> Errors.error (Context.inContextObject ic)) (\arg_ -> (\arg_ -> Serialization.printExpr (Serialization.parenthesize arg_)) (Serde.writeExpression arg_)) (Coder.encodeTerm (Environment.JavaEnvironment {
      Environment.javaEnvironmentAliases = Environment.Aliases {
        Environment.aliasesCurrentNamespace = (Module.Namespace "test"),
      ...
      Environment.javaEnvironmentGraph = g}) term Lexical.emptyContext g)
