...
common constraints = Model.CommonProperties {
...
encodeFieldType rname order ft cx =  
  let fname = (Core.fieldTypeName ft) 
      ...
      forType = (\mn -> \mx -> \t -> (\x -> case x of
              Core.TypeMaybe v0 -> (forType (Just 0) mx v0)
              Core.TypeSet v0 -> (forType mn Nothing v0)
              _ -> (forTypeDefault mn mx t)) (Rewriting.deannotateType t))
      forTypeDefault = (\mn -> \mx -> \t -> Eithers.map (\_cp ->  
...
encodeLiteralType lt =  
  ...
  in ((\x -> case x of
    ...
    Core.LiteralTypeFloat v0 -> ((\x -> case x of
      ...
      Core.FloatTypeFloat64 -> (xsd "double")) v0)
    Core.LiteralTypeInteger v0 -> ((\x -> case x of
      ...
      Core.IntegerTypeUint64 -> (xsd "unsignedLong")) v0)
...
property iri = Model.PropertyShape {
...
withType name desc =  
  let subj = (Syntax.descriptionSubject desc) 
      ...
      subjRes = ((\x -> case x of
              ...
              Syntax.NodeBnode v0 -> (Syntax.ResourceBnode v0)) subj)
      triple = Syntax.Triple {
