...
validateEdge checkValue showValue labelForVertexId typ el =
      ...
      let failWith = edgeError showValue el
          ...
          checkProperties =
                  Maybes.map (\arg_ -> failWith (prepend "Invalid property" arg_)) (validateProperties checkValue (Model.edgeTypeProperties typ) (Model.edgeProperties el))
          checkOut =
...
validateElement checkValue showValue labelForVertexId typ el =
    (\x -> case x of
...
validateVertex checkValue showValue typ el =
      ...
      let failWith = vertexError showValue el
          ...
          checkProperties =
                  Maybes.map (\arg_ -> failWith (prepend "Invalid property" arg_)) (validateProperties checkValue (Model.vertexTypeProperties typ) (Model.vertexProperties el))
      in (checkAll [
        checkLabel,
        checkId,
        checkProperties])
...
vertexLabelMismatch expected actual =
    Strings.cat [
      "expected ",
