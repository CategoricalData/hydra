...
elementCoder mparent schema source vidType eidType cx g =
      ...
      in case (Rewriting.deannotateType source) of
...
encodeProperty cx fields adapter =
      ...
      let fname = Core.fieldTypeName (Util.adapterSource adapter) 
          ...
          isMaybe =
                  case ftyp of
                    ...
                    _ -> False
          encodeValue =
                  \v -> Eithers.map (\x -> Just x) (Util.coderEncode (Util.adapterCoder adapter) cx (Core.Field {
      ...
      in (Maybes.maybe (Logic.ifElse isMaybe (Right Nothing) (Left (Context.InContext {
        ...
        Context.inContextContext = cx}))) (\value -> Logic.ifElse isMaybe (case (Rewriting.deannotateTerm value) of
