...
applyPattern :: (Context.Context -> String -> [([String], String)] -> Core.Term -> Either (Context.InContext Error.Error) [Core.Term])
...
decodeEdgeLabel :: (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.Error) Model.EdgeLabel)
decodeEdgeLabel cx g t = (Eithers.map (\_x -> Model.EdgeLabel _x) (Core_.string cx g t))

-- | Decode an edge specification from a term
decodeEdgeSpec :: (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.Error) Mapping.EdgeSpec)
...
decodeElementSpec :: (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.Error) Mapping.ElementSpec)
...
decodePropertyKey :: (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.Error) Model.PropertyKey)
...
decodePropertySpec :: (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.Error) Mapping.PropertySpec)
...
decodeValueSpec :: (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.Error) Mapping.ValueSpec)
...
decodeVertexLabel :: (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.Error) Model.VertexLabel)
...
decodeVertexSpec :: (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.Error) Mapping.VertexSpec)
...
evalPath :: (Context.Context -> [String] -> Core.Term -> Either (Context.InContext Error.Error) [Core.Term])
...
evalStep :: (Context.Context -> String -> Core.Term -> Either (Context.InContext Error.Error) [Core.Term])
evalStep cx step term = (Logic.ifElse (Strings.null step) (Right [
  ...
  Core.TermRecord v0 -> (Maybes.maybe (Left (Context.InContext {
    Context.inContextObject = (Error.ErrorOther (Error.OtherError (Strings.cat2 (Strings.cat2 "No such field " step) " in record"))),
  ...
  _ -> (Left (Context.InContext {
    Context.inContextObject = (Error.ErrorOther (Error.OtherError (Strings.cat2 "Can't traverse through term for step " step))),
    Context.inContextContext = cx}))) (Rewriting.deannotateTerm term)))

-- | Extract a list from a term and apply a decoder to each element
expectList :: (Context.Context -> Graph.Graph -> (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.Error) t0) -> Core.Term -> Either (Context.InContext Error.Error) [t0])
...
parseElementSpec :: (t0 -> t1 -> Mapping.Schema t2 t3 t4 -> Mapping.ElementSpec -> Either t5 (Model.Label, (Context.Context -> Core.Term -> Either (Context.InContext Error.Error) [Model.Element t4])))
...
parsePattern :: (t0 -> t1 -> String -> Either t2 (Context.Context -> Core.Term -> Either (Context.InContext Error.Error) [Core.Term]))
...
parsePropertySpec :: (t0 -> t1 -> Mapping.Schema t2 t3 t4 -> Mapping.PropertySpec -> Either t5 (Context.Context -> Core.Term -> Either (Context.InContext Error.Error) [(Model.PropertyKey, t4)]))
...
readField :: (Context.Context -> M.Map Core.Name t0 -> Core.Name -> (t0 -> Either (Context.InContext Error.Error) t1) -> Either (Context.InContext Error.Error) t1)
...
readInjection :: (Context.Context -> Graph.Graph -> [(Core.Name, (Core.Term -> Either (Context.InContext Error.Error) t0))] -> Core.Term -> Either (Context.InContext Error.Error) t0)
readInjection cx g cases encoded = (Eithers.bind (Core_.map cx (\k -> Eithers.map (\_n -> Core.Name _n) (Core_.string cx g k)) (\_v -> Right _v) g encoded) (\mp ->  
  ...
  in (Logic.ifElse (Lists.null entries) (Left (Context.InContext {
    Context.inContextObject = (Error.ErrorOther (Error.OtherError "empty injection")),
    ...
    in (Logic.ifElse (Lists.null matching) (Left (Context.InContext {
      Context.inContextObject = (Error.ErrorOther (Error.OtherError (Strings.cat2 "unexpected field: " (Core.unName key)))),
...
readRecord :: (Context.Context -> Graph.Graph -> (M.Map Core.Name Core.Term -> Either (Context.InContext Error.Error) t0) -> Core.Term -> Either (Context.InContext Error.Error) t0)
...
requireUnique :: (Context.Context -> String -> (t0 -> Either (Context.InContext Error.Error) [t1]) -> t0 -> Either (Context.InContext Error.Error) t1)
requireUnique cx context fun term = (Eithers.bind (fun term) (\results -> Logic.ifElse (Lists.null results) (Left (Context.InContext {
  Context.inContextObject = (Error.ErrorOther (Error.OtherError (Strings.cat2 "No value found: " context))),
  ...
  Context.inContextObject = (Error.ErrorOther (Error.OtherError (Strings.cat2 "Multiple values found: " context))),
  Context.inContextContext = cx})))))
