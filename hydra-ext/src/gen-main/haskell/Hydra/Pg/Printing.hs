...
printEdge printValue edge =  
  let label = (Model.unEdgeLabel (Model.edgeLabel edge)) 
      ...
      props = (Strings.intercalate ", " (Lists.map (\p -> printProperty printValue (Pairs.first p) (Pairs.second p)) (Maps.toList (Model.edgeProperties edge))))
  in (Strings.cat [
    id,
    ": ",
    "(",
...
printGraph printValue graph = (printLazyGraph printValue (Model.LazyGraph {
...
printProperty printValue key value = (Strings.cat [
...
printVertex printValue vertex =  
  let label = (Model.unVertexLabel (Model.vertexLabel vertex)) 
      ...
      props = (Strings.intercalate ", " (Lists.map (\p -> printProperty printValue (Pairs.first p) (Pairs.second p)) (Maps.toList (Model.vertexProperties vertex))))
  in (Strings.cat [
    id,
    ": (",
    label,
