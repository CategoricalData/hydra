...
adjacentEdge :: ((Graph.Graph -> Core.Term -> Either Error.DecodingError t0) -> Graph.Graph -> Core.Term -> Either Error.DecodingError (Model.AdjacentEdge t0))
adjacentEdge v cx raw = (Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermRecord v0 ->  
...
edgeLabel cx raw = (Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermWrap v0 -> (Eithers.map (\b -> Model.EdgeLabel b) ((\raw -> Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
...
propertyKey cx raw = (Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermWrap v0 -> (Eithers.map (\b -> Model.PropertyKey b) ((\raw -> Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
    Core.TermLiteral v1 -> ((\x -> case x of
      Core.LiteralString v2 -> (Right v2)
      _ -> (Left (Error.DecodingError "expected string literal"))) v1)
    _ -> (Left (Error.DecodingError "expected literal"))) stripped) (Lexical.stripAndDereferenceTermEither cx raw)) (Core.wrappedTermBody v0)))
...
propertyType t cx raw = (Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermRecord v0 ->  
    let fieldMap = (Helpers.toFieldMap v0)
    in (Eithers.bind (Helpers.requireField "key" propertyKey fieldMap cx) (\field_key -> Eithers.bind (Helpers.requireField "value" t fieldMap cx) (\field_value -> Eithers.bind (Helpers.requireField "required" (\cx -> \raw -> Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
      Core.TermLiteral v1 -> ((\x -> case x of
        Core.LiteralBoolean v2 -> (Right v2)
        _ -> (Left (Error.DecodingError "expected boolean literal"))) v1)
      _ -> (Left (Error.DecodingError "expected literal"))) stripped) (Lexical.stripAndDereferenceTermEither cx raw)) fieldMap cx) (\field_required -> Right (Model.PropertyType {
...
vertexLabel cx raw = (Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermWrap v0 -> (Eithers.map (\b -> Model.VertexLabel b) ((\raw -> Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
    Core.TermLiteral v1 -> ((\x -> case x of
      Core.LiteralString v2 -> (Right v2)
      _ -> (Left (Error.DecodingError "expected string literal"))) v1)
    _ -> (Left (Error.DecodingError "expected literal"))) stripped) (Lexical.stripAndDereferenceTermEither cx raw)) (Core.wrappedTermBody v0)))
