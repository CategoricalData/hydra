...
def edge_property_to_graphson(encode_value: Callable[[T0], hydra.util.Flow[T1, T2]], prop: tuple[hydra.pg.model.PropertyKey, T0]) -> hydra.util.Flow[T1, tuple[hydra.pg.graphson.syntax.PropertyKey, T2]]:
...
def adjacent_edge_to_graphson(encode_value: Callable[[T0], hydra.util.Flow[T1, hydra.pg.graphson.syntax.Value]], edge: hydra.pg.model.AdjacentEdge[T0]) -> hydra.util.Flow[T1, tuple[hydra.pg.graphson.syntax.EdgeLabel, hydra.pg.graphson.syntax.AdjacentEdge]]:
    @lru_cache(1)
...
@lru_cache(1)
def graphson_vertex_to_json_coder() -> hydra.util.Coder[T0, T1, hydra.pg.graphson.syntax.Vertex, hydra.json.model.Value]:
    return hydra.util.Coder((lambda v: hydra.lib.flows.pure(hydra.pg.graphson.coder.vertex_to_json(v))), (lambda _: hydra.lib.flows.fail("decoding GraphSON JSON is currently unsupported")))

def vertex_property_to_graphson(encode_value: Callable[[T0], hydra.util.Flow[T1, hydra.pg.graphson.syntax.Value]], prop: tuple[hydra.pg.model.PropertyKey, T0]) -> hydra.util.Flow[T1, tuple[hydra.pg.graphson.syntax.PropertyKey, hydra.pg.graphson.syntax.VertexPropertyValue]]:
...
def pg_vertex_with_adjacent_edges_to_graphson_vertex(encode_value: Callable[[T0], hydra.util.Flow[T1, hydra.pg.graphson.syntax.Value]], vae: hydra.pg.model.VertexWithAdjacentEdges[T0]) -> hydra.util.Flow[T1, hydra.pg.graphson.syntax.Vertex]:
    @lru_cache(1)
...
def pg_vertex_with_adjacent_edges_to_json(encode_value: Callable[[T0], hydra.util.Flow[T1, hydra.pg.graphson.syntax.Value]], vertex: hydra.pg.model.VertexWithAdjacentEdges[T0]) -> hydra.util.Flow[T1, hydra.json.model.Value]:
