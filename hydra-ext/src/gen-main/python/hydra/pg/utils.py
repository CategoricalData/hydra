...
def pg_element_to_json(schema: hydra.pg.mapping.Schema[T0, T1, T2], el: hydra.pg.model.Element[T2], cx: hydra.context.Context):
    r"""Convert a property graph element to JSON."""

    return (lambda v1: (lambda vertex: hydra.lib.eithers.bind(schema.vertex_ids.decode(cx, vertex.id), (lambda term: (label_json := cast(hydra.json.model.Value, hydra.json.model.ValueString(vertex.label.value)), hydra.lib.eithers.map((lambda props_json: cast(hydra.json.model.Value, hydra.json.model.ValueObject(hydra.lib.maps.from_list(hydra.lib.maybes.cat((Just(("label", label_json)), Just(("id", cast(hydra.json.model.Value, hydra.json.model.ValueString(hydra.show.core.term(term))))), props_json)))))), hydra.lib.logic.if_else(hydra.lib.maps.null(vertex.properties), (lambda : Right(Nothing())), (lambda : hydra.lib.eithers.map((lambda p: Just(("properties", cast(hydra.json.model.Value, hydra.json.model.ValueObject(hydra.lib.maps.from_list(p)))))), hydra.lib.eithers.map_list((lambda pair: (key := hydra.lib.pairs.first(pair), v := hydra.lib.pairs.second(pair), hydra.lib.eithers.bind(schema.property_values.decode(cx, v), (lambda term2: Right((key.value, cast(hydra.json.model.Value, hydra.json.model.ValueString(hydra.show.core.term(term2))))))))[2]), hydra.lib.maps.to_list(vertex.properties)))))))[1])))(v1.value) if isinstance(v1, hydra.pg.model.ElementVertex) else (lambda edge: hydra.lib.eithers.bind(schema.edge_ids.decode(cx, edge.id), (lambda term: hydra.lib.eithers.bind(schema.vertex_ids.decode(cx, edge.out), (lambda term_out: hydra.lib.eithers.bind(schema.vertex_ids.decode(cx, edge.in_), (lambda term_in: (label_json := cast(hydra.json.model.Value, hydra.json.model.ValueString(edge.label.value)), hydra.lib.eithers.map((lambda props_json: cast(hydra.json.model.Value, hydra.json.model.ValueObject(hydra.lib.maps.from_list(hydra.lib.maybes.cat((Just(("label", label_json)), Just(("id", cast(hydra.json.model.Value, hydra.json.model.ValueString(hydra.show.core.term(term))))), Just(("out", cast(hydra.json.model.Value, hydra.json.model.ValueString(hydra.show.core.term(term_out))))), Just(("in", cast(hydra.json.model.Value, hydra.json.model.ValueString(hydra.show.core.term(term_in))))), props_json)))))), hydra.lib.logic.if_else(hydra.lib.maps.null(edge.properties), (lambda : Right(Nothing())), (lambda : hydra.lib.eithers.map((lambda p: Just(("properties", cast(hydra.json.model.Value, hydra.json.model.ValueObject(hydra.lib.maps.from_list(p)))))), hydra.lib.eithers.map_list((lambda pair: (key := hydra.lib.pairs.first(pair), v := hydra.lib.pairs.second(pair), hydra.lib.eithers.bind(schema.property_values.decode(cx, v), (lambda term2: Right((key.value, cast(hydra.json.model.Value, hydra.json.model.ValueString(hydra.show.core.term(term2))))))))[2]), hydra.lib.maps.to_list(edge.properties)))))))[1])))))))(v1.value) if isinstance(v1, hydra.pg.model.ElementEdge) else hydra.dsl.python.unsupported("no matching case in inline union elimination"))(el)
