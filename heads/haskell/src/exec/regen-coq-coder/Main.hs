-- | Temporary one-shot exec: regenerate only Hydra.Coq.Coder from its DSL source.
-- Used while iterating on packages/hydra-coq/src/main/haskell/Hydra/Sources/Coq/Coder.hs
...
main :: IO ()
main = do
  putStrLn "Regenerating only Hydra.Coq.Coder..."
  let universe = mainModules ++ hydraExtModules
  let modsToGen = [CoqCoder.module_]
