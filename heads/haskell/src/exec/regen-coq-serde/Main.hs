-- | Temporary one-shot exec: regenerate only Hydra.Coq.Serde from its DSL source.
-- Used while iterating on packages/hydra-coq/src/main/haskell/Hydra/Sources/Coq/Serde.hs
...
main :: IO ()
main = do
  putStrLn "Regenerating only Hydra.Coq.Serde..."
  let universe = mainModules ++ hydraExtModules
  let modsToGen = [CoqSerde.module_]
