-- | Temporary one-shot exec: regenerate only Hydra.Coq.Language from its DSL source.
-- Used while iterating on packages/hydra-coq/src/main/haskell/Hydra/Sources/Coq/Language.hs
...
main :: IO ()
main = do
  putStrLn "Regenerating only Hydra.Coq.Language..."
  let universe = mainModules ++ hydraExtModules
  let modsToGen = [CoqLanguage.module_]
