-- | Temporary one-shot exec: regenerate only Hydra.Coq.Environment from its DSL source.
-- Used while iterating on packages/hydra-coq/src/main/haskell/Hydra/Sources/Coq/Environment.hs
-- during the #337 DSL promotion. Delete once the Coq coder port is done.
module Main where
...
main :: IO ()
main = do
  putStrLn "Regenerating only Hydra.Coq.Environment..."
  let universe = mainModules ++ hydraExtModules
  let modsToGen = [CoqEnvironment.module_]
