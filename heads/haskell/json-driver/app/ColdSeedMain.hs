...
-- coders). moduleToLispDialect + wrapLongScalaText are inlined below (they need
-- only the published Lisp coder + kernel), so this module never imports
-- Hydra.ExtGeneration. Hydra.Go.Coder is dropped too (hydra-go is not published
-- at 0.17.1); the "go" target dispatch is removed accordingly.
...
-- ---------------------------------------------------------------------------
-- #376 cold-only seeder: helpers inlined from Hydra.ExtGeneration so this
-- module does NOT import ExtGeneration (which transitively pulls the unpublished
-- ext/coq/wasm coders). These depend only on the published Lisp coder + kernel.
-- Kept verbatim from ExtGeneration; if the originals change, mirror them here.
-- ---------------------------------------------------------------------------
...
-- Scala line-wrap constants (from Hydra.ExtGeneration).
maxLineLength, commaBreakThreshold, arrowBreakThreshold :: Int
maxLineLength = 120
