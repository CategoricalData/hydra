...
@main def bootstrap(args: String*): Unit =
  ...
  val fileCount = tgt match
    ...
    case "typescript" => Generation.writeTypeScript(outMain + "/typescript", allMainMods, modsToGenerate)
  ...
  if includeTests then
    ...
    testFileCount = tgt match
      ...
      case "typescript" => Generation.writeTypeScript(outTest + "/typescript", allUniverse, testModsToEmit)
