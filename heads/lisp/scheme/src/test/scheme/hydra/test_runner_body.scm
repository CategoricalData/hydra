...
(define (t-match type-name default . case-fields)
  ...
  (list 'function (list 'elimination
    (list 'union (make-hydra_core_model_case_statement type-name default
                   (map (lambda (f)
  ...
  (let* ((native-graph graph)
         ...
         (apply-args (lambda (impl-term args)
                       (let loop ((f impl-term) (as args))
                         (if (null? as) f
                             (loop (list 'application (make-hydra_core_model_application f (car as)))
                                   (cdr as))))))
         (prims-list (hydra_overlay_scheme_lib_maps_to_list (hydra_core_graph_graph-primitives native-graph)))
         (patched-list
           (map (lambda (entry)
                  (let* ((name (car entry))
                         (prim (cdr entry))
                         (di (hydra_core_packaging_primitive_definition-default_implementation
                               (hydra_core_graph_primitive-definition prim))))
                                ...
                                (lambda (_g)
                                  (lambda (args)
                                    (let ((applied (apply-args impl-term args)))
                                      ((((hydra_core_reduction_reduce_term cx) native-graph) #t) applied))))))))))
                prims-list))
         (patched-map (hydra_overlay_scheme_lib_maps_from_list patched-list)))
    (make-hydra_core_graph_graph
      (hydra_core_graph_graph-bound_terms graph)
      (hydra_core_graph_graph-bound_types graph)
      (hydra_core_graph_graph-class_constraints graph)
      (hydra_core_graph_graph-lambda_variables graph)
      (hydra_core_graph_graph-metadata graph)
      patched-map
      (hydra_core_graph_graph-schema_types graph)
      (hydra_core_graph_graph-type_variables graph))))
...
(define (run-string-comparison-test path expected-str actual-str)
  ...
  (guard (exn (#t
               ...
               (newline)
               (list 0 1 0)))
    (let ((e (if (string? expected-str) expected-str (if expected-str (obj->string expected-str) "#f")))
          (a (if (string? actual-str) actual-str (if actual-str (obj->string actual-str) "#f"))))
      (if (equal? e a)
          (list 1 0 0)
          (let ((e-norm (normalize-type-var-names e))
                (a-norm (normalize-type-var-names a)))
            (if (equal? e-norm a-norm)
                (list 1 0 0)
                (begin
                  (display (string-append "FAIL: " path "\n"))
                  (display (string-append "  Expected: " e "\n"))
                  (display (string-append "  Actual:   " a "\n"))
                (list 0 1 0))))))))
      ...
      (let ((result (((hydra_core_inference_infer_type_of cx) graph)
        ...
        (if (eq? (car result) 'left)
            ...
            (let* ((pair-val (cadr result))
                   ...
                   (expected-str (guard (exn (#t #f)) (show-type-scheme expected-ts)))
                   (actual-str (guard (exn (#t #f)) (show-type-scheme result-scheme))))
...
(define (run-validate-core-term-test path tc)
  (guard (exn (#t (list 0 0 1)))  ;; skip if validate module not loaded
    (let* ((graph (get-test-graph))
           ...
           (result (hydra_core_validate_model_term typed graph input)))
            ...
            (cond
              ...
              ((eq? case-type 'effectful)
               ...
               (guard (exn (#t
                            ...
                            (display (string-append "  EXCEPTION: " (obj->string exn) "\n"))
                            (list 0 1 0)))
                 (let ((actual ((hydra_core_testing_effectful_test_case-actual case-data) '()))
                       (expected ((hydra_core_testing_effectful_test_case-expected case-data) '())))
              ...
              ((eq? case-type 'universal)
               ...
               (let ((actual ((hydra_core_testing_universal_test_case-actual case-data) '()))
                     (expected ((hydra_core_testing_universal_test_case-expected case-data) '())))
                 (if (equal? actual expected)
                   (list 1 0 0)
                   (begin
                     (display (string-append "FAIL: " full "\n"))
