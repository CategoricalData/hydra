...
def test_bind__bind_add():
    ...
    assert (hydra.monads.bind(hydra.monads.pure(10), (lambda n: hydra.monads.pure(hydra.lib.math.add(n, 5)))).value(None, hydra.compute.Trace((), (), FrozenDict({})))) == (hydra.compute.FlowState(Just(15), None, hydra.compute.Trace((), (), FrozenDict({}))))
...
def test_error_traces__error_traces_are_in_the_right_order():

    assert (hydra.monads.with_trace("one", hydra.monads.with_trace("two", hydra.monads.fail("oops"))).value(None, hydra.compute.Trace((), (), FrozenDict({})))) == (hydra.compute.FlowState(Nothing(), None, hydra.compute.Trace((), ("Error: oops (one > two)",), FrozenDict({}))))
