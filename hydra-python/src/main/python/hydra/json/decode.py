...
def expect_array(value: hydra.json.model.Value) -> Either[str, frozenlist[hydra.json.model.Value]]:
    ...
    match value:
        case hydra.json.model.ValueArray(value=arr):
            return cast(Either[str, frozenlist[hydra.json.model.Value]], Right(arr))
        
        case _:
            return cast(Either[str, frozenlist[hydra.json.model.Value]], Left("expected array"))
...
def expect_object(value: hydra.json.model.Value) -> Either[str, FrozenDict[str, hydra.json.model.Value]]:
    ...
    match value:
        case hydra.json.model.ValueObject(value=obj):
            return cast(Either[str, FrozenDict[str, hydra.json.model.Value]], Right(obj))
        
        case _:
            return cast(Either[str, FrozenDict[str, hydra.json.model.Value]], Left("expected object"))
    ...
    match stripped:
        ...
        case hydra.core.TypeMaybe(value=inner_type):
            def decode_just(arr: frozenlist[hydra.json.model.Value]) -> Either[str, hydra.core.Term]:
                return hydra.lib.eithers.map((lambda v: cast(hydra.core.Term, hydra.core.TermMaybe(cast(Maybe[hydra.core.Term], Just(v))))), from_json(types, inner_type, hydra.lib.lists.head(arr)))
            def decode_maybe_array(arr: frozenlist[hydra.json.model.Value]) -> Either[str, hydra.core.Term]:
                def len() -> int:
                    return hydra.lib.lists.length(arr)
                return hydra.lib.logic.if_else(hydra.lib.equality.equal(len(), 0), (lambda : cast(Either[str, hydra.core.Term], Right(cast(hydra.core.Term, hydra.core.TermMaybe(cast(Maybe[hydra.core.Term], Nothing())))))), (lambda : hydra.lib.logic.if_else(hydra.lib.equality.equal(len(), 1), (lambda : decode_just(arr)), (lambda : cast(Either[str, hydra.core.Term], Left("expected single-element array for Just"))))))
            match value:
        ...
        case hydra.core.TypeUnion(value=rt2):
            ...
            def try_field(key: str, val: Maybe[hydra.json.model.Value], ft: hydra.core.FieldType) -> Maybe[Either[str, hydra.core.Term]]:
                return hydra.lib.logic.if_else(hydra.lib.equality.equal(ft.name.value, key), (lambda : cast(Maybe[Either[str, hydra.core.Term]], Just(decode_variant(key, val, ft.type)))), (lambda : cast(Maybe[Either[str, hydra.core.Term]], Nothing())))
            def find_and_decode(key: str, val: Maybe[hydra.json.model.Value], fts: frozenlist[hydra.core.FieldType]) -> Either[str, hydra.core.Term]:
                return hydra.lib.logic.if_else(hydra.lib.lists.null(fts), (lambda : cast(Either[str, hydra.core.Term], Left(hydra.lib.strings.cat(("unknown variant: ", key))))), (lambda : hydra.lib.maybes.maybe(find_and_decode(key, val, hydra.lib.lists.tail(fts)), (lambda r: r), try_field(key, val, hydra.lib.lists.head(fts)))))
            ...
            def process_union(obj: FrozenDict[str, hydra.json.model.Value]) -> Either[str, hydra.core.Term]:
                return hydra.lib.logic.if_else(hydra.lib.equality.equal(hydra.lib.lists.length(hydra.lib.maps.keys(obj)), 1), (lambda : decode_single_key(obj)), (lambda : cast(Either[str, hydra.core.Term], Left("expected single-key object for union"))))
            def obj_result() -> Either[str, FrozenDict[str, hydra.json.model.Value]]:
                return expect_object(value)
            return hydra.lib.eithers.either((lambda err: cast(Either[str, hydra.core.Term], Left(err))), (lambda obj: process_union(obj)), obj_result())
        ...
        case hydra.core.TypeWrap(value=wn):
            ...
            return hydra.lib.maybes.maybe(cast(Either[str, hydra.core.Term], Left(hydra.lib.strings.cat(("unknown wrapped type: ", wn.type_name.value)))), (lambda lt: decode_and_wrap(lt)), looked_up())
