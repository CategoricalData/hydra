...
def vertex(cx: hydra.graph.Graph, raw: hydra.core.Term) -> Either[hydra.util.DecodingError, int]:
    ...
    return hydra.lib.eithers.either((lambda err: cast(Either[hydra.util.DecodingError, int], Left(hydra.util.DecodingError(err)))), (lambda stripped: _hoist_hydra_decode_topology_vertex_3(stripped)), hydra.lexical.strip_and_dereference_term_either(cx, raw))
