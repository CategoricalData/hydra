...
def infer_many[T0](cx: hydra.typing.InferenceContext, pairs: frozenlist[Tuple[hydra.core.Term, str]]) -> hydra.compute.Flow[T0, Tuple[frozenlist[hydra.core.Term], Tuple[frozenlist[hydra.core.Type], hydra.typing.TypeSubst]]]:
    ...
    return hydra.lib.logic.if_else(hydra.lib.lists.null(pairs), hydra.lib.flows.pure((cast(frozenlist[hydra.core.Term], ()), (cast(frozenlist[hydra.core.Type], ()), hydra.substitution.id_type_subst))), dflt())
...
def infer_type_of_let[T0](cx: hydra.typing.InferenceContext, let0: hydra.core.Let) -> hydra.compute.Flow[T0, hydra.typing.InferenceResult]:
    ...
    def restore_let(iterm: hydra.core.Term) -> hydra.core.Term:
        ...
        e = result[1]
        binding_map2 = cast(FrozenDict[hydra.core.Name, hydra.core.Binding], hydra.lib.maps.from_list(hydra.lib.lists.map((lambda b: (b.name, b)), binding_list)))
...
def infer_type_of_map[T0](cx: hydra.typing.InferenceContext, m: FrozenDict[hydra.core.Term, hydra.core.Term]) -> hydra.compute.Flow[T0, hydra.typing.InferenceResult]:
    return hydra.lib.flows.bind(cast(hydra.compute.Flow[T0, hydra.core.Name], hydra.schemas.fresh_name), (lambda kvar: hydra.lib.flows.bind(cast(hydra.compute.Flow[T0, hydra.core.Name], hydra.schemas.fresh_name), (lambda vvar: hydra.lib.logic.if_else(hydra.lib.maps.null(m), hydra.lib.flows.pure(yield_(build_type_application_term((kvar, vvar), cast(hydra.core.Term, hydra.core.TermMap(cast(FrozenDict[hydra.core.Term, hydra.core.Term], hydra.lib.maps.empty())))), cast(hydra.core.Type, hydra.core.TypeMap(hydra.core.MapType(cast(hydra.core.Type, hydra.core.TypeVariable(kvar)), cast(hydra.core.Type, hydra.core.TypeVariable(vvar))))), hydra.substitution.id_type_subst)), hydra.lib.flows.bind(infer_many(cx, hydra.lib.lists.map((lambda k: (k, "map key")), hydra.lib.maps.keys(m))), (lambda kresults: (kterms := kresults[0], ktypes := kresults[1][0], ksubst := kresults[1][1], hydra.lib.flows.bind(infer_many(cx, hydra.lib.lists.map((lambda v: (v, "map value")), hydra.lib.maps.elems(m))), (lambda vresults: (vterms := vresults[0], vtypes := vresults[1][0], vsubst := vresults[1][1], kcons := hydra.lib.lists.map((lambda t: hydra.typing.TypeConstraint(cast(hydra.core.Type, hydra.core.TypeVariable(kvar)), t, "map key")), ktypes), vcons := hydra.lib.lists.map((lambda t: hydra.typing.TypeConstraint(cast(hydra.core.Type, hydra.core.TypeVariable(vvar)), t, "map value")), vtypes), map_constraints(cx, (lambda subst: yield_(cast(hydra.core.Term, hydra.core.TermMap(cast(FrozenDict[hydra.core.Term, hydra.core.Term], hydra.lib.maps.from_list(hydra.lib.lists.zip(kterms, vterms))))), cast(hydra.core.Type, hydra.core.TypeMap(hydra.core.MapType(cast(hydra.core.Type, hydra.core.TypeVariable(kvar)), cast(hydra.core.Type, hydra.core.TypeVariable(vvar))))), hydra.substitution.compose_type_subst_list((ksubst, vsubst, subst)))), hydra.lib.lists.concat((kcons, vcons))))[5])))[3])))))))
...
def infer_types_of_temporary_bindings[T0](cx: hydra.typing.InferenceContext, bins: frozenlist[hydra.core.Binding]) -> hydra.compute.Flow[T0, Tuple[frozenlist[hydra.core.Term], Tuple[frozenlist[hydra.core.Type], hydra.typing.TypeSubst]]]:
    ...
    return hydra.lib.logic.if_else(hydra.lib.lists.null(bins), hydra.lib.flows.pure((cast(frozenlist[hydra.core.Term], ()), (cast(frozenlist[hydra.core.Type], ()), hydra.substitution.id_type_subst))), dflt())
...
def initial_type_context[T0](g: hydra.graph.Graph) -> hydra.compute.Flow[T0, hydra.typing.TypeContext]:
    def to_pair[T1](tuple2: Tuple[hydra.core.Name, hydra.core.Binding]) -> hydra.compute.Flow[T1, Tuple[hydra.core.Name, hydra.core.Type]]:
        name = tuple2[0]
        el = tuple2[1]
        return hydra.lib.maybes.maybe(hydra.lib.flows.fail(hydra.lib.strings.cat(("untyped element: ", name.value))), (lambda ts: hydra.lib.flows.pure((name, hydra.schemas.type_scheme_to_f_type(ts)))), el.type)
