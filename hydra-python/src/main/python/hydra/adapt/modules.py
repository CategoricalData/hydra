...
def adapted_module_definitions[T0](lang: hydra.coders.Language, mod: hydra.module.Module) -> hydra.compute.Flow[hydra.graph.Graph, frozenlist[hydra.module.Definition]]:
    ...
    def classify[T1, T2](adapters: FrozenDict[hydra.core.Type, hydra.compute.Adapter[hydra.graph.Graph, T1, T2, hydra.core.Type, hydra.core.Term, hydra.core.Term]], pair: Tuple[hydra.core.Binding, hydra.core.TypeApplicationTerm]) -> hydra.compute.Flow[hydra.graph.Graph, hydra.module.Definition]:
        ...
        typ = tt.type
        name = el.name
        return hydra.lib.logic.if_else(hydra.annotations.is_native_type(el), hydra.lib.flows.bind(hydra.lib.flows.bind(hydra.decode.core.type(term), (lambda core_typ: adapt_type_to_language(lang, core_typ))), (lambda adapted_typ: hydra.lib.flows.pure(cast(hydra.module.Definition, hydra.module.DefinitionType(hydra.module.TypeDefinition(name, adapted_typ)))))), hydra.lib.optionals.maybe(hydra.lib.flows.fail(hydra.lib.strings.cat2("no adapter for element ", name.value)), (lambda adapter: hydra.lib.flows.bind(adapter.coder.encode(term), (lambda adapted: hydra.lib.flows.pure(cast(hydra.module.Definition, hydra.module.DefinitionTerm(hydra.module.TermDefinition(name, adapted, adapter.target))))))), hydra.lib.maps.lookup(typ, adapters)))
