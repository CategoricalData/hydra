...
def dependency_namespaces(binds: bool, with_prims: bool, with_noms: bool, with_schema: bool, els: frozenlist[hydra.core.Binding]) -> hydra.compute.Flow[hydra.graph.Graph, frozenset[hydra.module.Namespace]]:
    ...
    return hydra.lib.flows.bind(cast(hydra.compute.Flow[hydra.graph.Graph, hydra.graph.Graph], hydra.monads.get_state()), (lambda cx: (dep_names := (lambda el: (term := el.term, data_names := hydra.rewriting.term_dependency_names(binds, with_prims, with_noms, term), schema_names := (lambda : hydra.lib.logic.if_else(with_schema, (lambda : hydra.lib.maybes.maybe(cast(frozenset[hydra.core.Name], hydra.lib.sets.empty()), (lambda ts: hydra.rewriting.type_dependency_names(True, ts.type)), el.type)), (lambda : cast(frozenset[hydra.core.Name], hydra.lib.sets.empty())))), hydra.lib.logic.if_else(is_encoded_type(hydra.rewriting.deannotate_term(term)), (lambda : hydra.lib.flows.bind(hydra.monads.with_trace("dependency namespace", hydra.monads.either_to_flow((lambda v1: v1.value), hydra.decode.core.type(cx, term))), (lambda typ: hydra.lib.flows.pure(hydra.lib.sets.unions((data_names, schema_names(), hydra.rewriting.type_dependency_names(True, typ))))))), (lambda : hydra.lib.flows.pure(hydra.lib.sets.unions((data_names, schema_names()))))))[3]), hydra.lib.flows.bind(hydra.lib.flows.map_list(cast(Callable[[hydra.core.Binding], hydra.compute.Flow[hydra.graph.Graph, frozenset[hydra.core.Name]]], (lambda x1: dep_names(x1))), els), (lambda names_list: hydra.lib.flows.pure(hydra.lib.sets.from_list(hydra.lib.maybes.cat(hydra.lib.lists.map(hydra.names.namespace_of, hydra.lib.sets.to_list(hydra.lib.sets.delete(hydra.constants.placeholder_name, hydra.lib.sets.unions(names_list))))))))))[1]))
...
def f_type_to_type_scheme(typ: hydra.core.Type) -> hydra.core.Type:
    ...
    def gather_forall(vars: frozenlist[hydra.core.Name], typ2: hydra.core.Type) -> hydra.core.Type:
        match hydra.rewriting.deannotate_type(typ2):
            ...
            case _:
                return hydra.core.TypeScheme(hydra.lib.lists.reverse(vars), typ2, cast(Maybe[FrozenDict[hydra.core.Name, hydra.core.TypeVariableMetadata]], Nothing()))
...
def field_types(t: hydra.core.Type) -> hydra.compute.Flow[hydra.graph.Graph, FrozenDict[hydra.core.Name, hydra.core.Type]]:
    ...
    return hydra.lib.flows.bind(cast(hydra.compute.Flow[hydra.graph.Graph, hydra.graph.Graph], hydra.monads.get_state()), (lambda cx: (to_map := (lambda fields: cast(FrozenDict[hydra.core.Name, hydra.core.Type], hydra.lib.maps.from_list(hydra.lib.lists.map((lambda ft: cast(tuple[hydra.core.Name, hydra.core.Type], (ft.name, ft.type))), fields)))), "inline match expressions are unsupported")[1]))
...
def graph_as_types(sg: hydra.graph.Graph) -> hydra.compute.Flow[hydra.graph.Graph, FrozenDict[hydra.core.Name, hydra.core.Type]]:
    ...
    return hydra.lib.flows.bind(cast(hydra.compute.Flow[hydra.graph.Graph, hydra.graph.Graph], hydra.monads.get_state()), (lambda cx: (els := (lambda : hydra.lib.maps.elems(sg.elements)), to_pair := (lambda el: hydra.lib.flows.bind(hydra.monads.with_trace(hydra.lib.strings.cat2("graph as types: ", el.name.value), hydra.monads.either_to_flow((lambda v1: v1.value), hydra.decode.core.type(cx, el.term))), (lambda typ: hydra.lib.flows.pure(cast(tuple[hydra.core.Name, hydra.core.Type], (el.name, typ)))))), hydra.lib.flows.bind(hydra.lib.flows.map_list(cast(Callable[[hydra.core.Binding], hydra.compute.Flow[hydra.graph.Graph, tuple[hydra.core.Name, hydra.core.Type]]], (lambda x1: to_pair(x1))), els()), (lambda pairs: hydra.lib.flows.pure(cast(FrozenDict[hydra.core.Name, hydra.core.Type], hydra.lib.maps.from_list(pairs))))))[2]))
...
def schema_graph_to_typing_environment(g: hydra.graph.Graph) -> hydra.compute.Flow[T0, FrozenDict[hydra.core.Name, hydra.core.TypeScheme]]:
    def to_type_scheme(vars: frozenlist[hydra.core.Name], typ: hydra.core.Type) -> hydra.core.Type:
        match hydra.rewriting.deannotate_type(typ):
            ...
            case _:
                return hydra.core.TypeScheme(hydra.lib.lists.reverse(vars), typ, cast(Maybe[FrozenDict[hydra.core.Name, hydra.core.TypeVariableMetadata]], Nothing()))
    def to_pair(el: hydra.core.Binding) -> hydra.compute.Flow[hydra.graph.Graph, Maybe[tuple[hydra.core.Name, hydra.core.TypeScheme]]]:
        return hydra.lib.flows.bind(cast(hydra.compute.Flow[hydra.graph.Graph, hydra.graph.Graph], hydra.monads.get_state()), (lambda cx: (for_term := (lambda term: "inline match expressions are unsupported"), hydra.lib.flows.bind(hydra.lib.maybes.maybe(hydra.lib.flows.map((lambda typ: cast(Maybe[hydra.core.TypeScheme], Just(f_type_to_type_scheme(typ)))), hydra.monads.either_to_flow((lambda v1: v1.value), hydra.decode.core.type(cx, el.term))), (lambda ts: hydra.lib.logic.if_else(hydra.lib.equality.equal(ts, hydra.core.TypeScheme(cast(frozenlist[hydra.core.Name], ()), cast(hydra.core.Type, hydra.core.TypeVariable(hydra.core.Name("hydra.core.TypeScheme"))), cast(Maybe[FrozenDict[hydra.core.Name, hydra.core.TypeVariableMetadata]], Nothing()))), (lambda : hydra.lib.flows.map(cast(Callable[[hydra.core.TypeScheme], Maybe[hydra.core.TypeScheme]], (lambda x1: hydra.lib.maybes.pure(x1))), hydra.monads.either_to_flow((lambda v1: v1.value), hydra.decode.core.type_scheme(cx, el.term)))), (lambda : hydra.lib.logic.if_else(hydra.lib.equality.equal(ts, hydra.core.TypeScheme(cast(frozenlist[hydra.core.Name], ()), cast(hydra.core.Type, hydra.core.TypeVariable(hydra.core.Name("hydra.core.Type"))), cast(Maybe[FrozenDict[hydra.core.Name, hydra.core.TypeVariableMetadata]], Nothing()))), (lambda : hydra.lib.flows.map((lambda decoded: cast(Maybe[hydra.core.TypeScheme], Just(to_type_scheme(cast(frozenlist[hydra.core.Name], ()), decoded)))), hydra.monads.either_to_flow((lambda v1: v1.value), hydra.decode.core.type(cx, el.term)))), (lambda : for_term(hydra.rewriting.deannotate_term(el.term))))))), el.type), (lambda mts: hydra.lib.flows.pure(hydra.lib.maybes.map((lambda ts: cast(tuple[hydra.core.Name, hydra.core.TypeScheme], (el.name, ts))), mts)))))[1]))
...
def graph_to_inference_context(graph: hydra.graph.Graph) -> hydra.compute.Flow[T0, hydra.typing.InferenceContext]:
    ...
    return hydra.lib.flows.bind(schema_graph_to_typing_environment(schema()), (lambda schema_types: hydra.lib.flows.pure(hydra.typing.InferenceContext(schema_types, prim_types(), var_types(), cast(FrozenDict[hydra.core.Name, hydra.core.TypeVariableMetadata], hydra.lib.maps.empty()), False))))
...
def instantiate_type_scheme(scheme: hydra.core.TypeScheme) -> hydra.compute.Flow[T0, hydra.core.TypeScheme]:
    ...
    return hydra.lib.flows.bind(fresh_names(hydra.lib.lists.length(old_vars)), (lambda new_vars: (subst := (lambda : hydra.typing.TypeSubst(cast(FrozenDict[hydra.core.Name, hydra.core.Type], hydra.lib.maps.from_list(hydra.lib.lists.zip(old_vars, hydra.lib.lists.map((lambda x: cast(hydra.core.Type, hydra.core.TypeVariable(x))), new_vars)))))), name_subst := (lambda : cast(FrozenDict[hydra.core.Name, hydra.core.Name], hydra.lib.maps.from_list(hydra.lib.lists.zip(old_vars, new_vars)))), renamed_constraints := (lambda : hydra.lib.maybes.map((lambda old_constraints: cast(FrozenDict[hydra.core.Name, hydra.core.TypeVariableMetadata], hydra.lib.maps.from_list(hydra.lib.lists.map((lambda kv: cast(tuple[hydra.core.Name, hydra.core.TypeVariableMetadata], (hydra.lib.maybes.from_maybe(hydra.lib.pairs.first(kv), hydra.lib.maps.lookup(hydra.lib.pairs.first(kv), name_subst())), hydra.lib.pairs.second(kv)))), hydra.lib.maps.to_list(old_constraints))))), scheme.constraints)), hydra.lib.flows.pure(hydra.core.TypeScheme(new_vars, hydra.substitution.subst_in_type(subst(), scheme.type), renamed_constraints())))[3]))
...
def type_to_type_scheme(t0: hydra.core.Type) -> hydra.core.Type:
    ...
    def helper(vars: frozenlist[hydra.core.Name], t: hydra.core.Type) -> hydra.core.Type:
        match hydra.rewriting.deannotate_type(t):
            ...
            case _:
                return hydra.core.TypeScheme(hydra.lib.lists.reverse(vars), t, cast(Maybe[FrozenDict[hydra.core.Name, hydra.core.TypeVariableMetadata]], Nothing()))
...
def resolve_type(typ: hydra.core.Type) -> hydra.compute.Flow[hydra.graph.Graph, Maybe[hydra.core.Type]]:
    ...
    return hydra.lib.flows.bind(cast(hydra.compute.Flow[hydra.graph.Graph, hydra.graph.Graph], hydra.monads.get_state()), (lambda cx: "inline match expressions are unsupported"))
