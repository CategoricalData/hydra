...
def make_elements(omit_trivial: bool, ns: hydra.module.Namespace, lname: str, pat: hydra.grammar.Pattern) -> frozenlist[Tuple[str, hydra.core.Type]]:
    ...
    trivial = hydra.lib.logic.if_else(omit_trivial, (), ((lname, hydra.core.TypeUnit(None)),))
    def descend[T0](n: str, f: Callable[[frozenlist[Tuple[str, hydra.core.Type]]], T0], p: hydra.grammar.Pattern) -> T0:
        ...
        return f(hydra.lib.logic.if_else(is_complex(p), hydra.lib.lists.cons((lname, hydra.core.TypeVariable(to_name(ns, hydra.lib.lists.head(cpairs)[0]))), cpairs), hydra.lib.logic.if_else(hydra.lib.lists.null(cpairs), ((lname, hydra.core.TypeUnit(None)),), hydra.lib.lists.cons((lname, hydra.lib.lists.head(cpairs)[1]), hydra.lib.lists.tail(cpairs)))))
    ...
    def for_pat(pat2: hydra.grammar.Pattern) -> frozenlist[Tuple[str, hydra.core.Type]]:
        match pat2:
            case hydra.grammar.PatternAlternatives(value=pats):
                return for_record_or_union(False, lambda fields: hydra.core.TypeUnion(hydra.core.RowType(hydra.constants.placeholder_name, fields)), pats)
            ...
            case hydra.grammar.PatternNonterminal(value=s):
                return ((lname, hydra.core.TypeVariable(to_name(ns, s.value))),)
            
            case hydra.grammar.PatternOption(value=p):
                return mod("Option", lambda x: hydra.core.TypeOptional(x), p)
            
            case hydra.grammar.PatternPlus(value=p2):
                return mod("Elmt", lambda x: hydra.core.TypeList(x), p2)
            
            case hydra.grammar.PatternRegex():
                return ((lname, hydra.core.TypeLiteral(hydra.core.LiteralTypeString(None))),)
            
            case hydra.grammar.PatternSequence(value=pats2):
                return for_record_or_union(True, lambda fields: hydra.core.TypeRecord(hydra.core.RowType(hydra.constants.placeholder_name, fields)), pats2)
            
            case hydra.grammar.PatternStar(value=p3):
                return mod("Elmt", lambda x: hydra.core.TypeList(x), p3)
...
def wrap_type(t: hydra.core.Type) -> hydra.core.Type:
    ...
    match t:
        ...
        case _:
            return hydra.core.TypeWrap(hydra.core.WrappedType(hydra.core.Name("Placeholder"), t))
