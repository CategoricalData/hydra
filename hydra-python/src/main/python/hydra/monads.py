...
def bind[T0, T1, T2](l: hydra.compute.Flow[T0, T1], r: Callable[[T1], hydra.compute.Flow[T0, T2]]) -> hydra.compute.Flow[T0, T2]:
    def q(s0: T0, t0: hydra.compute.Trace) -> hydra.compute.FlowState[T0, T2]:
        fs1 = l.value(s0, t0)
        return hydra.lib.optionals.maybe(hydra.compute.FlowState(Nothing(), fs1.state, fs1.trace), lambda v: r(v).value(fs1.state, fs1.trace), fs1.value)
...
get_state = hydra.compute.Flow(lambda s0, t0: (fs1 := pure(None).value(s0, t0), v := fs1.value, s := fs1.state, t := fs1.trace, hydra.lib.optionals.maybe(hydra.compute.FlowState(Nothing(), s, t), lambda _: hydra.compute.FlowState(Just(s), s, t), v))[4])

def map[T0, T1, T2](f: Callable[[T0], T1], f1: hydra.compute.Flow[T2, T0]) -> hydra.compute.Flow[T2, T1]:
    return hydra.compute.Flow(lambda s0, t0: (f2 := f1.value(s0, t0), hydra.compute.FlowState(hydra.lib.optionals.map(f, f2.value), f2.state, f2.trace))[1])
...
def mutate_trace[T0, T1](mutate: Callable[[hydra.compute.Trace], hydra.mantle.Either[str, hydra.compute.Trace]], restore: Callable[[hydra.compute.Trace, hydra.compute.Trace], hydra.compute.Trace], f: hydra.compute.Flow[T0, T1]) -> hydra.compute.Flow[T0, T1]:
    ...
    return hydra.compute.Flow(lambda s0, t0: (for_left := lambda msg: hydra.compute.FlowState(Nothing(), s0, push_error(msg, t0)), for_right := lambda t1: (f2 := f.value(s0, t1), hydra.compute.FlowState(f2.value, f2.state, restore(t0, f2.trace)))[1], choose(for_left, for_right, mutate(t0)))[2])

def optional_to_list[T0](mx: Maybe[T0]) -> frozenlist[T0]:
    return hydra.lib.optionals.maybe((), lambda v1: hydra.lib.lists.pure(v1), mx)
...
def warn[T0, T1](msg: str, b: hydra.compute.Flow[T0, T1]) -> hydra.compute.Flow[T0, T1]:
    return hydra.compute.Flow(lambda s0, t0: (f1 := b.value(s0, t0), add_message := lambda t: hydra.compute.Trace(t.stack, hydra.lib.lists.cons(hydra.lib.strings.cat(("Warning: ", msg)), t.messages), t.other), hydra.compute.FlowState(f1.value, f1.state, add_message(f1.trace)))[2])
...
def with_flag[T0, T1](flag: hydra.core.Name, f: hydra.compute.Flow[T0, T1]) -> hydra.compute.Flow[T0, T1]:
    def mutate(t: hydra.compute.Trace) -> hydra.mantle.Either[str, hydra.compute.Trace]:
        return hydra.lib.logic.if_else(False, hydra.mantle.EitherLeft("never happens"), hydra.mantle.EitherRight(hydra.compute.Trace(t.stack, t.messages, hydra.lib.maps.insert(flag, hydra.core.TermLiteral(hydra.core.LiteralBoolean(True)), t.other))))
...
def with_trace[T0, T1](msg: str, f: hydra.compute.Flow[T0, T1]) -> hydra.compute.Flow[T0, T1]:
    def mutate(t: hydra.compute.Trace) -> hydra.mantle.Either[str, hydra.compute.Trace]:
        return hydra.lib.logic.if_else(hydra.lib.equality.gte(hydra.lib.lists.length(t.stack), hydra.constants.max_trace_depth), hydra.mantle.EitherLeft("maximum trace depth exceeded. This may indicate an infinite loop"), hydra.mantle.EitherRight(hydra.compute.Trace(hydra.lib.lists.cons(msg, t.stack), t.messages, t.other)))
