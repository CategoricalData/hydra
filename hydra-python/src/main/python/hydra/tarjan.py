...
def adjacency_lists_to_graph(edges0: frozenlist[tuple[T0, frozenlist[T0]]]) -> tuple[FrozenDict[int, frozenlist[int]], Callable[[int], T0]]:
    ...
    def key_to_vertex() -> FrozenDict[T0, int]:
        return hydra.lib.maps.from_list(hydra.lib.lists.map((lambda vk_neighbors: (v := (lambda : hydra.lib.pairs.first(vk_neighbors)), k_neighbors := (lambda : hydra.lib.pairs.second(vk_neighbors)), k := (lambda : hydra.lib.pairs.first(k_neighbors())), (k(), v()))[3]), indexed_edges()))
    def vertex_map() -> FrozenDict[int, T0]:
        return hydra.lib.maps.from_list(hydra.lib.lists.map((lambda vk_neighbors: (v := (lambda : hydra.lib.pairs.first(vk_neighbors)), k_neighbors := (lambda : hydra.lib.pairs.second(vk_neighbors)), k := (lambda : hydra.lib.pairs.first(k_neighbors())), (v(), k()))[3]), indexed_edges()))
    def graph() -> FrozenDict[int, frozenlist[int]]:
        return hydra.lib.maps.from_list(hydra.lib.lists.map((lambda vk_neighbors: (v := (lambda : hydra.lib.pairs.first(vk_neighbors)), k_neighbors := (lambda : hydra.lib.pairs.second(vk_neighbors)), neighbors := (lambda : hydra.lib.pairs.second(k_neighbors())), (v(), hydra.lib.maybes.map_maybe((lambda k: hydra.lib.maps.lookup(k, key_to_vertex())), neighbors())))[3]), indexed_edges()))
...
def pop_stack_until(v: int) -> hydra.compute.Flow[hydra.topology.TarjanState, frozenlist[int]]:
    ...
    def go(acc: frozenlist[int]) -> hydra.compute.Flow[hydra.topology.TarjanState, frozenlist[int]]:
        ...
        return hydra.lib.flows.bind(hydra.monads.get_state(), (lambda st: hydra.lib.logic.if_else(hydra.lib.lists.null(st.stack), (lambda : hydra.lib.flows.fail("popStackUntil: empty stack")), (lambda : succeed(st)))))
    return go(())
