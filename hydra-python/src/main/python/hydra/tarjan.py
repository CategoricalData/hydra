...
def adjacency_lists_to_graph[T0](edges0: frozenlist[tuple[T0, frozenlist[T0]]]) -> tuple[FrozenDict[int, frozenlist[int]], Callable[[int], T0]]:
    def sorted_edges() -> frozenlist[tuple[T0, frozenlist[T0]]]:
        return hydra.lib.lists.sort_on(cast(Callable[[tuple[T0, frozenlist[T0]]], T0], (lambda x1: hydra.lib.pairs.first(x1))), edges0)
    ...
    def key_to_vertex() -> FrozenDict[T0, int]:
        return cast(FrozenDict[T0, int], hydra.lib.maps.from_list(hydra.lib.lists.map((lambda vk_neighbors: (v := (lambda : hydra.lib.pairs.first(vk_neighbors)), k_neighbors := (lambda : hydra.lib.pairs.second(vk_neighbors)), k := (lambda : hydra.lib.pairs.first(k_neighbors())), cast(tuple[T0, int], (k(), v())))[3]), indexed_edges())))
    def vertex_map() -> FrozenDict[int, T0]:
        return cast(FrozenDict[int, T0], hydra.lib.maps.from_list(hydra.lib.lists.map((lambda vk_neighbors: (v := (lambda : hydra.lib.pairs.first(vk_neighbors)), k_neighbors := (lambda : hydra.lib.pairs.second(vk_neighbors)), k := (lambda : hydra.lib.pairs.first(k_neighbors())), cast(tuple[int, T0], (v(), k())))[3]), indexed_edges())))
    def graph() -> FrozenDict[int, frozenlist[int]]:
        return cast(FrozenDict[int, frozenlist[int]], hydra.lib.maps.from_list(hydra.lib.lists.map((lambda vk_neighbors: (v := (lambda : hydra.lib.pairs.first(vk_neighbors)), k_neighbors := (lambda : hydra.lib.pairs.second(vk_neighbors)), neighbors := (lambda : hydra.lib.pairs.second(k_neighbors())), cast(tuple[int, frozenlist[int]], (v(), hydra.lib.maybes.map_maybe((lambda k: hydra.lib.maps.lookup(k, key_to_vertex())), neighbors()))))[3]), indexed_edges())))
...
def strong_connect(graph: FrozenDict[int, frozenlist[int]], v: int) -> hydra.compute.Flow[hydra.topology.TarjanState, None]:
    ...
    return hydra.lib.flows.bind(cast(hydra.compute.Flow[hydra.topology.TarjanState, hydra.topology.TarjanState], hydra.monads.get_state()), (lambda st: (i := st.counter, new_st := (lambda : hydra.topology.TarjanState(hydra.lib.math.add(i, 1), hydra.lib.maps.insert(v, i, st.indices), hydra.lib.maps.insert(v, i, st.low_links), hydra.lib.lists.cons(v, st.stack), hydra.lib.sets.insert(v, st.on_stack), st.sccs)), neighbors := (lambda : hydra.lib.maps.find_with_default(cast(frozenlist[int], ()), v, graph)), process_neighbor := (lambda w: (low_link := (lambda st_: (low_v := (lambda : hydra.lib.maps.find_with_default(hydra.constants.max_int32, v, st_.low_links)), idx_w := (lambda : hydra.lib.maps.find_with_default(hydra.constants.max_int32, w, st_.indices)), hydra.lib.flows.bind(hydra.monads.modify((lambda s: hydra.topology.TarjanState(s.counter, s.indices, hydra.lib.maps.insert(v, hydra.lib.equality.min(low_v(), idx_w()), s.low_links), s.stack, s.on_stack, s.sccs))), (lambda _: hydra.lib.flows.pure(None))))[2]), hydra.lib.flows.bind(cast(hydra.compute.Flow[hydra.topology.TarjanState, hydra.topology.TarjanState], hydra.monads.get_state()), (lambda st_: hydra.lib.logic.if_else(hydra.lib.logic.not_(hydra.lib.maps.member(w, st_.indices)), (lambda : hydra.lib.flows.bind(strong_connect(graph, w), (lambda _: hydra.lib.flows.bind(cast(hydra.compute.Flow[hydra.topology.TarjanState, hydra.topology.TarjanState], hydra.monads.get_state()), (lambda st_after: (low_v := (lambda : hydra.lib.maps.find_with_default(hydra.constants.max_int32, v, st_after.low_links)), low_w := (lambda : hydra.lib.maps.find_with_default(hydra.constants.max_int32, w, st_after.low_links)), hydra.lib.flows.bind(hydra.monads.modify((lambda s: hydra.topology.TarjanState(s.counter, s.indices, hydra.lib.maps.insert(v, hydra.lib.equality.min(low_v(), low_w()), s.low_links), s.stack, s.on_stack, s.sccs))), (lambda _2: hydra.lib.flows.pure(None))))[2]))))), (lambda : hydra.lib.logic.if_else(hydra.lib.sets.member(w, st_.on_stack), (lambda : low_link(st_)), (lambda : hydra.lib.flows.pure(None))))))))[1]), hydra.lib.flows.bind(hydra.monads.put_state(new_st()), (lambda _: hydra.lib.flows.bind(hydra.lib.flows.map_list(process_neighbor, neighbors()), (lambda _2: hydra.lib.flows.bind(cast(hydra.compute.Flow[hydra.topology.TarjanState, hydra.topology.TarjanState], hydra.monads.get_state()), (lambda st_final: (low_v := (lambda : hydra.lib.maps.find_with_default(hydra.constants.max_int32, v, st_final.low_links)), idx_v := (lambda : hydra.lib.maps.find_with_default(hydra.constants.max_int32, v, st_final.indices)), hydra.lib.logic.if_else(hydra.lib.equality.equal(low_v(), idx_v()), (lambda : hydra.lib.flows.bind(pop_stack_until(v), (lambda comp: hydra.lib.flows.bind(hydra.monads.modify((lambda s: hydra.topology.TarjanState(s.counter, s.indices, s.low_links, s.stack, s.on_stack, hydra.lib.lists.cons(comp, s.sccs)))), (lambda _3: hydra.lib.flows.pure(None)))))), (lambda : hydra.lib.flows.pure(None))))[2])))))))[4]))
