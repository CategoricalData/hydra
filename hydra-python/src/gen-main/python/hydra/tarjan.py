...
def pop_stack_until(v: hydra.topology.Vertex, st0: hydra.topology.TarjanState) -> tuple[frozenlist[hydra.topology.Vertex], hydra.topology.TarjanState]:
    r"""Pop vertices off the stack until the given vertex is reached, collecting the current strongly connected component."""

    def go(acc: frozenlist[hydra.topology.Vertex], st: hydra.topology.TarjanState) -> tuple[frozenlist[hydra.topology.Vertex], hydra.topology.TarjanState]:
        @lru_cache(1)
        def x() -> hydra.topology.Vertex:
            return hydra.lib.lists.head(st.stack)
        @lru_cache(1)
        def xs() -> frozenlist[hydra.topology.Vertex]:
        ...
        @lru_cache(1)
        def acc_() -> frozenlist[hydra.topology.Vertex]:
...
def strong_connect(graph: FrozenDict[hydra.topology.Vertex, frozenlist[hydra.topology.Vertex]], v: hydra.topology.Vertex, st: hydra.topology.TarjanState) -> hydra.topology.TarjanState:
    ...
    @lru_cache(1)
    def neighbors() -> frozenlist[hydra.topology.Vertex]:
        return hydra.lib.maps.find_with_default((), v, graph)
    def process_neighbor(st_: hydra.topology.TarjanState, w: hydra.topology.Vertex) -> hydra.topology.TarjanState:
...
def strongly_connected_components(graph: FrozenDict[hydra.topology.Vertex, frozenlist[hydra.topology.Vertex]]) -> frozenlist[frozenlist[hydra.topology.Vertex]]:
    ...
    @lru_cache(1)
    def verts() -> frozenlist[hydra.topology.Vertex]:
