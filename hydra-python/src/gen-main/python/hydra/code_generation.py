...
def build_schema_map(g: hydra.graph.Graph) -> FrozenDict[hydra.core.Name, hydra.core.Type]:
    r"""Build a schema map (Name -> Type) from a graph's schema."""
    
    return hydra.lib.maybes.maybe(hydra.lib.maps.empty(), (lambda schema_graph: hydra.lib.maps.from_list(hydra.lib.maybes.cat(hydra.lib.lists.map((lambda binding: (result := hydra.decode.core.type(schema_graph, binding.term), hydra.lib.eithers.either((lambda _err: Nothing()), (lambda typ: Just((binding.name, hydra.rewriting.deannotate_type(typ)))), result))[1]), schema_graph.elements)))), g.schema)
...
def modules_to_graph(bs_graph: hydra.graph.Graph, universe_modules: frozenlist[hydra.module.Module], modules: frozenlist[hydra.module.Module]) -> hydra.graph.Graph:
    ...
    def schema_graph() -> hydra.graph.Graph:
        return hydra.lexical.elements_to_graph(bs_graph, Nothing(), schema_elements())
    return hydra.lexical.elements_to_graph(bs_graph, Just(schema_graph()), data_elements())
...
def decode_module_from_json(bs_graph: hydra.graph.Graph, universe_modules: frozenlist[hydra.module.Module], do_strip_type_schemes: bool, json_val: hydra.json.model.Value) -> Either[str, hydra.module.Module]:
    ...
    @lru_cache(1)
    def post_process() -> Callable[[hydra.module.Module], hydra.module.Module]:
        return hydra.lib.logic.if_else(do_strip_type_schemes, (lambda : (lambda m: strip_module_type_schemes(m))), (lambda : (lambda m: m)))
    return hydra.lib.eithers.either((lambda err: Left(err)), (lambda term: hydra.lib.eithers.either((lambda dec_err: Left(dec_err.value)), (lambda mod: Right(post_process(mod))), hydra.decode.module.module(graph(), term))), hydra.json.decode.from_json(schema_map(), mod_type, json_val))
...
def format_type_binding(binding: hydra.core.Binding) -> hydra.compute.Flow[hydra.graph.Graph, str]:
    r"""Format a type binding for the lexicon."""
    
    return hydra.lib.flows.bind(hydra.monads.get_state(), (lambda g: hydra.lib.flows.bind(hydra.monads.either_to_flow((lambda v1: v1.value), hydra.decode.core.type(g, binding.term)), (lambda typ: hydra.lib.flows.pure(hydra.lib.strings.cat2(hydra.lib.strings.cat2(hydra.lib.strings.cat2("  ", binding.name.value), " = "), hydra.show.core.type(typ)))))))

def generate_coder_modules(codec: Callable[[T0], hydra.compute.Flow[hydra.graph.Graph, Maybe[T1]]], bs_graph: hydra.graph.Graph, universe_modules: frozenlist[hydra.module.Module], type_modules: frozenlist[T0]) -> hydra.compute.Flow[T2, frozenlist[T1]]:
    r"""Generate encoder or decoder modules for a list of type modules."""
    
    @lru_cache(1)
    def graph() -> hydra.graph.Graph:
        return modules_to_graph(bs_graph, universe_modules, universe_modules)
    return hydra.lib.maybes.maybe(hydra.lib.flows.fail("No schema graph available"), (lambda schema_graph: hydra.monads.with_state(schema_graph, hydra.lib.flows.map((lambda results: hydra.lib.maybes.cat(results)), hydra.lib.flows.map_list(codec, type_modules)))), graph().schema)

def generate_lexicon(graph: hydra.graph.Graph) -> hydra.compute.Flow[hydra.graph.Graph, str]:
    ...
    return hydra.lib.flows.bind(hydra.lib.flows.map_list((lambda b: format_type_binding(b)), sorted_types()), (lambda type_lines: (term_lines := hydra.lib.lists.map((lambda b: format_term_binding(b)), sorted_terms()), primitive_lines := hydra.lib.lists.map((lambda p: format_primitive(p)), sorted_primitives()), hydra.lib.flows.pure(hydra.lib.strings.cat2(hydra.lib.strings.cat2(hydra.lib.strings.cat2(hydra.lib.strings.cat2(hydra.lib.strings.cat2("Primitives:\n", hydra.lib.strings.unlines(primitive_lines)), "\nTypes:\n"), hydra.lib.strings.unlines(type_lines)), "\nTerms:\n"), hydra.lib.strings.unlines(term_lines))))[2]))

def generate_source_files(print_definitions: Callable[[hydra.module.Module, frozenlist[hydra.module.Definition]], hydra.compute.Flow[hydra.graph.Graph, FrozenDict[T0, T1]]], lang: hydra.coders.Language, do_expand: bool, do_hoist_case_statements: bool, do_hoist_polymorphic_let_bindings: bool, bs_graph: hydra.graph.Graph, universe_modules: frozenlist[hydra.module.Module], mods_to_generate: frozenlist[hydra.module.Module]) -> hydra.compute.Flow[hydra.graph.Graph, frozenlist[tuple[T0, T1]]]:
    ...
    @lru_cache(1)
    def constraints() -> hydra.coders.LanguageConstraints:
        return lang.constraints
    ...
    def data_graph() -> hydra.graph.Graph:
        return hydra.lexical.elements_to_graph(bs_graph, Just(schema_graph()), data_elements())
    return hydra.lib.flows.bind(hydra.lib.logic.if_else(hydra.lib.lists.null(type_modules_to_generate()), (lambda : hydra.lib.flows.pure(())), (lambda : hydra.monads.with_trace("generate type modules", (name_lists := hydra.lib.lists.map((lambda m: hydra.lib.lists.map((lambda e: e.name), hydra.lib.lists.filter((lambda e: hydra.annotations.is_native_type(e)), m.elements))), type_modules_to_generate()), hydra.lib.flows.bind(hydra.adapt.simple.schema_graph_to_definitions(constraints(), schema_graph(), name_lists), (lambda schema_result: (def_lists := hydra.lib.pairs.second(schema_result), hydra.monads.with_state(schema_graph(), hydra.lib.flows.map((lambda xs: hydra.lib.lists.concat(xs)), hydra.lib.flows.map_list((lambda p: (mod := hydra.lib.pairs.first(p), defs := hydra.lib.pairs.second(p), hydra.monads.with_trace(hydra.lib.strings.cat2("type module ", mod.namespace.value), hydra.lib.flows.map((lambda m: hydra.lib.maps.to_list(m)), print_definitions(mod, hydra.lib.lists.map((lambda d: cast(hydra.module.Definition, hydra.module.DefinitionType(d))), defs)))))[2]), hydra.lib.lists.zip(type_modules_to_generate(), def_lists)))))[1])))[1]))), (lambda schema_files: hydra.lib.flows.bind(hydra.lib.logic.if_else(hydra.lib.lists.null(term_modules_to_generate()), (lambda : hydra.lib.flows.pure(())), (lambda : hydra.monads.with_trace("generate term modules", (namespaces := hydra.lib.lists.map((lambda m: m.namespace), term_modules_to_generate()), hydra.lib.flows.bind(hydra.adapt.simple.data_graph_to_definitions(constraints(), do_expand, do_hoist_case_statements, do_hoist_polymorphic_let_bindings, data_graph(), namespaces), (lambda data_result: (g1 := hydra.lib.pairs.first(data_result), def_lists := hydra.lib.pairs.second(data_result), refresh_module := (lambda els, m: hydra.module.Module(m.namespace, hydra.lib.maybes.cat(hydra.lib.lists.map((lambda e: hydra.lib.lists.find((lambda b: hydra.lib.equality.equal(b.name, e.name)), els)), m.elements)), m.term_dependencies, m.type_dependencies, m.description)), refreshed_mods := hydra.lib.lists.map((lambda m: refresh_module(g1.elements, m)), term_modules_to_generate()), hydra.monads.with_state(g1, hydra.lib.flows.map((lambda xs: hydra.lib.lists.concat(xs)), hydra.lib.flows.map_list((lambda p: (mod := hydra.lib.pairs.first(p), defs := hydra.lib.pairs.second(p), hydra.monads.with_trace(hydra.lib.strings.cat2("term module ", mod.namespace.value), hydra.lib.flows.map((lambda m: hydra.lib.maps.to_list(m)), print_definitions(mod, hydra.lib.lists.map((lambda d: cast(hydra.module.Definition, hydra.module.DefinitionTerm(d))), defs)))))[2]), hydra.lib.lists.zip(refreshed_mods, def_lists)))))[4])))[1]))), (lambda term_files: hydra.lib.flows.pure(hydra.lib.lists.concat2(schema_files, term_files))))))

def infer_and_generate_lexicon(bs_graph: hydra.graph.Graph, kernel_modules: frozenlist[hydra.module.Module]) -> hydra.compute.Flow[T0, str]:
    ...
    return hydra.lib.flows.bind(hydra.inference.infer_graph_types(g0()), (lambda g1: hydra.monads.with_state(g1, generate_lexicon(g1))))

def infer_modules(bs_graph: hydra.graph.Graph, universe_mods: frozenlist[hydra.module.Module], target_mods: frozenlist[hydra.module.Module]) -> hydra.compute.Flow[T0, frozenlist[hydra.module.Module]]:
    ...
    return hydra.lib.flows.bind(hydra.inference.infer_graph_types(g0()), (lambda g1: (inferred_elements := g1.elements, is_type_module := (lambda mod: hydra.lib.lists.null(hydra.lib.lists.filter((lambda e: hydra.lib.logic.not_(hydra.annotations.is_native_type(e))), mod.elements))), refresh_module := (lambda m: hydra.lib.logic.if_else(is_type_module(m), (lambda : m), (lambda : hydra.module.Module(m.namespace, hydra.lib.maybes.cat(hydra.lib.lists.map((lambda e: hydra.lib.lists.find((lambda b: hydra.lib.equality.equal(b.name, e.name)), inferred_elements)), m.elements)), m.term_dependencies, m.type_dependencies, m.description)))), hydra.lib.flows.pure(hydra.lib.lists.map((lambda x1: refresh_module(x1)), target_mods)))[3]))
