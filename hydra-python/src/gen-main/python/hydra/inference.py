...
def infer_many(fcx: hydra.context.Context, cx: hydra.graph.Graph, pairs: frozenlist[tuple[hydra.core.Term, str]]) -> Either[hydra.context.InContext[hydra.error.OtherError], tuple[tuple[frozenlist[hydra.core.Term], tuple[frozenlist[hydra.core.Type], tuple[hydra.typing.TypeSubst, FrozenDict[hydra.core.Name, hydra.core.TypeVariableMetadata]]]], hydra.context.Context]]:
...
def infer_type_of_case_statement(fcx: hydra.context.Context, cx: hydra.graph.Graph, case_stmt: hydra.core.CaseStatement) -> Either[hydra.context.InContext[hydra.error.OtherError], hydra.typing.InferenceResult]:
    ...
    return hydra.lib.eithers.bind(hydra.schemas.require_schema_type(fcx, cx.schema_types, tname), (lambda st_rp: (schema_type := hydra.lib.pairs.first(st_rp), fcx2 := hydra.lib.pairs.second(st_rp), svars := schema_type.variables, stype := schema_type.type, hydra.lib.eithers.bind(hydra.extract.core.union_type(fcx2, tname, stype), (lambda sfields: hydra.lib.eithers.bind(hydra.lib.eithers.map_maybe((lambda t: infer_type_of_term(fcx2, cx, t, hydra.lib.strings.cat(("case ", tname.value, ".<default>")))), dflt), (lambda dflt_rp: (dflt_result := dflt_rp, fcx3 := hydra.lib.maybes.from_maybe(fcx2, hydra.lib.maybes.map((lambda v1: v1.context), dflt_rp)), hydra.lib.eithers.bind(infer_many(fcx3, cx, hydra.lib.lists.map((lambda f: (f.term, hydra.lib.strings.cat(("case ", tname.value, ".", f.name.value)))), cases)), (lambda case_rp: (case_results := hydra.lib.pairs.first(case_rp), fcx4 := hydra.lib.pairs.second(case_rp), iterms := hydra.lib.pairs.first(case_results), itypes := hydra.lib.pairs.first(hydra.lib.pairs.second(case_results)), isubst := hydra.lib.pairs.first(hydra.lib.pairs.second(hydra.lib.pairs.second(case_results))), case_elem_constraints := hydra.lib.pairs.second(hydra.lib.pairs.second(hydra.lib.pairs.second(case_results))), codv_result := hydra.schemas.fresh_name(fcx4), codv := hydra.lib.pairs.first(codv_result), fcx5 := hydra.lib.pairs.second(codv_result), cod := cast(hydra.core.Type, hydra.core.TypeVariable(codv)), case_map := hydra.lib.maps.from_list(hydra.lib.lists.map((lambda ft: (ft.name, ft.type)), sfields)), dflt_constraints := hydra.monads.maybe_to_list(hydra.lib.maybes.map((lambda r: hydra.typing.TypeConstraint(cod, hydra.substitution.subst_in_type(isubst, r.type), "match default")), dflt_result)), case_constraints := hydra.lib.maybes.cat(hydra.lib.lists.zip_with((lambda fname, itype: hydra.lib.maybes.map((lambda ftype: hydra.typing.TypeConstraint(itype, cast(hydra.core.Type, hydra.core.TypeFunction(hydra.core.FunctionType(ftype, cod))), "case type")), hydra.lib.maps.lookup(fname, case_map))), fnames(), itypes)), dflt_class_constraints := hydra.lib.maybes.from_maybe(hydra.lib.maps.empty(), hydra.lib.maybes.map((lambda v1: v1.class_constraints), dflt_result)), all_elem_constraints := merge_class_constraints(case_elem_constraints, dflt_class_constraints), hydra.lib.eithers.bind(map_constraints(fcx5, cx, (lambda subst: yield_with_constraints(fcx5, build_type_application_term(svars, cast(hydra.core.Term, hydra.core.TermFunction(cast(hydra.core.Function, hydra.core.FunctionElimination(cast(hydra.core.Elimination, hydra.core.EliminationUnion(hydra.core.CaseStatement(tname, hydra.lib.maybes.map((lambda v1: v1.term), dflt_result), hydra.lib.lists.zip_with((lambda n, t: hydra.core.Field(n, t)), fnames(), iterms))))))))), cast(hydra.core.Type, hydra.core.TypeFunction(hydra.core.FunctionType(hydra.schemas.nominal_application(tname, hydra.lib.lists.map((lambda x: cast(hydra.core.Type, hydra.core.TypeVariable(x))), svars)), cod))), hydra.substitution.compose_type_subst_list(hydra.lib.lists.concat((hydra.monads.maybe_to_list(hydra.lib.maybes.map((lambda v1: v1.subst), dflt_result)), (isubst, subst)))), hydra.substitution.subst_in_class_constraints(subst, all_elem_constraints))), hydra.lib.lists.concat((dflt_constraints, case_constraints))), (lambda mc_result: Right(mc_result))))[15])))[2])))))[4]))

def infer_type_of_collection(fcx: hydra.context.Context, cx: hydra.graph.Graph, typ_cons: Callable[[hydra.core.Type], hydra.core.Type], trm_cons: Callable[[frozenlist[hydra.core.Term]], hydra.core.Term], desc: str, class_names: frozenset[hydra.core.Name], els: frozenlist[hydra.core.Term]) -> Either[hydra.context.InContext[hydra.error.OtherError], hydra.typing.InferenceResult]:
...
def infer_type_of_optional(fcx: hydra.context.Context, cx: hydra.graph.Graph, m: Maybe[hydra.core.Term]) -> Either[hydra.context.InContext[hydra.error.OtherError], hydra.typing.InferenceResult]:
    ...
    return infer_type_of_collection(fcx, cx, (lambda x: cast(hydra.core.Type, hydra.core.TypeMaybe(x))), (lambda x1: trm_cons(x1)), "optional element", hydra.lib.sets.empty(), hydra.lib.maybes.maybe((), (lambda x1: hydra.lib.lists.singleton(x1)), m))
