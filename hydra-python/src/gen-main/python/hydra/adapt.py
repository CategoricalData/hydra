...
def adapt_data_graph(constraints: hydra.coders.LanguageConstraints, do_expand: bool, els0: frozenlist[hydra.core.Binding], cx: hydra.context.Context, graph0: hydra.graph.Graph) -> Either[str, tuple[hydra.graph.Graph, frozenlist[hydra.core.Binding]]]:
    ...
    def transform_term(g: hydra.graph.Graph, term: hydra.core.Term) -> hydra.core.Term:
        ...
        def t1() -> hydra.core.Term:
            return hydra.rewriting.unshadow_variables(push_type_apps_inward(term))
        @lru_cache(1)
        def t2() -> hydra.core.Term:
            return hydra.rewriting.unshadow_variables(hydra.lib.logic.if_else(do_expand, (lambda : push_type_apps_inward(hydra.reduction.eta_expand_term_new(tx, t1()))), (lambda : t1())))
        return hydra.rewriting.lift_lambda_above_let(t2())
