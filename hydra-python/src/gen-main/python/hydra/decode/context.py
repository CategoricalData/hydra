...
def context(cx: hydra.graph.Graph, raw: hydra.core.Term):
    def _hoist_hydra_decode_context_context_1(cx, v1):
        match v1:
            ...
            case _:
                return Left(hydra.error.DecodingError("expected record of type hydra.context.Context"))
    return hydra.lib.eithers.either((lambda err: Left(hydra.error.DecodingError(err))), (lambda stripped: _hoist_hydra_decode_context_context_1(cx, stripped)), hydra.lexical.strip_and_dereference_term_either(cx, raw))

def in_context(e: Callable[[hydra.graph.Graph, hydra.core.Term], Either[hydra.error.DecodingError, T0]], cx: hydra.graph.Graph, raw: hydra.core.Term):
    def _hoist_hydra_decode_context_in_context_1(cx, e, v1):
        match v1:
            ...
            case _:
                return Left(hydra.error.DecodingError("expected record of type hydra.context.InContext"))
    return hydra.lib.eithers.either((lambda err: Left(hydra.error.DecodingError(err))), (lambda stripped: _hoist_hydra_decode_context_in_context_1(cx, e, stripped)), hydra.lexical.strip_and_dereference_term_either(cx, raw))
