...
def data_graph_to_definitions(constraints: hydra.coders.LanguageConstraints, do_expand: bool, do_hoist_case_statements: bool, do_hoist_polymorphic_let_bindings: bool, graph: hydra.graph.Graph, name_lists: frozenlist[frozenlist[hydra.core.Name]]) -> hydra.compute.Flow[hydra.graph.Graph, tuple[hydra.graph.Graph, frozenlist[frozenlist[hydra.module.TermDefinition]]]]:
    ...
    return hydra.lib.flows.bind(hydra.lib.logic.if_else(do_hoist_case_statements, (lambda : (gterm0 := hydra.schemas.graph_as_term(graph), (gterm1 := hydra.rewriting.unshadow_variables(gterm0), (new_elements := hydra.schemas.term_as_graph(gterm1), hydra.lib.flows.pure(hydra.graph.Graph(new_elements, graph.environment, graph.types, graph.body, graph.primitives, graph.schema)))[1])[1])[1]), (lambda : hydra.lib.flows.pure(graph))), (lambda graphu0: hydra.lib.flows.bind(hydra.lib.logic.if_else(do_hoist_case_statements, (lambda : hydra.hoisting.hoist_case_statements_in_graph(graphu0)), (lambda : hydra.lib.flows.pure(graphu0))), (lambda graphh1: hydra.lib.flows.bind(hydra.lib.logic.if_else(do_hoist_case_statements, (lambda : (gterm2 := hydra.schemas.graph_as_term(graphh1), (gterm3 := hydra.rewriting.unshadow_variables(gterm2), (new_elements2 := hydra.schemas.term_as_graph(gterm3), hydra.lib.flows.pure(hydra.graph.Graph(new_elements2, graphh1.environment, graphh1.types, graphh1.body, graphh1.primitives, graphh1.schema)))[1])[1])[1]), (lambda : hydra.lib.flows.pure(graphh1))), (lambda graphu1: hydra.lib.flows.bind(hydra.lib.logic.if_else(do_expand, (lambda : hydra.inference.infer_graph_types(graphu1)), (lambda : hydra.lib.flows.pure(graphu1))), (lambda graphi1: hydra.lib.flows.bind(adapt_data_graph(constraints, do_expand, graphi1), (lambda graph1: hydra.lib.flows.bind(hydra.inference.infer_graph_types(graph1), (lambda graph2: hydra.lib.flows.bind(hydra.lib.logic.if_else(do_hoist_polymorphic_let_bindings, (lambda : (process_binding := (lambda binding: (term := binding.term, wrapped_let := hydra.dsl.python.unsupported("inline match expressions are not yet supported"), hoisted_let := hydra.hoisting.hoist_polymorphic_let_bindings(wrapped_let), result_term := hydra.lib.logic.if_else(hydra.lib.lists.null(hoisted_let.bindings), (lambda : hoisted_let.body), (lambda : cast(hydra.core.Term, hydra.core.TermLet(hoisted_let)))), hydra.core.Binding(binding.name, result_term, binding.type))[4]), (new_bindings := hydra.lib.lists.map(process_binding, hydra.lib.maps.elems(graph2.elements)), (new_elements3 := hydra.lib.maps.from_list(hydra.lib.lists.map((lambda b: (b.name, b)), new_bindings)), hydra.lib.flows.pure(hydra.graph.Graph(new_elements3, graph2.environment, graph2.types, graph2.body, graph2.primitives, graph2.schema)))[1])[1])[1]), (lambda : hydra.lib.flows.pure(graph2))), (lambda graphh2: hydra.lib.flows.bind(hydra.lib.logic.if_else(do_hoist_polymorphic_let_bindings, (lambda : (flatten_binding := (lambda binding: hydra.core.Binding(binding.name, hydra.rewriting.flatten_let_terms(binding.term), binding.type)), (new_bindings_f := hydra.lib.lists.map(flatten_binding, hydra.lib.maps.elems(graphh2.elements)), (new_elements_f := hydra.lib.maps.from_list(hydra.lib.lists.map((lambda b: (b.name, b)), new_bindings_f)), hydra.lib.flows.pure(hydra.graph.Graph(new_elements_f, graphh2.environment, graphh2.types, graphh2.body, graphh2.primitives, graphh2.schema)))[1])[1])[1]), (lambda : hydra.lib.flows.pure(graphh2))), (lambda graphf2: hydra.lib.flows.bind(hydra.lib.logic.if_else(do_hoist_polymorphic_let_bindings, (lambda : (gterm4 := hydra.schemas.graph_as_term(graphf2), (gterm5 := hydra.rewriting.unshadow_variables(gterm4), (new_elements4 := hydra.schemas.term_as_graph(gterm5), hydra.lib.flows.pure(hydra.graph.Graph(new_elements4, graphf2.environment, graphf2.types, graphf2.body, graphf2.primitives, graphf2.schema)))[1])[1])[1]), (lambda : hydra.lib.flows.pure(graphf2))), (lambda graphu2: (to_def := (lambda el: (ts := hydra.lib.maybes.from_just(el.type), hydra.module.TermDefinition(el.name, el.term, ts))[1]), hydra.lib.flows.pure((graphu2, hydra.lib.lists.map((lambda names: hydra.lib.lists.map(to_def, hydra.lib.lists.map((lambda n: hydra.lib.maybes.from_just(hydra.lib.maps.lookup(n, graphu2.elements))), names))), name_lists))))[1]))))))))))))))))))
