...
def qualified_snake_name(name: hydra.core.Name) -> str:
    ...
    def snake_parts() -> frozenlist[str]:
        return hydra.lib.lists.map((lambda p: hydra.formatting.convert_case_camel_to_lower_snake(p)), parts)
    @lru_cache(1)
    def joined() -> str:
...
def encode_function(dialect: hydra.ext.lisp.syntax.Dialect, cx: T0, g: T1, fun: hydra.core.Function) -> Either[hydra.ext.lisp.syntax.Expression, hydra.ext.lisp.syntax.Expression]:
    match fun:
        case hydra.core.FunctionLambda(value=lam):
            @lru_cache(1)
            def param() -> str:
                return hydra.formatting.convert_case_camel_to_lower_snake(hydra.formatting.sanitize_with_underscores(hydra.ext.lisp.language.lisp_reserved_words(), lam.parameter.value))
            return hydra.lib.eithers.bind(encode_term(dialect, cx, g, lam.body), (lambda body: Right(lisp_lambda_expr((param(),), body))))

        case hydra.core.FunctionPrimitive(value=name):
            return Right(lisp_var(hydra.formatting.convert_case_camel_to_lower_snake(hydra.formatting.sanitize_with_underscores(hydra.ext.lisp.language.lisp_reserved_words(), name.value))))
