...
def get_assertion_type(term: hydra.core.Term):
    def _hoist_hydra_ext_java_testing_get_assertion_type_1(v1):
        match v1:
    ...
    def _hoist_hydra_ext_java_testing_get_assertion_type_2(v1):
        match v1:
            case hydra.core.LiteralBinary():
                return "assertArrayEquals"

            case hydra.core.LiteralFloat(value=fv):
                return _hoist_hydra_ext_java_testing_get_assertion_type_1(fv)

            case _:
                return "assertEquals"
    match hydra.rewriting.deannotate_term(term):
        case hydra.core.TermLiteral(value=lit):
            return _hoist_hydra_ext_java_testing_get_assertion_type_2(lit)
...
def term_to_java(term: hydra.core.Term, g: hydra.graph.Graph) -> Either[str, str]:
    ...
    return hydra.lib.eithers.bimap((lambda ic: hydra.show.errors.error(ic.object)), (lambda arg_: hydra.serialization.print_expr(hydra.serialization.parenthesize(hydra.ext.java.serde.write_expression(arg_)))), hydra.ext.java.coder.encode_term(hydra.ext.java.environment.JavaEnvironment(hydra.ext.java.environment.Aliases(hydra.module.Namespace("test"), hydra.lib.maps.empty(), hydra.lib.sets.empty(), hydra.lib.sets.empty(), hydra.lib.sets.empty(), hydra.lib.sets.empty(), hydra.lib.sets.empty(), hydra.lib.maps.empty(), hydra.lib.sets.empty(), hydra.lib.maps.empty(), hydra.lib.sets.empty(), Nothing(), hydra.lib.sets.empty()), g), term, hydra.lexical.empty_context(), g))
