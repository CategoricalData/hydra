...
def python_binding_metadata(tc: hydra.graph.Graph, b: hydra.core.Binding) -> Maybe[hydra.core.Term]:
    r"""Like bindingMetadata, but skips metadata for trivial bindings."""
    
    return hydra.lib.logic.if_else(hydra.coder_utils.is_trivial_term(b.term), (lambda : Nothing()), (lambda : hydra.coder_utils.binding_metadata(tc, b)))
...
def analyze_python_function(v1: hydra.ext.python.helpers.PythonEnvironment, v2: hydra.core.Term) -> hydra.compute.Flow[T0, hydra.typing.FunctionStructure[hydra.ext.python.helpers.PythonEnvironment]]:
    ...
    return hydra.coder_utils.analyze_function_term_with((lambda x1, x2: python_binding_metadata(x1, x2)), (lambda x1: python_environment_get_graph(x1)), (lambda x1, x2: python_environment_set_graph(x1, x2)), v1, v2)
...
def analyze_python_function_inline(v1: hydra.ext.python.helpers.PythonEnvironment, v2: hydra.core.Term) -> hydra.compute.Flow[T0, hydra.typing.FunctionStructure[hydra.ext.python.helpers.PythonEnvironment]]:
    r"""Analyze a function term without recording binding metadata (for inline lambdas)."""
    
    return hydra.coder_utils.analyze_function_term_inline((lambda x1: python_environment_get_graph(x1)), (lambda x1, x2: python_environment_set_graph(x1, x2)), v1, v2)
...
def deconflict_variant_name(is_qualified: bool, env: hydra.ext.python.helpers.PythonEnvironment, union_name: hydra.core.Name, fname: hydra.core.Name) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, hydra.ext.python.syntax.Name]:
    ...
    return hydra.lib.flows.bind(hydra.monads.get_state(), (lambda pyg: (g := py_graph_graph(pyg), elements := hydra.lexical.graph_to_bindings(g), collision := hydra.lib.maybes.is_just(hydra.lib.lists.find((lambda b: hydra.lib.equality.equal(b.name.value, candidate_hydra_name().value)), elements)), hydra.lib.logic.if_else(collision, (lambda : hydra.lib.flows.pure(hydra.ext.python.syntax.Name(hydra.lib.strings.cat2(hydra.ext.python.names.variant_name(is_qualified, env, union_name, fname).value, "_")))), (lambda : hydra.lib.flows.pure(hydra.ext.python.names.variant_name(is_qualified, env, union_name, fname)))))[3]))
...
def in_graph_context(graph_flow: hydra.compute.Flow[hydra.graph.Graph, T0]) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, T0]:
    r"""Run a Flow Graph computation within a Flow PyGraph computation."""
...
def term_arity_with_primitives(graph: hydra.graph.Graph, term: hydra.core.Term) -> int:
    ...
    match hydra.rewriting.deannotate_and_detype_term(term):
        ...
        case hydra.core.TermVariable(value=name):
            return hydra.lib.maybes.maybe(0, (lambda el: hydra.lib.maybes.maybe(hydra.arity.term_arity(el.term), (lambda ts: hydra.arity.type_scheme_arity(ts)), el.type)), hydra.lexical.lookup_element(graph, name))
...
def encode_application_inner(env: hydra.ext.python.helpers.PythonEnvironment, fun: hydra.core.Term, hargs: frozenlist[hydra.ext.python.syntax.Expression], rargs: frozenlist[hydra.ext.python.syntax.Expression]):
    ...
    def _hoist_body_1(v1):
        match v1:
            ...
            case hydra.core.EliminationUnion(value=cs):
                return hydra.lib.flows.bind(encode_union_elimination_inline(env, cs, first_arg()), (lambda inline_expr: hydra.lib.flows.pure((with_rest(inline_expr), rargs))))
...
def encode_union_elimination_inline(env: hydra.ext.python.helpers.PythonEnvironment, cs: hydra.core.CaseStatement, py_arg: hydra.ext.python.syntax.Expression) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, hydra.ext.python.syntax.Expression]:
    r"""Encode a union elimination as an inline conditional chain (isinstance-based ternary)."""
    
    tname = cs.type_name
    mdefault = cs.default
    cases_ = cs.cases
    return hydra.lib.flows.bind(in_graph_context(hydra.schemas.require_union_type(tname)), (lambda rt: (is_enum := hydra.schemas.is_enum_row_type(rt), value_expr := hydra.ext.python.utils.project_from_expression(py_arg, hydra.ext.python.syntax.Name("value")), isinstance_primary := hydra.ext.python.utils.py_name_to_py_primary(hydra.ext.python.syntax.Name("isinstance")), hydra.lib.flows.bind(hydra.lib.maybes.maybe(hydra.lib.flows.pure(unsupported_expression("no matching case in inline union elimination")), (lambda dflt: encode_term_inline(env, False, dflt)), mdefault), (lambda py_default: (encode_branch := (lambda field: (fname := field.name, fterm := field.term, is_unit_variant := is_variant_unit_type(rt, fname), hydra.lib.flows.bind(deconflict_variant_name(True, env, tname, fname), (lambda py_variant_name: (isinstance_check := hydra.ext.python.utils.function_call(isinstance_primary, (py_arg, hydra.ext.python.utils.py_name_to_py_expression(py_variant_name))), hydra.lib.flows.bind(encode_term_inline(env, False, fterm), (lambda py_branch: (py_result := hydra.lib.logic.if_else(is_enum, (lambda : hydra.ext.python.utils.function_call(hydra.ext.python.utils.py_expression_to_py_primary(py_branch), (py_arg,))), (lambda : hydra.lib.logic.if_else(is_unit_variant, (lambda : hydra.ext.python.utils.function_call(hydra.ext.python.utils.py_expression_to_py_primary(py_branch), (value_expr,))), (lambda : hydra.ext.python.utils.function_call(hydra.ext.python.utils.py_expression_to_py_primary(py_branch), (value_expr,)))))), hydra.lib.flows.pure((isinstance_check, py_result)))[1])))[1])))[3]), hydra.lib.flows.bind(hydra.lib.flows.map_list((lambda x1: encode_branch(x1)), cases_), (lambda encoded_branches: (build_chain := (lambda else_expr, branch_pair: (check_expr := hydra.lib.pairs.first(branch_pair), result_expr := hydra.lib.pairs.second(branch_pair), cast(hydra.ext.python.syntax.Expression, hydra.ext.python.syntax.ExpressionConditional(hydra.ext.python.syntax.Conditional(hydra.ext.python.utils.py_expression_to_disjunction(result_expr), hydra.ext.python.utils.py_expression_to_disjunction(check_expr), else_expr))))[2]), hydra.lib.flows.pure(hydra.lib.lists.foldl((lambda x1, x2: build_chain(x1, x2)), py_default, hydra.lib.lists.reverse(encoded_branches))))[1])))[1])))[3]))
...
def encode_case_block(env: hydra.ext.python.helpers.PythonEnvironment, tname: hydra.core.Name, row_type: hydra.core.RowType, is_enum: bool, encode_body: Callable[[hydra.ext.python.helpers.PythonEnvironment, hydra.core.Term], hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, frozenlist[hydra.ext.python.syntax.Statement]]], field: hydra.core.Field) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, hydra.ext.python.syntax.CaseBlock]:
    ...
    return hydra.lib.flows.bind(hydra.lib.flows.pure(python_environment_set_graph(hydra.schemas.extend_graph_for_lambda(python_environment_get_graph(env), effective_lambda()), env)), (lambda env2: hydra.lib.flows.bind(deconflict_variant_name(True, env2, tname, fname), (lambda py_variant_name: (pattern := variant_closed_pattern(env2, tname, fname, py_variant_name, row_type, is_enum, v, should_capture()), hydra.lib.flows.bind(encode_body(env2, effective_body()), (lambda stmts: (py_body := hydra.ext.python.utils.indented_block(Nothing(), (stmts,)), hydra.lib.flows.pure(hydra.ext.python.syntax.CaseBlock(hydra.ext.python.utils.py_closed_pattern_to_py_patterns(pattern), Nothing(), py_body)))[1])))[1]))))
...
def with_bindings(v1: frozenlist[hydra.core.Binding], v2: hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, T0]) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, T0]:
    r"""Execute a computation with bindings added to the graph context."""
...
def encode_term_multiline_t_c_o(env: hydra.ext.python.helpers.PythonEnvironment, func_name: hydra.core.Name, param_names: frozenlist[hydra.core.Name], term: hydra.core.Term):
    ...
    def is_self_call() -> bool:
        match stripped_fun():
            ...
            case _:
                return False
    return hydra.lib.logic.if_else(hydra.lib.logic.and_(is_self_call(), hydra.lib.equality.equal(hydra.lib.lists.length(gather_args()), hydra.lib.lists.length(param_names))), (lambda : hydra.lib.flows.bind(hydra.lib.flows.map_list((lambda a: encode_term_inline(env, False, a)), gather_args()), (lambda py_args: (assignments := hydra.lib.lists.map((lambda pair: (param_name := hydra.lib.pairs.first(pair), py_arg := hydra.lib.pairs.second(pair), hydra.ext.python.utils.assignment_statement(hydra.ext.python.names.encode_name(False, hydra.util.CaseConvention.LOWER_SNAKE, env, param_name), py_arg))[2]), hydra.lib.lists.zip(param_names, py_args)), continue_stmt := cast(hydra.ext.python.syntax.Statement, hydra.ext.python.syntax.StatementSimple((cast(hydra.ext.python.syntax.SimpleStatement, hydra.ext.python.syntax.SimpleStatementContinue()),))), hydra.lib.flows.pure(hydra.lib.lists.concat2(assignments, (continue_stmt,))))[2]))), (lambda : (gathered2 := hydra.coder_utils.gather_applications(term), (args2 := hydra.lib.pairs.first(gathered2), (body2 := hydra.lib.pairs.second(gathered2), hydra.lib.logic.if_else(hydra.lib.equality.equal(hydra.lib.lists.length(args2), 1), (lambda : (arg := hydra.lib.lists.head(args2), (_hoist_body_1 := (lambda v1: (lambda cs: (tname := cs.type_name, dflt := cs.default, cases_ := cs.cases, hydra.lib.flows.bind(in_graph_context(hydra.schemas.require_union_type(tname)), (lambda rt: (is_enum := hydra.schemas.is_enum_row_type(rt), is_full := is_cases_full(rt, cases_), hydra.lib.flows.bind(encode_term_inline(env, False, arg), (lambda py_arg: hydra.lib.flows.bind(hydra.lib.flows.map_list((lambda v12: encode_case_block(env, tname, rt, is_enum, (lambda e2, t2: encode_term_multiline_t_c_o(e2, func_name, param_names, t2)), v12)), deduplicate_case_variables(cases_)), (lambda py_cases: hydra.lib.flows.bind(encode_default_case_block((lambda t2: encode_term_inline(env, False, t2)), is_full, dflt, tname), (lambda py_dflt: (subj := cast(hydra.ext.python.syntax.SubjectExpression, hydra.ext.python.syntax.SubjectExpressionSimple(cast(hydra.ext.python.syntax.NamedExpression, hydra.ext.python.syntax.NamedExpressionSimple(py_arg)))), match_stmt := cast(hydra.ext.python.syntax.Statement, hydra.ext.python.syntax.StatementCompound(cast(hydra.ext.python.syntax.CompoundStatement, hydra.ext.python.syntax.CompoundStatementMatch(hydra.ext.python.syntax.MatchStatement(subj, hydra.lib.lists.concat2(py_cases, py_dflt)))))), hydra.lib.flows.pure((match_stmt,)))[2])))))))[2])))[3])(v1.value) if isinstance(v1, hydra.core.EliminationUnion) else hydra.lib.flows.bind(encode_term_inline(env, False, term), (lambda expr: hydra.lib.flows.pure((hydra.ext.python.utils.return_single(expr),))))), _hoist_body_2 := (lambda v1: (lambda e: _hoist_body_1(e))(v1.value) if isinstance(v1, hydra.core.FunctionElimination) else hydra.lib.flows.bind(encode_term_inline(env, False, term), (lambda expr: hydra.lib.flows.pure((hydra.ext.python.utils.return_single(expr),))))), _hoist_body_3 := (lambda v1: (lambda f: _hoist_body_2(f))(v1.value) if isinstance(v1, hydra.core.TermFunction) else hydra.lib.flows.bind(encode_term_inline(env, False, term), (lambda expr: hydra.lib.flows.pure((hydra.ext.python.utils.return_single(expr),))))), _hoist_body_3(hydra.rewriting.deannotate_and_detype_term(body2)))[3])[1]), (lambda : hydra.lib.flows.bind(encode_term_inline(env, False, term), (lambda expr: hydra.lib.flows.pure((hydra.ext.python.utils.return_single(expr),)))))))[1])[1])[1]))
...
def encode_term_assignment(env: hydra.ext.python.helpers.PythonEnvironment, name: hydra.core.Name, term: hydra.core.Term, ts: hydra.core.TypeScheme, comment: Maybe[str]) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, hydra.ext.python.syntax.Statement]:
    ...
    return hydra.lib.flows.bind(analyze_python_function(env, term), (lambda fs: (tparams := fs.type_params, params := fs.params, bindings := fs.bindings, body := fs.body, doms := fs.domains, mcod := fs.codomain, env2 := fs.environment, tc := env2.graph, binding := hydra.core.Binding(name, term, Just(ts)), is_complex := hydra.coder_utils.is_complex_binding(tc, binding), is_trivial := hydra.coder_utils.is_trivial_term(term), hydra.lib.logic.if_else(hydra.lib.logic.and_(is_complex, hydra.lib.logic.not_(is_trivial)), (lambda : with_bindings(bindings, hydra.lib.flows.bind(hydra.lib.flows.map_list((lambda v1: encode_binding_as(env2, v1)), bindings), (lambda binding_stmts: encode_function_definition(env2, name, tparams, params, body, doms, mcod, comment, binding_stmts))))), (lambda : hydra.lib.flows.bind(encode_term_inline(env2, False, body), (lambda body_expr: (py_name := hydra.ext.python.names.encode_name(False, hydra.util.CaseConvention.LOWER_SNAKE, env2, name), hydra.lib.flows.pure(hydra.ext.python.utils.annotated_statement(comment, hydra.ext.python.utils.assignment_statement(py_name, body_expr))))[1])))))[11]))

def encode_term_multiline(env: hydra.ext.python.helpers.PythonEnvironment, term: hydra.core.Term):
    ...
    def body() -> hydra.core.Term:
        return hydra.lib.pairs.second(gathered())
    return hydra.lib.logic.if_else(hydra.lib.equality.equal(hydra.lib.lists.length(args()), 1), (lambda : (arg := hydra.lib.lists.head(args()), (_hoist_body_1 := (lambda v1: (lambda cs: (tname := cs.type_name, dflt := cs.default, cases_ := cs.cases, hydra.lib.flows.bind(in_graph_context(hydra.schemas.require_union_type(tname)), (lambda rt: (is_enum := hydra.schemas.is_enum_row_type(rt), is_full := is_cases_full(rt, cases_), hydra.lib.flows.bind(encode_term_inline(env, False, arg), (lambda py_arg: hydra.lib.flows.bind(hydra.lib.flows.map_list((lambda v12: encode_case_block(env, tname, rt, is_enum, (lambda e2, t: encode_term_multiline(e2, t)), v12)), deduplicate_case_variables(cases_)), (lambda py_cases: hydra.lib.flows.bind(encode_default_case_block((lambda t: encode_term_inline(env, False, t)), is_full, dflt, tname), (lambda py_dflt: (subj := cast(hydra.ext.python.syntax.SubjectExpression, hydra.ext.python.syntax.SubjectExpressionSimple(cast(hydra.ext.python.syntax.NamedExpression, hydra.ext.python.syntax.NamedExpressionSimple(py_arg)))), match_stmt := cast(hydra.ext.python.syntax.Statement, hydra.ext.python.syntax.StatementCompound(cast(hydra.ext.python.syntax.CompoundStatement, hydra.ext.python.syntax.CompoundStatementMatch(hydra.ext.python.syntax.MatchStatement(subj, hydra.lib.lists.concat2(py_cases, py_dflt)))))), hydra.lib.flows.pure((match_stmt,)))[2])))))))[2])))[3])(v1.value) if isinstance(v1, hydra.core.EliminationUnion) else dflt_logic()), _hoist_body_2 := (lambda v1: (lambda e: _hoist_body_1(e))(v1.value) if isinstance(v1, hydra.core.FunctionElimination) else dflt_logic()), _hoist_body_3 := (lambda v1: (lambda f: _hoist_body_2(f))(v1.value) if isinstance(v1, hydra.core.TermFunction) else dflt_logic()), _hoist_body_3(hydra.rewriting.deannotate_and_detype_term(body())))[3])[1]), (lambda : dflt_logic()))
...
def encode_record_type(env: hydra.ext.python.helpers.PythonEnvironment, name: hydra.core.Name, row_type: hydra.core.RowType, comment: Maybe[str]) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, hydra.ext.python.syntax.Statement]:
    ...
    tfields = row_type.fields
    return hydra.lib.flows.bind(hydra.lib.flows.map_list((lambda v1: encode_field_type(env, v1)), tfields), (lambda py_fields: (const_stmts := encode_name_constants(env, name, tfields), body := hydra.ext.python.utils.indented_block(comment, (py_fields, const_stmts)), bound_vars := env.bound_type_variables, tparam_list := hydra.lib.pairs.first(bound_vars), m_generic_arg := generic_arg(tparam_list), args := hydra.lib.maybes.maybe(Nothing(), (lambda a: Just(hydra.ext.python.utils.py_expressions_to_py_args((a,)))), m_generic_arg), decs := Just(hydra.ext.python.syntax.Decorators((dataclass_decorator(),))), py_name := hydra.ext.python.names.encode_name(False, hydra.util.CaseConvention.PASCAL, env, name), no_type_params := (), hydra.lib.flows.pure(hydra.ext.python.utils.py_class_definition_to_py_statement(hydra.ext.python.syntax.ClassDefinition(decs, py_name, no_type_params, args, body))))[9]))
...
def encode_union_field(env: hydra.ext.python.helpers.PythonEnvironment, union_name: hydra.core.Name, field_type: hydra.core.FieldType) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, hydra.ext.python.syntax.Statement]:
    ...
    fname = field_type.name
    ftype = field_type.type
    return hydra.lib.flows.bind(in_graph_context(hydra.annotations.get_type_description(ftype)), (lambda fcomment: (is_unit := hydra.lib.equality.equal(hydra.rewriting.deannotate_type(ftype), cast(hydra.core.Type, hydra.core.TypeUnit())), hydra.lib.flows.bind(deconflict_variant_name(False, env, union_name, fname), (lambda var_name: (tparam_names := find_type_params(env, ftype), tparam_py_names := hydra.lib.lists.map((lambda x1: hydra.ext.python.names.encode_type_variable(x1)), tparam_names), field_params := hydra.lib.lists.map((lambda x1: hydra.ext.python.utils.py_name_to_py_type_parameter(x1)), tparam_py_names), body := hydra.lib.logic.if_else(is_unit, (lambda : hydra.ext.python.utils.indented_block(fcomment, (hydra.ext.python.utils.unit_variant_methods(var_name),))), (lambda : hydra.ext.python.utils.indented_block(fcomment, ()))), hydra.lib.flows.bind(hydra.lib.logic.if_else(is_unit, (lambda : hydra.lib.flows.pure(Nothing())), (lambda : hydra.lib.flows.bind(encode_type_quoted(env, ftype), (lambda quoted_type: hydra.lib.flows.pure(Just(variant_args(quoted_type, ()))))))), (lambda margs: hydra.lib.flows.pure(hydra.ext.python.utils.py_class_definition_to_py_statement(hydra.ext.python.syntax.ClassDefinition(Nothing(), var_name, field_params, margs, body))))))[4])))[1]))
...
def encode_union_type(env: hydra.ext.python.helpers.PythonEnvironment, name: hydra.core.Name, row_type: hydra.core.RowType, comment: Maybe[str]) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, frozenlist[hydra.ext.python.syntax.Statement]]:
    ...
    tfields = row_type.fields
    return hydra.lib.logic.if_else(hydra.schemas.is_enum_row_type(row_type), (lambda : hydra.lib.flows.bind(hydra.lib.flows.map_list((lambda v1: encode_enum_value_assignment(env, v1)), tfields), (lambda vals: (body := hydra.ext.python.utils.indented_block(comment, vals), enum_name := hydra.ext.python.syntax.Name("Enum"), args := Just(hydra.ext.python.utils.py_expressions_to_py_args((hydra.ext.python.utils.py_name_to_py_expression(enum_name),))), py_name := hydra.ext.python.names.encode_name(False, hydra.util.CaseConvention.PASCAL, env, name), type_const_stmt := hydra.ext.python.utils.dotted_assignment_statement(py_name, hydra.ext.python.names.encode_constant_for_type_name(env, name), hydra.ext.python.utils.function_call(hydra.ext.python.utils.py_name_to_py_primary(hydra.ext.python.names.encode_name(True, hydra.util.CaseConvention.PASCAL, env, hydra.core.Name("hydra.core.Name"))), (hydra.ext.python.utils.double_quoted_string(name.value),))), hydra.lib.flows.pure((hydra.ext.python.utils.py_class_definition_to_py_statement(hydra.ext.python.syntax.ClassDefinition(Nothing(), py_name, (), args, body)), type_const_stmt)))[5]))), (lambda : (const_stmts := encode_name_constants(env, name, tfields), hydra.lib.flows.bind(hydra.lib.flows.map_list((lambda v1: encode_union_field(env, name, v1)), tfields), (lambda field_stmts: (tparams := environment_type_parameters(env), union_alts := hydra.lib.lists.map((lambda v1: encode_union_field_alt(env, name, v1)), tfields), union_stmts := union_type_statements_for(env, hydra.ext.python.names.encode_name(False, hydra.util.CaseConvention.PASCAL, env, name), tparams, comment, hydra.ext.python.utils.or_expression(union_alts), const_stmts), hydra.lib.flows.pure(hydra.lib.lists.concat2(field_stmts, union_stmts)))[3])))[1]))

def encode_wrapped_type(env: hydra.ext.python.helpers.PythonEnvironment, name: hydra.core.Name, typ: hydra.core.Type, comment: Maybe[str]) -> hydra.compute.Flow[T0, frozenlist[hydra.ext.python.syntax.Statement]]:
...
def encode_type_assignment_inner(env: hydra.ext.python.helpers.PythonEnvironment, name: hydra.core.Name, typ: hydra.core.Type, comment: Maybe[str]) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, frozenlist[hydra.ext.python.syntax.Statement]]:
    ...
    while True:
        ...
        match stripped():
            case hydra.core.TypeForall(value=ft):
                return (tvar := ft.parameter, (body := ft.body, (new_env := extend_env_with_type_var(env, tvar), encode_type_assignment_inner(new_env, name, body, comment))[1])[1])[1]
            ...
            case hydra.core.TypeWrap(value=wt):
                return (inner_type := wt.body, encode_wrapped_type(env, name, inner_type, comment))[1]
...
def encode_type_assignment(env: hydra.ext.python.helpers.PythonEnvironment, name: hydra.core.Name, typ: hydra.core.Type, comment: Maybe[str]) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, frozenlist[frozenlist[hydra.ext.python.syntax.Statement]]]:
    ...
    return hydra.lib.flows.bind(encode_type_assignment_inner(env, name, typ, comment), (lambda def_stmts: hydra.lib.flows.pure(hydra.lib.lists.map((lambda s: (s,)), def_stmts))))
...
def with_updated_graph(v1: Callable[[hydra.graph.Graph], hydra.graph.Graph], v2: hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, T0]) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, T0]:
    r"""Execute a computation with an updated graph."""
