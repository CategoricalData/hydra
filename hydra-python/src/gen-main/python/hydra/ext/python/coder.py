...
def is_complex_variable_for_python(tc: hydra.typing.TypeContext, name: hydra.core.Name) -> bool:
    r"""Check if a variable reference indicates complexity, excluding non-nullary functions."""
    
    @lru_cache(1)
    def meta_lookup() -> Maybe[hydra.core.Term]:
        return hydra.lib.maps.lookup(name, tc.metadata)
    return hydra.lib.logic.if_else(hydra.lib.maybes.is_just(meta_lookup()), (lambda : hydra.lib.maybes.maybe(True, (lambda typ: hydra.lib.equality.equal(hydra.arity.type_arity(typ), 0)), hydra.lib.maps.lookup(name, tc.types))), (lambda : hydra.lib.logic.if_else(hydra.lib.sets.member(name, tc.lambda_variables), (lambda : True), (lambda : (type_lookup := hydra.lib.maps.lookup(name, tc.types), hydra.lib.logic.not_(hydra.lib.maybes.is_just(type_lookup)))[1]))))

def is_complex_term_for_python(tc: hydra.typing.TypeContext, t: hydra.core.Term) -> bool:
    r"""Check if a term needs lazy evaluation, excluding non-nullary function references."""
    ...
    match t:
        ...
        case hydra.core.TermVariable(value=name):
            return is_complex_variable_for_python(tc, name)
        
        case _:
            return hydra.lib.lists.foldl((lambda b, sub: hydra.lib.logic.or_(b, is_complex_term_for_python(tc, sub))), False, hydra.rewriting.subterms(t))

def is_complex_binding_for_python(tc: hydra.typing.TypeContext, b: hydra.core.Binding) -> bool:
    r"""Check if a binding needs to be treated as a function (Python-specific)."""
    ...
    def mts() -> Maybe[hydra.core.TypeScheme]:
        return b.type
    return hydra.lib.maybes.maybe(is_complex_term_for_python(tc, term()), (lambda ts: (is_polymorphic := hydra.lib.logic.not_(hydra.lib.lists.null(ts.variables)), is_non_nullary := hydra.lib.equality.gt(hydra.arity.type_arity(ts.type), 0), is_complex := is_complex_term_for_python(tc, term()), hydra.lib.logic.or_(hydra.lib.logic.or_(is_polymorphic, is_non_nullary), is_complex))[3]), mts())

def binding_metadata_for_python(tc: hydra.typing.TypeContext, b: hydra.core.Binding) -> Maybe[hydra.core.Term]:
    r"""Produces metadata for a binding if it is complex (Python-specific)."""
    
    return hydra.lib.logic.if_else(is_complex_binding_for_python(tc, b), (lambda : Just(cast(hydra.core.Term, hydra.core.TermLiteral(cast(hydra.core.Literal, hydra.core.LiteralBoolean(True)))))), (lambda : Nothing()))
...
def analyze_python_function(v1: hydra.ext.python.helpers.PythonEnvironment, v2: hydra.core.Term) -> hydra.compute.Flow[T0, hydra.typing.FunctionStructure[hydra.ext.python.helpers.PythonEnvironment]]:
    r"""Analyze a function term with Python-specific TypeContext management."""
    
    return hydra.coder_utils.analyze_function_term_with((lambda x1, x2: binding_metadata_for_python(x1, x2)), (lambda x1: python_environment_get_type_context(x1)), (lambda x1, x2: python_environment_set_type_context(x1, x2)), v1, v2)
...
def deconflict_variant_name(is_qualified: bool, env: hydra.ext.python.helpers.PythonEnvironment, union_name: hydra.core.Name, fname: hydra.core.Name) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, hydra.ext.python.syntax.Name]:
    ...
    return hydra.lib.flows.bind(hydra.monads.get_state(), (lambda pyg: (g := py_graph_graph(pyg), elements := g.elements, collision := hydra.lib.maybes.is_just(hydra.lib.lists.find((lambda b: hydra.lib.equality.equal(b.name.value, candidate_hydra_name().value)), elements)), hydra.lib.logic.if_else(collision, (lambda : hydra.lib.flows.pure(hydra.ext.python.syntax.Name(hydra.lib.strings.cat2(hydra.ext.python.names.variant_name(is_qualified, env, union_name, fname).value, "_")))), (lambda : hydra.lib.flows.pure(hydra.ext.python.names.variant_name(is_qualified, env, union_name, fname)))))[3]))
...
def encode_union_elimination_inline(env: hydra.ext.python.helpers.PythonEnvironment, cs: hydra.core.CaseStatement, py_arg: hydra.ext.python.syntax.Expression) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, hydra.ext.python.syntax.Expression]:
    ...
    def tname() -> hydra.core.Name:
        return cs.type_name
    @lru_cache(1)
    def mdefault() -> Maybe[hydra.core.Term]:
        return cs.default
    @lru_cache(1)
    def cases_() -> frozenlist[hydra.core.Field]:
        return cs.cases
...
def encode_case_block(env: hydra.ext.python.helpers.PythonEnvironment, tname: hydra.core.Name, row_type: hydra.core.RowType, is_enum: bool, encode_body: Callable[[hydra.ext.python.helpers.PythonEnvironment, hydra.core.Term], hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, frozenlist[hydra.ext.python.syntax.Statement]]], field: hydra.core.Field) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, hydra.ext.python.syntax.CaseBlock]:
    ...
    @lru_cache(1)
    def v() -> hydra.core.Name:
        return effective_lambda().parameter
    @lru_cache(1)
    def raw_body() -> hydra.core.Term:
        return effective_lambda().body
    @lru_cache(1)
...
lru_cache_decorator = cast(hydra.ext.python.syntax.NamedExpression, hydra.ext.python.syntax.NamedExpressionSimple(hydra.ext.python.utils.function_call(cast(hydra.ext.python.syntax.Primary, hydra.ext.python.syntax.PrimarySimple(cast(hydra.ext.python.syntax.Atom, hydra.ext.python.syntax.AtomName(hydra.ext.python.syntax.Name("lru_cache"))))), (py_int(1),))))
...
use_inline_type_params = use_inline_type_params_for(hydra.ext.python.utils.target_python_version)
...
def encode_term_assignment(env: hydra.ext.python.helpers.PythonEnvironment, name: hydra.core.Name, term: hydra.core.Term, ts: hydra.core.TypeScheme, comment: Maybe[str]) -> hydra.compute.Flow[hydra.ext.python.helpers.PyGraph, hydra.ext.python.syntax.Statement]:
    ...
    return hydra.lib.flows.bind(analyze_python_function(env, term), (lambda fs: (tparams := fs.type_params, params := fs.params, bindings := fs.bindings, body := fs.body, doms := fs.domains, mcod := fs.codomain, env2 := fs.environment, tc := env2.type_context, binding := hydra.core.Binding(name, term, Just(ts)), is_complex := is_complex_binding_for_python(tc, binding), hydra.lib.logic.if_else(is_complex, (lambda : with_bindings(bindings, hydra.lib.flows.bind(hydra.lib.flows.map_list((lambda v1: encode_binding_as(env2, v1)), bindings), (lambda binding_stmts: encode_function_definition(env2, name, tparams, params, body, doms, mcod, comment, binding_stmts))))), (lambda : hydra.lib.flows.bind(encode_term_inline(env2, False, body), (lambda body_expr: (py_name := hydra.ext.python.names.encode_name(False, hydra.util.CaseConvention.LOWER_SNAKE, env2, name), hydra.lib.flows.pure(hydra.ext.python.utils.annotated_statement(comment, hydra.ext.python.utils.assignment_statement(py_name, body_expr))))[1])))))[10]))
    ...
    match stripped():
        ...
        case hydra.core.TypeWrap(value=wt):
            ...
            def inner_type() -> hydra.core.Type:
                return wt.body
            return encode_wrapped_type(env, name, inner_type(), comment)
...
def with_let(v1: hydra.ext.python.helpers.PythonEnvironment, v2: hydra.core.Let, v3: Callable[[hydra.ext.python.helpers.PythonEnvironment], T0]) -> T0:
    r"""Execute a computation with let context (adds let bindings to TypeContext)."""
    
    return hydra.schemas.with_let_context((lambda x1: python_environment_get_type_context(x1)), (lambda x1, x2: python_environment_set_type_context(x1, x2)), (lambda x1, x2: binding_metadata_for_python(x1, x2)), v1, v2, v3)
...
def with_lambda(v1: hydra.ext.python.helpers.PythonEnvironment, v2: hydra.core.Lambda, v3: Callable[[hydra.ext.python.helpers.PythonEnvironment], T0]) -> T0:
    r"""Execute a computation with lambda context (adds lambda parameter to TypeContext)."""
