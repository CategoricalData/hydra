...
def to_type_declarations_from(namespaces: hydra.module.Namespaces[hydra.ext.haskell.ast.ModuleName], element_name: hydra.core.Name, typ: hydra.core.Type):
    ...
    return hydra.monads.with_trace(hydra.lib.strings.cat2("type definition ", element_name.value), hydra.lib.flows.bind(hydra.monads.get_state(), (lambda g: hydra.lib.flows.bind(hydra.schemas.is_serializable_by_name(element_name), (lambda is_ser: (deriv := hydra.ext.haskell.ast.Deriving(hydra.lib.logic.if_else(is_ser, (lambda : hydra.lib.lists.map((lambda x1: hydra.ext.haskell.utils.raw_name(x1)), ("Eq", "Ord", "Read", "Show"))), (lambda : ()))), unpack_result := hydra.ext.haskell.utils.unpack_forall_type(g, typ), vars := hydra.lib.pairs.first(unpack_result), t_ := hydra.lib.pairs.second(unpack_result), hd := decl_head(hname(), hydra.lib.lists.reverse(vars)), _dt_ := hydra.rewriting.deannotate_type(t_), _hoist_body_result := (hydra.lib.flows.bind(record_cons(lname(), _dt_.value.fields), (lambda cons: hydra.lib.flows.pure(cast(hydra.ext.haskell.ast.Declaration, hydra.ext.haskell.ast.DeclarationData(hydra.ext.haskell.ast.DataDeclaration(hydra.ext.haskell.ast.DataOrNewtype.DATA, (), hd, (cons,), (deriv,))))))) if isinstance(_dt_, hydra.core.TypeRecord) else hydra.lib.flows.bind(hydra.lib.flows.map_list((lambda x1: union_cons(g, lname(), x1)), _dt_.value.fields), (lambda cons: hydra.lib.flows.pure(cast(hydra.ext.haskell.ast.Declaration, hydra.ext.haskell.ast.DeclarationData(hydra.ext.haskell.ast.DataDeclaration(hydra.ext.haskell.ast.DataOrNewtype.DATA, (), hd, cons, (deriv,))))))) if isinstance(_dt_, hydra.core.TypeUnion) else hydra.lib.flows.bind(newtype_cons(element_name, _dt_.value.body), (lambda cons: hydra.lib.flows.pure(cast(hydra.ext.haskell.ast.Declaration, hydra.ext.haskell.ast.DeclarationData(hydra.ext.haskell.ast.DataDeclaration(hydra.ext.haskell.ast.DataOrNewtype.NEWTYPE, (), hd, (cons,), (deriv,))))))) if isinstance(_dt_, hydra.core.TypeWrap) else hydra.lib.flows.bind(adapt_type_to_haskell_and_encode(namespaces, typ), (lambda htype: hydra.lib.flows.pure(cast(hydra.ext.haskell.ast.Declaration, hydra.ext.haskell.ast.DeclarationType(hydra.ext.haskell.ast.TypeDeclaration(hd, htype))))))), hydra.lib.flows.bind(_hoist_body_result, (lambda decl: hydra.lib.flows.bind(hydra.annotations.get_type_description(typ), (lambda comments: hydra.lib.flows.bind(hydra.lib.logic.if_else(include_type_definitions, (lambda : hydra.lib.flows.bind(type_decl(namespaces, element_name, typ), (lambda decl_: hydra.lib.flows.pure((decl_,))))), (lambda : hydra.lib.flows.pure(()))), (lambda tdecls: (main_decl := hydra.ext.haskell.ast.DeclarationWithComments(decl, comments), name_decls_ := name_decls(g, namespaces, element_name, typ), hydra.lib.flows.pure(hydra.lib.lists.concat(((main_decl,), name_decls_, tdecls))))[2])))))))[7])))))
