...
def encode_term(depth: int, namespaces: hydra.module.Namespaces[hydra.ext.haskell.syntax.ModuleName], term: hydra.core.Term, cx: hydra.context.Context, g: hydra.graph.Graph):
    ...
    match hydra.rewriting.deannotate_term(term):
        ...
        case hydra.core.TermUnion(value=injection):
            ...
            @lru_cache(1)
            def lhs() -> hydra.ext.haskell.syntax.Expression:
                return cast(hydra.ext.haskell.syntax.Expression, hydra.ext.haskell.syntax.ExpressionVariable(hydra.ext.haskell.utils.union_field_reference(hydra.lib.sets.from_list(hydra.lib.maps.keys(g.bound_terms)), namespaces, sname, fn)))
            @lru_cache(1)
    ...
    def to_decl(comments: Maybe[str], hname_: hydra.ext.haskell.syntax.Name, term_: hydra.core.Term, bindings: Maybe[hydra.ext.haskell.syntax.LocalBindings]) -> Either[hydra.context.InContext[hydra.errors.Error], hydra.ext.haskell.syntax.DeclarationWithComments]:
        match hydra.rewriting.deannotate_term(term_):
            case hydra.core.TermLet(value=let_term):
                ...
                env = let_term.body
                def to_binding(hname_: hydra.ext.haskell.syntax.Name, hterm_: hydra.ext.haskell.syntax.Expression) -> hydra.ext.haskell.syntax.LocalBinding:
            ...
            case _:
                return hydra.lib.eithers.bind(encode_term(0, namespaces, term_, cx, g), (lambda hterm: (vb := hydra.ext.haskell.utils.simple_value_binding(hname_, hterm, bindings), scheme_constraints := typ.constraints, scheme_classes := type_scheme_constraints_to_class_map(scheme_constraints), hydra.lib.eithers.bind(hydra.annotations.get_type_classes(cx, g, hydra.rewriting.remove_types_from_term(term)), (lambda explicit_classes: (combined_classes := hydra.lib.maps.union(scheme_classes, explicit_classes), hydra.lib.eithers.bind(encode_type_with_class_assertions(namespaces, combined_classes, typ.type, cx, g), (lambda htype: (decl := cast(hydra.ext.haskell.syntax.Declaration, hydra.ext.haskell.syntax.DeclarationTypedBinding(hydra.ext.haskell.syntax.TypedBinding(hydra.ext.haskell.syntax.TypeSignature(hname_, htype), rewrite_value_binding(vb)))), Right(hydra.ext.haskell.syntax.DeclarationWithComments(decl, comments)))[1])))[1])))[3]))
