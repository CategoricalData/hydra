...
def write_data_name(dn: hydra.ext.scala.meta.Data_Name) -> hydra.ast.Expr:
    r"""Convert a data name to an expression."""

    return hydra.serialization.cst(dn.value.value)

def write_name(name: hydra.ext.scala.meta.Name) -> hydra.ast.Expr:
    r"""Convert a name to an expression."""

    match name:
        case hydra.ext.scala.meta.NameValue(value=s):
            return hydra.serialization.cst(s)

        case _:
            raise TypeError("Unsupported Name")

def write_type_name(tn: hydra.ext.scala.meta.Type_Name) -> hydra.ast.Expr:
    r"""Convert a type name to an expression."""

    return hydra.serialization.cst(tn.value)

def write_type_param(tp: hydra.ext.scala.meta.Type_Param) -> hydra.ast.Expr:
    r"""Convert a type parameter to an expression."""

    return write_name(tp.name)

def write_type(typ: hydra.ext.scala.meta.Type):
    def _hoist_hydra_ext_scala_serde_write_type_1(v1):
        match v1:
            case hydra.ext.scala.meta.Type_RefName(value=name):
    ...
    def _hoist_hydra_ext_scala_serde_write_type_2(v1):
        match v1:
            case hydra.ext.scala.meta.Type_FunctionTypeFunction(value=tf):
                @lru_cache(1)
                def dom() -> hydra.ext.scala.meta.Type:
    ...
    match typ:
        case hydra.ext.scala.meta.TypeRef(value=tr):
            return _hoist_hydra_ext_scala_serde_write_type_1(tr)

        case hydra.ext.scala.meta.TypeApply(value=ta):
            fun = ta.tpe
            args = ta.args
        ...
        case hydra.ext.scala.meta.TypeFunctionType(value=ft):
            return _hoist_hydra_ext_scala_serde_write_type_2(ft)

        case hydra.ext.scala.meta.TypeLambda(value=tl):
            params = tl.tparams
            body = tl.tpe
        ...
        case hydra.ext.scala.meta.TypeVar(value=tv):
            return write_type_name(tv.name)

        case _:
            raise TypeError("Unsupported Type")

def write_data_param(dp: hydra.ext.scala.meta.Data_Param) -> hydra.ast.Expr:
...
def write_init(init: hydra.ext.scala.meta.Init) -> hydra.ast.Expr:
    r"""Convert an init to an expression."""

    return write_type(init.tpe)

def write_mod(m: hydra.ext.scala.meta.Mod) -> hydra.ast.Expr:
    r"""Convert a modifier to an expression."""

    match m:
        case hydra.ext.scala.meta.ModCase():
            return hydra.serialization.cst("case")

        case hydra.ext.scala.meta.ModSealed():
            return hydra.serialization.cst("sealed")

        case hydra.ext.scala.meta.ModAbstract():
            return hydra.serialization.cst("abstract")

        case hydra.ext.scala.meta.ModFinal():
            return hydra.serialization.cst("final")

        case hydra.ext.scala.meta.ModOverride():
            return hydra.serialization.cst("override")

        case hydra.ext.scala.meta.ModImplicit():
            return hydra.serialization.cst("implicit")

        case hydra.ext.scala.meta.ModLazy():
            return hydra.serialization.cst("lazy")

        case hydra.ext.scala.meta.ModPrivate():
            return hydra.serialization.cst("private")

        case hydra.ext.scala.meta.ModProtected():
            return hydra.serialization.cst("protected")

        case _:
            raise TypeError("Unsupported Mod")

def write_importer(imp: hydra.ext.scala.meta.Importer) -> hydra.ast.Expr:
    ...
    def ref_name() -> str:
        match ref:
            case hydra.ext.scala.meta.Data_RefName(value=dn):
    ...
    def for_importees():
        ...
        def _hoist_for_importees_2(v1):
            match v1:
                case hydra.ext.scala.meta.ImporteeWildcard():
                    return hydra.serialization.cst("*")

                case hydra.ext.scala.meta.ImporteeName(value=in_):
...
def write_import_export_stat(ie: hydra.ext.scala.meta.ImportExportStat) -> hydra.ast.Expr:
    r"""Convert an import/export statement to an expression."""

    match ie:
        case hydra.ext.scala.meta.ImportExportStatImport(value=imp):
...
def write_case(c: hydra.ext.scala.meta.Case) -> hydra.ast.Expr:
...
def write_data_function_data(ft: hydra.ext.scala.meta.Data_FunctionData) -> hydra.ast.Expr:
    r"""Convert function data to an expression."""

    match ft:
        case hydra.ext.scala.meta.Data_FunctionDataFunction(value=f):
            params = f.params
            body = f.body
            @lru_cache(1)
...
def write_data_ref(ref: hydra.ext.scala.meta.Data_Ref) -> hydra.ast.Expr:
    r"""Convert a data reference to an expression."""

    match ref:
        case hydra.ext.scala.meta.Data_RefName(value=name):
            return write_data_name(name)

        case hydra.ext.scala.meta.Data_RefSelect(value=sel):
            return write_data_select(sel)

        case _:
            raise TypeError("Unsupported Data_Ref")

def write_data_select(sel: hydra.ext.scala.meta.Data_Select) -> hydra.ast.Expr:
    r"""Convert a data select to an expression."""

    arg = sel.qual
    name = sel.name
    return hydra.serialization.ifx(dot_op, write_term(arg), write_term(cast(hydra.ext.scala.meta.Data, hydra.ext.scala.meta.DataRef(cast(hydra.ext.scala.meta.Data_Ref, hydra.ext.scala.meta.Data_RefName(name))))))

def write_defn(def_: hydra.ext.scala.meta.Defn) -> hydra.ast.Expr:
    r"""Convert a definition to an expression."""

    match def_:
        case hydra.ext.scala.meta.DefnDef(value=dd):
            name = dd.name
            tparams = dd.tparams
            paramss = dd.paramss
            scod = dd.decltpe
        ...
        case hydra.ext.scala.meta.DefnType(value=dt):
            name = dt.name
            tparams = dt.tparams
            body = dt.body
        ...
        case hydra.ext.scala.meta.DefnVal(value=dv):
            mods = dv.mods
            pats = dv.pats
            typ = dv.decltpe
            rhs = dv.rhs
            @lru_cache(1)
            def first_pat() -> hydra.ext.scala.meta.Pat:
                return hydra.lib.lists.head(pats)
            @lru_cache(1)
            def pat_name():
                def _hoist_pat_name_1(v1):
                    match v1:
                        case hydra.ext.scala.meta.PatVar(value=pv):
                            return pv.name
        ...
        case hydra.ext.scala.meta.DefnClass(value=dc):
            mods = dc.mods
            name = dc.name
            tparams = dc.tparams
            ctor = dc.ctor
        ...
        case hydra.ext.scala.meta.DefnEnum(value=de):
            name = de.name
            tparams = de.tparams
        ...
        case hydra.ext.scala.meta.DefnEnumCase(value=dec):
            name = dec.name
            ctor = dec.ctor
            inits = dec.inits
            paramss = ctor.paramss
            @lru_cache(1)
            def all_params() -> frozenlist[hydra.ext.scala.meta.Data_Param]:
...
def write_pat(pat: hydra.ext.scala.meta.Pat) -> hydra.ast.Expr:
    r"""Convert a pattern to an expression."""

    match pat:
        case hydra.ext.scala.meta.PatExtract(value=pe):
            fun = pe.fun
            args = pe.args
        ...
        case hydra.ext.scala.meta.PatWildcard():
            return hydra.serialization.cst("_")

        case _:
            raise TypeError("Unsupported Pat")

def write_stat(stat: hydra.ext.scala.meta.Stat) -> hydra.ast.Expr:
    r"""Convert a statement to an expression."""

    match stat:
        case hydra.ext.scala.meta.StatTerm(value=t):
            return write_term(t)

        case hydra.ext.scala.meta.StatDefn(value=def_):
            return write_defn(def_)

        case hydra.ext.scala.meta.StatImportExport(value=ie):
            return write_import_export_stat(ie)

        case _:
            raise TypeError("Unsupported Stat")

def write_term(term: hydra.ext.scala.meta.Data) -> hydra.ast.Expr:
    r"""Convert a term to an expression."""

    match term:
        case hydra.ext.scala.meta.DataLit(value=lit):
            return write_lit(lit)

        case hydra.ext.scala.meta.DataRef(value=ref):
            return write_data_ref(ref)

        case hydra.ext.scala.meta.DataApply(value=app):
            fun = app.fun
            args = app.args
        ...
        case hydra.ext.scala.meta.DataFunctionData(value=ft):
            return write_data_function_data(ft)

        case hydra.ext.scala.meta.DataBlock(value=blk):
            stats = blk.stats
...
def write_pkg(pkg: hydra.ext.scala.meta.Pkg) -> hydra.ast.Expr:
