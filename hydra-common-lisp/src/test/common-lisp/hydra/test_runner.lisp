...
(defun run-json-coder-test (path tc)
  (let ((cx (empty-context)) (graph (get-test-graph)))
    (handler-case
      (let ((coder-result (funcall (funcall (funcall hydra_ext_org_json_coder_json_coder (a :type tc)) cx) graph)))
        (if (eq (first coder-result) :left)
            (progn (format t "FAIL: ~A~%  JSON coder creation failed: ~A~%" path (second coder-result)) (list 0 1 0))
            (let* ((coder (second coder-result))
                   (encode-fn (cdr (assoc :encode coder)))
                   (decode-fn (cdr (assoc :decode coder)))
                   (encode-result (funcall (funcall encode-fn cx) (a :term tc))))
              (if (eq (first encode-result) :left)
                  (progn (format t "FAIL: ~A~%  JSON encode failed~%" path) (list 0 1 0))
                  (let ((encoded (second encode-result)))
                    (if (not (terms-match-p (a :json tc) encoded))
                        ...
                        (let ((decode-result (funcall (funcall decode-fn cx) encoded)))
                          (if (eq (first decode-result) :left)
                              (progn (format t "FAIL: ~A~%  JSON decode failed~%" path) (list 0 1 0))
...
(defun run-json-roundtrip-test (path tc)
  (let ((cx (empty-context)) (graph (get-test-graph)))
    (handler-case
      (let ((coder-result (funcall (funcall (funcall hydra_ext_org_json_coder_json_coder (a :type tc)) cx) graph)))
        (if (eq (first coder-result) :left)
            (progn (format t "FAIL: ~A~%  JSON coder creation failed~%" path) (list 0 1 0))
            (let* ((coder (second coder-result))
                   (encode-fn (cdr (assoc :encode coder)))
                   (decode-fn (cdr (assoc :decode coder)))
                   (encode-result (funcall (funcall encode-fn cx) (a :term tc))))
              (if (eq (first encode-result) :left)
                  (progn (format t "FAIL: ~A~%  JSON encode failed~%" path) (list 0 1 0))
                  (let ((decode-result (funcall (funcall decode-fn cx) (second encode-result))))
                    (if (eq (first decode-result) :left)
                        (progn (format t "FAIL: ~A~%  JSON decode failed~%" path) (list 0 1 0))
                        (if (terms-match-p (a :term tc) (second decode-result))
                            (list 1 0 0)
    ...
    (handler-case
      (let ((coder-result (funcall (funcall (funcall hydra_ext_org_json_coder_json_coder (a :type tc)) cx) graph)))
        (if (eq (first coder-result) :left)
            ...
            (let* ((coder (second coder-result))
                   (decode-fn (cdr (assoc :decode coder)))
                   (decode-result (funcall (funcall decode-fn cx) (a :json tc)))
                   (expected (a :expected tc)))
              (cond
