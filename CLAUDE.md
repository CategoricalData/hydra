...
| `/improve-docs()` | Usually called at the end of a session. If there is anything you have learned during the session which would improve CLAUDE.md or the user documentation, then edit the docs, taking care to also keep them concise. Communicate with the user about major changes like splitting one document up into two. |
