...
#
# Usage: ./python-to-haskell.sh [--output DIR] [--types-only] [--kernel-only]
...
echo "Step 3: Generating Haskell code (via Python host)..."
STEP_START=$(date +%s)
"$SCRIPT_DIR/invoke-python-host.sh" --target haskell --output "$OUTPUT_BASE" --include-tests $EXTRA_FLAGS 2>&1
STEP_END=$(date +%s)
