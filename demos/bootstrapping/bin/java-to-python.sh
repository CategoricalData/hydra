...
#
# Usage: ./java-to-python.sh [--output DIR] [--types-only] [--kernel-only]
...
echo "=========================================="
echo "Java to Python bootstrapping demo"
echo "=========================================="
echo ""
...
echo "Step 3: Generating Python code (via Java host)..."
STEP_START=$(date +%s)
"$SCRIPT_DIR/invoke-java-host.sh" --target python --output "$OUTPUT_BASE" --include-tests $EXTRA_FLAGS 2>&1
STEP_END=$(date +%s)
