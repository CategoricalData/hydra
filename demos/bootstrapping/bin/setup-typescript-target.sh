...
# Hand-written test runtime: test runner (heads/) + testEnv + jsonBindings
# (overlay/). The runner lives in heads/typescript/src/test/; the hand-written
# test modules (testEnv with the standardPrimitives()-backed primitives map,
# jsonBindings) live in overlay/typescript/.../src/test/ (migrated in #434) and
# are NOT generated by the host — bootstrap.ts skip-emits hydra.core.test.testEnv, so
# the hand-written file must be copied in or reduceTerm gets an empty primitives
# map and every reduction test fails. (heads/.../hydra/test/ is empty.)
...
HYDRA_TS_TEST_OVERLAY="$HYDRA_ROOT/overlay/typescript/hydra-kernel/src/test/typescript/hydra/test"
for f in "$HYDRA_TS_TEST_OVERLAY"/*.ts; do
    [ -f "$f" ] || continue
