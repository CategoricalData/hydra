...
dataRowCsvCoder :: Coder (DataRow String) String Error
dataRowCsvCoder = Coder encode decode
  where
    encode (DataRow cells) = Right $ encodeCsvLine $ Y.catMaybes cells
    decode line = Right $ DataRow $ fmap Just $ decodeCsvLine line

tableCsvCoder :: Bool -> Coder (Table String) [String] Error
...
type DomainCoders t v = M.Map t (Coder v String Error)

relationCsvCoder :: (Ord t, Show t) => DomainCoders t v -> RM.RelationSchema t -> Bool -> InferenceContext -> Either Error (Coder (RM.Relation v) [String] Error)
...
rowCsvCoder :: (Ord t, Show t) => DomainCoders t v -> RM.RelationSchema t -> InferenceContext -> Either Error (Coder (RM.Row v) String Error)
rowCsvCoder coderMap schema cx = do
  ...
  where
    ...
    findCoder typ = case M.lookup typ coderMap of
      ...
      Just c -> pure c
    encodeCells coders = CM.zipWithM (\c v -> coderEncode c v) coders
    decodeCells coders = CM.zipWithM (\c v -> coderDecode c v) coders
