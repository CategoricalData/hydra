...
def checkDuplicateBindings(path: hydra.accessors.AccessorPath)(bindings: Seq[hydra.core.Binding]): Option[hydra.error.core.InvalidTermError] =
  {
  val names: Seq[hydra.core.Name] = hydra.lib.lists.map[hydra.core.Binding, hydra.core.Name]((x: hydra.core.Binding) => (x.name))(bindings)
  val dup: Option[hydra.core.Name] = hydra.validate.core.findDuplicate(names)
...
def checkTerm(path: hydra.accessors.AccessorPath)(term: hydra.core.Term): Option[hydra.error.core.InvalidTermError] =
  ...
  case hydra.core.Term.record(v_Term_record_rec) => hydra.validate.core.checkDuplicateFields(path)(hydra.lib.lists.map[hydra.core.Field,
     hydra.core.Name]((x: hydra.core.Field) => (x.name))(v_Term_record_rec.fields))
  case _ => None

def findDuplicate[T0](names: Seq[T0]): Option[T0] =
  {
  val result: Tuple2[scala.collection.immutable.Set[T0], Option[T0]] = hydra.lib.lists.foldl[Tuple2[scala.collection.immutable.Set[T0],
     Option[T0]], T0]((acc: Tuple2[scala.collection.immutable.Set[T0], Option[T0]]) =>
    (name: T0) =>
    {
    val seen: scala.collection.immutable.Set[T0] = hydra.lib.pairs.first[scala.collection.immutable.Set[T0], Option[T0]](acc)
    {
      val dup: Option[T0] = hydra.lib.pairs.second[scala.collection.immutable.Set[T0], Option[T0]](acc)
...
def term(g: hydra.graph.Graph)(t: hydra.core.Term): Option[hydra.error.core.InvalidTermError] =
  ...
  val checkResult: Option[hydra.error.core.InvalidTermError] = hydra.validate.core.checkTerm(path)(trm)
