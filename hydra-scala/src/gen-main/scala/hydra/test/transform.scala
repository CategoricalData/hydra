...
def transformToCompiledTests(tg: hydra.testing.TestGroup): Option[hydra.testing.TestGroup] =
  ...
  lazy val transformedCases: Seq[hydra.testing.TestCaseWithMetadata] = hydra.lib.maybes.cat[hydra.testing.TestCaseWithMetadata](hydra.lib.lists.map[hydra.testing.TestCaseWithMetadata, Option[hydra.testing.TestCaseWithMetadata]]((tc: hydra.testing.TestCaseWithMetadata) => hydra.test.transform.transformTestCase(tc))(`cases_`))
  lazy val transformedSubgroups: Seq[hydra.testing.TestGroup] = hydra.lib.maybes.cat[hydra.testing.TestGroup](hydra.lib.lists.map[hydra.testing.TestGroup, Option[hydra.testing.TestGroup]]((sg: hydra.testing.TestGroup) => hydra.test.transform.transformToCompiledTests(sg))(subgroups))
  hydra.lib.logic.ifElse[Option[hydra.testing.TestGroup]](hydra.lib.logic.and(hydra.lib.lists.`null`[hydra.testing.TestCaseWithMetadata](transformedCases))(hydra.lib.lists.`null`[hydra.testing.TestGroup](transformedSubgroups)))(None)(Some(hydra.testing.TestGroup(`name_`, desc, transformedSubgroups, transformedCases)))
}
...
def transformTestCase(tcm: hydra.testing.TestCaseWithMetadata): Option[hydra.testing.TestCaseWithMetadata] =
  ...
  tc match
    case hydra.testing.TestCase.caseConversion(v_TestCase_caseConversion_ccase) => {
      ...
      Some(hydra.testing.TestCaseWithMetadata(`name_`, hydra.testing.TestCase.delegatedEvaluation(hydra.testing.DelegatedEvaluationTestCase(hydra.test.transform.buildConvertCaseCall(fromConv)(toConv)(fromStr), hydra.core.Term.literal(hydra.core.Literal.string(toStr)))), desc, `tags_`))
    }
    ...
    case hydra.testing.TestCase.topologicalSort(v_TestCase_topologicalSort_tscase) => {
      ...
      Some(hydra.testing.TestCaseWithMetadata(`name_`, hydra.testing.TestCase.delegatedEvaluation(hydra.testing.DelegatedEvaluationTestCase(hydra.test.transform.buildTopologicalSortCall(adjList), hydra.test.transform.encodeEitherListList(expected))), desc, `tags_`))
    }
...
def buildTopologicalSortCall(adjList: Seq[Tuple2[Int, Seq[Int]]]): hydra.core.Term =
  hydra.core.Term.application(hydra.core.Application(hydra.core.Term.variable("hydra.sorting.topologicalSort"), hydra.test.transform.encodeAdjacencyList(adjList)))

def buildTopologicalSortSCCCall(adjList: Seq[Tuple2[Int, Seq[Int]]]): hydra.core.Term =
  hydra.core.Term.application(hydra.core.Application(hydra.core.Term.variable("hydra.sorting.topologicalSortComponents"), hydra.test.transform.encodeAdjacencyList(adjList)))
...
def encodeAdjacencyList(pairs: Seq[Tuple2[Int, Seq[Int]]]): hydra.core.Term =
  ...
  hydra.core.Term.pair(Tuple2(hydra.test.transform.encodeInt(hydra.lib.pairs.first[Int, Seq[Int]](p)), hydra.core.Term.list(hydra.lib.lists.map[Int, hydra.core.Term]((d: Int) => hydra.test.transform.encodeInt(d))(hydra.lib.pairs.second[Int, Seq[Int]](p))))))(pairs))
