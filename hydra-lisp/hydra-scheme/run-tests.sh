...
if command -v guile > /dev/null 2>&1; then
    SCHEME_CMD="guile --fresh-auto-compile -L src/gen-main/scheme -L src/gen-test/scheme -L src/main/scheme -s run-tests.scm"
...
EXIT_CODE=0
OUTPUT=$($SCHEME_CMD 2>&1 | tee /dev/stderr) || EXIT_CODE=$?
