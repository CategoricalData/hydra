  ...
  (cond
   ...
   ((string= target "haskell")
    (list (symbol-value 'hydra_ext_haskell_coder_module_to_haskell)
          ...
          "haskell"))
   ((string= target "clojure")
    (let ((mtl (symbol-value 'hydra_ext_lisp_coder_module_to_lisp))
          (pte (symbol-value 'hydra_ext_lisp_serde_program_to_expr)))
      (list (lambda (mod)
              (lambda (defs)
                (lambda (cx)
                  (lambda (g)
                    (let ((result (funcall (funcall (funcall (funcall (funcall mtl (list :clojure nil)) mod) defs) cx) g)))
                      (if (eq (car result) :left)
                        ...
                        (let* ((program (cadr result))
                               ...
                               (fp (format "%s.clj" (bootstrap-namespace-to-path ns-val))))
...
Write output to OUT-DIR. UNIVERSE-MODS is the full set; MODS-TO-GENERATE is the subset to generate."
  (let* ((bs-graph (bootstrap-graph))
         ...
         (do-hoist-case (caddr flags))
         (do-hoist-poly (cadddr flags))
         (t0 (float-time))
