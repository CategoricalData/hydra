...
(defvar hydra_encode_tabular_header_row (lambda (x) (list :wrap (make-hydra_core_wrapped_term "hydra.tabular.HeaderRow" (funcall (lambda (xs) (list :list (funcall (hydra_lib_lists_map (lambda (x) (list :literal (list :string x)))) xs))) (funcall (lambda (v) v) x))))))
