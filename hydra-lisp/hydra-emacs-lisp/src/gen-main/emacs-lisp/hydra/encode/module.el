...
(defvar hydra_encode_module_file_extension (lambda (x) (list :wrap (make-hydra_core_wrapped_term "hydra.module.FileExtension" (funcall (lambda (x) (list :literal (list :string x))) (funcall (lambda (v) v) x))))))

(defvar hydra_encode_module_namespace (lambda (x) (list :wrap (make-hydra_core_wrapped_term "hydra.module.Namespace" (funcall (lambda (x) (list :literal (list :string x))) (funcall (lambda (v) v) x))))))

(defvar hydra_encode_module_module (lambda (x) (list :record (make-hydra_core_record "hydra.module.Module" (list (make-hydra_core_field "namespace" (hydra_encode_module_namespace (funcall (lambda (v) (hydra_module_module-namespace v)) x))) (make-hydra_core_field "elements" (funcall (lambda (xs) (list :list (funcall (hydra_lib_lists_map hydra_encode_core_binding) xs))) (funcall (lambda (v) (hydra_module_module-elements v)) x))) (make-hydra_core_field "termDependencies" (funcall (lambda (xs) (list :list (funcall (hydra_lib_lists_map hydra_encode_module_namespace) xs))) (funcall (lambda (v) (hydra_module_module-term_dependencies v)) x))) (make-hydra_core_field "typeDependencies" (funcall (lambda (xs) (list :list (funcall (hydra_lib_lists_map hydra_encode_module_namespace) xs))) (funcall (lambda (v) (hydra_module_module-type_dependencies v)) x))) (make-hydra_core_field "description" (funcall (lambda (opt) (list :maybe (funcall (hydra_lib_maybes_map (lambda (x) (list :literal (list :string x)))) opt))) (funcall (lambda (v) (hydra_module_module-description v)) x))))))))
