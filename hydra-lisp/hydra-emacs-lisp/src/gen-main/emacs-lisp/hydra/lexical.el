...
(defvar hydra_lexical_dereference_variable (lambda (graph) (lambda (name) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (funcall (hydra_lib_strings_cat2 "no such element: ") (funcall (lambda (v) v) name))))) (lambda (right_) (list :right right_))) (funcall (hydra_lexical_lookup_element graph) name)))))
