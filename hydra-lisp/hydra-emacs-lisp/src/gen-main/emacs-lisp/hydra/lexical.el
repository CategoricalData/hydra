...
(defvar hydra_lexical_lookup_element (lambda (graph) (lambda (name) (funcall (hydra_lib_maybes_map (lambda (term) (make-hydra_core_binding name term (funcall (hydra_lib_maps_lookup name) (funcall (lambda (v) (hydra_graph_graph-bound_types v)) graph))))) (funcall (hydra_lib_maps_lookup name) (funcall (lambda (v) (hydra_graph_graph-bound_terms v)) graph))))))

(defvar hydra_lexical_dereference_element (lambda (graph) (lambda (name) (funcall (hydra_lexical_lookup_element graph) name))))
...
(defvar hydra_lexical_dereference_variable (lambda (graph) (lambda (name) (funcall (funcall (hydra_lib_maybes_maybe (list :left (funcall (hydra_lib_strings_cat2 "no such element: ") (funcall (lambda (v) v) name)))) (lambda (right_) (list :right right_))) (funcall (hydra_lexical_lookup_element graph) name)))))
...
(defvar hydra_lexical_get_field (lambda (cx) (lambda (m) (lambda (fname) (lambda (decode) (funcall (funcall (hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context (list :other (funcall (hydra_lib_strings_cat2 (funcall (hydra_lib_strings_cat2 "expected field ") (funcall (lambda (v) v) fname))) " not found")) cx))) decode) (funcall (hydra_lib_maps_lookup fname) m)))))))
...
(defvar hydra_lexical_require_primitive (lambda (cx) (lambda (graph) (lambda (name) (funcall (funcall (hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context (list :other (funcall (hydra_lib_strings_cat2 "no such primitive function: ") (funcall (lambda (v) v) name))) cx))) (lambda (x) (list :right x))) (funcall (hydra_lexical_lookup_primitive graph) name))))))

(defvar hydra_lexical_require_primitive_type (lambda (cx) (lambda (tx) (lambda (name) (let ((mts (funcall (hydra_lib_maps_lookup name) (hydra_lib_maps_from_list (funcall (hydra_lib_lists_map (lambda (_gpt_p) (list (funcall (lambda (v) (hydra_graph_primitive-name v)) _gpt_p) (funcall (lambda (v) (hydra_graph_primitive-type v)) _gpt_p)))) (hydra_lib_maps_elems (funcall (lambda (v) (hydra_graph_graph-primitives v)) tx))))))) (funcall (funcall (hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context (list :other (funcall (hydra_lib_strings_cat2 "no such primitive function: ") (funcall (lambda (v) v) name))) cx))) (lambda (ts) (list :right ts))) mts))))))

(defvar hydra_lexical_resolve_term (lambda (graph) (lambda (name) (let ((recurse (lambda (term) (let ((stripped (hydra_rewriting_deannotate_term term))) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :variable) (funcall (lambda (name_) (funcall (hydra_lexical_resolve_term graph) name_)) match_value)) (t (list :just term)))) (cadr match_target))) stripped))))) (funcall (funcall (hydra_lib_maybes_maybe (list :nothing)) recurse) (funcall (hydra_lexical_lookup_term graph) name))))))

(defvar hydra_lexical_require_term (lambda (cx) (lambda (graph) (lambda (name) (funcall (funcall (hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context (list :other (funcall (hydra_lib_strings_cat2 "no such element: ") (funcall (lambda (v) v) name))) cx))) (lambda (x) (list :right x))) (funcall (hydra_lexical_resolve_term graph) name))))))
