...
(defvar hydra_environment_definition_as_type_application_term (lambda (cx) (lambda (el) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (make-hydra_context_in_context (list :other "missing element type") cx)))) (lambda (ts) (list :right (make-hydra_core_type_application_term (funcall (lambda (v) (hydra_core_binding-term v)) el) (funcall (lambda (v) (hydra_core_type_scheme-type v)) ts))))) (funcall (lambda (v) (hydra_core_binding-type v)) el)))))
