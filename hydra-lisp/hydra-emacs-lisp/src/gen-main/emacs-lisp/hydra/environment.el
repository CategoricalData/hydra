...
(defvar hydra_environment_definition_as_type_application_term (lambda (el) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (list :extraction (list :unexpected_shape (make-hydra_errors_unexpected_shape_error "typed binding" "untyped binding")))))) (lambda (ts) (list :right (make-hydra_core_type_application_term (funcall (lambda (v) (hydra_core_binding-term v)) el) (funcall (lambda (v) (hydra_core_type_scheme-type v)) ts))))) (funcall (lambda (v) (hydra_core_binding-type v)) el))))
...
(defvar hydra_environment_graph_as_types (lambda (graph) (lambda (els) (let ((to_pair (lambda (el) (funcall (hydra_lib_eithers_map (lambda (typ) (list (funcall (lambda (v) (hydra_core_binding-name v)) el) typ))) (funcall (hydra_decode_core_type graph) (funcall (lambda (v) (hydra_core_binding-term v)) el)))))) (funcall (hydra_lib_eithers_map hydra_lib_maps_from_list) (funcall (hydra_lib_eithers_map_list to_pair) els))))))
...
(defvar hydra_environment_types_to_definitions (lambda (type_map) (let ((to_element (lambda (pair) (let ((name (hydra_lib_pairs_first pair))) (make-hydra_core_binding name (hydra_encode_core_type (hydra_lib_pairs_second pair)) (list :nothing)))))) (funcall (hydra_lib_lists_map to_element) (hydra_lib_maps_to_list type_map)))))
