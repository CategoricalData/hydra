...
(defvar hydra_ext_java_utils_add_in_scope_var (lambda (name) (lambda (aliases) (make-hydra_ext_java_helpers_aliases (funcall (lambda (v) (hydra_ext_java_helpers_aliases-current_namespace v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-packages v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-branch_vars v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-recursive_vars v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-in_scope_type_params v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-polymorphic_locals v)) aliases) (funcall (hydra_lib_sets_insert name) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-in_scope_java_vars v)) aliases)) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-var_renames v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-lambda_vars v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-type_var_subst v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-trusted_type_vars v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-method_codomain v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-thunked_vars v)) aliases)))))
...
(defvar hydra_ext_java_utils_add_java_type_parameter (lambda (rt) (lambda (t_) (lambda (cx) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :reference) (funcall (lambda (rt1) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :class_or_interface) (funcall (lambda (cit) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :class) (funcall (lambda (ct) (let* ((anns (funcall (lambda (v) (hydra_ext_java_syntax_class_type-annotations v)) ct)) (args (funcall (lambda (v) (hydra_ext_java_syntax_class_type-arguments v)) ct)) (id (funcall (lambda (v) (hydra_ext_java_syntax_class_type-identifier v)) ct)) (qual (funcall (lambda (v) (hydra_ext_java_syntax_class_type-qualifier v)) ct))) (list :right (list :reference (list :class_or_interface (list :class (make-hydra_ext_java_syntax_class_type anns qual id (funcall (hydra_lib_lists_concat2 args) (list (list :reference rt)))))))))) match_value)) ((equal (car match_target) :interface) (funcall (lambda (_) (list :left (make-hydra_context_in_context (list :other "expected a Java class type") cx))) match_value)))) (cadr match_target))) cit)) match_value)) ((equal (car match_target) :variable) (funcall (lambda (tv) (list :right (hydra_ext_java_utils_java_type_variable_to_type tv))) match_value)) ((equal (car match_target) :array) (funcall (lambda (_) (list :left (make-hydra_context_in_context (list :other "expected a Java class or interface type, or a variable") cx))) match_value)))) (cadr match_target))) rt1)) match_value)) ((equal (car match_target) :primitive) (funcall (lambda (_) (list :left (make-hydra_context_in_context (list :other "expected a reference type") cx))) match_value)))) (cadr match_target))) t_)))))

(defvar hydra_ext_java_utils_add_var_rename (lambda (original) (lambda (renamed) (lambda (aliases) (make-hydra_ext_java_helpers_aliases (funcall (lambda (v) (hydra_ext_java_helpers_aliases-current_namespace v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-packages v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-branch_vars v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-recursive_vars v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-in_scope_type_params v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-polymorphic_locals v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-in_scope_java_vars v)) aliases) (funcall (funcall (hydra_lib_maps_insert original) renamed) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-var_renames v)) aliases)) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-lambda_vars v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-type_var_subst v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-trusted_type_vars v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-method_codomain v)) aliases) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-thunked_vars v)) aliases))))))
...
(defvar hydra_ext_java_utils_lookup_java_var_name (lambda (aliases) (lambda (name) (funcall (funcall (hydra_lib_maybes_cases (funcall (hydra_lib_maps_lookup name) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-var_renames v)) aliases))) (lambda () name)) (lambda (renamed) renamed)))))
...
(defvar hydra_ext_java_utils_name_to_java_name (lambda (aliases) (lambda (name) (let* ((qn (hydra_names_qualify_name name)) (local (funcall (lambda (v) (hydra_module_qualified_name-local v)) qn)) (ns_ (funcall (lambda (v) (hydra_module_qualified_name-namespace v)) qn))) (if (hydra_ext_java_utils_is_escaped (funcall (lambda (v) v) name)) (hydra_ext_java_utils_sanitize_java_name local) (funcall (funcall (hydra_lib_maybes_cases ns_) (lambda () local)) (lambda (gname) (let* ((parts (funcall (funcall (hydra_lib_maybes_cases (funcall (hydra_lib_maps_lookup gname) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-packages v)) aliases))) (lambda () (funcall (hydra_lib_strings_split_on ".") (funcall (lambda (v) v) gname)))) (lambda (pkg_name) (funcall (hydra_lib_lists_map (lambda (i) (funcall (lambda (v) v) i))) (funcall (lambda (v) v) pkg_name))))) (all_parts (funcall (hydra_lib_lists_concat2 parts) (list (hydra_ext_java_utils_sanitize_java_name local))))) (funcall (hydra_lib_strings_intercalate ".") all_parts)))))))))
...
(defvar hydra_ext_java_utils_unique_var_name_go (lambda (aliases) (lambda (base) (lambda (n) (let ((candidate (funcall (hydra_lib_strings_cat2 base) (hydra_lib_literals_show_int32 n)))) (if (funcall (hydra_lib_sets_member candidate) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-in_scope_java_vars v)) aliases)) (funcall (funcall (hydra_ext_java_utils_unique_var_name_go aliases) base) (funcall (hydra_lib_math_add n) 1)) candidate))))))

(defvar hydra_ext_java_utils_unique_var_name (lambda (aliases) (lambda (name) (if (funcall (hydra_lib_sets_member name) (funcall (lambda (v) (hydra_ext_java_helpers_aliases-in_scope_java_vars v)) aliases)) (funcall (funcall (hydra_ext_java_utils_unique_var_name_go aliases) (funcall (lambda (v) v) name)) 2) name))))
