...
(defvar hydra_ext_org_json_decoding_decode_array (lambda (decode_elem) (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :array) (funcall (lambda (a) (funcall (hydra_lib_eithers_map_list decode_elem) a)) match_value)) (t (list :left "expected an array")))) (cadr match_target)))))
...
(defvar hydra_ext_org_json_decoding_decode_object (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :object) (funcall (lambda (o) (list :right o)) match_value)) (t (list :left "expected an object")))) (cadr match_target))))

(defvar hydra_ext_org_json_decoding_decode_string (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :string) (funcall (lambda (s) (list :right s)) match_value)) (t (list :left "expected a string")))) (cadr match_target))))
