...
(defvar hydra_ext_org_json_decoding_decode_optional_field (lambda (decode_value) (lambda (name) (lambda (m) (((hydra_lib_maybes_maybe (list :right (list :nothing))) (lambda (v) ((hydra_lib_eithers_map (lambda (x) (list :just x))) (decode_value v)))) ((hydra_lib_maps_lookup name) m))))))
