...
(defvar hydra_resolution_dereference_type (lambda (cx) (lambda (graph) (lambda (name) (let ((mel (funcall (hydra_lexical_dereference_element graph) name))) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :right (list :nothing)))) (lambda (el) (funcall (hydra_lib_eithers_map hydra_lib_maybes_pure) (funcall (funcall (hydra_lib_eithers_bimap (lambda (_wc_e) (make-hydra_context_in_context _wc_e cx))) (lambda (_wc_a) _wc_a)) (funcall (funcall (hydra_lib_eithers_bimap (lambda (_e) (list :other (funcall (lambda (v) v) _e)))) (lambda (_a) _a)) (funcall (hydra_decode_core_type graph) (funcall (lambda (v) (hydra_core_binding-term v)) el))))))) mel))))))
