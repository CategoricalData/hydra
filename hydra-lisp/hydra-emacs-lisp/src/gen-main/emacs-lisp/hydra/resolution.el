...
(defvar hydra_resolution_find_field_type (lambda (cx) (lambda (fname) (lambda (fields) (let ((matching_fields (funcall (hydra_lib_lists_filter (lambda (ft) (funcall (hydra_lib_equality_equal (funcall (lambda (v) v) (funcall (lambda (v) (hydra_core_field_type-name v)) ft))) (funcall (lambda (v) v) fname)))) fields))) (if (hydra_lib_lists_null matching_fields) (list :left (list :resolution (list :no_matching_field (make-hydra_errors_no_matching_field_error fname)))) (if (funcall (hydra_lib_equality_equal (hydra_lib_lists_length matching_fields)) 1) (list :right (funcall (lambda (v) (hydra_core_field_type-type v)) (hydra_lib_lists_head matching_fields))) (list :left (list :extraction (list :multiple_fields (make-hydra_errors_multiple_fields_error fname)))))))))))
