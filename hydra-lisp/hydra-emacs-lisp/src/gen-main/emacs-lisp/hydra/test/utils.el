...
(defvar hydra_test_utils_infer_term (lambda (g) (lambda (term) (funcall (funcall (hydra_lib_eithers_bimap (lambda (ic) (hydra_show_error_error (funcall (lambda (v) (hydra_context_in_context-object v)) ic)))) (lambda (x) (funcall (lambda (v) (hydra_typing_inference_result-term v)) x))) (funcall (funcall (hydra_inference_infer_in_graph_context hydra_lexical_empty_context) g) term)))))
