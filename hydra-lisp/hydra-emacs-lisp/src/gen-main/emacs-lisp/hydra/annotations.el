...
(defvar hydra_annotations_get_debug_id (lambda (cx) (((hydra_lib_maybes_maybe (list :right (list :nothing))) (lambda (term) ((hydra_lib_eithers_map hydra_lib_maybes_pure) (((hydra_extract_core_string cx) (make-hydra_graph_graph hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_sets_empty hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_sets_empty)) term)))) ((hydra_annotations_get_attr hydra_constants_key_debug_id) cx))))

(defvar hydra_annotations_debug_if (lambda (cx) (lambda (debug_id) (lambda (message) ((hydra_lib_eithers_bind (hydra_annotations_get_debug_id cx)) (lambda (mid) (if ((hydra_lib_equality_equal mid) (list :just debug_id)) (list :left (make-hydra_context_in_context (list :other message) cx)) (list :right nil))))))))
...
(defvar hydra_annotations_fail_on_flag (lambda (cx) (lambda (flag) (lambda (msg) ((hydra_lib_eithers_bind ((hydra_annotations_has_flag cx) flag)) (lambda (val) (if val (list :left (make-hydra_context_in_context (list :other msg) cx)) (list :right nil))))))))
...
(defvar hydra_annotations_get_description (lambda (cx) (lambda (graph) (lambda (anns) (((hydra_lib_maybes_maybe (list :right (list :nothing))) (lambda (term) ((hydra_lib_eithers_map hydra_lib_maybes_pure) (((hydra_extract_core_string cx) graph) term)))) ((hydra_lib_maps_lookup "description") anns))))))

(defvar hydra_annotations_term_annotation_internal (lambda (term) (let ((get_ann (lambda (t_) ((lambda (match_target) ((lambda (match_value) (cond ((equal (car match_target) :annotated) ((lambda (a) (list :just a)) match_value)) (t (list :nothing)))) (cadr match_target))) t_)))) ((((hydra_annotations_aggregate_annotations get_ann) (lambda (at) ((lambda (v) (hydra_core_annotated_term-body v)) at))) (lambda (at) ((lambda (v) (hydra_core_annotated_term-annotation v)) at))) term))))
...
(defvar hydra_annotations_get_type (lambda (graph) (lambda (anns) (((hydra_lib_maybes_maybe (list :right (list :nothing))) (lambda (dat) ((hydra_lib_eithers_map hydra_lib_maybes_pure) ((hydra_decode_core_type graph) dat)))) ((hydra_lib_maps_lookup hydra_constants_key_type) anns)))))
...
(defvar hydra_annotations_get_type_classes (lambda (cx) (lambda (graph) (lambda (term) (let ((decode_class (lambda (term) (let ((by_name (hydra_lib_maps_from_list (list (list "equality" (list :equality nil)) (list "ordering" (list :ordering nil)))))) ((hydra_lib_eithers_bind ((((hydra_extract_core_unit_variant cx) "hydra.classes.TypeClass") graph) term)) (lambda (fn_) (((hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context (list :other ((hydra_lib_strings_cat2 "unexpected: expected type class, got ") (hydra_show_core_term term))) cx))) (lambda (x) (list :right x))) ((hydra_lib_maps_lookup fn_) by_name)))))))) (((hydra_lib_maybes_maybe (list :right hydra_lib_maps_empty)) (lambda (term) (((((hydra_extract_core_map cx) (lambda (t_) (((hydra_lib_eithers_bimap (lambda (de) (make-hydra_context_in_context (list :other ((lambda (v) v) de)) cx))) (lambda (x) x)) ((hydra_decode_core_name graph) t_)))) (((hydra_extract_core_set_of cx) decode_class) graph)) graph) term))) ((hydra_annotations_get_term_annotation hydra_constants_key_classes) term)))))))
...
(defvar hydra_annotations_is_native_type (lambda (el) (let ((is_flagged_as_first_class_type ((hydra_lib_maybes_from_maybe nil) ((hydra_lib_maybes_map (lambda (_) t)) ((hydra_annotations_get_term_annotation hydra_constants_key_first_class_type) ((lambda (v) (hydra_core_binding-term v)) el)))))) (((hydra_lib_maybes_maybe nil) (lambda (ts) ((hydra_lib_logic_and ((hydra_lib_equality_equal ts) (make-hydra_core_type_scheme (list) (list :variable "hydra.core.Type") (list :nothing)))) (hydra_lib_logic_not is_flagged_as_first_class_type)))) ((lambda (v) (hydra_core_binding-type v)) el)))))
