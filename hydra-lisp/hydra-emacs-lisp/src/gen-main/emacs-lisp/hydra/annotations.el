...
(defvar hydra_annotations_aggregate_annotations (lambda (get_value) (lambda (get_x) (lambda (get_anns) (lambda (t_) (letrec ((to_pairs (lambda (rest) (lambda (t_) (funcall (funcall (hydra_lib_maybes_maybe rest) (lambda (yy) (funcall (to_pairs (funcall (hydra_lib_lists_cons (hydra_lib_maps_to_list (get_anns yy))) rest)) (get_x yy)))) (get_value t_)))))) (hydra_lib_maps_from_list (hydra_lib_lists_concat (funcall (to_pairs (list)) t_)))))))))
...
(defvar hydra_annotations_get_debug_id (lambda (cx) (funcall (funcall (hydra_lib_maybes_maybe (list :right (list :nothing))) (lambda (term) (funcall (hydra_lib_eithers_map hydra_lib_maybes_pure) (funcall (funcall (hydra_extract_core_string cx) (make-hydra_graph_graph hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_sets_empty hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_sets_empty)) term)))) (funcall (hydra_annotations_get_attr hydra_constants_key_debug_id) cx))))
...
(defvar hydra_annotations_get_attr_with_default (lambda (key) (lambda (def_) (lambda (cx) (funcall (hydra_lib_maybes_from_maybe def_) (funcall (hydra_annotations_get_attr key) cx))))))
...
(defvar hydra_annotations_get_type (lambda (graph) (lambda (anns) (funcall (funcall (hydra_lib_maybes_maybe (list :right (list :nothing))) (lambda (dat) (funcall (hydra_lib_eithers_map hydra_lib_maybes_pure) (funcall (hydra_decode_core_type graph) dat)))) (funcall (hydra_lib_maps_lookup hydra_constants_key_type) anns)))))
...
(defvar hydra_annotations_get_type_classes (lambda (cx) (lambda (graph) (lambda (term) (let ((decode_class (lambda (term) (let ((by_name (hydra_lib_maps_from_list (list (list "equality" (list :equality nil)) (list "ordering" (list :ordering nil)))))) (funcall (hydra_lib_eithers_bind (funcall (funcall (funcall (hydra_extract_core_unit_variant cx) "hydra.classes.TypeClass") graph) term)) (lambda (fn_) (funcall (funcall (hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context (list :other (funcall (hydra_lib_strings_cat2 "unexpected: expected type class, got ") (hydra_show_core_term term))) cx))) (lambda (x) (list :right x))) (funcall (hydra_lib_maps_lookup fn_) by_name)))))))) (funcall (funcall (hydra_lib_maybes_maybe (list :right hydra_lib_maps_empty)) (lambda (term) (funcall (funcall (funcall (funcall (hydra_extract_core_map cx) (lambda (t_) (funcall (funcall (hydra_lib_eithers_bimap (lambda (de) (make-hydra_context_in_context (list :other (funcall (lambda (v) v) de)) cx))) (lambda (x) x)) (funcall (hydra_decode_core_name graph) t_)))) (funcall (funcall (hydra_extract_core_set_of cx) decode_class) graph)) graph) term))) (funcall (hydra_annotations_get_term_annotation hydra_constants_key_classes) term)))))))
