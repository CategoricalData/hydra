...
(defvar hydra_extract_core_cases (lambda (name) (lambda (graph) (lambda (term0) (funcall (hydra_lib_eithers_bind (funcall (hydra_lexical_strip_and_dereference_term graph) term0)) (lambda (term) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :function) (funcall (lambda (function_) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :elimination) (funcall (lambda (elimination) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :union) (funcall (lambda (cs) (if (funcall (hydra_lib_equality_equal (funcall (lambda (v) v) (funcall (lambda (v) (hydra_core_case_statement-type_name v)) cs))) (funcall (lambda (v) v) name)) (list :right cs) (list :left (list :extraction (list :unexpected_shape (make-hydra_errors_unexpected_shape_error (funcall (hydra_lib_strings_cat2 "case statement for type ") (funcall (lambda (v) v) name)) (hydra_show_core_term term))))))) match_value)) (t (list :left (list :extraction (list :unexpected_shape (make-hydra_errors_unexpected_shape_error "case statement" (hydra_show_core_term term)))))))) (cadr match_target))) elimination)) match_value)) (t (list :left (list :extraction (list :unexpected_shape (make-hydra_errors_unexpected_shape_error "case statement" (hydra_show_core_term term)))))))) (cadr match_target))) function_)) match_value)) (t (list :left (list :extraction (list :unexpected_shape (make-hydra_errors_unexpected_shape_error "case statement" (hydra_show_core_term term)))))))) (cadr match_target))) term)))))))
