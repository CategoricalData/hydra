...
(def hydra_annotations_get_type_classes (fn [cx] (fn [graph] (fn [term] (let [decode_class (fn [term2] (let [by_name (hydra_lib_maps_from_list (list (list "equality" (list :equality nil)) (list "ordering" (list :ordering nil))))] ((hydra_lib_eithers_bind ((((hydra_extract_core_unit_variant cx) "hydra.classes.TypeClass") graph) term2)) (fn [fn_] (((hydra_lib_maybes_maybe (fn [] (list :left (->hydra_context_in_context (list :other ((hydra_lib_strings_cat2 "unexpected: expected type class, got ") (hydra_show_core_term term2))) cx)))) (fn [x] (list :right x))) ((hydra_lib_maps_lookup fn_) by_name))))))] (((hydra_lib_maybes_maybe (fn [] (list :right hydra_lib_maps_empty))) (fn [term2] (((((hydra_extract_core_map cx) (fn [t_] (((hydra_lib_eithers_bimap (fn [de] (->hydra_context_in_context (list :other ((fn [v] v) de)) cx))) (fn [x] x)) ((hydra_decode_core_name graph) t_)))) (((hydra_extract_core_set_of cx) decode_class) graph)) graph) term2))) ((hydra_annotations_get_term_annotation hydra_constants_key_classes) term)))))))
