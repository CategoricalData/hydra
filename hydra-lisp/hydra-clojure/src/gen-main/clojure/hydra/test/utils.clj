(ns hydra.test.utils
  (:require [hydra.context :refer :all] [hydra.inference :refer :all] [hydra.lexical :refer :all] [hydra.lib.eithers :refer :all] [hydra.show.error :refer :all] [hydra.testing :refer :all] [hydra.typing :refer :all]
))

(declare hydra_test_utils_infer_term hydra_test_utils_infer_test_case hydra_test_utils_infer_test_group_terms)

(def hydra_test_utils_infer_term (fn [g] (fn [term] (((hydra_lib_eithers_bimap (fn [ic] (hydra_show_error_error ((fn [v] (:object v)) ic)))) (fn [x] ((fn [v] (:term v)) x))) (((hydra_inference_infer_in_graph_context hydra_lexical_empty_context) g) term)))))
