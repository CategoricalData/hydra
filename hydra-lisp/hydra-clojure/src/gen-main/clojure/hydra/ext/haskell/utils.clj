...
(def hydra_ext_haskell_utils_element_reference (fn [namespaces] (fn [name] (let [qname (hydra_names_qualify_name name) local ((fn [v] (:local v)) qname) esc_local (hydra_ext_haskell_utils_sanitize_haskell_name local) namespace_pair ((fn [v] (:focus v)) namespaces) gmod ((fn [v] v) (hydra_lib_pairs_second namespace_pair)) gname (hydra_lib_pairs_first namespace_pair) mns ((fn [v] (:namespace v)) qname) namespaces_map ((fn [v] (:mapping v)) namespaces)] (((hydra_lib_maybes_cases ((fn [v] (:namespace v)) qname)) (hydra_ext_haskell_utils_simple_name local)) (fn [ns_] (((hydra_lib_maybes_cases ((hydra_lib_maps_lookup ns_) namespaces_map)) (hydra_ext_haskell_utils_simple_name local)) (fn [mn] (let [alias_str ((fn [v] v) mn)] (if ((hydra_lib_equality_equal ns_) gname) (hydra_ext_haskell_utils_simple_name esc_local) (hydra_ext_haskell_utils_raw_name (hydra_lib_strings_cat (list alias_str "." (hydra_ext_haskell_utils_sanitize_haskell_name local))))))))))))))
