(ns hydra.ext.java.utils
  (:require [hydra.context :refer :all] [hydra.core :refer :all] [hydra.error :refer :all] [hydra.ext.java.helpers :refer :all] [hydra.ext.java.language :refer :all] [hydra.ext.java.names :refer :all] [hydra.ext.java.syntax :refer :all] [hydra.formatting :refer :all] [hydra.lib.eithers :refer :all] [hydra.lib.equality :refer :all] [hydra.lib.lists :refer :all] [hydra.lib.literals :refer :all] [hydra.lib.logic :refer :all] [hydra.lib.maps :refer :all] [hydra.lib.math :refer :all] [hydra.lib.maybes :refer :all] [hydra.lib.pairs :refer :all] [hydra.lib.sets :refer :all] [hydra.lib.strings :refer :all] [hydra.module :refer :all] [hydra.names :refer :all]
))
...
(def hydra_ext_java_utils_add_java_type_parameter (fn [rt] (fn [t_] (fn [cx] ((fn [match_target] ((fn [match_value] (cond (= (first match_target) :reference) ((fn [rt1] ((fn [match_target] ((fn [match_value] (cond (= (first match_target) :class_or_interface) ((fn [cit] ((fn [match_target] ((fn [match_value] (cond (= (first match_target) :class) ((fn [ct] (let [anns ((fn [v] (:annotations v)) ct) args ((fn [v] (:arguments v)) ct) id ((fn [v] (:identifier v)) ct) qual ((fn [v] (:qualifier v)) ct)] (list :right (list :reference (list :class_or_interface (list :class (->hydra_ext_java_syntax_class_type anns qual id ((hydra_lib_lists_concat2 args) (list (list :reference rt)))))))))) match_value) (= (first match_target) :interface) ((fn [_] (list :left (->hydra_context_in_context (list :other "expected a Java class type") cx))) match_value))) (second match_target))) cit)) match_value) (= (first match_target) :variable) ((fn [tv] (list :right (hydra_ext_java_utils_java_type_variable_to_type tv))) match_value) (= (first match_target) :array) ((fn [_] (list :left (->hydra_context_in_context (list :other "expected a Java class or interface type, or a variable") cx))) match_value))) (second match_target))) rt1)) match_value) (= (first match_target) :primitive) ((fn [_] (list :left (->hydra_context_in_context (list :other "expected a reference type") cx))) match_value))) (second match_target))) t_)))))

(def hydra_ext_java_utils_add_var_rename (fn [original] (fn [renamed] (fn [aliases] (->hydra_ext_java_helpers_aliases ((fn [v] (:current_namespace v)) aliases) ((fn [v] (:packages v)) aliases) ((fn [v] (:branch_vars v)) aliases) ((fn [v] (:recursive_vars v)) aliases) ((fn [v] (:in_scope_type_params v)) aliases) ((fn [v] (:polymorphic_locals v)) aliases) ((fn [v] (:in_scope_java_vars v)) aliases) (((hydra_lib_maps_insert original) renamed) ((fn [v] (:var_renames v)) aliases)) ((fn [v] (:lambda_vars v)) aliases) ((fn [v] (:type_var_subst v)) aliases) ((fn [v] (:trusted_type_vars v)) aliases) ((fn [v] (:method_codomain v)) aliases) ((fn [v] (:thunked_vars v)) aliases))))))
...
(def hydra_ext_java_utils_java_type_to_java_reference_type (fn [t_] (fn [cx] ((fn [match_target] ((fn [match_value] (cond (= (first match_target) :reference) ((fn [rt] (list :right rt)) match_value) (= (first match_target) :primitive) ((fn [_] (list :left (->hydra_context_in_context (list :other "expected a Java reference type") cx))) match_value))) (second match_target))) t_))))
...
(def hydra_ext_java_utils_to_java_array_type (fn [t_] (fn [cx] ((fn [match_target] ((fn [match_value] (cond (= (first match_target) :reference) ((fn [rt] ((fn [match_target] ((fn [match_value] (cond (= (first match_target) :class_or_interface) ((fn [cit] (list :right (list :reference (list :array (->hydra_ext_java_syntax_array_type (list (list)) (list :class_or_interface cit)))))) match_value) (= (first match_target) :array) ((fn [at] (let [old_dims ((fn [v] v) ((fn [v] (:dims v)) at)) new_dims ((hydra_lib_lists_concat2 old_dims) (list (list))) variant ((fn [v] (:variant v)) at)] (list :right (list :reference (list :array (->hydra_ext_java_syntax_array_type new_dims variant)))))) match_value) (= (first match_target) :variable) ((fn [_] (list :left (->hydra_context_in_context (list :other "don't know how to make Java reference type into array type") cx))) match_value))) (second match_target))) rt)) match_value) (= (first match_target) :primitive) ((fn [_] (list :left (->hydra_context_in_context (list :other "don't know how to make Java type into array type") cx))) match_value))) (second match_target))) t_))))
