...
(:use :cl :hydra.annotations :hydra.constants :hydra.core :hydra.formatting :hydra.lib.equality :hydra.lib.lists :hydra.lib.literals :hydra.lib.logic :hydra.lib.maps :hydra.lib.math :hydra.lib.maybes :hydra.lib.pairs :hydra.lib.sets :hydra.lib.strings :hydra.module :hydra.util)
(:export :hydra_names_qualify_name :hydra_names_compact_name :hydra_names_normal_type_variable :hydra_names_fresh_name :hydra_names_fresh_names :hydra_names_local_name_of :hydra_names_namespace_of :hydra_names_namespace_to_file_path :hydra_names_qname :hydra_names_unique_label :hydra_names_unqualify_name))

(in-package :hydra.names)

(cl:defvar hydra_names_qualify_name (cl:lambda (name) (let ((parts (hydra_lib_lists_reverse ((hydra_lib_strings_split_on ".") ((cl:lambda (v) v) name))))) (if ((hydra_lib_equality_equal 1) (hydra_lib_lists_length parts)) (make-hydra_module_qualified_name (list :nothing) ((cl:lambda (v) v) name)) (make-hydra_module_qualified_name (list :just ((hydra_lib_strings_intercalate ".") (hydra_lib_lists_reverse (hydra_lib_lists_tail parts)))) (hydra_lib_lists_head parts))))))

(cl:defvar hydra_names_compact_name (cl:lambda (namespaces) (cl:lambda (name) (let* ((qual_name (hydra_names_qualify_name name)) (local ((cl:lambda (v) (hydra_module_qualified_name-local v)) qual_name)) (mns ((cl:lambda (v) (hydra_module_qualified_name-namespace v)) qual_name))) (((hydra_lib_maybes_maybe (cl:lambda () ((cl:lambda (v) v) name))) (cl:lambda (ns_) (((hydra_lib_maybes_maybe (cl:lambda () local)) (cl:lambda (pre) (hydra_lib_strings_cat (cl:list pre ":" local)))) ((hydra_lib_maps_lookup ns_) namespaces)))) mns)))))
...
(cl:defvar hydra_names_unqualify_name (cl:lambda (qname) (let ((prefix (((hydra_lib_maybes_maybe (cl:lambda () "")) (cl:lambda (n) ((hydra_lib_strings_cat2 ((cl:lambda (v) v) n)) "."))) ((cl:lambda (v) (hydra_module_qualified_name-namespace v)) qname)))) ((hydra_lib_strings_cat2 prefix) ((cl:lambda (v) (hydra_module_qualified_name-local v)) qname)))))
