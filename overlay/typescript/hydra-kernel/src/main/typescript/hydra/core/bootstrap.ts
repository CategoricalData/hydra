...
const main = async (): Promise<void> => {
  ...
  // is missing files that the hand-written `Hydra.Haskell.Dsl.Syntax`
  ...
  if (opts.includeTests) {
    ...
    // #546/#547/#575/#588: the kernel test suite references hydra.core.test.build.* ->
    ...
    // with "Unknown variable: hydra.build.test.modules.allTests". Mirrors the
  ...
  // contains a serialized hydra.core.packaging.Module. We use the bootstrap
  // schema map (typesByName from hydra/json/bootstrap.ts) so the decoder
  // knows about hydra.core.packaging.Module without needing a populated graph.
  // Pipeline:
  //   raw JSON → hydra.core.json.model.Value (tagged-union shape) via toHydraJson
  ...
  // without them, every module that calls `hydra.core.lib.maps.empty` etc.
  ...
  const modName = { value: "hydra.core.packaging.Module" };
  ...
  // namespace hydra.core.overlay.<lang>.lib.<sub>, where the runtime implementations live
  ...
  const redirectLibRefs = (src: string): string => {
    ...
    const oldPfx = "hydra.core.lib.";
    const newPfx = "hydra.core.overlay." + langSeg + ".lib.";
    // Protect string literals "hydra.core.lib.X" (data, not import refs) from rewriting.
    const sentinel = " HYDRALIBNAME ";
    let s = src.split("\"hydra.core.lib.").join("\"" + sentinel);
    for (const sub of subs) {
      // member access, import-terminators, and "from hydra.core.lib import X" (python).
      s = s.split(oldPfx + sub + ".").join(newPfx + sub + ".");
      s = s.split(oldPfx + sub + ";").join(newPfx + sub + ";");
      s = s.split(oldPfx + sub + "\n").join(newPfx + sub + "\n");
      s = s.split(oldPfx + sub + " ").join(newPfx + sub + " ");
      s = s.split("hydra.core.lib import " + sub).join("hydra.core.overlay." + langSeg + ".lib import " + sub);
    }
    return s.split(sentinel).join("hydra.core.lib.");
  };
  ...
  // (`Hydra.Haskell.Dsl.*`) import the corresponding generated modules
  // (`Hydra.Haskell.*`).
  const emitPkgs = new Set<string>(["hydra-kernel"]);
  if (opts.target === "haskell") emitPkgs.add("hydra-haskell");
  // hydra-build's test modules (hydra.core.test.build.*) ARE emitted alongside the
  ...
  // Skip-emit: hydra.core.test.testEnv is hand-written (it builds the primitives map
  ...
  const testSkipEmitModuleNames = new Set<string>(["hydra.core.test.testEnv"]);
  ...
  if (twoPassLibTargets.has(opts.target)) {
    ...
    const isLibMod = (m: unknown): boolean => {
      ...
      return name.startsWith("hydra.core.lib.");
    };
