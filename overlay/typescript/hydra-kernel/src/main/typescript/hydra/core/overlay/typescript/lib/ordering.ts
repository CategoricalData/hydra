// Hand-written runtime: hydra.core.lib.ordering primitives.
//
...
// Declared-variant order for each hydra.core.model union family, transcribed from
...
// hand-authored hydra.core.reflect DSL module (Sources/Kernel/Terms/Reflect.hs).
...
// Returns -1, 0, or 1. Exported for reuse by hydra.core.lib.equality (equal(a, b)
// := compareValues(a, b) === 0).
export const compareValues = (a: unknown, b: unknown): number => {
  ...
  // ({tag:"left"|"right"}), and hydra.core.model unions (Term, Type, Literal, ...).
...
// Hydra `compare` returns a hydra.core.util.Comparison value:
