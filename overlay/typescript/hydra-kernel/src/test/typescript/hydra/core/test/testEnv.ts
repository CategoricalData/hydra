...
// hydra.core.test.test_graph module. Mirrors the role of
...
//   hydra.core.test.testEnv.testContext :: InferenceContext
//   hydra.core.test.testEnv.testGraph   :: Map Name Type -> Map Name Term -> Graph
//
...
//
// The TypeScript coder filters hydra.core.test.testEnv from emitted output (via
...
// are the evaluator-essential namespaces (hydra.core.annotations, etc.) plus
// the type-bearing modules (hydra.core.model, hydra.core.graph, etc.) that
...
const _cachedKernelTypes: ReadonlyMap<Name, TypeScheme> = maps.fromList(
  _kernelLoaded.types.map(([k, ts]) => {
    ...
    return [k, { variables: vars, body, constraints: tsAny.constraints ?? maps.empty } as unknown as TypeScheme] as const;
  }));
...
const buildGraph = (testTypes: ReadonlyMap<Name, Type>, testTerms: ReadonlyMap<Name, Term>): Graph => {
  ...
  const wrappedTestTypes: ReadonlyMap<Name, TypeScheme> = maps.fromList(
    maps.toList(testTypes).map(([k, v]) => {
      ...
      return [k, { variables: vars, body, constraints: maps.empty } as unknown as TypeScheme] as const;
    }));
  ...
  // assertions for tests like `hydra.core.lib.maps.lookup` see kernel-type
