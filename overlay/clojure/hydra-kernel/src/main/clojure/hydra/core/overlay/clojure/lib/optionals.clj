(ns hydra.core.overlay.clojure.lib.optionals)

;; Optional representation: nil, (:given val), or (:none) for native optionals.
;; IMPORTANT: Do NOT treat (:optional ...) as a native optional — that's
;; the Hydra Term.Optional term constructor, not an optional value.

(defn maybe-nothing? [m]
  (or (nil? m)
      (and (sequential? m) (empty? m))
      (and (sequential? m) (= (first m) :none))))

(defn maybe-value [m]
  (cond
    (and (sequential? m) (= (first m) :given)) (second m)
    :else m))

;; bind :: Maybe a -> (a -> Maybe b) -> Maybe b
(def hydra_overlay_clojure_lib_optionals_bind
  (fn [m] (fn [f]
    (if (maybe-nothing? m)
      (list :none)
      (f (maybe-value m))))))

;; givens :: [Maybe a] -> [a]
(def hydra_overlay_clojure_lib_optionals_givens
  (fn [ms]
    (loop [rest_ ms acc ()]
      (if (empty? rest_)
        (reverse acc)
        (let [m (first rest_)]
          (if (not (maybe-nothing? m))
            (recur (next rest_) (cons (maybe-value m) acc))
            (recur (next rest_) acc)))))))

;; with_default :: a -> Maybe a -> a
;; Thunk-aware: if def_ is a zero-arg fn, only called when Maybe is Nothing
(def hydra_overlay_clojure_lib_optionals_with_default
  (fn [def_] (fn [m]
    (if (maybe-nothing? m)
      (if (fn? def_) (def_) def_)
      (maybe-value m)))))

;; is_given :: Maybe a -> Bool
(def hydra_overlay_clojure_lib_optionals_is_given
  (fn [m] (not (maybe-nothing? m))))

;; is_none :: Maybe a -> Bool
(def hydra_overlay_clojure_lib_optionals_is_none
  (fn [m] (maybe-nothing? m)))

;; map :: (a -> b) -> Maybe a -> Maybe b
(def hydra_overlay_clojure_lib_optionals_map
  (fn [f] (fn [m]
    (if (maybe-nothing? m)
      (if (or (nil? m) (and (sequential? m) (empty? m)))
        nil  ;; preserve nil/empty representation for term-level
        (list :none))
      (let [result (f (maybe-value m))]
        (if (and (sequential? m) (= (first m) :given))
          (list :given result)
          result))))))

;; map_optional :: (a -> Maybe b) -> [a] -> [b]
(def hydra_overlay_clojure_lib_optionals_map_optional
  (fn [f] (fn [xs]
    (loop [rest_ (seq xs) acc ()]
      (if (nil? rest_)
        (reverse acc)
        (let [result (f (first rest_))]
          (if (not (maybe-nothing? result))
            (recur (next rest_) (cons (maybe-value result) acc))
            (recur (next rest_) acc))))))))

;; apply :: Maybe (a -> b) -> Maybe a -> Maybe b
(def hydra_overlay_clojure_lib_optionals_apply
  (fn [mf] (fn [ma]
    (if (maybe-nothing? mf)
      (list :none)
      (if (maybe-nothing? ma)
        (list :none)
        (list :given ((maybe-value mf) (maybe-value ma))))))))

;; match :: Maybe a -> b -> (a -> b) -> b
;; Thunk-aware: if nothing-val is a zero-arg fn, only called when Maybe is Nothing
(def hydra_overlay_clojure_lib_optionals_match
  (fn [m] (fn [nothing-val] (fn [just-fn]
    (if (maybe-nothing? m)
      (if (fn? nothing-val) (nothing-val) nothing-val)
      (just-fn (maybe-value m)))))))

;; compose :: (a -> Maybe b) -> (b -> Maybe c) -> a -> Maybe c
(def hydra_overlay_clojure_lib_optionals_compose
  (fn [f] (fn [g] (fn [x]
    (let [result (f x)]
      (if (maybe-nothing? result)
        (list :none)
        (g (maybe-value result))))))))

;; given :: a -> Maybe a
(def hydra_overlay_clojure_lib_optionals_given
  (fn [x] (list :given x)))

;; to_list :: Maybe a -> [a]
(def hydra_overlay_clojure_lib_optionals_to_list
  (fn [m]
    (if (maybe-nothing? m)
      ()
      (list (maybe-value m)))))
