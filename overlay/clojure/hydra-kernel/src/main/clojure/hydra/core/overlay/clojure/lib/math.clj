(ns hydra.core.overlay.clojure.lib.math)

;; Constraint-polymorphic ('numeric') dispatch for add/sub/mul/negate.
;;
;; These vars serve two distinct call contracts, both real:
;;
;;  1. The interpreter path: registered as `hydra.core.lib.math.add` etc. via `p/prim2`/`p/prim1` with
...
;;     own self-hosted Clojure source (e.g. hydra.core.names, hydra.core.lexical, hydra.core.rewriting) and
...
;;
;; This mirrors the Haskell host's numericBinary/numericUnary (Hydra.Core.Overlay.Haskell.Lib.Math) and
...
(defn- numeric-literal [op-name t]
  (if (and (sequential? t) (= (first t) :literal))
    (second t)
    (throw (ex-info (str "hydra.core.lib.math." op-name ": expected a numeric literal term") {}))))
...
(defn- numeric-binary-term [op-name int-op float-op x y]
  (let [lx (numeric-literal op-name x)
    ...
    (when (not= lx-kind ly-kind)
      (throw (ex-info (str "hydra.core.lib.math." op-name ": operands are not the same numeric kind") {})))
    (cond
      (= lx-kind :integer)
      (let [vx (second lx) vy (second ly)
            vx-tag (first vx) vy-tag (first vy)]
        (when (not= vx-tag vy-tag)
          (throw (ex-info (str "hydra.core.lib.math." op-name ": integer operands differ in precision") {})))
        (list :literal (list :integer (list vx-tag (wrap-int vx-tag (int-op (second vx) (second vy)))))))
      (= lx-kind :float)
      (let [vx (second lx) vy (second ly)
            vx-tag (first vx) vy-tag (first vy)]
        (when (not= vx-tag vy-tag)
          (throw (ex-info (str "hydra.core.lib.math." op-name ": float operands differ in precision") {})))
        (let [r (float-op (second vx) (second vy))]
          (list :literal (list :float (list vx-tag (if (= vx-tag :float32) (to-float32 r) (double r)))))))
      :else (throw (ex-info (str "hydra.core.lib.math." op-name ": operand is not numeric") {})))))
...
(defn- integral-literal [op-name t]
  (if (and (sequential? t) (= (first t) :literal) (= (first (second t)) :integer))
    (second (second t))
    (throw (ex-info (str "hydra.core.lib.math." op-name ": expected an integer literal term") {}))))
...
(defn- divide-term [x y]
  (let [lx (numeric-literal "divide" x)
    ...
    (when (or (not= (first lx) :float) (not= (first ly) :float))
      (throw (ex-info "hydra.core.lib.math.divide: operands are not the same fractional kind" {})))
    (let [vx (second lx) vy (second ly)
          vx-tag (first vx) vy-tag (first vy)]
      (when (not= vx-tag vy-tag)
        (throw (ex-info "hydra.core.lib.math.divide: float operands differ in precision" {})))
