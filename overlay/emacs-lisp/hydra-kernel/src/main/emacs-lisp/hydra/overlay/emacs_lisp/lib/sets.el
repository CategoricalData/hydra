...
(defvar hydra_overlay_emacs_lisp_lib_sets_delete
  (lambda (x)
...
(defvar hydra_overlay_emacs_lisp_lib_sets_difference
  (lambda (s1)
...
(defvar hydra_overlay_emacs_lisp_lib_sets_insert
  (lambda (x)
...
(defvar hydra_overlay_emacs_lisp_lib_sets_intersection
  (lambda (s1)
...
;; map :: (a -> b) -> Set a -> Set b
(defvar hydra_overlay_emacs_lisp_lib_sets_map
  (lambda (f)
...
;; member :: a -> Set a -> Bool
(defvar hydra_overlay_emacs_lisp_lib_sets_member
  (lambda (x)
...
;; null :: Set a -> Bool
(defvar hydra_overlay_emacs_lisp_lib_sets_null
  (lambda (s)
    "Check if a set is empty."
    (hydra-set-empty-p s)))

;; singleton :: a -> Set a
(defvar hydra_overlay_emacs_lisp_lib_sets_singleton
  (lambda (x)
...
;; size :: Set a -> Int
(defvar hydra_overlay_emacs_lisp_lib_sets_size
  (lambda (s)
    "Get the size of a set."
    (hydra-set-size s)))

;; to_list :: Set a -> [a]
(defvar hydra_overlay_emacs_lisp_lib_sets_to_list
  (lambda (s)
    "Convert a set to a sorted list."
    (hydra-set-sorted-list s)))

;; union :: Set a -> Set a -> Set a
(defvar hydra_overlay_emacs_lisp_lib_sets_union
  (lambda (s1)
...
;; unions :: [Set a] -> Set a
(defvar hydra_overlay_emacs_lisp_lib_sets_unions
  (lambda (sets)
