...
;; max :: Int -> Int -> Int
(defvar hydra_overlay_emacs_lisp_lib_math_max
  (lambda (a)
    (lambda (b)
      (max a b))))

;; maybe_div :: Int -> Int -> Maybe Int
(defvar hydra_overlay_emacs_lisp_lib_math_maybe_div
  (lambda (a)
    (lambda (b)
      (if (= b 0)
          (list :none)
...
(defvar hydra_overlay_emacs_lisp_lib_math_maybe_mod
  (lambda (a)
    (lambda (b)
      (if (= b 0)
          (list :none)
...
(defvar hydra_overlay_emacs_lisp_lib_math_maybe_pred
  (lambda (n)
    (if (= n -2147483648)
        (list :none)
        (list :given (1- n)))))
...
(defvar hydra_overlay_emacs_lisp_lib_math_maybe_rem
  (lambda (a)
    (lambda (b)
      (if (= b 0)
          (list :none)
...
(defvar hydra_overlay_emacs_lisp_lib_math_maybe_succ
  (lambda (n)
    (if (= n 2147483647)
        (list :none)
        (list :given (1+ n)))))
...
;; min :: Int -> Int -> Int
(defvar hydra_overlay_emacs_lisp_lib_math_min
  (lambda (a)
    (lambda (b)
      (min a b))))
