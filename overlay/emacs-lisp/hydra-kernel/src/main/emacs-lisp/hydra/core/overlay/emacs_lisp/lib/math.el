...
;;     self-hosted Emacs Lisp source (e.g. hydra.core.names, hydra.core.lexical) as ordinary arithmetic
...
(defun hydra--numeric-literal (op-name term)
  (if (eq (car term) :literal)
      (cadr term)
    (error "hydra.core.lib.math.%s: expected a numeric literal term" op-name)))
...
(defun hydra--numeric-binary-term (op-name int-op float-op x y)
  (let* ((lx (hydra--numeric-literal op-name x))
    ...
    (unless (eq lx-kind ly-kind)
      (error "hydra.core.lib.math.%s: operands are not the same numeric kind" op-name))
    (cond
      ((eq lx-kind :integer)
       (let* ((vx (cadr lx)) (vy (cadr ly)) (vx-tag (car vx)) (vy-tag (car vy)))
         (unless (eq vx-tag vy-tag)
           (error "hydra.core.lib.math.%s: integer operands differ in precision" op-name))
         (list :literal (list :integer (list vx-tag (hydra--wrap-int vx-tag (funcall int-op (cadr vx) (cadr vy))))))))
      ((eq lx-kind :float)
       (let* ((vx (cadr lx)) (vy (cadr ly)) (vx-tag (car vx)) (vy-tag (car vy)))
         (unless (eq vx-tag vy-tag)
           (error "hydra.core.lib.math.%s: float operands differ in precision" op-name))
      ...
      (t (error "hydra.core.lib.math.%s: operand is not numeric" op-name)))))
...
(defun hydra--integral-literal (op-name term)
  (if (and (eq (car term) :literal) (eq (car (cadr term)) :integer))
      (cadr (cadr term))
    (error "hydra.core.lib.math.%s: expected an integer literal term" op-name)))
