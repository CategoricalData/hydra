    ...
    (let* ((std-prims (standard-library))
           ...
           (kernel-schemas
             (mapcar (lambda (entry)
                     ...
                     (hydra_overlay_emacs_lisp_lib_maps_to_list boot-types-raw)))
           (test-schemas
             (mapcar (lambda (entry)
                       (list (car entry) (funcall type-to-ts (cadr entry))))
                     (hydra_overlay_emacs_lisp_lib_maps_to_list test-types)))
           (schema-types
             (hydra_overlay_emacs_lisp_lib_maps_from_list (append kernel-schemas test-schemas)))
           (prim-map
             (hydra_overlay_emacs_lisp_lib_maps_from_list
               (mapcar (lambda (p) (list (car p) (cdr p))) std-prims)))
           (bound-terms
             (hydra_overlay_emacs_lisp_lib_maps_from_list
               (append (hydra-annotation-bindings)
                       (hydra_overlay_emacs_lisp_lib_maps_to_list test-terms)))))
...
(provide 'hydra.core.test.testEnv)
