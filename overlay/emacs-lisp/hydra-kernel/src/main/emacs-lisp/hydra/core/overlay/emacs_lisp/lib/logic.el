...
;; and :: Bool -> Bool -> Bool
(defvar hydra_overlay_emacs_lisp_lib_logic_and
  (lambda (a)
...
;; if_else :: Bool -> a -> a -> a
(defvar hydra_overlay_emacs_lisp_lib_logic_if_else
  (lambda (cond-val)
...
;; not :: Bool -> Bool
(defvar hydra_overlay_emacs_lisp_lib_logic_not
  (lambda (x)
    "Compute the logical NOT of a boolean value."
    (not x)))

;; or :: Bool -> Bool -> Bool
(defvar hydra_overlay_emacs_lisp_lib_logic_or
  (lambda (a)
    "Compute the logical OR of two boolean values."
    (lambda (b)
      (if (or a b) t nil))))

(provide 'hydra.core.lib.logic)
