;;; hydra-prims.el --- Hydra Emacs Lisp primitive infrastructure -*- lexical-binding: t -*-
;;; Provides TermCoder constructors and primitive builders (prim0-prim3).
;;; Ported from Common Lisp prims.lisp.

(require 'cl-lib)

;; ============================================================================
;; Empty InferenceContext placeholder
;; ============================================================================

;; After #446 the primitive implementation carrier no longer receives an
;; InferenceContext, but the coder encode/decode functions still take cx. The
;; prim bodies use this empty-InferenceContext placeholder, mirroring the
;; Haskell host's `primCx = emptyInferenceContext`, Python's
;; `PRIM_CX = InferenceContext(fresh_type_variable_count=0, trace=())`, and the
;; Scheme/Clojure hosts' `(make-hydra_core_typing_inference_context 0 '())`.
;; InferenceContext has two fields: freshTypeVariableCount (int32) and trace (a list).
(defconst hydra-prim-cx (make-hydra_core_typing_inference_context 0 nil)
  "Empty InferenceContext used by primitive coders after #446 dropped cx from the carrier.")

;; ============================================================================
;; Type scheme helpers
;; ============================================================================

(defun make-arity-type (n)
  "Build a dummy function type with the given arity."
  (if (<= n 0) (list :unit)
    (list :function (make-hydra_core_model_function_type (list :unit) (make-arity-type (1- n))))))

(defun make-prim-type-scheme (arity)
  (make-hydra_core_model_type_scheme nil (make-arity-type arity) nil))

(defun make-prim-def-from-arity (name arity)
  "Build a PrimitiveDefinition (#156 shape) from name + arity (for annotation primitives)."
  (let* ((ts (make-prim-type-scheme arity))
         (sig (funcall hydra_core_scoping_type_scheme_to_term_signature ts)))
    (make-hydra_core_packaging_primitive_definition name (list :none) sig t t (list :none))))

(defun collect-type-vars-ordered (typ)
  "Collect type variable names from a Hydra type in order of first appearance."
  (let ((seen (make-hash-table :test 'equal))
        (result nil))
    (cl-labels ((visit (t_)
                  (when (consp t_)
                    (let ((tag (car t_)))
                      (cond
                       ((eq tag :variable)
                        (let ((v (cadr t_)))
                          (unless (gethash v seen)
                            (puthash v t seen)
                            (push v result))))
                       ((eq tag :function)
                        (let ((ft (cadr t_)))
                          (visit (hydra_core_model_function_type-domain ft))
                          (visit (hydra_core_model_function_type-codomain ft))))
                       ((eq tag :list) (visit (cadr t_)))
                       ((eq tag :set) (visit (cadr t_)))
                       ((eq tag :optional) (visit (cadr t_)))
                       ((eq tag :map)
                        (let ((mt (cadr t_)))
                          (visit (hydra_core_model_map_type-keys mt))
                          (visit (hydra_core_model_map_type-values mt))))
                       ((eq tag :pair)
                        (let ((pt (cadr t_)))
                          (visit (hydra_core_model_pair_type-first pt))
                          (visit (hydra_core_model_pair_type-second pt))))
                       ((eq tag :either)
                        (let ((et (cadr t_)))
                          (visit (hydra_core_model_either_type-left et))
                          (visit (hydra_core_model_either_type-right et)))))))))
      (visit typ)
      (nreverse result))))

(defun wrap-constraints (classes)
  "Convert class-name strings to TypeClassConstraint.simple variants (#156)."
  (mapcar (lambda (c) (list :simple c)) classes))

(defun build-type-scheme (variables inputs output &optional constraints)
  "Build a TypeScheme from TermCoder types."
  (let* ((out-type (or (hydra_core_graph_term_coder-type output) (list :unit)))
         (fun-type (cl-reduce (lambda (result-type input-tc)
                                (let ((in-type (or (hydra_core_graph_term_coder-type input-tc) (list :unit))))
                                  (list :function (make-hydra_core_model_function_type in-type result-type))))
                              (reverse inputs) :initial-value out-type))
         (all-vars (collect-type-vars-ordered fun-type))
         ;; Exclude qualified names (containing dots) — those are nominal type references
         (detected-vars (cl-remove-if (lambda (v) (string-match-p "\\." v)) all-vars))
         (vars (if variables variables detected-vars))
         ;; After #156, TypeVariableConstraints.classes is Seq[TypeClassConstraint].
         ;; TypeScheme.constraints is a plain Map (#683); an empty map means "no constraints".
         (constraint-map
          (funcall hydra_overlay_emacs_lisp_lib_maps_from_list
                   (mapcar (lambda (entry)
                             (list (car entry)
                                   (make-hydra_core_model_type_variable_constraints
                                    (wrap-constraints (cdr entry)))))
                           constraints))))
    (make-hydra_core_model_type_scheme vars fun-type constraint-map)))

(defun build-prim-def (pname variables inputs output constraints)
  "Build a PrimitiveDefinition (#156 shape) from name + signature."
  (let* ((ts (build-type-scheme variables inputs output constraints))
         (sig (funcall hydra_core_scoping_type_scheme_to_term_signature ts)))
    (make-hydra_core_packaging_primitive_definition pname (list :none) sig t t (list :none))))

;; ============================================================================
;; Error helpers
;; ============================================================================

(defun wrap-other-error (_cx result)
  "Pass through Either Error results (no wrapping needed after InContext removal)."
  result)

;; ============================================================================
;; TermCoder constructors — each carries its actual Hydra type
;; ============================================================================

(defun tc-bigint ()
  (make-hydra_core_graph_term_coder (list :literal (list :integer (list :bigint nil)))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_bigint g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :integer (list :bigint v))))))))

(defun tc-boolean ()
  (make-hydra_core_graph_term_coder (list :literal (list :boolean nil))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_boolean g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :boolean v)))))))

;; #727: v arrives as a (coefficient . scale) cons pair (the real, scale-preserving Decimal
;; representation -- see overlay/emacs_lisp/lib/literals.el), so it's passed through
;; unchanged, not coerced to a lossy float.
(defun tc-decimal ()
  (make-hydra_core_graph_term_coder (list :literal (list :decimal nil))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_decimal g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :decimal v)))))))

(defun tc-float32 ()
  (make-hydra_core_graph_term_coder (list :literal (list :float (list :float32 nil)))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_float32 g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :float (list :float32 (float v)))))))))

(defun tc-float64 ()
  (make-hydra_core_graph_term_coder (list :literal (list :float (list :float64 nil)))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_float64 g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :float (list :float64 (float v)))))))))

(defun tc-int8 ()
  (make-hydra_core_graph_term_coder (list :literal (list :integer (list :int8 nil)))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_int8 g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :integer (list :int8 v))))))))

(defun tc-int16 ()
  (make-hydra_core_graph_term_coder (list :literal (list :integer (list :int16 nil)))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_int16 g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :integer (list :int16 v))))))))

(defun tc-int32 ()
  (make-hydra_core_graph_term_coder (list :literal (list :integer (list :int32 nil)))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_int32 g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :integer (list :int32 v))))))))

(defun tc-int64 ()
  (make-hydra_core_graph_term_coder (list :literal (list :integer (list :int64 nil)))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_int64 g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :integer (list :int64 v))))))))

(defun tc-uint8 ()
  (make-hydra_core_graph_term_coder (list :literal (list :integer (list :uint8 nil)))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_uint8 g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :integer (list :uint8 v))))))))

(defun tc-uint16 ()
  (make-hydra_core_graph_term_coder (list :literal (list :integer (list :uint16 nil)))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_uint16 g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :integer (list :uint16 v))))))))

(defun tc-uint32 ()
  (make-hydra_core_graph_term_coder (list :literal (list :integer (list :uint32 nil)))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_uint32 g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :integer (list :uint32 v))))))))

(defun tc-uint64 ()
  (make-hydra_core_graph_term_coder (list :literal (list :integer (list :uint64 nil)))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_uint64 g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :integer (list :uint64 v))))))))

(defun tc-string ()
  (make-hydra_core_graph_term_coder (list :literal (list :string nil))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_string g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :string v)))))))

(defun tc-binary ()
  (make-hydra_core_graph_term_coder (list :literal (list :binary nil))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall hydra_core_extract_model_binary g) t_))))
    (lambda (cx) (ignore cx) (lambda (v) (list :right (list :literal (list :binary v)))))))

;; Container TermCoders

(defun tc-list (el-coder)
  (make-hydra_core_graph_term_coder (list :list (hydra_core_graph_term_coder-type el-coder))
    (lambda (cx) (lambda (g) (lambda (t_)
      (funcall (funcall (funcall hydra_core_extract_model_list_of
                                 (lambda (term) (funcall (funcall (funcall (hydra_core_graph_term_coder-encode el-coder) cx) g) term)))
                        g) t_))))
    (lambda (cx) (lambda (lst)
      (let ((result nil))
        (catch 'fail
          (dolist (item lst)
            (let ((r (funcall (funcall (hydra_core_graph_term_coder-decode el-coder) cx) item)))
              (if (eq (car r) :left) (throw 'fail r)
                (push (cadr r) result))))
          (list :right (list :list (nreverse result)))))))))

(defun tc-set (el-coder)
  (make-hydra_core_graph_term_coder (list :set (hydra_core_graph_term_coder-type el-coder))
    (lambda (cx) (lambda (g) (lambda (t_)
      (funcall (funcall (funcall hydra_core_extract_model_set_of
                                 (lambda (term) (funcall (funcall (funcall (hydra_core_graph_term_coder-encode el-coder) cx) g) term)))
                        g) t_))))
    (lambda (cx) (lambda (s)
      (let ((result nil))
        (catch 'fail
          (dolist (item (funcall hydra_overlay_emacs_lisp_lib_sets_to_list s))
            (let ((r (funcall (funcall (hydra_core_graph_term_coder-decode el-coder) cx) item)))
              (if (eq (car r) :left) (throw 'fail r)
                (push (cadr r) result))))
          (list :right (list :set (funcall hydra_overlay_emacs_lisp_lib_sets_from_list (nreverse result))))))))))

(defun tc-map (key-coder val-coder)
  (make-hydra_core_graph_term_coder (list :map (make-hydra_core_model_map_type (hydra_core_graph_term_coder-type key-coder) (hydra_core_graph_term_coder-type val-coder)))
    (lambda (cx) (lambda (g) (lambda (t_)
      (funcall (funcall (funcall (funcall hydra_core_extract_model_map
                                          (lambda (term) (funcall (funcall (funcall (hydra_core_graph_term_coder-encode key-coder) cx) g) term)))
                                 (lambda (term) (funcall (funcall (funcall (hydra_core_graph_term_coder-encode val-coder) cx) g) term)))
                        g) t_))))
    (lambda (cx) (lambda (m)
      (let ((result nil))
        (catch 'fail
          (dolist (pair (funcall hydra_overlay_emacs_lisp_lib_maps_to_list m))
            (let ((kr (funcall (funcall (hydra_core_graph_term_coder-decode key-coder) cx) (funcall hydra_overlay_emacs_lisp_lib_pairs_first pair))))
              (if (eq (car kr) :left) (throw 'fail kr)
                (let ((vr (funcall (funcall (hydra_core_graph_term_coder-decode val-coder) cx) (funcall hydra_overlay_emacs_lisp_lib_pairs_second pair))))
                  (if (eq (car vr) :left) (throw 'fail vr)
                    (push (list (cadr kr) (cadr vr)) result))))))
          (list :right (list :map (funcall hydra_overlay_emacs_lisp_lib_maps_from_list (nreverse result))))))))))

(defun tc-optional (el-coder)
  (make-hydra_core_graph_term_coder (list :optional (hydra_core_graph_term_coder-type el-coder))
    (lambda (cx) (lambda (g) (lambda (t_)
      (funcall (funcall (funcall hydra_core_extract_model_optional_term
                                 (lambda (term) (funcall (funcall (funcall (hydra_core_graph_term_coder-encode el-coder) cx) g) term)))
                        g) t_))))
    (lambda (cx) (lambda (mv)
      (cond
       ((null mv) (list :right (list :optional nil)))
       ((and (consp mv) (eq (car mv) :none)) (list :right (list :optional nil)))
       ((and (consp mv) (eq (car mv) :given))
        (let ((r (funcall (funcall (hydra_core_graph_term_coder-decode el-coder) cx) (cadr mv))))
          (if (eq (car r) :left) r (list :right (list :optional (cadr r))))))
       (t (let ((r (funcall (funcall (hydra_core_graph_term_coder-decode el-coder) cx) mv)))
            (if (eq (car r) :left) r (list :right (list :optional (cadr r)))))))))))

(defun tc-either (left-coder right-coder)
  (make-hydra_core_graph_term_coder (list :either (make-hydra_core_model_either_type (hydra_core_graph_term_coder-type left-coder) (hydra_core_graph_term_coder-type right-coder)))
    (lambda (cx) (lambda (g) (lambda (t_)
      (funcall (funcall (funcall (funcall hydra_core_extract_model_either_term
                                          (lambda (term) (funcall (funcall (funcall (hydra_core_graph_term_coder-encode left-coder) cx) g) term)))
                                 (lambda (term) (funcall (funcall (funcall (hydra_core_graph_term_coder-encode right-coder) cx) g) term)))
                        g) t_))))
    (lambda (cx) (lambda (ev)
      (if (eq (car ev) :left)
          (let ((r (funcall (funcall (hydra_core_graph_term_coder-decode left-coder) cx) (cadr ev))))
            (if (eq (car r) :left) r (list :right (list :either (list :left (cadr r))))))
        (let ((r (funcall (funcall (hydra_core_graph_term_coder-decode right-coder) cx) (cadr ev))))
          (if (eq (car r) :left) r (list :right (list :either (list :right (cadr r)))))))))))

(defun tc-pair (first-coder second-coder)
  (make-hydra_core_graph_term_coder (list :pair (make-hydra_core_model_pair_type (hydra_core_graph_term_coder-type first-coder) (hydra_core_graph_term_coder-type second-coder)))
    (lambda (cx) (lambda (g) (lambda (t_)
      (funcall (funcall (funcall (funcall hydra_core_extract_model_pair
                                          (lambda (term) (funcall (funcall (funcall (hydra_core_graph_term_coder-encode first-coder) cx) g) term)))
                                 (lambda (term) (funcall (funcall (funcall (hydra_core_graph_term_coder-encode second-coder) cx) g) term)))
                        g) t_))))
    (lambda (cx) (lambda (p)
      (let ((fr (funcall (funcall (hydra_core_graph_term_coder-decode first-coder) cx) (funcall hydra_overlay_emacs_lisp_lib_pairs_first p))))
        (if (eq (car fr) :left) fr
          (let ((sr (funcall (funcall (hydra_core_graph_term_coder-decode second-coder) cx) (funcall hydra_overlay_emacs_lisp_lib_pairs_second p))))
            (if (eq (car sr) :left) sr
              (list :right (list :pair (list (cadr fr) (cadr sr))))))))))))

;; Effect / unit / named coders (#494)
;;
;; effect<t> is transparent in emacs-lisp (effect<t> = t), so tc-effect delegates to its inner
;; coder. These coders exist so the inference graph can resolve the hydra.lib.{effects,files,text}.*
;; names; the real I/O is performed by the relocated hydra.<lang>.lib.* runtimes (reached via the
;; bootstrap redirect), not through these stub primitive impls.

(defun tc-effect (inner-coder)
  (make-hydra_core_graph_term_coder (list :effect (hydra_core_graph_term_coder-type inner-coder))
    (lambda (cx) (lambda (g) (lambda (t_) (funcall (funcall (funcall (hydra_core_graph_term_coder-encode inner-coder) cx) g) t_))))
    (lambda (cx) (lambda (v) (funcall (funcall (hydra_core_graph_term_coder-decode inner-coder) cx) v)))))

(defun tc-unit ()
  (make-hydra_core_graph_term_coder (list :unit nil)
    (lambda (_cx) (lambda (_g) (lambda (_t) (list :right nil))))
    (lambda (_cx) (lambda (_v) (list :right (list :unit))))))

(defun tc-named (type-name)
  (make-hydra_core_graph_term_coder (list :variable type-name)
    (lambda (_cx) (lambda (_g) (lambda (t_) (list :right t_))))
    (lambda (_cx) (lambda (t_) (list :right t_)))))

;; TermCoder for the void type. Never legitimately exercised: void has no inhabitants.
(defun tc-void ()
  (make-hydra_core_graph_term_coder (list :void nil)
    (lambda (_cx) (lambda (_g) (lambda (t_) (list :right t_))))
    (lambda (_cx) (lambda (t_) (list :right t_)))))

;; Term/variable passthrough coders

(defun tc-variable (name)
  (make-hydra_core_graph_term_coder (list :variable name)
    (lambda (_cx) (lambda (_g) (lambda (t_) (list :right t_))))
    (lambda (_cx) (lambda (t_) (list :right t_)))))

(defun tc-term ()
  (make-hydra_core_graph_term_coder (list :variable "hydra.core.model.Term")
    (lambda (_cx) (lambda (_g) (lambda (t_) (list :right t_))))
    (lambda (_cx) (lambda (t_) (list :right t_)))))

;; Comparison coder

(defun tc-comparison ()
  (make-hydra_core_graph_term_coder (list :variable "hydra.core.util.Comparison")
    (lambda (cx) (lambda (g) (lambda (t_)
      (let ((r (funcall (funcall (funcall hydra_core_extract_model_unit_variant "hydra.core.util.Comparison") g) t_)))
        (if (eq (car r) :left) r
          (let ((variant-name (cadr r)))
            (cond
             ((equal variant-name "lessThan")    (list :right :lt))
             ((equal variant-name "equalTo")     (list :right :eq))
             ((equal variant-name "greaterThan") (list :right :gt))
             (t (list :left (list :other (make-hydra_core_errors_other_error (format "unknown comparison: %s" variant-name))))))))))))
    (lambda (_cx) (lambda (c)
      (let ((variant-name
             (cond
              ((and (consp c) (eq (car c) :less_than)) "lessThan")
              ((and (consp c) (eq (car c) :equal_to)) "equalTo")
              ((and (consp c) (eq (car c) :greater_than)) "greaterThan")
              ((or (eq c :lt) (and (numberp c) (< c 0))) "lessThan")
              ((or (eq c :eq) (and (numberp c) (zerop c))) "equalTo")
              ((or (eq c :gt) (and (numberp c) (> c 0))) "greaterThan")
              (t (error "not a comparison: %S" c)))))
        (list :right (list :inject (make-hydra_core_model_injection "hydra.core.util.Comparison"
                                    (make-hydra_core_model_field variant-name (list :unit))))))))))

;; Function coders

(defun tc-function (dom cod)
  (make-hydra_core_graph_term_coder (list :function (make-hydra_core_model_function_type (hydra_core_graph_term_coder-type dom) (hydra_core_graph_term_coder-type cod)))
    (lambda (cx) (lambda (_g) (lambda (_t) (list :left (list :other (make-hydra_core_errors_other_error "cannot encode term to a function"))))))
    (lambda (cx) (lambda (_v) (list :left (list :other (make-hydra_core_errors_other_error "cannot decode functions to terms")))))))

(defun tc-function-with-reduce (reduce-fn dom cod)
  "TermCoder for function types, using a reducer to bridge term-level to native."
  (make-hydra_core_graph_term_coder (list :function (make-hydra_core_model_function_type (hydra_core_graph_term_coder-type dom) (hydra_core_graph_term_coder-type cod)))
    (lambda (cx) (lambda (g) (lambda (fun-term)
      (list :right
            (lambda (x)
              (let ((arg-result (funcall (funcall (hydra_core_graph_term_coder-decode dom) cx) x)))
                (when (eq (car arg-result) :left)
                  (error "function_with_reduce: failed to encode argument"))
                (let* ((arg-term (cadr arg-result))
                       (app-term (list :application (make-hydra_core_model_application fun-term arg-term)))
                       (reduce-result (funcall reduce-fn cx g app-term)))
                  (when (eq (car reduce-result) :left)
                    (error "function_with_reduce: failed to reduce application"))
                  (let* ((result-term (cadr reduce-result))
                         (decode-result (funcall (funcall (funcall (hydra_core_graph_term_coder-encode cod) cx) g) result-term)))
                    (when (eq (car decode-result) :left)
                      (error "function_with_reduce: failed to decode result"))
                    (cadr decode-result)))))))))
    (lambda (cx) (lambda (_v) (list :left (list :other (make-hydra_core_errors_other_error "cannot decode functions to terms")))))))

;; ============================================================================
;; Primitive constructors
;; ============================================================================

(defun default-fallback-primitive (pname variables inputs output &optional constraints)
  "Build a Primitive for a kernel primitive with no native Emacs Lisp implementation, but
which declares a portable, cross-compilable defaultImplementation term (see
hydra.core.lib.defaults/hydra_core_lib_defaults_default_implementations). Its implementation folds call
args into an Application chain over the term and evaluates via reduceTerm, rather than running
hand-written Lisp logic. Ported from Common Lisp's default-fallback-primitive (#609 Stage 4 —
NOT YET SLOT-VALIDATED; the defaults symbol name below is inferred from the naming convention
shared by Java/Python/Scala/TS/Clojure/Common-Lisp, not confirmed against a real
dist/emacs-lisp build)."
  (let* ((ts (build-type-scheme variables inputs output constraints))
         (sig (funcall hydra_core_scoping_type_scheme_to_term_signature ts))
         (default-impl (cdr (assoc pname hydra_core_lib_defaults_default_implementations #'equal)))
         (definition (progn
                       (when (null default-impl)
                         (error "default-fallback-primitive: no defaultImplementation for %s" pname))
                       (make-hydra_core_packaging_primitive_definition pname (list :none) sig t t (list :given default-impl)))))
    (make-hydra_core_graph_primitive definition
      (lambda (g)
        (lambda (args)
          (let ((applied (cl-reduce (lambda (fn-term arg) (list :application (make-hydra_core_model_application fn-term arg)))
                                     args :initial-value default-impl)))
            (funcall (funcall (funcall (funcall hydra_core_reduction_reduce_term hydra-prim-cx) g) t) applied)))))))

(defun prim0 (pname value-fn variables output &optional constraints)
  (make-hydra_core_graph_primitive (build-prim-def pname variables nil output constraints)
    (lambda (_g) (lambda (_args)
      (let ((result (funcall (funcall (hydra_core_graph_term_coder-decode output) hydra-prim-cx) (funcall value-fn))))
        (wrap-other-error hydra-prim-cx result))))))

(defun prim1 (pname compute variables input1 output &optional constraints)
  (make-hydra_core_graph_primitive (build-prim-def pname variables (list input1) output constraints)
    (lambda (g) (lambda (args)
      (let ((check (funcall (funcall (funcall hydra_core_extract_model_n_args pname) 1) args)))
        (if (eq (car check) :left) check
          (let ((r1 (funcall (funcall (funcall (hydra_core_graph_term_coder-encode input1) hydra-prim-cx) g) (car args))))
            (if (eq (car r1) :left) (wrap-other-error hydra-prim-cx r1)
              (let ((result (funcall (funcall (hydra_core_graph_term_coder-decode output) hydra-prim-cx) (funcall compute (cadr r1)))))
                (wrap-other-error hydra-prim-cx result))))))))))

(defun prim2 (pname compute variables input1 input2 output &optional constraints)
  (make-hydra_core_graph_primitive (build-prim-def pname variables (list input1 input2) output constraints)
    (lambda (g) (lambda (args)
      (let ((check (funcall (funcall (funcall hydra_core_extract_model_n_args pname) 2) args)))
        (if (eq (car check) :left) check
          (let ((r1 (funcall (funcall (funcall (hydra_core_graph_term_coder-encode input1) hydra-prim-cx) g) (car args))))
            (if (eq (car r1) :left) (wrap-other-error hydra-prim-cx r1)
              (let ((r2 (funcall (funcall (funcall (hydra_core_graph_term_coder-encode input2) hydra-prim-cx) g) (cadr args))))
                (if (eq (car r2) :left) (wrap-other-error hydra-prim-cx r2)
                  (let ((result (funcall (funcall (hydra_core_graph_term_coder-decode output) hydra-prim-cx)
                                         (funcall (funcall compute (cadr r1)) (cadr r2)))))
                    (wrap-other-error hydra-prim-cx result))))))))))))

(defun prim3 (pname compute variables input1 input2 input3 output &optional constraints)
  (make-hydra_core_graph_primitive (build-prim-def pname variables (list input1 input2 input3) output constraints)
    (lambda (g) (lambda (args)
      (let ((check (funcall (funcall (funcall hydra_core_extract_model_n_args pname) 3) args)))
        (if (eq (car check) :left) check
          (let ((r1 (funcall (funcall (funcall (hydra_core_graph_term_coder-encode input1) hydra-prim-cx) g) (car args))))
            (if (eq (car r1) :left) (wrap-other-error hydra-prim-cx r1)
              (let ((r2 (funcall (funcall (funcall (hydra_core_graph_term_coder-encode input2) hydra-prim-cx) g) (cadr args))))
                (if (eq (car r2) :left) (wrap-other-error hydra-prim-cx r2)
                  (let ((r3 (funcall (funcall (funcall (hydra_core_graph_term_coder-encode input3) hydra-prim-cx) g) (nth 2 args))))
                    (if (eq (car r3) :left) (wrap-other-error hydra-prim-cx r3)
                      (let ((result (funcall (funcall (hydra_core_graph_term_coder-decode output) hydra-prim-cx)
                                             (funcall (funcall (funcall compute (cadr r1)) (cadr r2)) (cadr r3)))))
                        (wrap-other-error hydra-prim-cx result))))))))))))))

;; Mirror of Hydra.Core.Dsl.Prims.lazyArgs / Clojure's lazy-args / Scheme's lazy-args (#391,
;; #585, #586): mark the value parameters at the given (0-based) positions of a
;; primitive's signature as lazy. Used at prim*-based registration sites in libraries.el
;; to record which arguments coders must thunk in hosts that distinguish strict from
;; lazy evaluation. Without this, the hand-rolled prim0-prim3 registry helpers silently
;; default every parameter to strict, so a target coder's isLazy-driven thunk-wrapping
;; (which reads is_lazy from graph_primitives(g), the single source of truth per #391)
;; treats every registry-built primitive as fully strict -- eagerly evaluating both
;; branches of e.g. if_else/optionals.match regardless of which is taken, causing
;; infinite recursion on self-recursive definitions and O(2^depth) blowup in any
;; self-hosted interpretation that walks the resulting eagerly-expanded terms.
(defun lazy-args (idxs prim)
  (let* ((def0 (hydra_core_graph_primitive-definition prim))
         (sig (hydra_core_packaging_primitive_definition-signature def0))
         (params (hydra_core_typing_term_signature-parameters sig))
         (marked (let ((i 0))
                   (mapcar (lambda (p)
                             (prog1
                                 (if (memq i idxs)
                                     (make-hydra_core_typing_parameter
                                      (hydra_core_typing_parameter-name p)
                                      (hydra_core_typing_parameter-description p)
                                      (hydra_core_typing_parameter-type p)
                                      t)
                                   p)
                               (setq i (1+ i))))
                           params)))
         (sig2 (make-hydra_core_typing_term_signature
                (hydra_core_typing_term_signature-type_parameters sig)
                marked
                (hydra_core_typing_term_signature-result sig)))
         (def2 (make-hydra_core_packaging_primitive_definition
                (hydra_core_packaging_primitive_definition-name def0)
                (hydra_core_packaging_primitive_definition-metadata def0)
                sig2
                (hydra_core_packaging_primitive_definition-is_pure def0)
                (hydra_core_packaging_primitive_definition-is_total def0)
                (hydra_core_packaging_primitive_definition-default_implementation def0))))
    (make-hydra_core_graph_primitive
     def2
     (hydra_core_graph_primitive-implementation prim))))

(provide 'hydra-prims)
