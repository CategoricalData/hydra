(define-library (hydra overlay scheme lib sets)
  (import (scheme base)
          (scheme write)
          (ice-9 vlist)
  ...
  (export hydra_overlay_scheme_lib_sets_delete
          ...
          hydra_overlay_scheme_lib_sets_is_empty
          ...
          hydra_overlay_scheme_lib_sets_unions)
  (begin
    ...
    (define hydra_overlay_scheme_lib_sets_delete
      (lambda (x)
        (lambda (s)
    ...
    (define hydra_overlay_scheme_lib_sets_difference
      (lambda (s1)
        (lambda (s2)
    ...
    (define hydra_overlay_scheme_lib_sets_insert
      (lambda (x)
        (lambda (s)
          (vhash-cons x #t (ensure-vhset s)))))

    ;; Compute the intersection of two sets.
    (define hydra_overlay_scheme_lib_sets_intersection
      (lambda (s1)
        (lambda (s2)
          (let ((vh2 (ensure-vhset s2)))
            (vhash-fold (lambda (k v acc)
    ...
    (define hydra_overlay_scheme_lib_sets_map
      (lambda (f)
        (lambda (s)
    ...
    (define hydra_overlay_scheme_lib_sets_member
      (lambda (x)
        (lambda (s)
          (if (vhash-assoc x (ensure-vhset s)) #t #f))))

    ;; Check if a set is empty.
    (define hydra_overlay_scheme_lib_sets_is_empty
      (lambda (s)
        (cond
    ...
    (define hydra_overlay_scheme_lib_sets_singleton
      (lambda (x)
    ...
    (define hydra_overlay_scheme_lib_sets_size
      (lambda (s)
        (if (vlist? s)
    ...
    (define hydra_overlay_scheme_lib_sets_to_list
      (lambda (s)
        (let ((seen (make-hash-table)))
    ...
    (define hydra_overlay_scheme_lib_sets_union
      (lambda (s1)
        (lambda (s2)
    ...
    (define hydra_overlay_scheme_lib_sets_unions
      (lambda (sets)
        (let loop ((rest sets) (acc vlist-null))
          (if (null? rest)
              acc
              (loop (cdr rest)
                    ((hydra_overlay_scheme_lib_sets_union acc) (car rest)))))))))
