(define-library (hydra overlay scheme lib math)
  ...
  (begin
    ...
    ;;
    ;;  1. The interpreter path: registered as hydra.core.lib.math.add etc. via prim2/prim1 with a
    ...
    ;;     self-hosted Scheme source (e.g. hydra.core.names, hydra.core.lexical) as ordinary arithmetic over
    ;;     raw native numbers -- no Term involved at all.
    ;;
    ;; This mirrors the Haskell host's numericBinary/numericUnary (Hydra.Core.Overlay.Haskell.Lib.Math)
    ...
    (define (numeric-literal op-name t)
      (if (eq? (car t) 'literal)
          (cadr t)
          (error (string-append "hydra.core.lib.math." op-name ": expected a numeric literal term"))))
    ...
    (define (numeric-binary-term op-name int-op float-op x y)
      (let* ((lx (numeric-literal op-name x))
        ...
        (if (not (eq? lx-kind ly-kind))
            (error (string-append "hydra.core.lib.math." op-name ": operands are not the same numeric kind")))
        (cond
          ((eq? lx-kind 'integer)
           (let* ((vx (cadr lx)) (vy (cadr ly)) (vx-tag (car vx)) (vy-tag (car vy)))
             (if (not (eq? vx-tag vy-tag))
                 (error (string-append "hydra.core.lib.math." op-name ": integer operands differ in precision")))
             (list 'literal (list 'integer (list vx-tag (wrap-int vx-tag (int-op (cadr vx) (cadr vy))))))))
          ((eq? lx-kind 'float)
           (let* ((vx (cadr lx)) (vy (cadr ly)) (vx-tag (car vx)) (vy-tag (car vy)))
             (if (not (eq? vx-tag vy-tag))
                 (error (string-append "hydra.core.lib.math." op-name ": float operands differ in precision")))
             (let ((r (float-op (cadr vx) (cadr vy))))
               (list 'literal (list 'float (list vx-tag (if (eq? vx-tag 'float32) (snap-to-float32 r) r)))))))
          (else (error (string-append "hydra.core.lib.math." op-name ": operand is not numeric"))))))
    ...
    (define (integral-literal op-name t)
      (if (and (eq? (car t) 'literal) (eq? (car (cadr t)) 'integer))
          (cadr (cadr t))
          (error (string-append "hydra.core.lib.math." op-name ": expected an integer literal term"))))
    ...
    (define (divide-term x y)
      (let* ((lx (numeric-literal "divide" x))
        ...
        (if (or (not (eq? (car lx) 'float)) (not (eq? (car ly) 'float)))
            (error "hydra.core.lib.math.divide: operands are not the same fractional kind"))
        (let* ((vx (cadr lx)) (vy (cadr ly)) (vx-tag (car vx)) (vy-tag (car vy)))
          (if (not (eq? vx-tag vy-tag))
              (error "hydra.core.lib.math.divide: float operands differ in precision"))
