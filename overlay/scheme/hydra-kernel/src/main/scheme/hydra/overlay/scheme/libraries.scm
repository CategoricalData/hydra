          ...
          (list
            ...
            (cons (prim-name def:hydra_lib_files_copy) (prim3 (prim-name def:hydra_lib_files_copy)
                                               (lambda (recursive) (lambda (source) (lambda (destination)
                                                 (((hydra_lib_files_copy recursive) source) destination))))
            ...
            (cons (prim-name def:hydra_lib_files_remove_directory) (prim2 (prim-name def:hydra_lib_files_remove_directory)
                                               (lambda (recursive) (lambda (path) ((hydra_lib_files_remove_directory recursive) path)))
                                               #f bool fp (eff (tc-either ferr unit))))
        ...
        (let ((eff (lambda (c) (tc-effect c))))
          (list
            ...
            (cons (prim-name def:hydra_lib_system_read_stdin) (prim0 (prim-name def:hydra_lib_system_read_stdin)
                                               (lambda () hydra_lib_system_read_stdin)
