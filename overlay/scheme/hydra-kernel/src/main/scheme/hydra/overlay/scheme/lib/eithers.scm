(define-library (hydra overlay scheme lib eithers)
  (import (scheme base)
          (hydra overlay scheme lib equality)
          (hydra overlay scheme lib sets))
  (export hydra_overlay_scheme_lib_eithers_bimap
  ...
  (begin
    ...
    (define hydra_overlay_scheme_lib_eithers_bimap
      (lambda (f)
        (lambda (g)
          (lambda (e)
    ...
    (define hydra_overlay_scheme_lib_eithers_bind
      (lambda (e)
        (lambda (f)
          (if (eq? (either-tag e) 'left)
              e
    ...
    (define hydra_overlay_scheme_lib_eithers_either
      (lambda (f)
        (lambda (g)
          (lambda (e)
            (if (eq? (either-tag e) 'left)
    ...
    (define hydra_overlay_scheme_lib_eithers_is_left
      (lambda (e)
    ...
    (define hydra_overlay_scheme_lib_eithers_map
      (lambda (f)
        (lambda (e)
          (if (eq? (either-tag e) 'left)
              e
    ...
    (define hydra_overlay_scheme_lib_eithers_map_list
      (lambda (f)
        (lambda (xs)
    ...
    (define hydra_overlay_scheme_lib_eithers_map_optional
      (lambda (f)
        (lambda (m)
          (if (or (null? m) (and (pair? m) (eq? (car m) 'none)))
              (list 'right (list 'none))
              (let ((val (if (and (pair? m) (or (eq? (car m) 'given) (eq? (car m) 'optional)))
                             (cadr m) m)))
                (let ((result (f val)))
                  (if (eq? (either-tag result) 'left)
                      result
    ...
    (define hydra_overlay_scheme_lib_eithers_map_set
      (lambda (f)
        (lambda (s)
          (let loop ((rest (hydra_overlay_scheme_lib_sets_to_list s)) (acc '()))
            (if (null? rest)
                (list 'right (hydra_overlay_scheme_lib_sets_from_list (reverse acc)))
                (let ((result (f (car rest))))
                  (if (eq? (either-tag result) 'left)
                      result
                      (loop (cdr rest) (cons (either-val result) acc)))))))))
    ...
    (define hydra_overlay_scheme_lib_eithers_partition
      (lambda (es)
    ...
    (define hydra_overlay_scheme_lib_eithers_fold_list
      (lambda (f)
        (lambda (init)
          (lambda (xs)
