...
(define-library (hydra overlay scheme lib regex)
  ...
  (begin
    ...
    ;; first runs the pattern through hydra.core.parse.regex, then renders the AST to POSIX ERE syntax via
    ;; hydra.core.print.posix.regex (Guile (ice-9 regex) is POSIX ERE), before handing the rendered
    ;; pattern to the native engine. An ill-formed pattern (rejected by hydra.core.parse.regex) is treated
