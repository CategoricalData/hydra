...
(defvar hydra_lisp_lib_files_append_file
  (lambda (path)
    (lambda (contents)
...
(defvar hydra_lisp_lib_files_create_directory
  (lambda (recursive)
    (lambda (path)
      (hydra-files-with-error path
...
(defvar hydra_lisp_lib_files_exists
  (lambda (path)
    (hydra-files-with-error path
...
(defvar hydra_lisp_lib_files_list_directory
  (lambda (path)
    (hydra-files-with-error path
      (let* ((dirpath (if (char= (char path (1- (length path))) #\/)
                          path
...
(defvar hydra_lisp_lib_files_read_file
  (lambda (path)
    (hydra-files-with-error path
...
(defvar hydra_lisp_lib_files_remove_file
  (lambda (path)
    (hydra-files-with-error path
...
(defvar hydra_lisp_lib_files_rename
  (lambda (source)
    (lambda (destination)
...
(defvar hydra_lisp_lib_files_write_file
  (lambda (path)
    (lambda (contents)
      (hydra-files-with-error path
        (with-open-file (out path :direction :output
