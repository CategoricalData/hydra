...
(defvar hydra_overlay_common_lisp_lib_sets_delete
  (lambda (x)
    (lambda (s)
      (cond
        ((null s) nil)
...
(defvar hydra_overlay_common_lisp_lib_sets_difference
  (lambda (s1)
    (lambda (s2)
      (let ((result nil))
...
(defvar hydra_overlay_common_lisp_lib_sets_insert
  (lambda (x)
    (lambda (s)
      (cond
...
(defvar hydra_overlay_common_lisp_lib_sets_intersection
  (lambda (s1)
    (lambda (s2)
      (let ((result nil))
        (sets-each s1 (lambda (x)
...
;; map :: (a -> b) -> Set a -> Set b
(defvar hydra_overlay_common_lisp_lib_sets_map
  (lambda (f)
    (lambda (s)
      (let ((result nil))
...
;; member :: a -> Set a -> Bool
(defvar hydra_overlay_common_lisp_lib_sets_member
  (lambda (x)
    (lambda (s) (sets-member-raw x s))))

;; null :: Set a -> Bool
(defvar hydra_overlay_common_lisp_lib_sets_is_empty
  (lambda (s) (sets-empty-p s)))

;; size :: Set a -> Int
(defvar hydra_overlay_common_lisp_lib_sets_size
  (lambda (s)
    (cond
      ((null s) 0)
...
;; singleton :: a -> Set a
(defvar hydra_overlay_common_lisp_lib_sets_singleton
  (lambda (x)
    (rb-insert nil x t)))

;; to_list :: Set a -> [a]
;; Sorted via generic-compare for determinism.
(defvar hydra_overlay_common_lisp_lib_sets_to_list
  (lambda (s) (sets-elements-sorted s)))

;; union :: Set a -> Set a -> Set a
(defvar hydra_overlay_common_lisp_lib_sets_union
  (lambda (s1)
    (lambda (s2)
      (cond
        ((null s1) s2)
...
;; unions :: [Set a] -> Set a
(defvar hydra_overlay_common_lisp_lib_sets_unions
  (lambda (sets)
    (let ((result nil))
