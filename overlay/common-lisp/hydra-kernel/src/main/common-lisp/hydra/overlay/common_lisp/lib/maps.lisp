...
;; ============================================================================
;; Public API: hydra_overlay_common_lisp_lib_maps_*
;; ============================================================================

;; alter :: (Maybe v -> Maybe v) -> k -> Map k v -> Map k v
(defvar hydra_overlay_common_lisp_lib_maps_alter
  (lambda (f)
    (lambda (k)
      (lambda (m)
...
(defvar hydra_overlay_common_lisp_lib_maps_bimap
  (lambda (fk)
    (lambda (fv)
      (lambda (m)
        (let ((result nil))
...
(defvar hydra_overlay_common_lisp_lib_maps_delete
  (lambda (k)
    (lambda (m)
      (cond
        ((null m) nil)
...
(defvar hydra_overlay_common_lisp_lib_maps_elems
  (lambda (m)
    (mapcar #'cdr (maps-entries-sorted m))))

;; empty :: Map k v
(defvar hydra_overlay_common_lisp_lib_maps_empty nil)

;; filter :: (v -> Bool) -> Map k v -> Map k v
(defvar hydra_overlay_common_lisp_lib_maps_filter
  (lambda (pred)
    (lambda (m)
      (let ((result nil))
        (cond
...
(defvar hydra_overlay_common_lisp_lib_maps_filter_with_key
  (lambda (pred)
    (lambda (m)
      (let ((result nil))
        (cond
...
(defvar hydra_overlay_common_lisp_lib_maps_find_with_default
  (lambda (def)
    (lambda (k)
      (lambda (m)
        (multiple-value-bind (v found) (maps-lookup-raw k m)
...
(defvar hydra_overlay_common_lisp_lib_maps_from_list
  (lambda (pairs)
    (let ((result nil))
      (dolist (p pairs result)
...
(defvar hydra_overlay_common_lisp_lib_maps_insert
  (lambda (k)
    (lambda (v)
      (lambda (m)
        (cond
...
(defvar hydra_overlay_common_lisp_lib_maps_keys
  (lambda (m)
    (mapcar #'car (maps-entries-sorted m))))

;; lookup :: k -> Map k v -> Maybe v
(defvar hydra_overlay_common_lisp_lib_maps_lookup
  (lambda (k)
    (lambda (m)
      (multiple-value-bind (v found) (maps-lookup-raw k m)
...
(defvar hydra_overlay_common_lisp_lib_maps_map
  (lambda (f)
    (lambda (m)
      (let ((result nil))
        (cond
...
(defvar hydra_overlay_common_lisp_lib_maps_map_keys
  (lambda (f)
    (lambda (m)
      (let ((result nil))
        (cond
...
(defvar hydra_overlay_common_lisp_lib_maps_member
  (lambda (k)
    (lambda (m)
      (nth-value 1 (maps-lookup-raw k m)))))

;; null :: Map k v -> Bool
(defvar hydra_overlay_common_lisp_lib_maps_null
  (lambda (m)
    (cond
      ((null m) t)
...
(defvar hydra_overlay_common_lisp_lib_maps_singleton
  (lambda (k)
    (lambda (v)
      (rb-insert nil k v))))

;; size :: Map k v -> Int
(defvar hydra_overlay_common_lisp_lib_maps_size
  (lambda (m)
    (cond
      ((null m) 0)
...
(defvar hydra_overlay_common_lisp_lib_maps_to_list
  (lambda (m)
...
(defvar hydra_overlay_common_lisp_lib_maps_union
  (lambda (m1)
    (lambda (m2)
      (cond
        ((null m1) m2)
