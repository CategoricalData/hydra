...
(defvar hydra_overlay_common_lisp_lib_lists_apply
  (lambda (fs)
    (lambda (xs)
      (loop for f in fs
            append (mapcar f xs)))))
...
(defvar hydra_overlay_common_lisp_lib_lists_bind
  (lambda (xs)
    (lambda (f)
      (loop for x in xs
            append (funcall f x)))))
...
(defvar hydra_overlay_common_lisp_lib_lists_concat
  (lambda (xss)
    (apply #'append xss)))

;; concat2 :: [a] -> [a] -> [a]
;; Concatenate two lists.
(defvar hydra_overlay_common_lisp_lib_lists_concat2
  (lambda (xs)
    (lambda (ys)
      (append xs ys))))
...
(defvar hydra_overlay_common_lisp_lib_lists_cons
  (lambda (x)
    (lambda (xs)
      (cons x xs))))
...
(defvar hydra_overlay_common_lisp_lib_lists_drop
  (lambda (n)
    (lambda (xs)
...
(defvar hydra_overlay_common_lisp_lib_lists_drop_while
  (lambda (pred)
    (lambda (xs)
      (loop for rest on xs
...
(defvar hydra_overlay_common_lisp_lib_lists_filter
  (lambda (pred)
    (lambda (xs)
...
(defvar hydra_overlay_common_lisp_lib_lists_find
  (lambda (pred)
    (lambda (xs)
...
(defvar hydra_overlay_common_lisp_lib_lists_foldl
  (lambda (f)
    (lambda (init)
      (lambda (xs)
        (let ((acc init))
...
(defvar hydra_overlay_common_lisp_lib_lists_foldr
  (lambda (f)
    (lambda (init)
      (lambda (xs)
        (let ((acc init))
...
(defvar hydra_overlay_common_lisp_lib_lists_group
  (lambda (xs)
    (if (null xs)
        nil
        (let ((groups nil)
...
(defvar hydra_overlay_common_lisp_lib_lists_intersperse
  (lambda (sep)
    (lambda (xs)
...
(defvar hydra_overlay_common_lisp_lib_lists_length
  (lambda (xs)
    (length xs)))

;; map :: (a -> b) -> [a] -> [b]
;; Map a function over a list.
(defvar hydra_overlay_common_lisp_lib_lists_map
  (lambda (f)
    (lambda (xs)
      (mapcar f xs))))
...
;; reverse :: [a] -> [a]
;; Reverse a list.
(defvar hydra_overlay_common_lisp_lib_lists_reverse
  (lambda (xs)
    (reverse xs)))

;; singleton :: a -> [a]
;; Create a single-element list.
(defvar hydra_overlay_common_lisp_lib_lists_singleton
  (lambda (x)
    (list x)))

;; sort :: [a] -> [a]
;; Sort a list.
(defvar hydra_overlay_common_lisp_lib_lists_sort
  (lambda (xs)
...
(defvar hydra_overlay_common_lisp_lib_lists_span
  (lambda (pred)
    (lambda (xs)
      (let ((acc nil))
        (loop for rest on xs
...
(defvar hydra_overlay_common_lisp_lib_lists_take
  (lambda (n)
    (lambda (xs)
      (loop for x in xs
            repeat n
...
;; transpose :: [[a]] -> [[a]]
;; Transpose a list of lists.
(defvar hydra_overlay_common_lisp_lib_lists_transpose
  (lambda (xss)
    (transpose-helper xss)))

;; uncons :: [a] -> Maybe (a, [a])
;; Split a list into its head and tail, returning Nothing if the list is empty.
(defvar hydra_overlay_common_lisp_lib_lists_uncons
  (lambda (xs)
    (if (null xs)
        (list :none)
...
(defvar hydra_overlay_common_lisp_lib_lists_zip
  (lambda (xs)
    (lambda (ys)
      (mapcar #'list xs ys))))
...
(defvar hydra_overlay_common_lisp_lib_lists_zip_with
  (lambda (f)
    (lambda (xs)
      (lambda (ys)
