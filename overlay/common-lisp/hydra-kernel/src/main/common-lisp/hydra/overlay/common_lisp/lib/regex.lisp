...
;; hydra.print.pcre.regex (cl-ppcre is a full Perl-5-syntax engine), before handing the rendered
;; pattern to the native engine. An ill-formed pattern (rejected by hydra.parse.regex) is treated as
...
(defun hydra--regex-to-native (pattern)
  (let ((parsed (funcall hydra.parse.regex:hydra_parse_regex_parse_regex pattern)))
    (if (eq (car parsed) :given)
        (list :given (funcall hydra.print.pcre.regex:hydra_print_pcre_regex_print_regex (cadr parsed)))
        (list :none))))
