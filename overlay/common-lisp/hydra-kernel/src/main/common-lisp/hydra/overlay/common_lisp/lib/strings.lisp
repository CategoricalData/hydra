...
(defvar hydra_overlay_common_lisp_lib_strings_cons
  (lambda (c)
    (lambda (s)
...
(defvar hydra_overlay_common_lisp_lib_strings_drop
  (lambda (n)
    (lambda (s)
      (if (>= n (length s))
          ""
...
(defvar hydra_overlay_common_lisp_lib_strings_from_list
  (lambda (codepoints)
...
(defvar hydra_overlay_common_lisp_lib_strings_head
  (lambda (s)
    (char-code (char s 0))))
...
(defvar hydra_overlay_common_lisp_lib_strings_is_infix_of
  (lambda (needle)
    (lambda (haystack)
...
(defvar hydra_overlay_common_lisp_lib_strings_is_prefix_of
  (lambda (prefix)
    (lambda (s)
...
(defvar hydra_overlay_common_lisp_lib_strings_is_suffix_of
  (lambda (suffix)
    (lambda (s)
...
(defvar hydra_overlay_common_lisp_lib_strings_length
  (lambda (s)
    (length s)))
...
(defvar hydra_overlay_common_lisp_lib_strings_null
  (lambda (s)
    (zerop (length s))))

;; replicate :: Int -> String -> String
;; Repeat a string n times.
(defvar hydra_overlay_common_lisp_lib_strings_replicate
  (lambda (n)
    (lambda (s)
      (let ((acc ""))
        (dotimes (i n acc)
...
;; Reverse a string.
(defvar hydra_overlay_common_lisp_lib_strings_reverse
  (lambda (s)
    (reverse s)))
...
(defvar hydra_overlay_common_lisp_lib_strings_split_on
  (lambda (sep)
    (lambda (s)
...
(defvar hydra_overlay_common_lisp_lib_strings_tail
  (lambda (s)
    (subseq s 1)))

;; take :: Int -> String -> String
;; Take the first n characters from a string.
(defvar hydra_overlay_common_lisp_lib_strings_take
  (lambda (n)
    (lambda (s)
...
(defvar hydra_overlay_common_lisp_lib_strings_to_list
  (lambda (s)
...
(defvar hydra_overlay_common_lisp_lib_strings_unwords
  (lambda (strs)
    (if (null strs)
        ""
        (let ((acc (car strs)))
...
(defvar hydra_overlay_common_lisp_lib_strings_words
  (lambda (s)
