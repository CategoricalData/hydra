...
;;     self-hosted Common Lisp source (e.g. hydra.core.names, hydra.core.lexical) as ordinary arithmetic
...
(defun hydra-numeric-literal (op-name term)
  (if (eq (first term) :literal)
      (second term)
      (error "hydra.core.lib.math.~a: expected a numeric literal term" op-name)))
...
(defun hydra-numeric-binary-term (op-name int-op float-op x y)
  (let* ((lx (hydra-numeric-literal op-name x))
    ...
    (unless (eq lx-kind ly-kind)
      (error "hydra.core.lib.math.~a: operands are not the same numeric kind" op-name))
    (cond
      ((eq lx-kind :integer)
       (let* ((vx (second lx)) (vy (second ly)) (vx-tag (first vx)) (vy-tag (first vy)))
         (unless (eq vx-tag vy-tag)
           (error "hydra.core.lib.math.~a: integer operands differ in precision" op-name))
         (list :literal (list :integer (list vx-tag (hydra-wrap-int vx-tag (funcall int-op (second vx) (second vy))))))))
      ((eq lx-kind :float)
       (let* ((vx (second lx)) (vy (second ly)) (vx-tag (first vx)) (vy-tag (first vy)))
         (unless (eq vx-tag vy-tag)
           (error "hydra.core.lib.math.~a: float operands differ in precision" op-name))
      ...
      (t (error "hydra.core.lib.math.~a: operand is not numeric" op-name)))))
...
(defun hydra-integral-literal (op-name term)
  (if (and (consp term) (eq (first term) :literal) (eq (first (second term)) :integer))
      (second (second term))
      (error "hydra.core.lib.math.~a: expected an integer literal term" op-name)))
