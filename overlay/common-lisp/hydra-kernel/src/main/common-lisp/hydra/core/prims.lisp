  ...
  (make-term_coder (list :set (term_coder-type el-coder))
    ...
    (lambda (cx)
      (lambda (s)
        (block decode
          (let ((result nil))
            (dolist (item (funcall hydra_overlay_common_lisp_lib_sets_to_list s))
              (let ((r (funcall (funcall (term_coder-decode el-coder) cx) item)))
                (if (eq (first r) :left)
                    (return-from decode r)
                    (push (second r) result))))
            (list :right (list :set (funcall hydra_overlay_common_lisp_lib_sets_from_list (nreverse result))))))))))
  ...
  (make-term_coder (list :map (make-map_type (term_coder-type key-coder) (term_coder-type val-coder)))
    ...
    (lambda (cx)
      (lambda (m)
        (block decode
          (let ((result nil))
            (dolist (pair (funcall hydra_overlay_common_lisp_lib_maps_to_list m))
              (let ((kr (funcall (funcall (term_coder-decode key-coder) cx) (funcall hydra_overlay_common_lisp_lib_pairs_first pair))))
                (if (eq (first kr) :left)
                    (return-from decode kr)
                    (let ((vr (funcall (funcall (term_coder-decode val-coder) cx) (funcall hydra_overlay_common_lisp_lib_pairs_second pair))))
                      (if (eq (first vr) :left)
                          (return-from decode vr)
                          (push (list (second kr) (second vr)) result))))))
            (list :right (list :map (funcall hydra_overlay_common_lisp_lib_maps_from_list (nreverse result))))))))))
...
(defun tc-pair (first-coder second-coder)
  (make-term_coder (list :pair (make-pair_type (term_coder-type first-coder) (term_coder-type second-coder)))
    ...
    (lambda (cx)
      (lambda (p)
        (let ((fr (funcall (funcall (term_coder-decode first-coder) cx) (funcall hydra_overlay_common_lisp_lib_pairs_first p))))
          (if (eq (first fr) :left) fr
              (let ((sr (funcall (funcall (term_coder-decode second-coder) cx) (funcall hydra_overlay_common_lisp_lib_pairs_second p))))
...
;; unit and nominal-by-name (e.g. hydra.core.file.FilePath) types used by the files/text signatures.
...
(defun tc-term ()
  (make-term_coder (list :variable "hydra.core.model.Term")
    (lambda (cx) (declare (ignore cx)) (lambda (g) (declare (ignore g)) (lambda (t_) (list :right t_))))
    (lambda (cx) (declare (ignore cx)) (lambda (t_) (list :right t_)))))
...
(defun tc-comparison ()
  (make-term_coder (list :variable "hydra.core.util.Comparison")
    (lambda (cx)
      (lambda (g)
        (lambda (t_)
    ...
    (lambda (cx) (declare (ignore cx))
      (lambda (c)
        (let ((variant-name (cond
          ...
          (list :right (list :inject (make-injection "hydra.core.util.Comparison"
