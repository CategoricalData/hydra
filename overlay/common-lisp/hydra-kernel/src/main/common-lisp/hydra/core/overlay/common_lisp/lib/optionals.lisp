(in-package :cl-user)

;; Optional representation: (list :given val) or (list :none)

;; Do NOT treat (:optional ...) as a native optional — that is Hydra's term-level
;; representation and collides with nested optionals like optional<optional<string>>.
(defun maybe-nothing-p (m)
  (or (null m)
      (and (consp m) (eq (first m) :none))))

(defun maybe-value (m)
  (cond
    ((and (consp m) (eq (first m) :given)) (second m))
    (t m)))

;; apply :: Maybe (a -> b) -> Maybe a -> Maybe b
(defvar hydra_overlay_common_lisp_lib_optionals_apply
  (lambda (mf)
    (lambda (mx)
      (if (maybe-nothing-p mf)
          (list :none)
          (if (maybe-nothing-p mx)
              (list :none)
              (list :given (funcall (maybe-value mf) (maybe-value mx))))))))

;; bind :: Maybe a -> (a -> Maybe b) -> Maybe b
(defvar hydra_overlay_common_lisp_lib_optionals_bind
  (lambda (m)
    (lambda (f)
      (if (maybe-nothing-p m)
          (list :none)
          (funcall f (maybe-value m))))))

;; givens :: [Maybe a] -> [a]
(defvar hydra_overlay_common_lisp_lib_optionals_givens
  (lambda (ms)
    (loop for m in ms
          unless (maybe-nothing-p m)
          collect (maybe-value m))))

;; compose :: (a -> Maybe b) -> (b -> Maybe c) -> a -> Maybe c
(defvar hydra_overlay_common_lisp_lib_optionals_compose
  (lambda (f)
    (lambda (g)
      (lambda (x)
        (let ((result (funcall f x)))
          (if (maybe-nothing-p result)
              (list :none)
              (funcall g (maybe-value result))))))))

;; with_default :: a -> Maybe a -> a
;; Thunk-aware: if def is a zero-arg function (thunk), only called when Maybe is Nothing
(defvar hydra_overlay_common_lisp_lib_optionals_with_default
  (lambda (def)
    (lambda (m)
      (if (maybe-nothing-p m)
          (if (functionp def) (funcall def) def)
          (maybe-value m)))))

;; is_given :: Maybe a -> Bool
(defvar hydra_overlay_common_lisp_lib_optionals_is_given
  (lambda (m)
    (not (maybe-nothing-p m))))

;; is_none :: Maybe a -> Bool
(defvar hydra_overlay_common_lisp_lib_optionals_is_none
  (lambda (m)
    (maybe-nothing-p m)))

;; map :: (a -> b) -> Maybe a -> Maybe b
(defvar hydra_overlay_common_lisp_lib_optionals_map
  (lambda (f)
    (lambda (m)
      (if (maybe-nothing-p m)
          (list :none)
          (list :given (funcall f (maybe-value m)))))))

;; map_optional :: (a -> Maybe b) -> [a] -> [b]
(defvar hydra_overlay_common_lisp_lib_optionals_map_optional
  (lambda (f)
    (lambda (xs)
      (loop for x in xs
            for result = (funcall f x)
            unless (maybe-nothing-p result)
            collect (maybe-value result)))))

;; match :: Maybe a -> b -> (a -> b) -> b
;; Thunk-aware: if def is a zero-arg function (thunk), only called when Maybe is Nothing
(defvar hydra_overlay_common_lisp_lib_optionals_match
  (lambda (m)
    (lambda (def)
      (lambda (f)
        (if (maybe-nothing-p m)
            (if (functionp def) (funcall def) def)
            (funcall f (maybe-value m)))))))

;; given :: a -> Maybe a
(defvar hydra_overlay_common_lisp_lib_optionals_given
  (lambda (x)
    (list :given x)))

;; to_list :: Maybe a -> [a]
(defvar hydra_overlay_common_lisp_lib_optionals_to_list
  (lambda (m)
    (if (maybe-nothing-p m)
        nil
        (list (maybe-value m)))))
