...
Mirrors ``hydra.core.util.PersistentMap`` from Hydra-Java. Iteration via
``keys()`` / ``values()`` / ``items()`` / ``__iter__`` is in **key-sorted
order** (Haskell semantics), achieved by sorting at iteration time using
``hydra.core.overlay.python.util._compare.compare``.
...
from hydra.core.overlay.python.util._compare import compare as _cmp
