...
from hydra.core.overlay.python.dsl.phantoms import primitive1
from hydra.core.overlay.python.dsl.prims import int32, prim1, string
from hydra.core.graph import TermCoder
...
def _infer_namespace_from_file(func: Any) -> str:
    ...
    try:
        ...
        # Look for patterns like: .../hydra/core/dsl/lib/strings.py
        # (post-#729 package-rooted grammar: the dsl/lib segments sit under hydra.core)
        path_parts = file_path.replace(os.path.sep, '/').split('/')

        # Find the hydra/core/dsl/lib part and extract the module name
        try:
            hydra_idx = path_parts.index('hydra')
            if (hydra_idx + 4 < len(path_parts) and
                path_parts[hydra_idx + 1] == 'core' and
                path_parts[hydra_idx + 2] == 'dsl' and
                path_parts[hydra_idx + 3] == 'lib'):
                # Extract the module name (without .py extension)
                module_file = path_parts[hydra_idx + 4]
                module_name = os.path.splitext(module_file)[0]
                return f"hydra.core.lib.{module_name}"
