"""Python implementations of hydra.core.lib.math primitives."""
...
def _numeric_binary_terms(x, y, op):
    ...
    if isinstance(lx, hydra.core.model.LiteralInteger) and isinstance(ly, hydra.core.model.LiteralInteger):
        return hydra.core.model.TermLiteral(hydra.core.model.LiteralInteger(
            _rewrap_integer(lx.value, op(lx.value.value, ly.value.value))))
    if isinstance(lx, hydra.core.model.LiteralFloat) and isinstance(ly, hydra.core.model.LiteralFloat):
        return hydra.core.model.TermLiteral(hydra.core.model.LiteralFloat(
            _rewrap_float(lx.value, op(lx.value.value, ly.value.value))))
    raise TypeError("hydra.core.lib.math: operands are not the same numeric kind")


def _numeric_unary_terms(x, op):
    import hydra.core.model
    lx = _numeric_literal(x)
    if isinstance(lx, hydra.core.model.LiteralInteger):
        return hydra.core.model.TermLiteral(hydra.core.model.LiteralInteger(
            _rewrap_integer(lx.value, op(lx.value.value))))
    if isinstance(lx, hydra.core.model.LiteralFloat):
        return hydra.core.model.TermLiteral(hydra.core.model.LiteralFloat(
            _rewrap_float(lx.value, op(lx.value.value))))
    raise TypeError("hydra.core.lib.math: operand is not numeric")


def _numeric_literal(term):
    import hydra.core.model
    if isinstance(term, hydra.core.model.TermLiteral):
        return term.value
    raise TypeError("hydra.core.lib.math: expected a literal term")


# Width (signed?, bits) per fixed-width IntegerValue variant, keyed by class NAME to avoid importing
# hydra.core.model at module-load time. Bigint has no entry — it keeps full arbitrary precision.
_INT_WIDTHS = {
...
# dividend) — mirrors Hydra.Core.Overlay.Haskell.Lib.Math's divTerm/modTerm/remTerm split (GHC's
...
def _integral_binary_terms(x, y, op):
    ...
    lx = _numeric_literal(x)
    ly = _numeric_literal(y)
    if not (isinstance(lx, hydra.core.model.LiteralInteger) and isinstance(ly, hydra.core.model.LiteralInteger)):
        raise TypeError("hydra.core.lib.math: operands are not the same integral kind")
    if ly.value.value == 0:
        return None_()
    return Given(hydra.core.model.TermLiteral(hydra.core.model.LiteralInteger(
        _rewrap_integer(lx.value, op(lx.value.value, ly.value.value)))))
...
def _integral_unary_bool_terms(x, op):
    ...
    raise TypeError("hydra.core.lib.math: operand is not integral")
...
def _divide_binary_terms(x, y, op):
    ...
    if isinstance(lx, hydra.core.model.LiteralFloat) and isinstance(ly, hydra.core.model.LiteralFloat):
        return hydra.core.model.TermLiteral(hydra.core.model.LiteralFloat(
            _rewrap_float(lx.value, op(lx.value.value, ly.value.value))))
    raise TypeError("hydra.core.lib.math.divide: operands are not the same fractional kind")
