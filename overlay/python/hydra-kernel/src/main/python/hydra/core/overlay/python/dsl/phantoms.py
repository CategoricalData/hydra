...
from hydra.core.util import CaseConvention, QualifiedName
from hydra.core.model import Binding, Field, Name, Term, Type
from hydra.core.packaging import Module, ModuleName
from hydra.core.typed import TypedBinding, TypedTerm
from hydra.core.overlay.python.dsl.python import FrozenDict, Optional, Given, None_
...
from hydra.core.overlay.python.dsl.phantom_literals import (
    bigint,
    binary,
    boolean,
    char,
...
def definition_in_namespace(ns: ModuleName, lname: str, term: TypedTerm[A]) -> "TypedBinding[A]":
...
def to_definition(tb: TypedBinding[A]):
    ...
    Mirrors Haskell's Hydra.Core.Dsl.Meta.Phantoms.toDefinition.
    """
    from hydra.core.packaging import DefinitionTerm, TermDefinition
...
def derive_primitive_name() -> Name:
    ...
    Returns:
        The fully qualified primitive name (e.g., "hydra.core.lib.strings.fromList")
    """
    ...
    # Extract library name from module (e.g., "hydra.core.dsl.lib.strings" -> "strings")
    ...
    camel_name = hydra.core.formatting.convert_case(
        CaseConvention.LOWER_SNAKE,
        CaseConvention.CAMEL,
        clean_name
    )

    # Construct full primitive name (post-#729 package-rooted grammar: hydra.core.lib.<sub>.<fn>)
    return Name(f"hydra.core.lib.{lib_name}.{camel_name}")


def doc(s: str, term: TypedTerm[A]) -> "TypedTerm[A]":
...
def first_class_type(typ: TypedTerm[Type]) -> TypedTerm[Type]:
    ...
    return annot(
        hydra.core.constants.key_first_class_type,
...
def fold(f: TypedTerm[A]) -> TypedTerm[B]:
    ...
    return apply(primitive(Name("hydra.core.lib.lists.foldl")), f)
...
def lam(v: str, body: TypedTerm[X]) -> TypedTerm[A]:
    """Create a lambda function with one parameter."""
    return TypedTerm[A](terms.lambda_(v, un_tterm(body)))


def let(name: str, value: TypedTerm[A], env: TypedTerm[B]) -> TypedTerm[B]:
...
def opt_cases(arg: TypedTerm[Optional[A]], if_nothing: TypedTerm[B], if_just: TypedTerm[A]) -> TypedTerm[B]:
    ...
    return primitive3(
        Name("hydra.core.lib.optionals.match"),
        arg,
        if_nothing,
        if_just
    )
...
def unary_function(f) -> TypedTerm[A]:
    ...
    match term:
        ...
        case terms.TermOptional(Given(_)):
            return TypedTerm[A](terms.primitive(Name("hydra.core.lib.optionals.given")))
...
def with_type_classes(classes: FrozenDict[Name, frozenset], term: TypedTerm[A]) -> TypedTerm[A]:
    ...
    return TypedTerm[A](hydra.core.annotations.set_type_classes(classes, un_tterm(term)))
