"""Python implementations of hydra.core.lib.regex primitives.

Patterns are Hydra-defined and translingual (docs/specification/regex.md). Each primitive first runs
the pattern through hydra.core.parse.regex, then renders the AST to PCRE syntax via hydra.core.print.pcre.regex
...
import hydra.core.parse.regex
import hydra.core.print.pcre.regex
from hydra.core.overlay.python.dsl.python import Given, None_, Optional, NONE_, frozenlist
...
def _to_native(pattern: str) -> Optional[str]:
    ...
    match hydra.core.parse.regex.parse_regex(pattern):
        case Given(value=ast):
            return Given(hydra.core.print.pcre.regex.print_regex(ast))
        case None_():
            return NONE_
