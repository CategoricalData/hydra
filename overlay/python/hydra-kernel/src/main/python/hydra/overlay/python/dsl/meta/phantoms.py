...
from hydra.overlay.python.dsl.python import FrozenDict, Optional, Given, None_
...
from hydra.overlay.python.dsl.meta.phantom_literals import (
    bigint,
    binary,
    boolean,
    char,
...
def opt_cases(arg: TypedTerm[Optional[A]], if_nothing: TypedTerm[B], if_just: TypedTerm[A]) -> TypedTerm[B]:
    ...
    return primitive3(
        Name("hydra.lib.optionals.match"),
        arg,
        if_nothing,
        if_just
    )
