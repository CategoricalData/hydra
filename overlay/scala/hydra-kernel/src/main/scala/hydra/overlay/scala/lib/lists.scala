...
object lists:
  ...
  def member[A](x: A)(xs: Seq[A]): Boolean = xs.contains(x)
  ...
  def group[A](xs: Seq[A]): Seq[Seq[A]] =
    if xs.isEmpty then Seq.empty
      ...
      same +: group(rest)
  def join[A](sep: Seq[A])(xss: Seq[Seq[A]]): Seq[A] =
  ...
  def at[A](i: Int)(xs: Seq[A]): Option[A] = if i >= 0 && i < xs.length then Some(xs(i)) else None
  def head[A](xs: Seq[A]): Option[A] = xs.headOption
  def init[A](xs: Seq[A]): Option[Seq[A]] = if xs.isEmpty then None else Some(xs.init)
  ...
  def sort[A](xs: Seq[A]): Seq[A] =
    ...
    else xs.sortWith((a, b) => ordering.lt(a)(b))
  def sortBy[A, B](f: A => B)(xs: Seq[A])(using ord: Ordering[B] = null): Seq[A] =
    if ord != null then xs.sortBy(f)(using ord)
    else xs.sortWith((a, b) => ordering.lt(f(a))(f(b)))
