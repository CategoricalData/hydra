...
object Libraries:
  ...
  private def eithersPrimitives(): Map[String, Primitive] =
    ...
    Map(
      ...
      hydra.lib.eithers.left.name -> mkPrimImpl(hydra.lib.eithers.left.name, tScheme(Seq("x", "y"),
        tFun(x, tEither(x, y))),
      ...
      hydra.lib.eithers.right.name -> mkPrimImpl(hydra.lib.eithers.right.name, tScheme(Seq("x", "y"),
        tFun(y, tEither(x, y))),
  ...
  private def listsPrimitives(): Map[String, Primitive] =
    ...
    Map(
      ...
      hydra.lib.lists.isEmpty.name -> mkPrimImpl(hydra.lib.lists.isEmpty.name, tScheme(Seq("a"),
        tFun(tList(a), tBool)),
  ...
  private def mapsPrimitives(): Map[String, Primitive] =
    ...
    Map(
      ...
      hydra.lib.maps.isEmpty.name -> mkPrimImpl(hydra.lib.maps.isEmpty.name, tSchemeConstrained(Seq(("k", Seq("ordering")), ("v", Seq.empty)),
        tFun(mapKV, tBool)),
  ...
  private def setsPrimitives(): Map[String, Primitive] =
    ...
    Map(
      ...
      hydra.lib.sets.isEmpty.name -> mkPrimImpl(hydra.lib.sets.isEmpty.name, tSchemeConstrained(aOrd,
        tFun(tSet(a), tBool)),
  ...
  private def stringsPrimitives(): Map[String, Primitive] =
    Map(
      ...
      hydra.lib.strings.isEmpty.name -> mkPrimImpl(hydra.lib.strings.isEmpty.name, tMono(tFun(tString, tBool)),
  ...
  private def literalsPrimitives(): Map[String, Primitive] =
    Map(
      ...
      hydra.lib.literals.binaryToBase64.name -> mkPrimImpl(hydra.lib.literals.binaryToBase64.name, tMono(tFun(tBinary, tString)),
      ...
      // Read primitives
      hydra.lib.literals.parseBigint.name -> mkPrimImpl(hydra.lib.literals.parseBigint.name, tMono(tFun(tString, tOpt(tBigint))),
      ...
      hydra.lib.literals.parseDecimal.name -> mkPrimImpl(hydra.lib.literals.parseDecimal.name, tMono(tFun(tString, tOpt(tDecimal))),
        impl1(s => mkMaybe(literals.parseDecimal(exString(s)).map(mkDecimal)))),
      hydra.lib.literals.parseFloat32.name -> mkPrimImpl(hydra.lib.literals.parseFloat32.name, tMono(tFun(tString, tOpt(tFloat32))),
        impl1(s => mkMaybe(literals.parseFloat32(exString(s)).map(mkFloat32)))),
      hydra.lib.literals.parseFloat64.name -> mkPrimImpl(hydra.lib.literals.parseFloat64.name, tMono(tFun(tString, tOpt(tFloat64))),
        impl1(s => mkMaybe(literals.parseFloat64(exString(s)).map(mkFloat64)))),
      hydra.lib.literals.parseInt8.name -> mkPrimImpl(hydra.lib.literals.parseInt8.name, tMono(tFun(tString, tOpt(tInt8))),
        impl1(s => mkMaybe(literals.parseInt8(exString(s)).map(mkInt8)))),
      hydra.lib.literals.parseInt16.name -> mkPrimImpl(hydra.lib.literals.parseInt16.name, tMono(tFun(tString, tOpt(tInt16))),
        impl1(s => mkMaybe(literals.parseInt16(exString(s)).map(mkInt16)))),
      hydra.lib.literals.parseInt32.name -> mkPrimImpl(hydra.lib.literals.parseInt32.name, tMono(tFun(tString, tOpt(tInt32))),
        impl1(s => mkMaybe(literals.parseInt32(exString(s)).map(mkInt32)))),
      hydra.lib.literals.parseInt64.name -> mkPrimImpl(hydra.lib.literals.parseInt64.name, tMono(tFun(tString, tOpt(tInt64))),
      ...
      hydra.lib.literals.parseUint8.name -> mkPrimImpl(hydra.lib.literals.parseUint8.name, tMono(tFun(tString, tOpt(tUint8))),
        impl1(s => mkMaybe(literals.parseUint8(exString(s)).map(mkUint8)))),
      hydra.lib.literals.parseUint16.name -> mkPrimImpl(hydra.lib.literals.parseUint16.name, tMono(tFun(tString, tOpt(tUint16))),
        impl1(s => mkMaybe(literals.parseUint16(exString(s)).map(mkUint16)))),
      hydra.lib.literals.parseUint32.name -> mkPrimImpl(hydra.lib.literals.parseUint32.name, tMono(tFun(tString, tOpt(tUint32))),
        impl1(s => mkMaybe(literals.parseUint32(exString(s)).map(mkUint32)))),
      hydra.lib.literals.parseUint64.name -> mkPrimImpl(hydra.lib.literals.parseUint64.name, tMono(tFun(tString, tOpt(tUint64))),
        impl1(s => mkMaybe(literals.parseUint64(exString(s)).map(mkUint64)))),
      // Show primitives
      hydra.lib.literals.printBigint.name -> mkPrimImpl(hydra.lib.literals.printBigint.name, tMono(tFun(tBigint, tString)),
      ...
      hydra.lib.literals.printDecimal.name -> mkPrimImpl(hydra.lib.literals.printDecimal.name, tMono(tFun(tDecimal, tString)),
        impl1(a => mkString(literals.printDecimal(exDecimal(a))))),
      hydra.lib.literals.printFloat32.name -> mkPrimImpl(hydra.lib.literals.printFloat32.name, tMono(tFun(tFloat32, tString)),
        impl1(a => mkString(literals.printFloat32(exFloat32(a))))),
      hydra.lib.literals.printFloat64.name -> mkPrimImpl(hydra.lib.literals.printFloat64.name, tMono(tFun(tFloat64, tString)),
        impl1(a => mkString(literals.printFloat64(exFloat64(a))))),
      hydra.lib.literals.printInt8.name -> mkPrimImpl(hydra.lib.literals.printInt8.name, tMono(tFun(tInt8, tString)),
        impl1(a => mkString(literals.printInt8(exInt8(a))))),
      hydra.lib.literals.printInt16.name -> mkPrimImpl(hydra.lib.literals.printInt16.name, tMono(tFun(tInt16, tString)),
        impl1(a => mkString(literals.printInt16(exInt16(a))))),
      hydra.lib.literals.printInt32.name -> mkPrimImpl(hydra.lib.literals.printInt32.name, tMono(tFun(tInt32, tString)),
        impl1(a => mkString(literals.printInt32(exInt32(a))))),
      hydra.lib.literals.printInt64.name -> mkPrimImpl(hydra.lib.literals.printInt64.name, tMono(tFun(tInt64, tString)),
        impl1(a => mkString(literals.printInt64(exInt64(a))))),
      hydra.lib.literals.printUint8.name -> mkPrimImpl(hydra.lib.literals.printUint8.name, tMono(tFun(tUint8, tString)),
        impl1(a => mkString(literals.printUint8(exUint8(a))))),
      hydra.lib.literals.printUint16.name -> mkPrimImpl(hydra.lib.literals.printUint16.name, tMono(tFun(tUint16, tString)),
        impl1(a => mkString(literals.printUint16(exUint16(a))))),
      hydra.lib.literals.printUint32.name -> mkPrimImpl(hydra.lib.literals.printUint32.name, tMono(tFun(tUint32, tString)),
        impl1(a => mkString(literals.printUint32(exUint32(a))))),
      hydra.lib.literals.printUint64.name -> mkPrimImpl(hydra.lib.literals.printUint64.name, tMono(tFun(tUint64, tString)),
      ...
      hydra.lib.literals.base64ToBinary.name -> mkPrimImpl(hydra.lib.literals.base64ToBinary.name, tMono(tFun(tString, tBinary)),
  ...
  private def pairsPrimitives(): Map[String, Primitive] =
    ...
    Map(
      ...
      hydra.lib.pairs.first.name -> mkPrimImpl(hydra.lib.pairs.first.name, tScheme(Seq("a", "b"),
        tFun(tPair(a, b), a)),
        impl1(p => exPair(p)._1)),
      hydra.lib.pairs.pair.name -> mkPrimImpl(hydra.lib.pairs.pair.name, tScheme(Seq("a", "b"),
