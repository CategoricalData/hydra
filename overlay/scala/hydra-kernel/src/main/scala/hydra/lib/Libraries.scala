...
// #473 Step 0: native primitive implementations were relocated to package hydra.scala.lib
...
import hydra.scala.lib.{chars, eithers, equality, lists, literals, logic, maps, math, optionals, pairs, regex, sets, strings}
    ...
    Map(
      ...
      hydra.lib.lists.transpose.name -> mkPrimImpl(hydra.lib.lists.transpose.name, tScheme(Seq("a"),
        ...
        impl1 { xss =>
          ...
          mkList(hydra.scala.lib.lists.transpose(innerLists).map(mkList))
        }),
