 ...
 * Mirrors Hydra.Core.Dsl.Terms (Haskell), hydra.core.dsl.terms (Python),
 * and hydra.core.overlay.java.dsl.Terms (Java). Scala stays slim because
 * hydra.core.model.Name is a String alias, the kernel constructors are
...
object Terms:
  ...
  def comparison(c: hydra.core.util.Comparison): Term =
  ...
  def first: Term = primitive("hydra.core.lib.pairs.first")
  def second: Term = primitive("hydra.core.lib.pairs.second")
