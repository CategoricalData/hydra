...
/**
 * Scala implementations of hydra.core.lib.files primitives (#494).
 *
 ...
 *
 * hydra.core.file.FilePath is a wrapped string, which the Scala coder renders as a plain
 ...
 * byte I/O. hydra.core.model.unit maps to Scala Unit, so mutating operations succeed with Right(()).
 */
object files:
  ...
  def status(followLinks: Boolean)(path: String): Either[FileError, hydra.core.file.FileStatus] =
    withFileError(path) {
  ...
  private def fileTypeFromMode(mode: Int): hydra.core.file.FileType =
    (mode & S_IFMT) match {
      case S_IFDIR  => hydra.core.file.FileType.directory
      case S_IFLNK  => hydra.core.file.FileType.link
      case S_IFBLK  => hydra.core.file.FileType.block
      case S_IFCHR  => hydra.core.file.FileType.character
      case S_IFIFO  => hydra.core.file.FileType.fifo
      case S_IFSOCK => hydra.core.file.FileType.socket
      case _        => hydra.core.file.FileType.regular
    }

  private def timespec(instant: _root_.java.time.Instant): hydra.core.time.Timespec =
    hydra.core.time.Timespec(instant.getEpochSecond, instant.getNano.toLong)
