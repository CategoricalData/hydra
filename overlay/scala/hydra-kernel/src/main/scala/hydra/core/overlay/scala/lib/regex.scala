...
// hydra.core.print.pcre.regex (Scala uses java.util.regex, a PCRE-like engine), before handing the
// rendered pattern to the native engine. An ill-formed pattern (rejected by hydra.core.parse.regex) is
...
object regex:
  private def toNative(pattern: String): Option[String] =
    hydra.core.parse.regex.parseRegex(pattern).map(hydra.core.print.pcre.regex.printRegex)
