...
/** Runtime dispatch for the constraint-polymorphic ('numeric') arithmetic primitives
  ...
  * `numericBinary`/`numericUnary` (see `Hydra.Core.Overlay.Haskell.Lib.Math`) and Java's
  * `NumericDispatch`.
  *
...
object NumericDispatch:
  ...
  def applyNativeBinary[A](opName: String, x: A, y: A): A =
    (x.asInstanceOf[Any], y.asInstanceOf[Any]) match
      ...
      case (a, _) => throw new RuntimeException(s"hydra.core.lib.math.$opName: operand is not numeric: $a")
  ...
  def applyNativeUnary[A](opName: String, x: A): A =
    x.asInstanceOf[Any] match
      ...
      case a => throw new RuntimeException(s"hydra.core.lib.math.$opName: operand is not numeric: $a")

  private def applyIntegerOp(opName: String, a: BigInt, b: BigInt): BigInt = opName match
    case "add" => a + b
    case "sub" => a - b
    case "mul" => a * b
    case _ => throw new RuntimeException(s"hydra.core.lib.math.$opName: unsupported integer op")

  private def applyFloatOp(opName: String, a: Double, b: Double): Double = opName match
    case "add" => a + b
    case "sub" => a - b
    case "mul" => a * b
    case _ => throw new RuntimeException(s"hydra.core.lib.math.$opName: unsupported float op")
