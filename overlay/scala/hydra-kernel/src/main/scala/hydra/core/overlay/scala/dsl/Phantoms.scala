...
import hydra.core.packaging.{Definition, EntityMetadata, Module, ModuleName, TermDefinition}
import hydra.core.typed.{TypedBinding, TypedTerm}
 ...
 * hydra.core.dsl.meta.Phantoms (Java) and hydra.core.dsl.meta.phantoms (Python).
 *
 * Because hydra.core.typed.TypedTerm[A] is a `type` alias for hydra.core.model.Term in
 ...
 * hydra.core.overlay.scala.dsl.meta.Defs.checkComplete, which verifies every
...
object Phantoms:
   ...
   *
   * Returns a Hydra `Inject(hydra.core.model.Term, literal, Inject(hydra.core.model.Literal,
   * boolean, Literal(Boolean(b))))` expression — NOT a raw `Literal(Boolean(b))`.
   * The outer `Inject` has typeName `hydra.core.model.Term`, so Hydra inference
  ...
  def metaBool(b: Boolean): TypedTerm[Term] =
    Terms.inject("hydra.core.model.Term", "literal",
      Terms.inject("hydra.core.model.Literal", "boolean",
  ...
  def cat2(a: TypedTerm[String], b: TypedTerm[String]): TypedTerm[String] =
    applyP("hydra.core.lib.strings.concat2", a, b)
