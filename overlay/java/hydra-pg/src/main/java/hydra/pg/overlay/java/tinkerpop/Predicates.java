...
import hydra.pg.tinkerpop.gremlin.RangeArgument;
import hydra.pg.tinkerpop.gremlin.TraversalPredicate;
...
final class Predicates {
    ...
    private static Object value(hydra.pg.tinkerpop.gremlin.GenericLiteralArgument a) {
    ...
    private static Object[] collection(
            hydra.core.overlay.java.util.Optional<hydra.pg.tinkerpop.gremlin.GenericLiteralArgument> opt) {
        if (opt.isNone()) {
            return new Object[0];
        }
    ...
    static TraversalPredicate fromP(P<?> p) {
        ...
        if (p instanceof org.apache.tinkerpop.gremlin.process.traversal.util.AndP) {
            ...
            return new TraversalPredicate.And(new hydra.pg.tinkerpop.gremlin.TwoTraversalPredicates(fromP(ps.get(0)), fromP(ps.get(1))));
        }
        if (p instanceof org.apache.tinkerpop.gremlin.process.traversal.util.OrP) {
            ...
            return new TraversalPredicate.Or(new hydra.pg.tinkerpop.gremlin.TwoTraversalPredicates(fromP(ps.get(0)), fromP(ps.get(1))));
        }
        ...
        switch (name) {
            ...
            case "within":  return new TraversalPredicate.Within(hydra.core.overlay.java.util.Optional.given(genLit(value)));
            case "without": return new TraversalPredicate.Without(hydra.core.overlay.java.util.Optional.given(genLit(value)));
    ...
    private static hydra.pg.tinkerpop.gremlin.GenericLiteralArgument genLit(Object v) {
        return new hydra.pg.tinkerpop.gremlin.GenericLiteralArgument.Value(BytecodeToHydra.literal(v));
    }

    private static hydra.pg.tinkerpop.gremlin.RangeArgument range(Object v) {
        ...
        return new hydra.pg.tinkerpop.gremlin.RangeArgument(genLit(lo), genLit(hi));
    }

    private static hydra.pg.tinkerpop.gremlin.StringArgument strArg(Object v) {
        return new hydra.pg.tinkerpop.gremlin.StringArgument.Value((String) v);
    }
}
