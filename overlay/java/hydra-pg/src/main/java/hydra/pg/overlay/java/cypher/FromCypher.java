...
import static hydra.pg.overlay.java.dsl.Queries.apply;
import static hydra.pg.overlay.java.dsl.Queries.query;
...
public class FromCypher extends MapperBase {
     ...
     */
    public static ComparisonOperator from(hydra.pg.cypher.openCypher.ComparisonOperator cypher) {
        return cypher.accept(new hydra.pg.cypher.openCypher.ComparisonOperator.Visitor<ComparisonOperator>() {
            @Override
            public ComparisonOperator visit(hydra.pg.cypher.openCypher.ComparisonOperator.Eq instance) {
                return new ComparisonOperator.Eq();
            }

            @Override
            public ComparisonOperator visit(hydra.pg.cypher.openCypher.ComparisonOperator.Neq instance) {
                return new ComparisonOperator.Neq();
            }

            @Override
            public ComparisonOperator visit(hydra.pg.cypher.openCypher.ComparisonOperator.Lt instance) {
                return new ComparisonOperator.Lt();
            }

            @Override
            public ComparisonOperator visit(hydra.pg.cypher.openCypher.ComparisonOperator.Gt instance) {
                return new ComparisonOperator.Gt();
            }

            @Override
            public ComparisonOperator visit(hydra.pg.cypher.openCypher.ComparisonOperator.Lte instance) {
                return new ComparisonOperator.Lte();
            }

            @Override
            public ComparisonOperator visit(hydra.pg.cypher.openCypher.ComparisonOperator.Gte instance) {
     ...
     * @return the converted property graph Expression
     */
    public static Expression from(hydra.pg.cypher.openCypher.Expression cypher) {
    ...
    public static VertexPattern from(NodePattern cypher) {
        return new VertexPattern(
                ...
                labelOf(cypher),
                cypher.properties.isGiven()
                        ? ConsList.fromList(from(cypher.properties.fromGiven())) : ConsList.empty(),
                ConsList.empty());
    }
    ...
    public static EdgeProjectionPattern from(List<PatternElementChain> cypher) {
        ...
        for (int i = cypher.size() - 1; i >= 0; i--) {
            ...
            chain.relationship.detail.ifGiven(FromCypher::checkRelationshipDetail);
            ...
            ConsList<PropertyPattern> props = ps.isGiven()
                    ? ConsList.fromList(from(ps.fromGiven()))
                    : ConsList.empty();
            cur = new EdgeProjectionPattern(dir, label, props, Optional.given(vp));
        }
     ...
     */
    public static Query from(hydra.pg.cypher.openCypher.Query cypher) {
        return cypher.accept(new hydra.pg.cypher.openCypher.Query.Visitor<Query>() {
            @Override
            public Query visit(hydra.pg.cypher.openCypher.Query.Regular instance) {
                return from(instance.value);
            }

            @Override
            public Query visit(hydra.pg.cypher.openCypher.Query.Standalone instance) {
                return unsupported();
            }
        });
    }
    ...
    public static Query from(SingleQuery cypher) {
        return cypher.accept(new hydra.pg.cypher.openCypher.SingleQuery.Visitor<Query>() {
            @Override
            public Query visit(hydra.pg.cypher.openCypher.SingleQuery.SinglePart instance) {
                return from(instance.value);
            }

            @Override
            public Query visit(hydra.pg.cypher.openCypher.SingleQuery.MultiPart instance) {
                return unsupported();
            }
        });
    }
    ...
    public static Query from(SinglePartQuery cypher) {
        ...
        cypher.return_.ifGiven(r -> subqueries.add(query(from(r.value))));
    ...
    public static Expression from(UnaryAddOrSubtractExpression cypher) {
        return cypher.operator.isGiven()
                ? unsupported()
                : from(cypher.expression);
    }
     ...
     */
    public static Variable from(hydra.pg.cypher.openCypher.Variable cypher) {
    ...
    private static void checkRelationshipDetail(RelationshipDetail cypher) {
        if (cypher.range.isGiven()) {
            throw new MapperException("not yet supported");
        }
        if (cypher.variable.isGiven()) {
            throw new MapperException("not yet supported");
        }
    }
    ...
    private static Optional<VertexLabel> labelOf(NodePattern cypher) {
        if (cypher.labels.isGiven()) {
            List<NodeLabel> labels = cypher.labels.fromGiven().value;
            if (labels.size() > 0) {
                if (labels.size() > 1) {
                    return unsupported("multiple vertex labels per pattern not yet supported");
                }

                return Optional.given(new VertexLabel(labels.get(0).value));
            }
        }
    ...
    private static Optional<EdgeLabel> labelOf(RelationshipPattern cypher) {
        if (cypher.detail.isGiven()) {
            RelationshipDetail detail = cypher.detail.fromGiven();
            if (detail.types.isGiven()) {
                List<RelTypeName> types = detail.types.fromGiven().value;
                if (types.size() > 0) {
                    if (types.size() > 1) {
                        return unsupported("multiple edge labels per pattern not yet supported");
                    }

                    return Optional.given(new EdgeLabel(types.get(0).value));
                }
            }
        }
