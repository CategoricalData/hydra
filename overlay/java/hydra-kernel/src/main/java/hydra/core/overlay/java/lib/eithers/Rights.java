...
public class Rights extends PrimitiveFunction {
    public static final Name NAME = new Name("hydra.core.lib.eithers.rights");
     ...
     */
    public static <A, B> List<B> apply(List<hydra.core.overlay.java.util.Either<A, B>> eithers) {
        ConsList<B> reversed = ConsList.empty();
        for (hydra.core.overlay.java.util.Either<A, B> either : eithers) {
            if (either.isRight()) {
                reversed = ConsList.cons(((hydra.core.overlay.java.util.Either.Right<A, B>) either).value, reversed);
            }
        }
