...
public class Abs extends PrimitiveFunction {
    ...
    protected Function<List<Term>, Function<Graph, Either<Error_, Term>>> implementation() {
        return args -> graph -> Either.right(NumericDispatch.unary("abs", ABS, args.get(0)));
    }

    private static final NumericDispatch.UnaryOp ABS = new NumericDispatch.UnaryOp() {
    ...
    /**
     * Computes the absolute value. This is the statically-typed entry point emitted by generated
     * code. It is generic and erased so that code polymorphic over a {@code numeric} type variable
     * can reference it as a {@code <A> A apply(A)} function value; the runtime numeric type is
     * recovered by dispatching on the operand's boxed class (see
     * {@link NumericDispatch#applyNativeUnary}).
     * @param num the number
     * @return the absolute value
     */
    public static <A> A apply(A num) {
        return NumericDispatch.applyNativeUnary("abs", ABS, num);
    }
}
