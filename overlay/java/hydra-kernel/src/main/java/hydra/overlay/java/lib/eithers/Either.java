...
public class Either extends PrimitiveFunction {
    ...
    @Override
    protected Function<List<Term>, Function<Graph, hydra.overlay.java.util.Either<Error_, Term>>> implementation() {
        return args -> graph -> {
            ...
            return hydra.overlay.java.lib.eithers.Map.apply(e -> e.accept(new hydra.overlay.java.util.Either.Visitor<Term, Term, Term>() {
                ...
                }), hydra.extract.Core.eitherTerm(t -> hydra.overlay.java.util.Either.right(t), t -> hydra.overlay.java.util.Either.right(t), graph, args.get(2)));
        };
    }
    ...
    public static <A, B, C> C apply(
        ...
        if (either.isLeft()) {
            return leftFn.apply(((hydra.overlay.java.util.Either.Left<A, B>) either).value);
        } else {
            return rightFn.apply(((hydra.overlay.java.util.Either.Right<A, B>) either).value);
        }
    }
}
