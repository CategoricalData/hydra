...
public class MaybeInit extends PrimitiveFunction {
    ...
    protected Function<List<Term>, Function<Graph, Either<Error_, Term>>> implementation() {
        return args -> graph -> hydra.lib.eithers.Map.apply((Function<List<Term>, Term>) l -> Terms.optional(MaybeInit.apply(l).map(Terms::list)), hydra.extract.Core.list(graph, args.get(0)));
    }
