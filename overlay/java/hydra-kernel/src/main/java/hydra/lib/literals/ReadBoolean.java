...
public class ReadBoolean extends PrimitiveFunction {
    ...
    protected Function<List<Term>, Function<Graph, Either<Error_, Term>>> implementation() {
        return args -> graph -> hydra.lib.eithers.Map.apply((Function<String, Term>) s -> Terms.optional(apply(s).map(Terms::boolean_)), hydra.extract.Core.string(graph, args.get(0)));
    }
