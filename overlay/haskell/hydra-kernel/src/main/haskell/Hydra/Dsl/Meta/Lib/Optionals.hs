...
apply = primitive2 (Prims.primName DefOptionals.apply)
...
bind = primitive2 (Prims.primName DefOptionals.bind)
...
cases m def f = primitive3 (Prims.primName DefOptionals.cases) m (asTerm def) (asTerm f)

-- | Filter out absent values from a list.
cat :: TypedTerm [Maybe a] -> TypedTerm [a]
cat = primitive1 (Prims.primName DefOptionals.cat)
...
toList = primitive1 (Prims.primName DefOptionals.toList)
