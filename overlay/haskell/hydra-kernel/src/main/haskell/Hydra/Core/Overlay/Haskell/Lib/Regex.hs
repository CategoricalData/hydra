-- | Haskell implementations of hydra.core.lib.regex primitives.
--
-- Patterns are Hydra-defined and translingual (docs/specification/regex.md): each primitive first
-- runs the pattern through hydra.core.parse.regex, then renders the resulting AST to POSIX ERE syntax via
-- hydra.core.print.posix.regex (Haskell's Text.Regex.TDFA is POSIX ERE), and only then hands the rendered
-- pattern to the native engine. An ill-formed pattern (one that hydra.core.parse.regex rejects) is treated
...
module Hydra.Core.Overlay.Haskell.Lib.Regex where

import qualified Text.Regex.TDFA as R

import qualified Hydra.Core.Parse.Regex as ParseRegex
import qualified Hydra.Core.Print.Posix.Regex as PrintPosixRegex
