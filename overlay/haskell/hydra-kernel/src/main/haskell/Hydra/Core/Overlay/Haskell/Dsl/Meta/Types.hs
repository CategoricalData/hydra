...
module Hydra.Core.Overlay.Haskell.Dsl.Meta.Types (
...
import Hydra.Core.Overlay.Haskell.Dsl.Meta.Core as Core hiding (name, unName)
...
nonNegativeInt32 = Hydra.Core.Overlay.Haskell.Dsl.Meta.Types.int32
...
polyConstrained params constraints t = Phantoms.record _TypeScheme [
  ...
  where
    constraintsTerm = Phantoms.map $ M.fromList
      [(name v, Core.typeVariableConstraints (Phantoms.set (Core.typeClassConstraintSimple . name <$> classes)))
