-- | Haskell implementations of hydra.core.lib.math primitives
{-# LANGUAGE RankNTypes #-}

module Hydra.Core.Overlay.Haskell.Lib.Math where
...
-- registered with identity (Term) coders (see Hydra.Core.Overlay.Haskell.Libraries) so that the
...
numericBinary opName op x y = case (numericLiteral opName x, numericLiteral opName y) of
  ...
  _ -> Prelude.error $ "hydra.core.lib.math." Prelude.++ opName Prelude.++ ": operands are not the same numeric kind"
...
numericUnary opName op x = case numericLiteral opName x of
  ...
  _ -> Prelude.error $ "hydra.core.lib.math." Prelude.++ opName Prelude.++ ": operand is not numeric"
...
numericLiteral opName term = case term of
  ...
  _ -> Prelude.error $ "hydra.core.lib.math." Prelude.++ opName Prelude.++ ": expected a literal term"
...
integerBinary opName op ix iy = case (ix, iy) of
  ...
  _ -> Prelude.error $ "hydra.core.lib.math." Prelude.++ opName Prelude.++ ": integer operands differ in precision"
...
floatBinary opName op fx fy = case (fx, fy) of
  ...
  _ -> Prelude.error $ "hydra.core.lib.math." Prelude.++ opName Prelude.++ ": float operands differ in precision"
...
-- identity (Term) coder (see Hydra.Core.Overlay.Haskell.Libraries) so that the runtime float type
...
divideTerm x y = case (fractionalLiteral x, fractionalLiteral y) of
  ...
  _ -> Prelude.error "hydra.core.lib.math.divide: operands are not the same fractional kind"

-- | Extract the literal from a term, failing loudly on any non-literal term.
fractionalLiteral :: Core.Term -> Core.Literal
fractionalLiteral term = case term of
  Core.TermLiteral lit -> lit
  _ -> Prelude.error "hydra.core.lib.math.divide: expected a literal term"
...
floatDivide fx fy = case (fx, fy) of
  ...
  _ -> Prelude.error "hydra.core.lib.math.divide: float operands differ in precision"
...
-- identity (Term) coders (see Hydra.Core.Overlay.Haskell.Libraries) so the runtime integer type
...
divTerm x y = case (integralLiteral "div" x, integralLiteral "div" y) of
  ...
  _ -> Prelude.error "hydra.core.lib.math.div: integer operands differ in precision"
...
modTerm x y = case (integralLiteral "mod" x, integralLiteral "mod" y) of
  ...
  _ -> Prelude.error "hydra.core.lib.math.mod: integer operands differ in precision"
...
remTerm x y = case (integralLiteral "rem" x, integralLiteral "rem" y) of
  ...
  _ -> Prelude.error "hydra.core.lib.math.rem: integer operands differ in precision"
...
integralLiteral opName term = case term of
  ...
  _ -> Prelude.error $ "hydra.core.lib.math." Prelude.++ opName Prelude.++ ": expected an integer literal term"
