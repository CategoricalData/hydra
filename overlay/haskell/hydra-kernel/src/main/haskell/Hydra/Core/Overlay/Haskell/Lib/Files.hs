-- | Haskell implementations of hydra.core.lib.files primitives

module Hydra.Core.Overlay.Haskell.Lib.Files where
...
-- | Classify a POSIX file status's file type as a hydra.core.file.FileType.
...
-- exposed by the portable "unix" API) to a hydra.core.time.Timespec.
