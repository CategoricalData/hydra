...
(define (run-json-coder-test path tc)
  (let ((cx (empty-context))
        (graph (get-test-graph)))
    (guard (exn (#t (begin (display (string-append "FAIL: " path "\n"))
                           (display (string-append "  EXCEPTION: " (error-object-message exn) "\n"))
                           (list 0 1 0))))
      (let ((coder-result (((hydra_ext_org_json_coder_json_coder
        ...
        (if (eq? (car coder-result) 'left)
            (begin (display (string-append "FAIL: " path "\n  JSON coder creation failed\n"))
                   (list 0 1 0))
            (let* ((coder (cadr coder-result))
                   (encode-fn (hydra_compute_coder-encode coder))
                   (decode-fn (hydra_compute_coder-decode coder))
                  ...
                  (let ((encoded (cadr encode-result)))
                    (if (not (terms-match? (hydra_testing_json_coder_test_case-json tc) encoded))
                        (begin (display (string-append "FAIL: " path "\n  JSON encode mismatch\n"))
                               (list 0 1 0))
                        (let ((decode-result ((decode-fn cx) encoded)))
                          (if (eq? (car decode-result) 'left)
                              (begin (display (string-append "FAIL: " path "\n  JSON decode failed\n"))
                                     (list 0 1 0))
                              (if (terms-match? (hydra_testing_json_coder_test_case-term tc)
                                                (cadr decode-result))
                                  (list 1 0 0)
                                  (begin (display (string-append "FAIL: " path "\n  Roundtrip mismatch\n"))
                                         (list 0 1 0))))))))))))))
        ...
        (if (eq? (car coder-result) 'left)
            ...
            (let* ((coder (cadr coder-result))
              ...
              (if (eq? (car encode-result) 'left)
                  (begin (display (string-append "FAIL: " path "\n  JSON encode failed\n"))
                         (list 0 1 0))
                  (let ((decode-result ((decode-fn cx) (cadr encode-result))))
                    (if (eq? (car decode-result) 'left)
                        (begin (display (string-append "FAIL: " path "\n  JSON decode failed\n"))
                               (list 0 1 0))
                        (if (terms-match? (hydra_testing_json_roundtrip_test_case-term tc)
                                          (cadr decode-result))
                            (list 1 0 0)
                            (begin (display (string-append "FAIL: " path "\n  Roundtrip mismatch\n"))
                                   (list 0 1 0))))))))))))
              ...
              (cond
                ...
                ((eq? (car expected) 'right)
                 (if (eq? (car decode-result) 'left)
                     (begin (display (string-append "FAIL: " path "\n  JSON decode failed\n"))
                            (list 0 1 0))
                     (if (terms-match? (cadr expected) (cadr decode-result))
                         (list 1 0 0)
                         (begin (display (string-append "FAIL: " path "\n  Decode mismatch\n"))
                                (list 0 1 0)))))
                (else (list 0 1 0)))))))))
