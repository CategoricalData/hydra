...
(defun hydra-run-json-coder-test (path tc)
  (let ((cx (hydra-empty-context)) (graph hydra--test-graph))
    (condition-case err
        (let ((coder-result (funcall (funcall (funcall hydra_ext_org_json_coder_json_coder
                                                       (cdr (assq :type tc))) cx) graph)))
          (if (eq (car coder-result) :left)
              (progn (message "FAIL: %s\n  JSON coder creation failed: %S" path (cadr coder-result)) (list 0 1 0))
            (let* ((coder (cadr coder-result))
                   (encode-fn (cdr (assq :encode coder)))
                   (decode-fn (cdr (assq :decode coder)))
                   (encode-result (funcall (funcall encode-fn cx) (cdr (assq :term tc)))))
              (if (eq (car encode-result) :left)
                  (progn (message "FAIL: %s\n  JSON encode failed" path) (list 0 1 0))
                  ...
                  (if (not (hydra-terms-match-p (cdr (assq :json tc)) encoded))
                    ...
                    (let ((decode-result (funcall (funcall decode-fn cx) encoded)))
                      (if (eq (car decode-result) :left)
                          (progn (message "FAIL: %s\n  JSON decode failed" path) (list 0 1 0))
  ...
  (let ((cx (hydra-empty-context)) (graph hydra--test-graph))
    (condition-case err
        (let ((coder-result (funcall (funcall (funcall hydra_ext_org_json_coder_json_coder
                                                       (cdr (assq :type tc))) cx) graph)))
          (if (eq (car coder-result) :left)
              (progn (message "FAIL: %s\n  JSON coder creation failed" path) (list 0 1 0))
            (let* ((coder (cadr coder-result))
                   (encode-fn (cdr (assq :encode coder)))
                   (decode-fn (cdr (assq :decode coder)))
                   (encode-result (funcall (funcall encode-fn cx) (cdr (assq :term tc)))))
              (if (eq (car encode-result) :left)
                  (progn (message "FAIL: %s\n  JSON encode failed" path) (list 0 1 0))
                (let ((decode-result (funcall (funcall decode-fn cx) (cadr encode-result))))
                  (if (eq (car decode-result) :left)
                      (progn (message "FAIL: %s\n  JSON decode failed" path) (list 0 1 0))
                    (if (hydra-terms-match-p (cdr (assq :term tc)) (cadr decode-result))
                        (list 1 0 0)
    ...
    (condition-case err
        (let ((coder-result (funcall (funcall (funcall hydra_ext_org_json_coder_json_coder
          ...
          (if (eq (car coder-result) :left)
            ...
            (let* ((coder (cadr coder-result))
                   (decode-fn (cdr (assq :decode coder)))
                   (decode-result (funcall (funcall decode-fn cx) (cdr (assq :json tc))))
                   (expected (cdr (assq :expected tc))))
              (cond
               ((eq (car expected) :left)
