...
(defvar hydra_lexical_elements_to_graph (lambda (parent) (lambda (schema_types) (lambda (elements) (let ((prims ((lambda (v) (hydra_graph_graph-primitives v)) parent))) (make-hydra_graph_graph ((lambda (v) (hydra_graph_graph-bound_terms v)) (((hydra_lexical_build_graph elements) hydra_lib_maps_empty) prims)) ((lambda (v) (hydra_graph_graph-bound_types v)) (((hydra_lexical_build_graph elements) hydra_lib_maps_empty) prims)) ((lambda (v) (hydra_graph_graph-class_constraints v)) (((hydra_lexical_build_graph elements) hydra_lib_maps_empty) prims)) ((lambda (v) (hydra_graph_graph-lambda_variables v)) (((hydra_lexical_build_graph elements) hydra_lib_maps_empty) prims)) ((lambda (v) (hydra_graph_graph-metadata v)) (((hydra_lexical_build_graph elements) hydra_lib_maps_empty) prims)) ((lambda (v) (hydra_graph_graph-primitives v)) (((hydra_lexical_build_graph elements) hydra_lib_maps_empty) prims)) schema_types ((lambda (v) (hydra_graph_graph-type_variables v)) (((hydra_lexical_build_graph elements) hydra_lib_maps_empty) prims))))))))
...
(defvar hydra_lexical_fields_of (lambda (t_) (let ((stripped (hydra_rewriting_deannotate_type t_))) ((lambda (match_target) ((lambda (match_value) (cond ((equal (car match_target) :forall) ((lambda (forall_type) (hydra_lexical_fields_of ((lambda (v) (hydra_core_forall_type-body v)) forall_type))) match_value)) ((equal (car match_target) :record) ((lambda (rt) ((lambda (v) (hydra_core_row_type-fields v)) rt)) match_value)) ((equal (car match_target) :union) ((lambda (rt) ((lambda (v) (hydra_core_row_type-fields v)) rt)) match_value)) (t (list)))) (cadr match_target))) stripped))))

(defvar hydra_lexical_get_field (lambda (cx) (lambda (m) (lambda (fname) (lambda (decode) (((hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context ((hydra_lib_strings_cat2 ((hydra_lib_strings_cat2 "expected field ") ((lambda (v) v) fname))) " not found") cx))) decode) ((hydra_lib_maps_lookup fname) m)))))))
...
(defvar hydra_lexical_require_element (lambda (cx) (lambda (graph) (lambda (name) (let ((show_all nil)) (let ((ellipsis (lambda (strings) (if ((hydra_lib_logic_and ((hydra_lib_equality_gt (hydra_lib_lists_length strings)) 3)) (hydra_lib_logic_not show_all)) ((hydra_lib_lists_concat2 ((hydra_lib_lists_take 3) strings)) (list "...")) strings)))) (let ((err_msg ((hydra_lib_strings_cat2 ((hydra_lib_strings_cat2 ((hydra_lib_strings_cat2 ((hydra_lib_strings_cat2 "no such element: ") ((lambda (v) v) name))) ". Available elements: {")) ((hydra_lib_strings_intercalate ", ") (ellipsis ((hydra_lib_lists_map (lambda (v) v)) (hydra_lib_maps_keys ((lambda (v) (hydra_graph_graph-bound_terms v)) graph))))))) "}"))) (((hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context err_msg cx))) (lambda (x) (list :right x))) ((hydra_lexical_dereference_element graph) name)))))))))

(defvar hydra_lexical_match_union (lambda (cx) (lambda (graph) (lambda (tname) (lambda (pairs) (lambda (term) (let ((stripped (hydra_rewriting_deannotate_and_detype_term term))) (let ((mapping (hydra_lib_maps_from_list pairs))) ((lambda (match_target) ((lambda (match_value) (cond ((equal (car match_target) :variable) ((lambda (name) ((hydra_lib_eithers_bind (((hydra_lexical_require_element cx) graph) name)) (lambda (el) (((((hydra_lexical_match_union cx) graph) tname) pairs) ((lambda (v) (hydra_core_binding-term v)) el))))) match_value)) ((equal (car match_target) :union) ((lambda (injection) (let ((exp (let ((fname ((lambda (v) (hydra_core_field-name v)) ((lambda (v) (hydra_core_injection-field v)) injection)))) (let ((val ((lambda (v) (hydra_core_field-term v)) ((lambda (v) (hydra_core_injection-field v)) injection)))) (((hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context ((hydra_lib_strings_cat2 ((hydra_lib_strings_cat2 ((hydra_lib_strings_cat2 "no matching case for field \"") ((lambda (v) v) fname))) "\" in union type ")) ((lambda (v) v) tname)) cx))) (lambda (f) (f val))) ((hydra_lib_maps_lookup fname) mapping)))))) (if ((hydra_lib_equality_equal ((lambda (v) v) ((lambda (v) (hydra_core_injection-type_name v)) injection))) ((lambda (v) v) tname)) exp (list :left (make-hydra_context_in_context ((hydra_lib_strings_cat2 ((hydra_lib_strings_cat2 ((hydra_lib_strings_cat2 "expected injection for type ") ((lambda (v) v) tname))) ", got ")) (hydra_show_core_term term)) cx))))) match_value)) (t (list :left (make-hydra_context_in_context (hydra_lib_strings_cat (list "expected inject(" ((lambda (v) v) tname) ") with one of {" ((hydra_lib_strings_intercalate ", ") ((hydra_lib_lists_map (lambda (pair) ((lambda (v) v) (hydra_lib_pairs_first pair)))) pairs)) "}, got " (hydra_show_core_term stripped))) cx))))) (cadr match_target))) stripped)))))))))
...
(defvar hydra_lexical_match_record (lambda (cx) (lambda (graph) (lambda (decode) (lambda (term) (let ((stripped (hydra_rewriting_deannotate_and_detype_term term))) ((lambda (match_target) ((lambda (match_value) (cond ((equal (car match_target) :record) ((lambda (record) (decode (hydra_lib_maps_from_list ((hydra_lib_lists_map (lambda (field) (list ((lambda (v) (hydra_core_field-name v)) field) ((lambda (v) (hydra_core_field-term v)) field)))) ((lambda (v) (hydra_core_record-fields v)) record))))) match_value)) (t (list :left (make-hydra_context_in_context ((hydra_lib_strings_cat2 "expected a record, got ") (hydra_show_core_term term)) cx))))) (cadr match_target))) stripped)))))))

(defvar hydra_lexical_require_primitive (lambda (cx) (lambda (graph) (lambda (name) (((hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context ((hydra_lib_strings_cat2 "no such primitive function: ") ((lambda (v) v) name)) cx))) (lambda (x) (list :right x))) ((hydra_lexical_lookup_primitive graph) name))))))

(defvar hydra_lexical_require_primitive_type (lambda (cx) (lambda (tx) (lambda (name) (let ((mts ((hydra_lib_maps_lookup name) (hydra_lib_maps_from_list ((hydra_lib_lists_map (lambda (_gpt_p) (list ((lambda (v) (hydra_graph_primitive-name v)) _gpt_p) ((lambda (v) (hydra_graph_primitive-type v)) _gpt_p)))) (hydra_lib_maps_elems ((lambda (v) (hydra_graph_graph-primitives v)) tx))))))) (((hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context ((hydra_lib_strings_cat2 "no such primitive function: ") ((lambda (v) v) name)) cx))) (lambda (ts) (list :right ts))) mts))))))
...
(defvar hydra_lexical_require_term (lambda (cx) (lambda (graph) (lambda (name) (((hydra_lib_maybes_maybe (list :left (make-hydra_context_in_context ((hydra_lib_strings_cat2 "no such element: ") ((lambda (v) v) name)) cx))) (lambda (x) (list :right x))) ((hydra_lexical_resolve_term graph) name))))))
