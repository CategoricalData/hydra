...
(def hydra_names_qualify_name (fn [name] (let [parts (hydra_lib_lists_reverse ((hydra_lib_strings_split_on ".") ((fn [v] v) name)))] (if ((hydra_lib_equality_equal 1) (hydra_lib_lists_length parts)) (->hydra_module_qualified_name nil ((fn [v] v) name)) (->hydra_module_qualified_name ((hydra_lib_strings_intercalate ".") (hydra_lib_lists_reverse (hydra_lib_lists_tail parts))) (hydra_lib_lists_head parts))))))

(def hydra_names_compact_name (fn [namespaces] (fn [name] (let [qual_name (hydra_names_qualify_name name) local ((fn [v] (:local v)) qual_name) mns ((fn [v] (:namespace v)) qual_name)] (((hydra_lib_maybes_maybe ((fn [v] v) name)) (fn [ns_] (((hydra_lib_maybes_maybe local) (fn [pre] (hydra_lib_strings_cat (list pre ":" local)))) ((hydra_lib_maps_lookup ns_) namespaces)))) mns)))))
...
(def hydra_names_unqualify_name (fn [qname] (let [prefix (((hydra_lib_maybes_maybe "") (fn [n] ((hydra_lib_strings_cat2 ((fn [v] v) n)) "."))) ((fn [v] (:namespace v)) qname))] ((hydra_lib_strings_cat2 prefix) ((fn [v] (:local v)) qname)))))
