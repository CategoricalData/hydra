...
(def hydra_annotations_aggregate_annotations (fn [get_value] (fn [get_x] (fn [get_anns] (fn [t_] (let [to_pairs (fn to_pairs [rest] (fn [t_] (((hydra_lib_maybes_maybe rest) (fn [yy] ((to_pairs ((hydra_lib_lists_cons (hydra_lib_maps_to_list (get_anns yy))) rest)) (get_x yy)))) (get_value t_))))] (hydra_lib_maps_from_list (hydra_lib_lists_concat ((to_pairs (list)) t_)))))))))
...
(def hydra_annotations_get_description (fn [cx] (fn [graph] (fn [anns] (((hydra_lib_maybes_maybe (list :right nil)) (fn [term] ((hydra_lib_eithers_map hydra_lib_maybes_pure) (((hydra_extract_core_string cx) graph) term)))) ((hydra_lib_maps_lookup "description") anns))))))

(def hydra_annotations_term_annotation_internal (fn [term] (let [get_ann (fn [t_] ((fn [match_target] ((fn [match_value] (cond (= (first match_target) :annotated) ((fn [a] a) match_value) :else nil)) (second match_target))) t_))] ((((hydra_annotations_aggregate_annotations get_ann) (fn [at] ((fn [v] (:body v)) at))) (fn [at] ((fn [v] (:annotation v)) at))) term))))
...
(def hydra_annotations_get_type (fn [graph] (fn [anns] (((hydra_lib_maybes_maybe (list :right nil)) (fn [dat] ((hydra_lib_eithers_map hydra_lib_maybes_pure) ((hydra_decode_core_type graph) dat)))) ((hydra_lib_maps_lookup hydra_constants_key_type) anns)))))

(def hydra_annotations_type_annotation_internal (fn [typ] (let [get_ann (fn [t_] ((fn [match_target] ((fn [match_value] (cond (= (first match_target) :annotated) ((fn [a] a) match_value) :else nil)) (second match_target))) t_))] ((((hydra_annotations_aggregate_annotations get_ann) (fn [at] ((fn [v] (:body v)) at))) (fn [at] ((fn [v] (:annotation v)) at))) typ))))
...
(def hydra_annotations_is_native_type (fn [el] (let [is_flagged_as_first_class_type ((hydra_lib_maybes_from_maybe false) ((hydra_lib_maybes_map (fn [_] true)) ((hydra_annotations_get_term_annotation hydra_constants_key_first_class_type) ((fn [v] (:term v)) el))))] (((hydra_lib_maybes_maybe false) (fn [ts] ((hydra_lib_logic_and ((hydra_lib_equality_equal ts) (->hydra_core_type_scheme (list) (list :variable "hydra.core.Type") nil))) (hydra_lib_logic_not is_flagged_as_first_class_type)))) ((fn [v] (:type v)) el)))))
