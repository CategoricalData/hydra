...
(defn -main [& _args]
  ...
  (require 'hydra.ext.org.json.coder)
  ;; Load test suite
  (require 'hydra.test.testSuite)
  ;; Build the test graph now that all namespaces are loaded
  (ensure-test-graph!)
  (println "DEBUG: schema types:" (count (:schema_types @test-graph)))
  (println "DEBUG: reduction ns exists?" (boolean (find-ns 'hydra.reduction)))
  (let [v (ns-resolve 'hydra.reduction 'hydra_reduction_reduce_term)]
    (println "DEBUG: reduce_term var?" (boolean v))
    (println "DEBUG: reduce_term bound?" (and v (bound? v)))
    (when (and v (bound? v))
      (println "DEBUG: reduce_term type:" (type @v))))
  ;; Run test suite
  (let [suite (deref (ns-resolve 'hydra.test.testSuite 'hydra_test_test_suite_all_tests))
        [pass fail skip] (run-test-group "" suite)]
    (println)
    (println (str "Results: " pass " passed, " fail " failed, " skip " skipped"))
    (println (str "Total:   " (+ pass fail skip)))
