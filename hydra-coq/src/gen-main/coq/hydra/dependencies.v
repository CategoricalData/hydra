...
(* Module dependencies *)
Require Import hydra.core hydra.rewriting hydra.coders hydra.lib.sets hydra.lib.logic hydra.variables hydra.packaging hydra.sorting hydra.lib.lists hydra.lib.maps hydra.lib.maybes hydra.lib.pairs hydra.lib.math hydra.lib.equality hydra.lib.strings hydra.lib.literals hydra.names hydra.strip hydra.errors hydra.lib.eithers hydra.graph hydra.lexical.

Definition typeNamesInType (t0 : Type) : Type_ -> (list) (t0) :=
  fun (typ0 : Type_) => let addNames := fun (t1 : Type) => fun (t2 : Type) => fun (names : t1) => fun (typ : t2) => names in ((((foldOverType) ((TraversalOrder_Pre) (tt))) (((addNames) ((list) (t0))) (Type_))) (sets.empty)) (typ0).
Arguments typeNamesInType {t0}.
Definition typeDependencyNames : bool -> Type_ -> (list) (Name) :=
  fun (withSchema : bool) => fun (typ : Type_) => (((logic.ifElse) (withSchema)) (((sets.union) ((freeVariablesInType) (typ))) ((typeNamesInType) (typ)))) ((freeVariablesInType) (typ)).
Definition topologicalSortTypeDefinitions : (list) (TypeDefinition) -> (list) ((list) (TypeDefinition)) :=
  fun (defs : (list) (TypeDefinition)) => let toPair := fun (def : TypeDefinition) => (pair) ((fun r_ => (typeDefinition_name) (r_)) (def)) ((sets.toList) (((typeDependencyNames) (false)) ((fun r_ => (typeScheme_type) (r_)) ((fun r_ => (typeDefinition_type) (r_)) (def))))) in let sorted := (topologicalSortComponents) (((lists.map) (toPair)) (defs)) in let nameToDef := (maps.fromList) (((lists.map) (fun (d : TypeDefinition) => (pair) ((fun r_ => (typeDefinition_name) (r_)) (d)) (d))) (defs)) in ((lists.map) (fun (names : (list) (Name)) => (maybes.cat) (((lists.map) (fun (n : Name) => ((maps.lookup) (n)) (nameToDef))) (names)))) (sorted).
Definition topologicalSortBindingMap : (list) ((prod) (Name) (Term)) -> (list) ((list) ((prod) (Name) (Term))) :=
  fun (bindingMap : (list) ((prod) (Name) (Term))) => let toPair := fun (name : Name) => (pair) (name) (((maybes.fromMaybe) ((Term_Literal) ((Literal_String) ("Impossible!"%string)))) (((maps.lookup) (name)) (bindingMap))) in let hasTypeAnnotation := (hydra_fix) (fun hasTypeAnnotation => fun (term_ : Term) => (fun x_ => match x_ with
| Term_Annotated v_ => (fun (at_ : AnnotatedTerm) => (hasTypeAnnotation) ((fun r_ => (annotatedTerm_body) (r_)) (at_))) (v_)
| _ => false
end) (term_)) in let bindings := (maps.toList) (bindingMap) in let keys := (sets.fromList) (((lists.map) (pairs.first)) (bindings)) in let depsOf := fun nameAndTerm => let term_ := (pairs.second) (nameAndTerm) in let name := (pairs.first) (nameAndTerm) in (pair) (name) ((((logic.ifElse) ((hasTypeAnnotation) (term_))) (nil)) ((sets.toList) (((sets.intersection) (keys)) ((freeVariablesInTerm) (term_))))) in ((lists.map) ((lists.map) (toPair))) ((topologicalSortComponents) (((lists.map) (depsOf)) (bindings))).
Definition toShortNames : (list) (Name) -> (list) ((prod) (Name) (Name)) :=
...
Definition topologicalSortBindings : (list) (Binding) -> (sum) ((list) ((list) (Name))) ((list) (Name)) :=
  fun (els : (list) (Binding)) => let adjlist := fun (e : Binding) => (pair) ((fun r_ => (binding_name) (r_)) (e)) ((sets.toList) (((((termDependencyNames) (false)) (true)) (true)) ((fun r_ => (binding_term) (r_)) (e)))) in (topologicalSort) (((lists.map) (adjlist)) (els)).
...
| _ => term2
end) (stripped) in (recurse) ((forTerm) (stripped)) in ((rewriteTerm) (simplify)) (term_)).

Definition simplifyTerm : Term -> Term :=
  simplifyTerm_bundle.
Definition replaceTypedefs : (list) ((prod) (Name) (TypeScheme)) -> Type_ -> Type_ :=
  fun (types : (list) ((prod) (Name) (TypeScheme))) => fun (typ0 : Type_) => let rewrite := (hydra_fix) (fun rewrite => fun (recurse : Type_ -> Type_) => fun (typ : Type_) => (fun x_ => match x_ with
...
Definition pruneLet : Let -> Let :=
  fun (l : Let) => let rootName := "[[[root]]]"%string in let bindingMap := (maps.fromList) (((lists.map) (fun (b : Binding) => (pair) ((fun r_ => (binding_name) (r_)) (b)) ((fun r_ => (binding_term) (r_)) (b)))) ((fun r_ => (let_bindings) (r_)) (l))) in let adj := fun (n : Name) => ((sets.intersection) ((sets.fromList) ((maps.keys) (bindingMap)))) ((freeVariablesInTerm) ((((logic.ifElse) (((equality.equal) (n)) (rootName))) ((fun r_ => (let_body) (r_)) (l))) ((maybes.fromJust) (((maps.lookup) (n)) (bindingMap))))) in let reachable := ((findReachableNodes) (adj)) (rootName) in let prunedBindings := ((lists.filter) (fun (b : Binding) => ((sets.member) ((fun r_ => (binding_name) (r_)) (b))) (reachable))) ((fun r_ => (let_bindings) (r_)) (l)) in (Build_Let) (prunedBindings) ((fun r_ => (let_body) (r_)) (l)).
...
Definition isLambda : Term -> bool :=
  isLambda_bundle.
Definition inlineType_bundle :=
  hydra_fix (fun (bundle_ : (list) ((prod) (Name) (Type_)) -> Type_ -> (sum) (Error) (Type_)) =>
...
Definition inlineType : (list) ((prod) (Name) (Type_)) -> Type_ -> (sum) (Error) (Type_) :=
  inlineType_bundle.
Definition flattenLetTerms : Term -> Term :=
  fun (term_ : Term) => let rewriteBinding := (hydra_fix) (fun rewriteBinding => fun (binding : Binding) => let val0 := (fun r_ => (binding_term) (r_)) (binding) in let t := (fun r_ => (binding_type) (r_)) (binding) in let key0 := (fun r_ => (binding_name) (r_)) (binding) in (fun x_ => match x_ with
| Term_Annotated v_ => (fun (at_ : AnnotatedTerm) => let val1 := (fun r_ => (annotatedTerm_body) (r_)) (at_) in let recursive := (rewriteBinding) ((Build_Binding) (key0) (val1) (t)) in let innerBinding := (pairs.first) (recursive) in let val2 := (fun r_ => (binding_term) (r_)) (innerBinding) in let deps := (pairs.second) (recursive) in let ann := (fun r_ => (annotatedTerm_annotation) (r_)) (at_) in (pair) ((Build_Binding) (key0) ((Term_Annotated) ((Build_AnnotatedTerm) (val2) (ann))) (t)) (deps)) (v_)
| Term_Let v_ => (fun (innerLet : Let) => let prefix := ((strings.cat2) ((fun w_ => w_) (key0))) ("_"%string) in let qualify := fun (n : Name) => ((strings.cat2) (prefix)) ((fun w_ => w_) (n)) in let toSubstPair := fun (b : Binding) => (pair) ((fun r_ => (binding_name) (r_)) (b)) ((qualify) ((fun r_ => (binding_name) (r_)) (b))) in let body1 := (fun r_ => (let_body) (r_)) (innerLet) in let bindings1 := (fun r_ => (let_bindings) (r_)) (innerLet) in let subst := (maps.fromList) (((lists.map) (toSubstPair)) (bindings1)) in let replaceVars := (substituteVariables) (subst) in let newBinding := fun (b : Binding) => (Build_Binding) ((qualify) ((fun r_ => (binding_name) (r_)) (b))) ((replaceVars) ((fun r_ => (binding_term) (r_)) (b))) ((fun r_ => (binding_type) (r_)) (b)) in let newBody := (replaceVars) (body1) in (pair) ((Build_Binding) (key0) (newBody) (t)) (((lists.map) (newBinding)) (bindings1))) (v_)
| _ => (pair) ((Build_Binding) (key0) (val0) (t)) (nil)
end) (val0)) in let flattenBodyLet := (hydra_fix) (fun flattenBodyLet => fun (bindings : (list) (Binding)) => fun (body : Term) => (fun x_ => match x_ with
| Term_Let v_ => (fun (innerLt : Let) => let innerBody := (fun r_ => (let_body) (r_)) (innerLt) in let innerBindings := (fun r_ => (let_bindings) (r_)) (innerLt) in ((flattenBodyLet) (((lists.concat2) (bindings)) (innerBindings))) (innerBody)) (v_)
| _ => (pair) (((lists.concat2) (nil)) (bindings)) (body)
end) (body)) in let flatten := fun recurse => fun term2 => let rewritten := (recurse) (term2) in (fun x_ => match x_ with
| Term_Let v_ => (fun (lt : Let) => let forResult := fun hr => ((lists.concat2) ((pairs.second) (hr))) ((lists.pure) ((pairs.first) (hr))) in let body := (fun r_ => (let_body) (r_)) (lt) in let bindings := (fun r_ => (let_bindings) (r_)) (lt) in let flattenedBindings := (lists.concat) (((lists.map) (fun (arg_ : Binding) => (forResult) ((rewriteBinding) (arg_)))) (bindings)) in let merged := ((flattenBodyLet) (flattenedBindings)) (body) in let newBindings := (pairs.first) (merged) in let newBody := (pairs.second) (merged) in (Term_Let) ((Build_Let) (newBindings) (newBody))) (v_)
| _ => rewritten
end) (rewritten) in ((rewriteTerm) (flatten)) (term_).
Definition definitionsWithDependencies (t0 : Type) : t0 -> hydra.graph.Graph -> (list) (Binding) -> (sum) (Error) ((list) (Binding)) :=
  fun (cx : t0) => fun (graph_ : hydra.graph.Graph) => fun (original : (list) (Binding)) => let depNames := fun (el : Binding) => (sets.toList) (((((termDependencyNames) (true)) (false)) (false)) ((fun r_ => (binding_term) (r_)) (el))) in let allDepNames := (lists.nub) (((lists.concat2) (((lists.map) (fun r_ => (binding_name) (r_))) (original))) ((lists.concat) (((lists.map) (depNames)) (original)))) in ((eithers.mapList) (fun (name : Name) => ((requireBinding) (graph_)) (name))) (allDepNames).
