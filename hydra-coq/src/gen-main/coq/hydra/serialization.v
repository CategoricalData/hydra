...
"%string
end) (ws) in let idt := fun (ws : Ws) => fun (s : string) => (fun x_ => match x_ with
...
| Expr_Indent v_ => (fun (indentExpr : IndentedExpression) => let style := (fun r_ => (indentedExpression_style) (r_)) (indentExpr) in let expr := (fun r_ => (indentedExpression_expr) (r_)) (indentExpr) in let lns := (strings.lines) ((printExpr) (expr)) in let ilns := (fun x_ => match x_ with
...
"%string)) (ilns)) (v_)
| Expr_Seq v_ => (fun (seqExpr : SeqExpr) => let sop := (fun r_ => (seqExpr_op) (r_)) (seqExpr) in let spadding := (fun r_ => (op_padding) (r_)) (sop) in let spadl := (fun r_ => (padding_left) (r_)) (spadding) in let spadr := (fun r_ => (padding_right) (r_)) (spadding) in let ssym := (fun w_ => w_) ((fun r_ => (op_symbol) (r_)) (sop)) in let separator := ((strings.cat2) (((strings.cat2) ((pad) (spadl))) (ssym))) ((pad) (spadr)) in let selements := (fun r_ => (seqExpr_elements) (r_)) (seqExpr) in let printedElements := ((lists.map) (fun (el : Expr) => ((idt) (spadr)) ((printExpr) (el)))) (selements) in ((strings.intercalate) (separator)) (printedElements)) (v_)
| Expr_Op v_ => (fun (opExpr : OpExpr) => let r := (fun r_ => (opExpr_rhs) (r_)) (opExpr) in let op := (fun r_ => (opExpr_op) (r_)) (opExpr) in let padding := (fun r_ => (op_padding) (r_)) (op) in let padl := (fun r_ => (padding_left) (r_)) (padding) in let padr := (fun r_ => (padding_right) (r_)) (padding) in let rhs := ((idt) (padr)) ((printExpr) (r)) in let sym := (fun w_ => w_) ((fun r_ => (op_symbol) (r_)) (op)) in let l := (fun r_ => (opExpr_lhs) (r_)) (opExpr) in let lhs := ((idt) (padl)) ((printExpr) (l)) in ((strings.cat2) (((strings.cat2) (((strings.cat2) (((strings.cat2) (lhs)) ((pad) (padl)))) (sym))) ((pad) (padr)))) (rhs)) (v_)
| Expr_Brackets v_ => (fun (bracketExpr : BracketExpr) => let style := (fun r_ => (bracketExpr_style) (r_)) (bracketExpr) in let nlBefore := (fun r_ => (blockStyle_newlineBeforeContent) (r_)) (style) in let pre := (((logic.ifElse) (nlBefore)) ("
"%string)) (""%string) in let nlAfter := (fun r_ => (blockStyle_newlineAfterContent) (r_)) (style) in let suf := (((logic.ifElse) (nlAfter)) ("
...
end) (ws) in let symbolLength := fun (s : Symbol) => (strings.length) ((fun w_ => w_) (s)) in let opLength := fun (op : Op) => let symLen := (symbolLength) ((fun r_ => (op_symbol) (r_)) (op)) in let padding := (fun r_ => (op_padding) (r_)) (op) in let rightLen := (wsLength) ((fun r_ => (padding_right) (r_)) (padding)) in let leftLen := (wsLength) ((fun r_ => (padding_left) (r_)) (padding)) in ((math.add) (symLen)) (((math.add) (leftLen)) (rightLen)) in let seqExprLength := fun (se : SeqExpr) => let sopLen := (opLength) ((fun r_ => (seqExpr_op) (r_)) (se)) in let numSeps := ((math.sub) ((lists.length) ((fun r_ => (seqExpr_elements) (r_)) (se)))) ((1)%Z) in let elementLens := ((lists.map) (expressionLength)) ((fun r_ => (seqExpr_elements) (r_)) (se)) in let totalElLen := (((lists.foldl) (math.add)) ((0)%Z)) (elementLens) in ((math.add) (totalElLen)) (((math.mul) (sopLen)) ((((logic.ifElse) (((equality.gt) (numSeps)) ((0)%Z))) (numSeps)) ((0)%Z))) in let opExprLength := fun (oe : OpExpr) => let rightLen := (expressionLength) ((fun r_ => (opExpr_rhs) (r_)) (oe)) in let opLen := (opLength) ((fun r_ => (opExpr_op) (r_)) (oe)) in let leftLen := (expressionLength) ((fun r_ => (opExpr_lhs) (r_)) (oe)) in ((math.add) (opLen)) (((math.add) (leftLen)) (rightLen)) in let indentedExpressionLength := fun (ie : IndentedExpression) => let indentLen := (fun x_ => match x_ with
| IndentStyle_AllLines v_ => (fun (s : string) => (strings.length) (s)) (v_)
| IndentStyle_SubsequentLines v_ => (fun (s : string) => (strings.length) (s)) (v_)
end) ((fun r_ => (indentedExpression_style) (r_)) (ie)) in let baseLen := (expressionLength) ((fun r_ => (indentedExpression_expr) (r_)) (ie)) in ((math.add) (baseLen)) (indentLen) in let bracketsLength := fun (brackets : Brackets) => ((math.add) ((symbolLength) ((fun r_ => (brackets_open) (r_)) (brackets)))) ((symbolLength) ((fun r_ => (brackets_close) (r_)) (brackets))) in let blockStyleLength := fun (style : BlockStyle) => let nlBeforeLen := (((logic.ifElse) ((fun r_ => (blockStyle_newlineBeforeContent) (r_)) (style))) ((1)%Z)) ((0)%Z) in let nlAfterLen := (((logic.ifElse) ((fun r_ => (blockStyle_newlineAfterContent) (r_)) (style))) ((1)%Z)) ((0)%Z) in let mindentLen := (((maybes.maybe) ((0)%Z)) (strings.length)) ((fun r_ => (blockStyle_indent) (r_)) (style)) in ((math.add) (mindentLen)) (((math.add) (nlBeforeLen)) (nlAfterLen)) in let bracketExprLength := fun (be : BracketExpr) => ((math.add) ((bracketsLength) ((fun r_ => (bracketExpr_brackets) (r_)) (be)))) (((math.add) ((expressionLength) ((fun r_ => (bracketExpr_enclosed) (r_)) (be)))) ((blockStyleLength) ((fun r_ => (bracketExpr_style) (r_)) (be)))) in (fun x_ => match x_ with
...
| _ => true
end) (a) in let assocLeft := fun (a : Associativity) => (fun x_ => match x_ with
...
| Expr_Op v_ => (fun (opExpr : OpExpr) => let rhs := (fun r_ => (opExpr_rhs) (r_)) (opExpr) in let rhs' := (parenthesize) (rhs) in let op := (fun r_ => (opExpr_op) (r_)) (opExpr) in let prec := (fun w_ => w_) ((fun r_ => (op_precedence) (r_)) (op)) in let lhs := (fun r_ => (opExpr_lhs) (r_)) (opExpr) in let lhs' := (parenthesize) (lhs) in let assoc := (fun r_ => (op_associativity) (r_)) (op) in let lhs2 := (fun x_ => match x_ with
| Expr_Op v_ => (fun (lopExpr : OpExpr) => let lop := (fun r_ => (opExpr_op) (r_)) (lopExpr) in let lprec := (fun w_ => w_) ((fun r_ => (op_precedence) (r_)) (lop)) in let lassoc := (fun r_ => (op_associativity) (r_)) (lop) in let comparison := ((equality.compare) (prec)) (lprec) in (fun x_ => match x_ with
...
| _ => lhs'
end) (lhs') in let rhs2 := (fun x_ => match x_ with
| Expr_Op v_ => (fun (ropExpr : OpExpr) => let rop := (fun r_ => (opExpr_op) (r_)) (ropExpr) in let rprec := (fun w_ => w_) ((fun r_ => (op_precedence) (r_)) (rop)) in let rassoc := (fun r_ => (op_associativity) (r_)) (rop) in let comparison := ((equality.compare) (prec)) (rprec) in (fun x_ => match x_ with
