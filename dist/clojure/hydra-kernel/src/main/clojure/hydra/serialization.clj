...
(def hydra_serialization_sep (fn [op] (fn [els] (((hydra_lib_maybes_maybe (fn [] (hydra_serialization_cst ""))) (fn [h] (((hydra_lib_lists_foldl (fn [acc] (fn [el] (((hydra_serialization_ifx op) acc) el)))) h) ((hydra_lib_lists_drop 1) els)))) (hydra_lib_lists_safe_head els)))))
...
(def hydra_serialization_custom_indent_block (fn [idt] (fn [els] (let [idt_op (->hydra_ast_op (hydra_serialization_sym "") (->hydra_ast_padding (list :space nil) (list :break_and_indent idt)) 0 (list :none nil))] (((hydra_lib_maybes_maybe (fn [] (hydra_serialization_cst ""))) (fn [head] (if ((hydra_lib_equality_equal (hydra_lib_lists_length els)) 1) head (((hydra_serialization_ifx idt_op) head) (hydra_serialization_newline_sep ((hydra_lib_lists_drop 1) els)))))) (hydra_lib_lists_safe_head els))))))
