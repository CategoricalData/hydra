(ns hydra.serialization
  (:require [hydra.ast :refer :all] [hydra.lib.equality :refer :all] [hydra.lib.lists :refer :all] [hydra.lib.literals :refer :all] [hydra.lib.logic :refer :all] [hydra.lib.math :refer :all] [hydra.lib.maybes :refer :all] [hydra.lib.pairs :refer :all] [hydra.lib.strings :refer :all] [hydra.util :refer :all]
))
...
(def hydra_serialization_symbol_sep (fn [symb] (fn [style] (fn [l] (let [break_count (hydra_lib_lists_length ((hydra_lib_lists_filter (fn [x_] x_)) (list ((fn [v] (:newline_before_content v)) style) ((fn [v] (:newline_after_content v)) style))))] (let [break (if ((hydra_lib_equality_equal break_count) 0) (list :space nil) (if ((hydra_lib_equality_equal break_count) 1) (list :break nil) (list :double_break nil)))] (let [comma_op (->hydra_ast_op (hydra_serialization_sym symb) (->hydra_ast_padding (list :none nil) break) 0 (list :none nil))] (((hydra_lib_maybes_maybe (fn [] (hydra_serialization_cst ""))) (fn [h] (((hydra_lib_lists_foldl (fn [acc] (fn [el] (((hydra_serialization_ifx comma_op) acc) el)))) h) ((hydra_lib_lists_drop 1) l)))) (hydra_lib_lists_maybe_head l)))))))))
...
(def hydra_serialization_curly_block (fn [style] (fn [e] (((hydra_serialization_curly_braces_list (list :nothing)) style) (list e)))))
...
(def hydra_serialization_sep (fn [op] (fn [els] (((hydra_lib_maybes_maybe (fn [] (hydra_serialization_cst ""))) (fn [h] (((hydra_lib_lists_foldl (fn [acc] (fn [el] (((hydra_serialization_ifx op) acc) el)))) h) ((hydra_lib_lists_drop 1) els)))) (hydra_lib_lists_maybe_head els)))))
...
(def hydra_serialization_custom_indent_block (fn [idt] (fn [els] (let [idt_op (->hydra_ast_op (hydra_serialization_sym "") (->hydra_ast_padding (list :space nil) (list :break_and_indent idt)) 0 (list :none nil))] (((hydra_lib_maybes_maybe (fn [] (hydra_serialization_cst ""))) (fn [head] (if ((hydra_lib_equality_equal (hydra_lib_lists_length els)) 1) head (((hydra_serialization_ifx idt_op) head) (hydra_serialization_newline_sep ((hydra_lib_lists_drop 1) els)))))) (hydra_lib_lists_maybe_head els))))))
...
(def hydra_serialization_full_block_style (->hydra_ast_block_style (list :just hydra_serialization_double_space) true true))
...
(def hydra_serialization_or_sep (fn [style] (fn [l] (let [newlines ((fn [v] (:newline_before_content v)) style)] (((hydra_lib_maybes_maybe (fn [] (hydra_serialization_cst ""))) (fn [h] (((hydra_lib_lists_foldl (fn [acc] (fn [el] (((hydra_serialization_ifx (hydra_serialization_or_op newlines)) acc) el)))) h) ((hydra_lib_lists_drop 1) l)))) (hydra_lib_lists_maybe_head l))))))
