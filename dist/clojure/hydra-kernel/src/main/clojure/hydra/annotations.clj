...
(def hydra_annotations_comments_from_field_type (fn [cx] (fn [g] (fn [ft] (((hydra_annotations_get_type_description cx) g) ((fn [v] (:type v)) ft))))))
...
(def hydra_annotations_debug_if (fn [cx] (fn [debug_id] (fn [message] ((hydra_lib_eithers_bind (hydra_annotations_get_debug_id cx)) (fn [mid] (if ((hydra_lib_equality_equal mid) (list :just debug_id)) (list :left (list :other message)) (list :right nil))))))))
...
(def hydra_annotations_has_flag (fn [cx] (fn [flag] (let [term (((hydra_annotations_get_attr_with_default flag) (list :literal (list :boolean false))) cx)] ((hydra_extract_core_boolean (->hydra_graph_graph hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_sets_empty hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_maps_empty hydra_lib_sets_empty)) term)))))

(def hydra_annotations_fail_on_flag (fn [cx] (fn [flag] (fn [msg] ((hydra_lib_eithers_bind ((hydra_annotations_has_flag cx) flag)) (fn [val] (if val (list :left (list :other msg)) (list :right nil))))))))
...
(def hydra_annotations_get_type_classes (fn [cx] (fn [graph] (fn [term] (let [decode_class (fn [term2] (let [by_name (hydra_lib_maps_from_list (list (list "equality" (list :equality nil)) (list "ordering" (list :ordering nil))))] ((hydra_lib_eithers_bind (((hydra_extract_core_unit_variant "hydra.classes.TypeClass") graph) term2)) (fn [fn_] (((hydra_lib_maybes_maybe (fn [] (list :left (list :extraction (list :unexpected_shape (->hydra_errors_unexpected_shape_error "type class" (hydra_show_core_term term2))))))) (fn [x] (list :right x))) ((hydra_lib_maps_lookup fn_) by_name))))))] (((hydra_lib_maybes_maybe (fn [] (list :right hydra_lib_maps_empty))) (fn [term2] ((((hydra_extract_core_map (fn [t_] (((hydra_lib_eithers_bimap (fn [de] (list :decoding de))) (fn [x] x)) ((hydra_decode_core_name graph) t_)))) ((hydra_extract_core_set_of decode_class) graph)) graph) term2))) ((hydra_annotations_get_term_annotation hydra_constants_key_classes) term)))))))
...
(def hydra_annotations_normalize_term_annotations (fn [term] (let [anns (hydra_annotations_term_annotation_internal term)] (let [stripped (hydra_strip_deannotate_term term)] (if (hydra_lib_maps_null anns) stripped (list :annotated (->hydra_core_annotated_term stripped anns)))))))

(def hydra_annotations_normalize_type_annotations (fn [typ] (let [anns (hydra_annotations_type_annotation_internal typ)] (let [stripped (hydra_strip_deannotate_type typ)] (if (hydra_lib_maps_null anns) stripped (list :annotated (->hydra_core_annotated_type stripped anns)))))))
...
(def hydra_annotations_set_term_annotation (fn [key] (fn [val] (fn [term] (let [term_ (hydra_strip_deannotate_term term)] (let [anns (((hydra_annotations_set_annotation key) val) (hydra_annotations_term_annotation_internal term))] (if (hydra_lib_maps_null anns) term_ (list :annotated (->hydra_core_annotated_term term_ anns)))))))))
