(ns hydra.names
  (:require [hydra.annotations :refer :all] [hydra.constants :refer :all] [hydra.core :refer :all] [hydra.formatting :refer :all] [hydra.lib.lists :refer :all] [hydra.lib.literals :refer :all] [hydra.lib.logic :refer :all] [hydra.lib.maps :refer :all] [hydra.lib.math :refer :all] [hydra.lib.maybes :refer :all] [hydra.lib.pairs :refer :all] [hydra.lib.sets :refer :all] [hydra.lib.strings :refer :all] [hydra.packaging :refer :all] [hydra.util :refer :all]
))
...
(def hydra_names_qualify_name (fn [name] (let [parts (hydra_lib_lists_reverse ((hydra_lib_strings_split_on ".") ((fn [v] v) name)))] (((hydra_lib_maybes_maybe (fn [] (->hydra_packaging_qualified_name (list :nothing) ((fn [v] v) name)))) (fn [uc] (let [local_name (hydra_lib_pairs_first uc)] (let [rest_reversed (hydra_lib_pairs_second uc)] (if (hydra_lib_lists_null rest_reversed) (->hydra_packaging_qualified_name (list :nothing) ((fn [v] v) name)) (->hydra_packaging_qualified_name (list :just ((hydra_lib_strings_intercalate ".") (hydra_lib_lists_reverse rest_reversed))) local_name)))))) (hydra_lib_lists_uncons parts)))))
