...
(def hydra_encode_parsing_parse_result (fn [a] (fn [match_target] ((fn [match_value] (cond (= (first match_target) :success) ((fn [y] (list :inject (->hydra_core_injection "hydra.parsing.ParseResult" (->hydra_core_field "success" ((hydra_encode_parsing_parse_success a) y))))) match_value) (= (first match_target) :failure) ((fn [y] (list :inject (->hydra_core_injection "hydra.parsing.ParseResult" (->hydra_core_field "failure" (hydra_encode_parsing_parse_error y))))) match_value))) (second match_target)))))
