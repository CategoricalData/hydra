...
def fold_accum_result(f: Callable[[T0, T1], Either[T2, tuple[T3, T0]]], cx: T0, xs: frozenlist[T1]) -> Either[T2, tuple[frozenlist[T3], T0]]:
    ...
    return hydra.lib.maybes.maybe((lambda : Right(((), cx))), (lambda p: hydra.lib.eithers.bind(f(cx, hydra.lib.pairs.first(p)), (lambda _r: hydra.lib.eithers.map((lambda _rest: (hydra.lib.lists.cons(hydra.lib.pairs.first(_r), hydra.lib.pairs.first(_rest)), hydra.lib.pairs.second(_rest))), fold_accum_result(f, hydra.lib.pairs.second(_r), hydra.lib.pairs.second(p)))))), hydra.lib.lists.uncons(xs))
...
def encode_term(subject: hydra.rdf.syntax.Resource, term: hydra.core.Term, cx: hydra.context.Context, g: hydra.graph.Graph) -> Either[hydra.errors.Error, tuple[frozenlist[hydra.rdf.syntax.Description], hydra.context.Context]]:
    ...
    match term:
        ...
        case hydra.core.TermWrap(value=wt):
            return hydra.lib.eithers.map((lambda _dr: (descs := hydra.lib.pairs.first(_dr), cx1 := hydra.lib.pairs.second(_dr), (hydra.lib.maybes.from_maybe((lambda : descs), hydra.lib.maybes.map((lambda p: hydra.lib.lists.cons(with_type(wt.type_name, hydra.lib.pairs.first(p)), hydra.lib.pairs.second(p))), hydra.lib.lists.uncons(descs))), cx1))[2]), encode_term(subject, wt.body, cx, g))
