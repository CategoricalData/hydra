...
r"""Language constraints for JSON Schema."""
...
@lru_cache(1)
def json_schema_language() -> hydra.coders.Language:
    r"""Language constraints for JSON Schema."""
    ...
    def type_predicate(_: T0) -> bool:
        return True
    return hydra.coders.Language(hydra.coders.LanguageName("hydra.json.schema"), hydra.coders.LanguageConstraints(elimination_variants(), literal_variants(), float_types(), function_variants(), integer_types(), term_variants(), type_variants(), (lambda x1: type_predicate(x1))))
