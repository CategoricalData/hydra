...
import hydra.core
import hydra.haskell.operators
...
def write_qualified_name(qname: hydra.haskell.syntax.QualifiedName) -> str:
    r"""Write a qualified name as a string."""

    qualifiers = qname.qualifiers
    unqual = qname.unqualified
    def h(name_part: hydra.haskell.syntax.NamePart) -> str:
...
def name_to_expr(name: hydra.haskell.syntax.Name):
    def _hoist_hydra_haskell_serde_name_to_expr_1(v1):
        match v1:
            case hydra.haskell.syntax.NameImplicit(value=qn):
                return hydra.lib.strings.cat2("?", write_qualified_name(qn))

            case hydra.haskell.syntax.NameNormal(value=qn):
                return write_qualified_name(qn)

            case hydra.haskell.syntax.NameParens(value=qn):
                return hydra.lib.strings.cat(("(", write_qualified_name(qn), ")"))

            case _:
                raise AssertionError("Unreachable: all variants handled")
    return hydra.serialization.cst(_hoist_hydra_haskell_serde_name_to_expr_1(name))

def literal_to_expr(lit: hydra.haskell.syntax.Literal):
...
def application_pattern_to_expr(app_pat: hydra.haskell.syntax.ApplicationPattern) -> hydra.ast.Expr:
...
def pattern_to_expr(pat: hydra.haskell.syntax.Pattern) -> hydra.ast.Expr:
    r"""Convert a pattern to an AST expression."""

    match pat:
        case hydra.haskell.syntax.PatternApplication(value=app):
            return application_pattern_to_expr(app)

        case hydra.haskell.syntax.PatternList(value=pats):
        ...
        case hydra.haskell.syntax.PatternLiteral(value=lit):
            return literal_to_expr(lit)

        case hydra.haskell.syntax.PatternName(value=name):
            return name_to_expr(name)

        case hydra.haskell.syntax.PatternParens(value=pat_):
            return hydra.serialization.parenthesize(pattern_to_expr(pat_))

        case hydra.haskell.syntax.PatternTuple(value=pats2):
            return hydra.serialization.paren_list(False, hydra.lib.lists.map((lambda x1: pattern_to_expr(x1)), pats2))

        case hydra.haskell.syntax.PatternWildcard():
            return hydra.serialization.cst("_")

        case _:
            raise TypeError("Unsupported Pattern")

def assertion_to_expr(sert: hydra.haskell.syntax.Assertion) -> hydra.ast.Expr:
    r"""Convert a type class assertion to an AST expression."""

    match sert:
        case hydra.haskell.syntax.AssertionClass(value=cls):
            return class_assertion_to_expr(cls)

        case hydra.haskell.syntax.AssertionTuple(value=serts):
            return hydra.serialization.paren_list(False, hydra.lib.lists.map((lambda x1: assertion_to_expr(x1)), serts))

        case _:
            raise AssertionError("Unreachable: all variants handled")

def class_assertion_to_expr(cls_asrt: hydra.haskell.syntax.ClassAssertion) -> hydra.ast.Expr:
...
def type_to_expr(htype: hydra.haskell.syntax.Type) -> hydra.ast.Expr:
    r"""Convert a Haskell type to an AST expression."""

    match htype:
        case hydra.haskell.syntax.TypeApplication(value=app_type):
            lhs = app_type.context
            rhs = app_type.argument
            return hydra.serialization.ifx(hydra.haskell.operators.app_op, type_to_expr(lhs), type_to_expr(rhs))

        case hydra.haskell.syntax.TypeCtx(value=ctx_type):
            ctx = ctx_type.ctx
            typ = ctx_type.type
            return hydra.serialization.ifx(hydra.haskell.operators.assert_op(), assertion_to_expr(ctx), type_to_expr(typ))

        case hydra.haskell.syntax.TypeFunction(value=fun_type):
            dom = fun_type.domain
            cod = fun_type.codomain
            return hydra.serialization.ifx(hydra.haskell.operators.arrow_op(), type_to_expr(dom), type_to_expr(cod))

        case hydra.haskell.syntax.TypeList(value=htype_):
            return hydra.serialization.bracket_list(hydra.serialization.inline_style, (type_to_expr(htype_),))

        case hydra.haskell.syntax.TypeTuple(value=types):
            return hydra.serialization.paren_list(False, hydra.lib.lists.map((lambda x1: type_to_expr(x1)), types))

        case hydra.haskell.syntax.TypeVariable(value=name):
            return name_to_expr(name)

        case _:
            raise TypeError("Unsupported Type")

def type_signature_to_expr(type_sig: hydra.haskell.syntax.TypeSignature) -> hydra.ast.Expr:
...
def alternative_to_expr(alt: hydra.haskell.syntax.Alternative) -> hydra.ast.Expr:
...
def application_expression_to_expr(app: hydra.haskell.syntax.ApplicationExpression) -> hydra.ast.Expr:
    r"""Convert a function application expression to an AST expression."""

    return hydra.serialization.ifx(hydra.haskell.operators.app_op, expression_to_expr(app.function), expression_to_expr(app.argument))

def case_expression_to_expr(case_expr: hydra.haskell.syntax.CaseExpression) -> hydra.ast.Expr:
...
def case_rhs_to_expr(rhs: hydra.haskell.syntax.CaseRhs) -> hydra.ast.Expr:
    r"""Convert a case right-hand side to an AST expression."""

    return expression_to_expr(rhs.value)

def construct_record_expression_to_expr(construct_record: hydra.haskell.syntax.ConstructRecordExpression) -> hydra.ast.Expr:
    r"""Convert a record construction expression to an AST expression."""

    name = construct_record.name
    updates = construct_record.fields
    def from_update(update: hydra.haskell.syntax.FieldUpdate) -> hydra.ast.Expr:
        fn = update.name
        val = update.value
        return hydra.serialization.ifx(hydra.haskell.operators.define_op(), name_to_expr(fn), expression_to_expr(val))
    @lru_cache(1)
...
def expression_to_expr(expr: hydra.haskell.syntax.Expression) -> hydra.ast.Expr:
    r"""Convert a Haskell expression to an AST expression."""

    match expr:
        case hydra.haskell.syntax.ExpressionApplication(value=app):
            return application_expression_to_expr(app)

        case hydra.haskell.syntax.ExpressionCase(value=cases):
            return case_expression_to_expr(cases)

        case hydra.haskell.syntax.ExpressionConstructRecord(value=r):
            return construct_record_expression_to_expr(r)

        case hydra.haskell.syntax.ExpressionDo(value=statements):
        ...
        case hydra.haskell.syntax.ExpressionIf(value=ifte):
            return if_expression_to_expr(ifte)

        case hydra.haskell.syntax.ExpressionLiteral(value=lit):
            return literal_to_expr(lit)

        case hydra.haskell.syntax.ExpressionLambda(value=lam):
            return hydra.serialization.parenthesize(lambda_expression_to_expr(lam))

        case hydra.haskell.syntax.ExpressionLet(value=let_expr):
            bindings = let_expr.bindings
            inner = let_expr.inner
            def encode_binding(binding: hydra.haskell.syntax.LocalBinding) -> hydra.ast.Expr:
        ...
        case hydra.haskell.syntax.ExpressionList(value=exprs):
        ...
        case hydra.haskell.syntax.ExpressionParens(value=expr_):
            return hydra.serialization.parenthesize(expression_to_expr(expr_))

        case hydra.haskell.syntax.ExpressionTuple(value=exprs2):
            return hydra.serialization.paren_list(False, hydra.lib.lists.map((lambda x1: expression_to_expr(x1)), exprs2))

        case hydra.haskell.syntax.ExpressionVariable(value=name):
            return name_to_expr(name)

        case _:
            raise TypeError("Unsupported Expression")

def if_expression_to_expr(if_expr: hydra.haskell.syntax.IfExpression) -> hydra.ast.Expr:
...
def lambda_expression_to_expr(lambda_expr: hydra.haskell.syntax.LambdaExpression) -> hydra.ast.Expr:
    ...
    return hydra.serialization.ifx(hydra.haskell.operators.lambda_op(), hydra.serialization.prefix("\\", head()), body())

def local_binding_to_expr(binding: hydra.haskell.syntax.LocalBinding) -> hydra.ast.Expr:
    r"""Convert a local binding to an AST expression."""

    match binding:
        case hydra.haskell.syntax.LocalBindingSignature(value=ts):
            return type_signature_to_expr(ts)

        case hydra.haskell.syntax.LocalBindingValue(value=vb):
            return value_binding_to_expr(vb)

        case _:
            raise AssertionError("Unreachable: all variants handled")

def right_hand_side_to_expr(rhs: hydra.haskell.syntax.RightHandSide) -> hydra.ast.Expr:
    r"""Convert a right-hand side to an AST expression."""

    return expression_to_expr(rhs.value)

def statement_to_expr(stmt: hydra.haskell.syntax.Statement) -> hydra.ast.Expr:
    r"""Convert a statement to an AST expression."""

    return expression_to_expr(stmt.value)

def value_binding_to_expr(vb: hydra.haskell.syntax.ValueBinding) -> hydra.ast.Expr:
    r"""Convert a value binding to an AST expression."""

    match vb:
        case hydra.haskell.syntax.ValueBindingSimple(value=simple_v_b):
...
def field_to_expr(field: hydra.haskell.syntax.Field) -> hydra.ast.Expr:
...
def field_with_comments_to_expr(field_with_comments: hydra.haskell.syntax.FieldWithComments) -> hydra.ast.Expr:
...
def constructor_to_expr(cons: hydra.haskell.syntax.Constructor) -> hydra.ast.Expr:
    r"""Convert a data constructor to an AST expression."""

    match cons:
        case hydra.haskell.syntax.ConstructorOrdinary(value=ord):
            name = ord.name
            types = ord.fields
        ...
        case hydra.haskell.syntax.ConstructorRecord(value=rec):
            name = rec.name
            fields = rec.fields
...
def constructor_with_comments_to_expr(cons_with_comments: hydra.haskell.syntax.ConstructorWithComments) -> hydra.ast.Expr:
...
def data_or_newtype_to_expr(kw: hydra.haskell.syntax.DataOrNewtype) -> hydra.ast.Expr:
    r"""Convert a data/newtype keyword to an AST expression."""

    match kw:
        case hydra.haskell.syntax.DataOrNewtype.DATA:
            return hydra.serialization.cst("data")

        case hydra.haskell.syntax.DataOrNewtype.NEWTYPE:
            return hydra.serialization.cst("newtype")

        case _:
            raise AssertionError("Unreachable: all variants handled")

def variable_to_expr(variable: hydra.haskell.syntax.Variable) -> hydra.ast.Expr:
    r"""Convert a type variable to an AST expression."""

    return name_to_expr(variable.value)

def declaration_head_to_expr(hd: hydra.haskell.syntax.DeclarationHead) -> hydra.ast.Expr:
    r"""Convert a declaration head to an AST expression."""

    match hd:
        case hydra.haskell.syntax.DeclarationHeadApplication(value=app_head):
        ...
        case hydra.haskell.syntax.DeclarationHeadSimple(value=name):
            return name_to_expr(name)

        case _:
            raise TypeError("Unsupported DeclarationHead")

def declaration_to_expr(decl: hydra.haskell.syntax.Declaration) -> hydra.ast.Expr:
    r"""Convert a declaration to an AST expression."""

    match decl:
        case hydra.haskell.syntax.DeclarationData(value=data_decl):
            kw = data_decl.keyword
            hd = data_decl.head
            cons = data_decl.constructors
            deriv = data_decl.deriving
            @lru_cache(1)
            def deriv_cat() -> frozenlist[hydra.haskell.syntax.Name]:
        ...
        case hydra.haskell.syntax.DeclarationType(value=type_decl):
            hd = type_decl.name
            typ = type_decl.type
        ...
        case hydra.haskell.syntax.DeclarationValueBinding(value=vb):
            return value_binding_to_expr(vb)

        case hydra.haskell.syntax.DeclarationTypedBinding(value=typed_binding):
...
def declaration_with_comments_to_expr(decl_with_comments: hydra.haskell.syntax.DeclarationWithComments) -> hydra.ast.Expr:
...
def import_export_spec_to_expr(spec: hydra.haskell.syntax.ImportExportSpec) -> hydra.ast.Expr:
    r"""Convert an import/export specification to an AST expression."""

    return name_to_expr(spec.name)

def import_to_expr(import_: hydra.haskell.syntax.Import) -> hydra.ast.Expr:
    ...
    mspec = import_.spec
    name = mod_name.value
    def hiding_sec(spec: hydra.haskell.syntax.SpecImport) -> hydra.ast.Expr:
        match spec:
            case hydra.haskell.syntax.SpecImportHiding(value=names):
...
def module_head_to_expr(module_head: hydra.haskell.syntax.ModuleHead) -> hydra.ast.Expr:
...
def module_to_expr(module: hydra.haskell.syntax.Module) -> hydra.ast.Expr:
