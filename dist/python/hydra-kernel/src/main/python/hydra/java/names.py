...
def java_package_name(parts: frozenlist[str]) -> hydra.java.syntax.PackageName:
    r"""Construct a Java package name from a list of string parts."""

    return hydra.java.syntax.PackageName(hydra.lib.lists.map((lambda p: hydra.java.syntax.Identifier(p)), parts))

@lru_cache(1)
def hydra_core_package_name() -> Maybe[hydra.java.syntax.PackageName]:
    r"""The hydra.core package name."""

    return Just(java_package_name(("hydra", "core")))

@lru_cache(1)
def hydra_util_package_name() -> Maybe[hydra.java.syntax.PackageName]:
...
@lru_cache(1)
def java_lang_package_name() -> Maybe[hydra.java.syntax.PackageName]:
    r"""The java.lang package name."""

    return Just(java_package_name(("java", "lang")))

@lru_cache(1)
def java_util_function_package_name() -> Maybe[hydra.java.syntax.PackageName]:
    r"""The java.util.function package name."""

    return Just(java_package_name(("java", "util", "function")))

@lru_cache(1)
def java_util_package_name() -> Maybe[hydra.java.syntax.PackageName]:
