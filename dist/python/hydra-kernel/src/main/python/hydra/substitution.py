...
def compose_type_subst_list(v1: _):
...
def subst_in_class_constraints(subst: hydra.typing.TypeSubst, constraints: FrozenDict[hydra.core.Name, hydra.core.TypeVariableMetadata]) -> FrozenDict[hydra.core.Name, hydra.core.TypeVariableMetadata]:
    ...
    return hydra.lib.lists.foldl((lambda acc, pair: (var_name := hydra.lib.pairs.first(pair), metadata := hydra.lib.pairs.second(pair), hydra.lib.maybes.maybe((lambda : insert_or_merge(var_name, metadata, acc)), (lambda target_type: (free_vars := hydra.lib.sets.to_list(hydra.variables.free_variables_in_type(target_type)), hydra.lib.lists.foldl((lambda acc2, free_var: insert_or_merge(free_var, metadata, acc2)), acc, free_vars))[1]), hydra.lib.maps.lookup(var_name, subst_map)))[2]), hydra.lib.maps.empty(), hydra.lib.maps.to_list(constraints))
...
def subst_in_context(subst: hydra.typing.TypeSubst, cx: hydra.graph.Graph) -> hydra.graph.Graph:
    ...
    cx2 = hydra.graph.Graph(cx.bound_terms, new_bound_types(), cx.class_constraints, cx.lambda_variables, cx.metadata, cx.primitives, cx.schema_types, cx.type_variables)
    return hydra.graph.Graph(cx2.bound_terms, cx2.bound_types, new_class_constraints(), cx2.lambda_variables, cx2.metadata, cx2.primitives, cx2.schema_types, cx2.type_variables)
