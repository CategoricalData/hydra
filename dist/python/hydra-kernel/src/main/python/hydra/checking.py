...
def all_equal(els: frozenlist[T0]) -> bool:
    return hydra.lib.maybes.maybe((lambda : True), (lambda uc: (h := hydra.lib.pairs.first(uc), t := hydra.lib.pairs.second(uc), hydra.lib.lists.foldl((lambda b, x: hydra.lib.logic.and_(b, hydra.lib.equality.equal(x, h))), True, t))[2]), hydra.lib.lists.uncons(els))

def apply_type_arguments_to_type(cx: T0, tx: hydra.graph.Graph, type_args: frozenlist[hydra.core.Type], t: hydra.core.Type):
    r"""Apply type arguments to a type, substituting forall-bound variables."""

    return hydra.lib.maybes.maybe((lambda : Right(t)), (lambda uc: (ah := hydra.lib.pairs.first(uc), at := hydra.lib.pairs.second(uc), _hoist_at_body_1 := (lambda v1: (lambda ft: (v := ft.parameter, tbody := ft.body, apply_type_arguments_to_type(cx, tx, at, hydra.substitution.subst_in_type(hydra.typing.TypeSubst(hydra.lib.maps.singleton(v, ah)), tbody)))[2])(v1.value) if isinstance(v1, hydra.core.TypeForall) else Left(cast(hydra.errors.Error, hydra.errors.ErrorExtraction(cast(hydra.errors.ExtractionError, hydra.errors.ExtractionErrorUnexpectedShape(hydra.errors.UnexpectedShapeError("forall type", hydra.lib.strings.cat((hydra.show.core.type(t), ". Trying to apply ", hydra.lib.literals.show_int32(hydra.lib.lists.length(type_args)), " type args: ", hydra.formatting.show_list((lambda x1: hydra.show.core.type(x1)), type_args), ". Context has vars: {", hydra.lib.strings.intercalate(", ", hydra.lib.lists.map((lambda v1: v1.value), hydra.lib.maps.keys(tx.bound_types))), "}"))))))))), _hoist_at_body_1(t))[3]), hydra.lib.lists.uncons(type_args))
...
def check_same_type(cx: T0, tx: hydra.graph.Graph, desc: str, types: frozenlist[hydra.core.Type]) -> Either[hydra.errors.Error, hydra.core.Type]:
    ...
    @lru_cache(1)
    def unequal_err() -> Either[hydra.errors.Error, T1]:
        return Left(cast(hydra.errors.Error, hydra.errors.ErrorChecking(cast(hydra.error.checking.CheckingError, hydra.error.checking.CheckingErrorUnequalTypes(hydra.error.checking.UnequalTypesError(types, desc))))))
    return hydra.lib.logic.if_else(types_all_effectively_equal(tx, types), (lambda : hydra.lib.maybes.maybe((lambda : unequal_err()), (lambda t: Right(t)), hydra.lib.lists.maybe_head(types))), (lambda : unequal_err()))
...
def type_of_either(cx: hydra.context.Context, tx: hydra.graph.Graph, type_args: frozenlist[hydra.core.Type], et: Either[hydra.core.Term, hydra.core.Term]) -> Either[hydra.errors.Error, tuple[hydra.core.Type, hydra.context.Context]]:
    ...
    @lru_cache(1)
    def arity_err() -> Either[hydra.errors.Error, T0]:
        return Left(cast(hydra.errors.Error, hydra.errors.ErrorChecking(cast(hydra.error.checking.CheckingError, hydra.error.checking.CheckingErrorTypeArityMismatch(hydra.error.checking.TypeArityMismatchError(cast(hydra.core.Type, hydra.core.TypeEither(hydra.core.EitherType(cast(hydra.core.Type, hydra.core.TypeUnit()), cast(hydra.core.Type, hydra.core.TypeUnit())))), 2, n(), type_args))))))
    return hydra.lib.maybes.maybe((lambda : arity_err()), (lambda uc0: (ta0 := hydra.lib.pairs.first(uc0), hydra.lib.maybes.maybe((lambda : arity_err()), (lambda uc1: (ta1 := hydra.lib.pairs.first(uc1), hydra.lib.logic.if_else(hydra.lib.equality.equal(n(), 2), (lambda : hydra.lib.eithers.either((lambda left_term: hydra.lib.eithers.bind(type_of(cx, tx, (), left_term), (lambda result: (left_type := hydra.lib.pairs.first(result), cx2 := hydra.lib.pairs.second(result), Right((cast(hydra.core.Type, hydra.core.TypeEither(hydra.core.EitherType(left_type, ta1))), cx2)))[2]))), (lambda right_term: hydra.lib.eithers.bind(type_of(cx, tx, (), right_term), (lambda result: (right_type := hydra.lib.pairs.first(result), cx2 := hydra.lib.pairs.second(result), Right((cast(hydra.core.Type, hydra.core.TypeEither(hydra.core.EitherType(ta0, right_type))), cx2)))[2]))), et)), (lambda : arity_err())))[1]), hydra.lib.lists.uncons(hydra.lib.pairs.second(uc0))))[1]), hydra.lib.lists.uncons(type_args))
...
def type_of_maybe(cx: hydra.context.Context, tx: hydra.graph.Graph, type_args: frozenlist[hydra.core.Type], mt: Maybe[hydra.core.Term]) -> Either[hydra.errors.Error, tuple[hydra.core.Type, hydra.context.Context]]:
    ...
    def for_nothing() -> Either[hydra.errors.Error, tuple[hydra.core.Type, hydra.context.Context]]:
        ...
        def n() -> int:
            return hydra.lib.lists.length(type_args)
        @lru_cache(1)
        def maybe_arity_err() -> Either[hydra.errors.Error, T0]:
            return Left(cast(hydra.errors.Error, hydra.errors.ErrorChecking(cast(hydra.error.checking.CheckingError, hydra.error.checking.CheckingErrorTypeArityMismatch(hydra.error.checking.TypeArityMismatchError(cast(hydra.core.Type, hydra.core.TypeMaybe(cast(hydra.core.Type, hydra.core.TypeUnit()))), 1, n(), type_args))))))
        return hydra.lib.logic.if_else(hydra.lib.equality.equal(n(), 1), (lambda : hydra.lib.maybes.maybe((lambda : maybe_arity_err()), (lambda ta0: Right((cast(hydra.core.Type, hydra.core.TypeMaybe(ta0)), cx))), hydra.lib.lists.maybe_head(type_args))), (lambda : maybe_arity_err()))
