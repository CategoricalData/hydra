...
def satisfy(pred: Callable[[int], bool]) -> hydra.parsing.Parser[int]:
    ...
    def parse(input: str) -> hydra.parsing.ParseResult[int]:
        ...
        return hydra.lib.maybes.maybe((lambda : cast(hydra.parsing.ParseResult, hydra.parsing.ParseResultFailure(hydra.parsing.ParseError("unexpected end of input", input)))), (lambda c: (rest := hydra.lib.strings.from_list(hydra.lib.lists.drop(1, codes())), hydra.lib.logic.if_else(pred(c), (lambda : cast(hydra.parsing.ParseResult, hydra.parsing.ParseResultSuccess(hydra.parsing.ParseSuccess(c, rest)))), (lambda : cast(hydra.parsing.ParseResult, hydra.parsing.ParseResultFailure(hydra.parsing.ParseError("character did not satisfy predicate", input))))))[1]), hydra.lib.lists.maybe_head(codes()))
    return hydra.parsing.Parser((lambda x1: parse(x1)))
