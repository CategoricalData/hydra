...
def type_dependencies(cx: hydra.context.Context, graph: hydra.graph.Graph, with_schema: bool, transform: Callable[[hydra.core.Type], hydra.core.Type], name: hydra.core.Name) -> Either[hydra.errors.Error, FrozenDict[hydra.core.Name, hydra.core.Type]]:
    r"""Get all type dependencies for a given type name (Either version)."""

    def require_type(name2: hydra.core.Name) -> Either[hydra.errors.Error, hydra.core.Type]:
        @lru_cache(1)
        ...
        return hydra.lib.eithers.bind(hydra.lexical.require_binding(graph, name2), (lambda el: hydra.lib.eithers.bimap((lambda _e: cast(hydra.errors.Error, hydra.errors.ErrorDecoding(_e))), (lambda _a: _a), hydra.decode.core.type(graph, el.term))))
    def to_pair(name2: hydra.core.Name) -> Either[hydra.errors.Error, tuple[hydra.core.Name, hydra.core.Type]]:
        return hydra.lib.eithers.map((lambda typ: (name2, transform(typ))), require_type(name2))
    def deps(seeds: frozenset[hydra.core.Name], names: FrozenDict[hydra.core.Name, hydra.core.Type]) -> Either[hydra.errors.Error, FrozenDict[hydra.core.Name, hydra.core.Type]]:
...
def is_serializable(cx: hydra.context.Context, graph: hydra.graph.Graph, el: hydra.core.Binding) -> Either[hydra.errors.Error, bool]:
...
def is_serializable_by_name(cx: hydra.context.Context, graph: hydra.graph.Graph, name: hydra.core.Name) -> Either[hydra.errors.Error, bool]:
