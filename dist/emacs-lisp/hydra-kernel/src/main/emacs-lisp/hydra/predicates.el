...
(defvar hydra_predicates_is_complex_variable (lambda (tc) (lambda (name) (let ((meta_lookup (funcall (hydra_lib_maps_lookup name) (funcall (lambda (v) (hydra_graph_graph-metadata v)) tc)))) (if (hydra_lib_maybes_is_just meta_lookup) t (if (funcall (hydra_lib_sets_member name) (funcall (lambda (v) (hydra_graph_graph-lambda_variables v)) tc)) t (let ((type_lookup (funcall (hydra_lib_maps_lookup name) (funcall (lambda (v) (hydra_graph_graph-bound_types v)) tc)))) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (let ((prim_lookup (funcall (hydra_lib_maps_lookup name) (funcall (lambda (v) (hydra_graph_graph-primitives v)) tc)))) (funcall (funcall (hydra_lib_maybes_maybe (lambda () t)) (lambda (prim) (funcall (hydra_lib_equality_gt (hydra_arity_type_scheme_arity (funcall (lambda (v) (hydra_graph_primitive-type v)) prim))) 0))) prim_lookup)))) (lambda (ts) (funcall (hydra_lib_equality_gt (hydra_arity_type_scheme_arity ts)) 0))) type_lookup))))))))
