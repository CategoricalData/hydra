...
(defvar hydra_ext_python_names_encode_name_qualified (lambda (env) (lambda (name) (let* ((bound_vars (hydra_lib_pairs_second (funcall (lambda (v) (hydra_ext_python_environment_python_environment-bound_type_variables v)) env))) (namespaces (funcall (lambda (v) (hydra_ext_python_environment_python_environment-namespaces v)) env)) (focus_pair (funcall (lambda (v) (hydra_packaging_namespaces-focus v)) namespaces)) (focus_ns (hydra_lib_pairs_first focus_pair)) (qual_name (hydra_names_qualify_name name)) (local (funcall (lambda (v) (hydra_packaging_qualified_name-local v)) qual_name)) (mns (funcall (lambda (v) (hydra_packaging_qualified_name-namespace v)) qual_name))) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (if (funcall (hydra_lib_equality_equal mns) (list :just focus_ns)) (if hydra_ext_python_names_use_future_annotations local (funcall (hydra_ext_python_serde_escape_python_string t) local)) (funcall (hydra_lib_strings_intercalate ".") (funcall (hydra_lib_lists_map hydra_ext_python_names_sanitize_python_name) (funcall (hydra_lib_strings_split_on ".") (funcall (lambda (v) v) name))))))) (lambda (n) n)) (funcall (hydra_lib_maps_lookup name) bound_vars))))))
