...
(defvar hydra_parsers_satisfy (lambda (pred) (let ((parse (lambda (input) (let ((codes (hydra_lib_strings_to_list input))) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :failure (make-hydra_parsing_parse_error "unexpected end of input" input)))) (lambda (c) (let ((rest (hydra_lib_strings_from_list (funcall (hydra_lib_lists_drop 1) codes)))) (if (pred c) (list :success (make-hydra_parsing_parse_success c rest)) (list :failure (make-hydra_parsing_parse_error "character did not satisfy predicate" input)))))) (hydra_lib_lists_safe_head codes)))))) parse)))
