...
(defvar hydra_lexical_build_graph (lambda (elements) (lambda (environment) (lambda (primitives) (let ((element_terms (hydra_lib_maps_from_list (funcall (hydra_lib_lists_map (lambda (b) (list (funcall (lambda (v) (hydra_core_binding-name v)) b) (funcall (lambda (v) (hydra_core_binding-term v)) b)))) elements)))) (let ((let_terms (hydra_lib_maps_from_list (funcall (hydra_lib_maybes_map_maybe (lambda (kv) (funcall (hydra_lib_maybes_map (lambda (t_) (list (hydra_lib_pairs_first kv) t_))) (hydra_lib_pairs_second kv)))) (hydra_lib_maps_to_list environment))))) (let ((merged_terms (funcall (hydra_lib_maps_union element_terms) let_terms))) (let ((filtered_terms (funcall (hydra_lib_maps_filter_with_key (lambda (k) (lambda (_v) (hydra_lib_logic_not (funcall (hydra_lib_maps_member k) primitives))))) merged_terms))) (let ((element_types (hydra_lib_maps_from_list (hydra_lib_maybes_cat (funcall (hydra_lib_lists_map (lambda (b) (funcall (hydra_lib_maybes_map (lambda (ts) (list (funcall (lambda (v) (hydra_core_binding-name v)) b) ts))) (funcall (lambda (v) (hydra_core_binding-type v)) b)))) elements))))) (let ((filtered_types (funcall (hydra_lib_maps_filter_with_key (lambda (k) (lambda (_v) (hydra_lib_logic_not (funcall (hydra_lib_maps_member k) primitives))))) element_types))) (make-hydra_graph_graph filtered_terms filtered_types hydra_lib_maps_empty (hydra_lib_sets_from_list (hydra_lib_maps_keys (funcall (hydra_lib_maps_filter (lambda (mt) (hydra_lib_maybes_is_nothing mt))) environment))) hydra_lib_maps_empty primitives hydra_lib_maps_empty hydra_lib_sets_empty)))))))))))
...
(defvar hydra_lexical_dereference_variable (lambda (graph) (lambda (name) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (list :resolution (list :no_such_binding (make-hydra_errors_no_such_binding_error name)))))) (lambda (right_) (list :right right_))) (funcall (hydra_lexical_lookup_binding graph) name)))))
...
(defvar hydra_lexical_get_field (lambda (m) (lambda (fname) (lambda (decode) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (list :resolution (list :no_matching_field (make-hydra_errors_no_matching_field_error fname)))))) decode) (funcall (hydra_lib_maps_lookup fname) m))))))
...
(defvar hydra_lexical_graph_with_primitives (lambda (built_in) (lambda (user_provided) (let ((to_map (lambda (ps) (hydra_lib_maps_from_list (funcall (hydra_lib_lists_map (lambda (p) (list (funcall (lambda (v) (hydra_graph_primitive-name v)) p) p))) ps))))) (let ((prims (funcall (hydra_lib_maps_union (to_map user_provided)) (to_map built_in)))) (funcall (funcall (hydra_lexical_build_graph (list)) hydra_lib_maps_empty) prims))))))
...
(defvar hydra_lexical_require_binding (lambda (graph) (lambda (name) (let ((show_all nil)) (let ((ellipsis (lambda (strings) (if (funcall (hydra_lib_logic_and (funcall (hydra_lib_equality_gt (hydra_lib_lists_length strings)) 3)) (hydra_lib_logic_not show_all)) (funcall (hydra_lib_lists_concat2 (funcall (hydra_lib_lists_take 3) strings)) (list "...")) strings)))) (let ((err_msg (funcall (hydra_lib_strings_cat2 (funcall (hydra_lib_strings_cat2 (funcall (hydra_lib_strings_cat2 (funcall (hydra_lib_strings_cat2 "no such element: ") (funcall (lambda (v) v) name))) ". Available elements: {")) (funcall (hydra_lib_strings_intercalate ", ") (ellipsis (funcall (hydra_lib_lists_map (lambda (v) v)) (hydra_lib_maps_keys (funcall (lambda (v) (hydra_graph_graph-bound_terms v)) graph))))))) "}"))) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (list :resolution (list :other err_msg))))) (lambda (x) (list :right x))) (funcall (hydra_lexical_lookup_binding graph) name))))))))

(defvar hydra_lexical_match_union (lambda (graph) (lambda (tname) (lambda (pairs) (lambda (term) (let ((stripped (hydra_strip_deannotate_and_detype_term term))) (let ((mapping (hydra_lib_maps_from_list pairs))) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :variable) (funcall (lambda (name) (funcall (hydra_lib_eithers_bind (funcall (hydra_lexical_require_binding graph) name)) (lambda (el) (funcall (funcall (funcall (hydra_lexical_match_union graph) tname) pairs) (funcall (lambda (v) (hydra_core_binding-term v)) el))))) match_value)) ((equal (car match_target) :inject) (funcall (lambda (injection) (let ((exp (let ((fname (funcall (lambda (v) (hydra_core_field-name v)) (funcall (lambda (v) (hydra_core_injection-field v)) injection)))) (let ((val (funcall (lambda (v) (hydra_core_field-term v)) (funcall (lambda (v) (hydra_core_injection-field v)) injection)))) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (list :resolution (list :no_matching_field (make-hydra_errors_no_matching_field_error fname)))))) (lambda (f) (f val))) (funcall (hydra_lib_maps_lookup fname) mapping)))))) (if (funcall (hydra_lib_equality_equal (funcall (lambda (v) v) (funcall (lambda (v) (hydra_core_injection-type_name v)) injection))) (funcall (lambda (v) v) tname)) exp (list :left (list :resolution (list :unexpected_shape (make-hydra_errors_unexpected_shape_error (funcall (hydra_lib_strings_cat2 "injection for type ") (funcall (lambda (v) v) tname)) (hydra_show_core_term term)))))))) match_value)) (t (list :left (list :resolution (list :unexpected_shape (make-hydra_errors_unexpected_shape_error (funcall (hydra_lib_strings_cat2 "injection for type ") (funcall (lambda (v) v) tname)) (hydra_show_core_term stripped)))))))) (cadr match_target))) stripped))))))))

(defvar hydra_lexical_match_unit_field (lambda (fname) (lambda (x) (list fname (lambda (ignored) (list :right x))))))

(defvar hydra_lexical_match_enum (lambda (graph) (lambda (tname) (lambda (pairs) (funcall (funcall (hydra_lexical_match_union graph) tname) (funcall (hydra_lib_lists_map (lambda (pair) (funcall (hydra_lexical_match_unit_field (hydra_lib_pairs_first pair)) (hydra_lib_pairs_second pair)))) pairs))))))

(defvar hydra_lexical_match_record (lambda (graph) (lambda (decode) (lambda (term) (let ((stripped (hydra_strip_deannotate_and_detype_term term))) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :record) (funcall (lambda (record) (decode (hydra_lib_maps_from_list (funcall (hydra_lib_lists_map (lambda (field) (list (funcall (lambda (v) (hydra_core_field-name v)) field) (funcall (lambda (v) (hydra_core_field-term v)) field)))) (funcall (lambda (v) (hydra_core_record-fields v)) record))))) match_value)) (t (list :left (list :resolution (list :unexpected_shape (make-hydra_errors_unexpected_shape_error "record" (hydra_show_core_term term)))))))) (cadr match_target))) stripped))))))

(defvar hydra_lexical_require_primitive (lambda (graph) (lambda (name) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (list :resolution (list :no_such_primitive (make-hydra_errors_no_such_primitive_error name)))))) (lambda (x) (list :right x))) (funcall (hydra_lexical_lookup_primitive graph) name)))))

(defvar hydra_lexical_require_primitive_type (lambda (tx) (lambda (name) (let ((mts (funcall (hydra_lib_maybes_map (lambda (_p) (funcall (lambda (v) (hydra_graph_primitive-type v)) _p))) (funcall (hydra_lib_maps_lookup name) (funcall (lambda (v) (hydra_graph_graph-primitives v)) tx))))) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (list :resolution (list :no_such_primitive (make-hydra_errors_no_such_primitive_error name)))))) (lambda (ts) (list :right ts))) mts)))))
...
(defvar hydra_lexical_require_term (lambda (graph) (lambda (name) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (list :resolution (list :no_such_binding (make-hydra_errors_no_such_binding_error name)))))) (lambda (x) (list :right x))) (funcall (hydra_lexical_resolve_term graph) name)))))

(defvar hydra_lexical_strip_and_dereference_term (lambda (graph) (lambda (term) (let ((stripped (hydra_strip_deannotate_and_detype_term term))) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :variable) (funcall (lambda (v) (funcall (hydra_lib_eithers_bind (funcall (hydra_lexical_require_term graph) v)) (lambda (t_) (funcall (hydra_lexical_strip_and_dereference_term graph) t_)))) match_value)) (t (list :right stripped)))) (cadr match_target))) stripped)))))
