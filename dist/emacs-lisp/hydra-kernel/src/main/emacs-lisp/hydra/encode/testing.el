...
(defvar hydra_encode_testing_test_case (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :universal) (funcall (lambda (y) (list :inject (make-hydra_core_injection "hydra.testing.TestCase" (make-hydra_core_field "universal" (hydra_encode_testing_universal_test_case y))))) match_value)))) (cadr match_target))))
