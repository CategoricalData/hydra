...
(defvar hydra_json_writer_json_string (lambda (s) (let ((hex_escape (lambda (c) (let ((hi (hydra_lib_strings_from_list (hydra_lib_lists_pure (funcall (hydra_lib_strings_char_at (funcall (hydra_lib_math_div c) 16)) "0123456789abcdef"))))) (let ((lo (hydra_lib_strings_from_list (hydra_lib_lists_pure (funcall (hydra_lib_strings_char_at (funcall (hydra_lib_math_mod c) 16)) "0123456789abcdef"))))) (funcall (hydra_lib_strings_cat2 (funcall (hydra_lib_strings_cat2 "\\u00") hi)) lo)))))) (let ((escape (lambda (c) (if (funcall (hydra_lib_equality_equal c) 34) "\\\"" (if (funcall (hydra_lib_equality_equal c) 92) "\\\\" (if (funcall (hydra_lib_equality_equal c) 8) "\\b" (if (funcall (hydra_lib_equality_equal c) 12) "\\f" (if (funcall (hydra_lib_equality_equal c) 10) "\\n" (if (funcall (hydra_lib_equality_equal c) 13) "\\r" (if (funcall (hydra_lib_equality_equal c) 9) "\\t" (if (funcall (hydra_lib_equality_lt c) 32) (hex_escape c) (hydra_lib_strings_from_list (hydra_lib_lists_pure c))))))))))))) (let ((escaped (hydra_lib_strings_cat (funcall (hydra_lib_lists_map escape) (hydra_lib_strings_to_list s))))) (funcall (hydra_lib_strings_cat2 (funcall (hydra_lib_strings_cat2 "\"") escaped)) "\""))))))
