...
(defvar hydra_scala_syntax_tree-variants (list :ref :stat :type :bounds :pat :member :ctor :template :mod :enumerator :importer :importee :case_tree :source :quasi))

(defvar hydra_scala_syntax_ref-variants (list :name :init))

(defvar hydra_scala_syntax_stat-variants (list :term :decl :defn :import_export))

(defvar hydra_scala_syntax_name-variants (list :value :anonymous :indeterminate))

(defvar hydra_scala_syntax_lit-variants (list :null :int :double :float :byte :short :char :long :boolean :unit :string :bytes :symbol))

(defvar hydra_scala_syntax_data-variants (list :lit :ref :interpolate :xml :apply :apply_using :apply_type :assign :return :throw :ascribe :annotate :tuple :block :end_marker :if :quoted_macro_expr :quoted_macro_type :spliced_macro_expr :match :try :try_with_handler :function_data :poly_function :partial_function :while :do :for :for_yield :new :new_anonymous :placeholder :eta :repeated :param))

(defvar hydra_scala_syntax_data_ref-variants (list :this :super :name :anonymous :select :apply_unary))
...
(defvar hydra_scala_syntax_data_function_data-variants (list :context_function :function))
...
(defvar hydra_scala_syntax_type-variants (list :ref :anonymous_name :apply :apply_infix :function_type :poly_function :implicit_function :tuple :with :and :or :refine :existential :annotate :lambda :macro :method :placeholder :by_name :repeated :var :typed_param :match))

(defvar hydra_scala_syntax_type_ref-variants (list :name :select :project :singleton))
...
(defvar hydra_scala_syntax_type_function_type-variants (list :function :context_function))
...
(defvar hydra_scala_syntax_pat-variants (list :var :wildcard :seq_wildcard :bind :alternative :tuple :repeated :extract :extract_infix :interpolate :xml :typed :macro :given))
...
(defvar hydra_scala_syntax_member-variants (list :term :type :term_param :type_param :self))

(defvar hydra_scala_syntax_member_data-variants (list :pkg :object))

(cl-defstruct hydra_scala_syntax_member_type name)

(defvar hydra_scala_syntax_decl-variants (list :val :var :def :type :given))
...
(defvar hydra_scala_syntax_defn-variants (list :val :var :given :enum :enum_case :repeated_enum_case :given_alias :extension_group :def :macro :type :class :trait :object))
...
(defvar hydra_scala_syntax_ctor-variants (list :primary :secondary))
...
(defvar hydra_scala_syntax_mod-variants (list :annot :private :protected :implicit :final :sealed :open :super :override :case :abstract :covariant :contravariant :lazy :val_param :var_param :infix :inline :using :opaque :transparent))
...
(defvar hydra_scala_syntax_enumerator-variants (list :generator :case_generator :val :guard))
...
(defvar hydra_scala_syntax_import_export_stat-variants (list :import :export))
...
(defvar hydra_scala_syntax_importee-variants (list :wildcard :given :given_all :name :rename :unimport))
...
(defvar hydra_scala_syntax_case_tree-variants (list :case :type_case))
