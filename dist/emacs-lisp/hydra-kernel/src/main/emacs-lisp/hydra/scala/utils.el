...
(defvar hydra_scala_utils_sprim (lambda (name) (let* ((qname (hydra_names_qualify_name name)) (local (hydra_scala_utils_scala_escape_name (funcall (lambda (v) (hydra_packaging_qualified_name-local v)) qname))) (prefix (funcall (lambda (v) v) (hydra_lib_maybes_from_just (funcall (lambda (v) (hydra_packaging_qualified_name-namespace v)) qname))))) (hydra_scala_utils_sname (funcall (hydra_lib_strings_cat2 (funcall (hydra_lib_strings_cat2 prefix) ".")) local)))))
