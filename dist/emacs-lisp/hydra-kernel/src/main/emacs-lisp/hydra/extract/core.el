...
(defvar hydra_extract_core_case_field (lambda (name) (lambda (n) (lambda (graph) (lambda (term) (let ((field_name n)) (funcall (hydra_lib_eithers_bind (funcall (funcall (hydra_extract_core_cases name) graph) term)) (lambda (cs) (let ((matching (funcall (hydra_lib_lists_find (lambda (f) (funcall (hydra_lib_equality_equal (funcall (lambda (v) v) (funcall (lambda (v) (hydra_core_field-name v)) f))) (funcall (lambda (v) v) field_name)))) (funcall (lambda (v) (hydra_core_case_statement-cases v)) cs)))) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (list :extraction (list :unexpected_shape (make-hydra_errors_unexpected_shape_error "matching case" "no matching case")))))) (lambda (mf) (list :right mf))) matching))))))))))

(defvar hydra_extract_core_decimal_literal (lambda (v) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :decimal) (funcall (lambda (d) (list :right d)) match_value)) (t (list :left (list :extraction (list :unexpected_shape (make-hydra_errors_unexpected_shape_error "decimal" (hydra_show_core_literal v)))))))) (cadr match_target))) v)))

(defvar hydra_extract_core_decimal (lambda (graph) (lambda (t_) (funcall (hydra_lib_eithers_bind (funcall (hydra_extract_core_literal graph) t_)) (lambda (l) (hydra_extract_core_decimal_literal l))))))
...
(defvar hydra_extract_core_field (lambda (fname) (lambda (mapping) (lambda (graph) (lambda (fields) (let ((matching_fields (funcall (hydra_lib_lists_filter (lambda (f) (funcall (hydra_lib_equality_equal (funcall (lambda (v) v) (funcall (lambda (v) (hydra_core_field-name v)) f))) (funcall (lambda (v) v) fname)))) fields))) (let ((no_match_err (list :left (list :extraction (list :unexpected_shape (make-hydra_errors_unexpected_shape_error (funcall (hydra_lib_strings_cat2 "field ") (funcall (lambda (v) v) fname)) "no matching field")))))) (if (hydra_lib_lists_null matching_fields) no_match_err (if (funcall (hydra_lib_equality_equal (hydra_lib_lists_length matching_fields)) 1) (funcall (funcall (hydra_lib_maybes_maybe (lambda () no_match_err)) (lambda (mf) (funcall (hydra_lib_eithers_bind (funcall (hydra_lexical_strip_and_dereference_term graph) (funcall (lambda (v) (hydra_core_field-term v)) mf))) (lambda (stripped) (mapping stripped))))) (hydra_lib_lists_maybe_head matching_fields)) (list :left (list :extraction (list :unexpected_shape (make-hydra_errors_unexpected_shape_error "single field" (funcall (hydra_lib_strings_cat2 "multiple fields named ") (funcall (lambda (v) v) fname)))))))))))))))
...
(defvar hydra_extract_core_let_binding (lambda (n) (lambda (graph) (lambda (term) (let ((name n)) (funcall (hydra_lib_eithers_bind (funcall (hydra_extract_core_let graph) term)) (lambda (let_expr) (let ((matching_bindings (funcall (hydra_lib_lists_filter (lambda (b) (funcall (hydra_lib_equality_equal (funcall (lambda (v) v) (funcall (lambda (v) (hydra_core_binding-name v)) b))) (funcall (lambda (v) v) name)))) (funcall (lambda (v) (hydra_core_let-bindings v)) let_expr)))) (let ((no_binding_err (list :left (list :extraction (list :no_such_binding (make-hydra_errors_no_such_binding_error name)))))) (if (hydra_lib_lists_null matching_bindings) no_binding_err (if (funcall (hydra_lib_equality_equal (hydra_lib_lists_length matching_bindings)) 1) (funcall (funcall (hydra_lib_maybes_maybe (lambda () no_binding_err)) (lambda (b) (list :right (funcall (lambda (v) (hydra_core_binding-term v)) b)))) (hydra_lib_lists_maybe_head matching_bindings)) (list :left (list :extraction (list :multiple_bindings (make-hydra_errors_multiple_bindings_error name)))))))))))))))
...
(defvar hydra_extract_core_list_head (lambda (graph) (lambda (term) (funcall (hydra_lib_eithers_bind (funcall (hydra_extract_core_list graph) term)) (lambda (l) (funcall (funcall (hydra_lib_maybes_maybe (lambda () (list :left (list :extraction (list :unexpected_shape (make-hydra_errors_unexpected_shape_error "non-empty list" "empty list")))))) (lambda (h) (list :right h))) (hydra_lib_lists_maybe_head l)))))))
