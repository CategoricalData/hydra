...
(defvar hydra_annotations_set_type_classes (lambda (m) (lambda (term) (let ((encode_class (lambda (tc) (funcall (lambda (match_target) (funcall (lambda (match_value) (cond ((equal (car match_target) :equality) (funcall (lambda (_) (list :inject (make-hydra_core_injection "hydra.classes.TypeClass" (make-hydra_core_field "equality" (list :unit nil))))) match_value)) ((equal (car match_target) :ordering) (funcall (lambda (_) (list :inject (make-hydra_core_injection "hydra.classes.TypeClass" (make-hydra_core_field "ordering" (list :unit nil))))) match_value)))) (cadr match_target))) tc)))) (let ((encode_pair (lambda (name_classes) (let ((name (hydra_lib_pairs_first name_classes))) (let ((classes (hydra_lib_pairs_second name_classes))) (list (hydra_encode_core_name name) (list :set (hydra_lib_sets_from_list (funcall (hydra_lib_lists_map encode_class) (hydra_lib_sets_to_list classes)))))))))) (let ((encoded (if (hydra_lib_maps_null m) (list :nothing) (list :just (list :map (hydra_lib_maps_from_list (funcall (hydra_lib_lists_map encode_pair) (hydra_lib_maps_to_list m)))))))) (funcall (funcall (hydra_annotations_set_term_annotation hydra_constants_key_classes) encoded) term)))))))
