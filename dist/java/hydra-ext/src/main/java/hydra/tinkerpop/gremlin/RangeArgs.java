...
public class RangeArgs implements Serializable, Comparable<RangeArgs> {
  public static final hydra.core.Name TYPE_ = new hydra.core.Name("hydra.tinkerpop.gremlin.RangeArgs");
  ...
  public final hydra.tinkerpop.gremlin.IntegerArgument min;

  public final hydra.tinkerpop.gremlin.IntegerArgument max;

  public RangeArgs (hydra.util.Maybe<hydra.tinkerpop.gremlin.TraversalScopeArgument> scope, hydra.tinkerpop.gremlin.IntegerArgument min, hydra.tinkerpop.gremlin.IntegerArgument max) {
    this.scope = scope;
    this.min = min;
    this.max = max;
  }
  ...
  public RangeArgs withScope(hydra.util.Maybe<hydra.tinkerpop.gremlin.TraversalScopeArgument> scope) {
    return new RangeArgs(scope, min, max);
  }

  public RangeArgs withMin(hydra.tinkerpop.gremlin.IntegerArgument min) {
    return new RangeArgs(scope, min, max);
  }

  public RangeArgs withMax(hydra.tinkerpop.gremlin.IntegerArgument max) {
    return new RangeArgs(scope, min, max);
  }
}
