// Note: this is an automatically generated file. Do not edit.

package hydra.haskell.syntax;

import java.io.Serializable;

/**
 * An 'as' pattern
 */
public class AsPattern implements Serializable, Comparable<AsPattern> {
  public static final hydra.core.Name TYPE_ = new hydra.core.Name("hydra.haskell.syntax.AsPattern");

  public static final hydra.core.Name NAME = new hydra.core.Name("name");

  public static final hydra.core.Name INNER = new hydra.core.Name("inner");

  /**
   * The bound name
   */
  public final hydra.haskell.syntax.Name name;

  /**
   * The inner pattern
   */
  public final hydra.haskell.syntax.Pattern inner;

  public AsPattern (hydra.haskell.syntax.Name name, hydra.haskell.syntax.Pattern inner) {
    this.name = name;
    this.inner = inner;
  }

  @Override
  public boolean equals(Object other) {
    if (!(other instanceof AsPattern)) {
      return false;
    }
    AsPattern o = (AsPattern) other;
    return java.util.Objects.equals(
      this.name,
      o.name) && java.util.Objects.equals(
      this.inner,
      o.inner);
  }

  @Override
  public int hashCode() {
    return 2 * java.util.Objects.hashCode(name) + 3 * java.util.Objects.hashCode(inner);
  }

  @Override
  @SuppressWarnings("unchecked")
  public int compareTo(AsPattern other) {
    int cmp = 0;
    cmp = hydra.util.Comparing.compare(
      name,
      other.name);
    if (cmp != 0) {
      return cmp;
    }
    return hydra.util.Comparing.compare(
      inner,
      other.inner);
  }

  public AsPattern withName(hydra.haskell.syntax.Name name) {
    return new AsPattern(name, inner);
  }

  public AsPattern withInner(hydra.haskell.syntax.Pattern inner) {
    return new AsPattern(name, inner);
  }
}
