// Note: this is an automatically generated file. Do not edit.

package hydra.haskell.syntax;

import java.io.Serializable;

/**
 * A 'let' expression
 */
public class LetExpression implements Serializable, Comparable<LetExpression> {
  public static final hydra.core.Name TYPE_ = new hydra.core.Name("hydra.haskell.syntax.LetExpression");

  public static final hydra.core.Name BINDINGS = new hydra.core.Name("bindings");

  public static final hydra.core.Name INNER = new hydra.core.Name("inner");

  /**
   * The local bindings
   */
  public final java.util.List<hydra.haskell.syntax.LocalBinding> bindings;

  /**
   * The body of the let expression
   */
  public final hydra.haskell.syntax.Expression inner;

  public LetExpression (java.util.List<hydra.haskell.syntax.LocalBinding> bindings, hydra.haskell.syntax.Expression inner) {
    this.bindings = bindings;
    this.inner = inner;
  }

  @Override
  public boolean equals(Object other) {
    if (!(other instanceof LetExpression)) {
      return false;
    }
    LetExpression o = (LetExpression) other;
    return java.util.Objects.equals(
      this.bindings,
      o.bindings) && java.util.Objects.equals(
      this.inner,
      o.inner);
  }

  @Override
  public int hashCode() {
    return 2 * java.util.Objects.hashCode(bindings) + 3 * java.util.Objects.hashCode(inner);
  }

  @Override
  @SuppressWarnings("unchecked")
  public int compareTo(LetExpression other) {
    int cmp = 0;
    cmp = hydra.util.Comparing.compare(
      bindings,
      other.bindings);
    if (cmp != 0) {
      return cmp;
    }
    return hydra.util.Comparing.compare(
      inner,
      other.inner);
  }

  public LetExpression withBindings(java.util.List<hydra.haskell.syntax.LocalBinding> bindings) {
    return new LetExpression(bindings, inner);
  }

  public LetExpression withInner(hydra.haskell.syntax.Expression inner) {
    return new LetExpression(bindings, inner);
  }
}
