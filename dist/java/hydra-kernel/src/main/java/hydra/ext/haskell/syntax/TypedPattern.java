...
public class TypedPattern implements Serializable, Comparable<TypedPattern> {
  public static final hydra.core.Name TYPE_ = new hydra.core.Name("hydra.ext.haskell.syntax.TypedPattern");
   ...
   * The inner pattern
   */
  public final hydra.ext.haskell.syntax.Pattern inner;
  ...
  public TypedPattern (hydra.ext.haskell.syntax.Pattern inner, hydra.ext.haskell.syntax.Type type) {
    this.inner = inner;
    this.type = type;
  }
  ...
  public TypedPattern withType(hydra.ext.haskell.syntax.Type type) {
    return new TypedPattern(inner, type);
  }
}
