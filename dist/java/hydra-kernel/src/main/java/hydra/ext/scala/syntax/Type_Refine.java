...
public class Type_Refine implements Serializable, Comparable<Type_Refine> {
  ...
  public Type_Refine (hydra.util.Maybe<hydra.ext.scala.syntax.Type> tpe, java.util.List<hydra.ext.scala.syntax.Stat> stats) {
    this.tpe = tpe;
    this.stats = stats;
  }
  ...
  public Type_Refine withStats(java.util.List<hydra.ext.scala.syntax.Stat> stats) {
    return new Type_Refine(tpe, stats);
  }
}
