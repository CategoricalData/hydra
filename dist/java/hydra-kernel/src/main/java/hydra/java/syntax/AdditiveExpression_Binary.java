// Note: this is an automatically generated file. Do not edit.

package hydra.java.syntax;

import java.io.Serializable;

public class AdditiveExpression_Binary implements Serializable, Comparable<AdditiveExpression_Binary> {
  public static final hydra.core.Name TYPE_ = new hydra.core.Name("hydra.java.syntax.AdditiveExpression_Binary");

  public static final hydra.core.Name LHS = new hydra.core.Name("lhs");

  public static final hydra.core.Name RHS = new hydra.core.Name("rhs");

  public final hydra.java.syntax.AdditiveExpression lhs;

  public final hydra.java.syntax.MultiplicativeExpression rhs;

  public AdditiveExpression_Binary (hydra.java.syntax.AdditiveExpression lhs, hydra.java.syntax.MultiplicativeExpression rhs) {
    this.lhs = lhs;
    this.rhs = rhs;
  }

  @Override
  public boolean equals(Object other) {
    if (!(other instanceof AdditiveExpression_Binary)) {
      return false;
    }
    AdditiveExpression_Binary o = (AdditiveExpression_Binary) other;
    return java.util.Objects.equals(
      this.lhs,
      o.lhs) && java.util.Objects.equals(
      this.rhs,
      o.rhs);
  }

  @Override
  public int hashCode() {
    return 2 * java.util.Objects.hashCode(lhs) + 3 * java.util.Objects.hashCode(rhs);
  }

  @Override
  @SuppressWarnings("unchecked")
  public int compareTo(AdditiveExpression_Binary other) {
    int cmp = 0;
    cmp = hydra.util.Comparing.compare(
      lhs,
      other.lhs);
    if (cmp != 0) {
      return cmp;
    }
    return hydra.util.Comparing.compare(
      rhs,
      other.rhs);
  }

  public AdditiveExpression_Binary withLhs(hydra.java.syntax.AdditiveExpression lhs) {
    return new AdditiveExpression_Binary(lhs, rhs);
  }

  public AdditiveExpression_Binary withRhs(hydra.java.syntax.MultiplicativeExpression rhs) {
    return new AdditiveExpression_Binary(lhs, rhs);
  }
}
