// Note: this is an automatically generated file. Do not edit.

package hydra.scala.syntax;

import java.io.Serializable;

public class Data_Match implements Serializable, Comparable<Data_Match> {
  public static final hydra.core.Name TYPE_ = new hydra.core.Name("hydra.scala.syntax.Data_Match");

  public static final hydra.core.Name EXPR = new hydra.core.Name("expr");

  public static final hydra.core.Name CASES = new hydra.core.Name("cases");

  public final hydra.scala.syntax.Data expr;

  public final java.util.List<hydra.scala.syntax.Case> cases;

  public Data_Match (hydra.scala.syntax.Data expr, java.util.List<hydra.scala.syntax.Case> cases) {
    this.expr = expr;
    this.cases = cases;
  }

  @Override
  public boolean equals(Object other) {
    if (!(other instanceof Data_Match)) {
      return false;
    }
    Data_Match o = (Data_Match) other;
    return java.util.Objects.equals(
      this.expr,
      o.expr) && java.util.Objects.equals(
      this.cases,
      o.cases);
  }

  @Override
  public int hashCode() {
    return 2 * java.util.Objects.hashCode(expr) + 3 * java.util.Objects.hashCode(cases);
  }

  @Override
  @SuppressWarnings("unchecked")
  public int compareTo(Data_Match other) {
    int cmp = 0;
    cmp = hydra.util.Comparing.compare(
      expr,
      other.expr);
    if (cmp != 0) {
      return cmp;
    }
    return hydra.util.Comparing.compare(
      cases,
      other.cases);
  }

  public Data_Match withExpr(hydra.scala.syntax.Data expr) {
    return new Data_Match(expr, cases);
  }

  public Data_Match withCases(java.util.List<hydra.scala.syntax.Case> cases) {
    return new Data_Match(expr, cases);
  }
}
