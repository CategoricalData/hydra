// Note: this is an automatically generated file. Do not edit.

package hydra.haskell.syntax;

import java.io.Serializable;

/**
 * An application type
 */
public class ApplicationType implements Serializable, Comparable<ApplicationType> {
  public static final hydra.core.Name TYPE_ = new hydra.core.Name("hydra.haskell.syntax.ApplicationType");

  public static final hydra.core.Name CONTEXT = new hydra.core.Name("context");

  public static final hydra.core.Name ARGUMENT = new hydra.core.Name("argument");

  /**
   * The type being applied
   */
  public final hydra.haskell.syntax.Type context;

  /**
   * The type argument
   */
  public final hydra.haskell.syntax.Type argument;

  public ApplicationType (hydra.haskell.syntax.Type context, hydra.haskell.syntax.Type argument) {
    this.context = context;
    this.argument = argument;
  }

  @Override
  public boolean equals(Object other) {
    if (!(other instanceof ApplicationType)) {
      return false;
    }
    ApplicationType o = (ApplicationType) other;
    return java.util.Objects.equals(
      this.context,
      o.context) && java.util.Objects.equals(
      this.argument,
      o.argument);
  }

  @Override
  public int hashCode() {
    return 2 * java.util.Objects.hashCode(context) + 3 * java.util.Objects.hashCode(argument);
  }

  @Override
  @SuppressWarnings("unchecked")
  public int compareTo(ApplicationType other) {
    int cmp = 0;
    cmp = hydra.util.Comparing.compare(
      context,
      other.context);
    if (cmp != 0) {
      return cmp;
    }
    return hydra.util.Comparing.compare(
      argument,
      other.argument);
  }

  public ApplicationType withContext(hydra.haskell.syntax.Type context) {
    return new ApplicationType(context, argument);
  }

  public ApplicationType withArgument(hydra.haskell.syntax.Type argument) {
    return new ApplicationType(context, argument);
  }
}
