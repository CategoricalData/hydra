...
public class PyGraph implements Serializable, Comparable<PyGraph> {
  public static final hydra.core.Name TYPE_ = new hydra.core.Name("hydra.python.environment.PyGraph");
   ...
   */
  public final hydra.python.environment.PythonModuleMetadata metadata;

  public PyGraph (hydra.graph.Graph graph, hydra.python.environment.PythonModuleMetadata metadata) {
    this.graph = graph;
  ...
  public PyGraph withMetadata(hydra.python.environment.PythonModuleMetadata metadata) {
    return new PyGraph(graph, metadata);
  }
}
