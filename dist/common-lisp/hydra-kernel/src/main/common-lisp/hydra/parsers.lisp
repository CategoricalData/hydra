...
(cl:defvar hydra_parsers_satisfy (cl:lambda (pred) (let ((parse (cl:lambda (input) (let ((codes (hydra_lib_strings_to_list input))) (((hydra_lib_maybes_maybe (cl:lambda () (list :failure (make-hydra_parsing_parse_error "unexpected end of input" input)))) (cl:lambda (c) (let ((rest (hydra_lib_strings_from_list ((hydra_lib_lists_drop 1) codes)))) (if (pred c) (list :success (make-hydra_parsing_parse_success c rest)) (list :failure (make-hydra_parsing_parse_error "character did not satisfy predicate" input)))))) (hydra_lib_lists_maybe_head codes)))))) parse)))
