...
| Term_Lambda v_ => (fun (l : Lambda) => let v := (fun r_ => (lambda_parameter) (r_)) (l) in let domain := (fun r_ => (lambda_domain) (r_)) (l) in let body := (fun r_ => (lambda_body) (r_)) (l) in (((logic.ifElse) (((maps.member) (v)) (m))) (let v2 := (((freshName) (v)) ((2)%Z)) (m) in let m2 := (((maps.insert) (v)) (v2)) ((((maps.insert) (v2)) (v2)) (m)) in (Term_Lambda) ((Build_Lambda) (v2) (domain) ((((f) (recurse)) (m2)) (body))))) ((Term_Lambda) ((Build_Lambda) (v) (domain) ((((f) (recurse)) ((((maps.insert) (v)) (v)) (m))) (body))))) (v_)
...
Definition freeVariablesInTypeSchemeSimple : forall (_ : TypeScheme) , (list) (Name) := fun (ts : TypeScheme) => let vars := (fun r_ => (typeScheme_variables) (r_)) (ts) in let t := (fun r_ => (typeScheme_type) (r_)) (ts) in ((sets.difference) ((freeVariablesInTypeSimple) (t))) ((sets.fromList) (vars)).
...
Definition freeVariablesInTypeScheme : forall (_ : TypeScheme) , (list) (Name) := fun (ts : TypeScheme) => let vars := (fun r_ => (typeScheme_variables) (r_)) (ts) in let t := (fun r_ => (typeScheme_type) (r_)) (ts) in ((sets.difference) ((freeVariablesInType) (t))) ((sets.fromList) (vars)).
...
Definition freeTypeVariablesInTerm : forall (_ : Term) , (list) (Name) := fun (term0 : Term) => let tryType := fun (tvars : (list) (Name)) => fun (typ : Type_) => ((sets.difference) ((freeVariablesInType) (typ))) (tvars) in let allOf := fun sets => (((lists.foldl) (sets.union)) (sets.empty)) (sets) in let getAll := (hydra_fix) (fun getAll => fun (vars : (list) (Name)) => fun (term_ : Term) => let recurse := (getAll) (vars) in let dflt := (allOf) (((lists.map) (recurse)) ((subterms) (term_))) in (fun x_ => match x_ with
