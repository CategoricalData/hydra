...
(* Module dependencies *)
Require Import hydra.annotations hydra.constants hydra.context hydra.core hydra.formatting hydra.lib.equality hydra.lib.lists hydra.lib.literals hydra.lib.logic hydra.lib.maps hydra.lib.math hydra.lib.maybes hydra.lib.pairs hydra.lib.sets hydra.lib.strings hydra.packaging hydra.util.

Definition qualifyName : forall (_ : Name) , QualifiedName := fun (name : Name) => let parts := (lists.reverse) (((strings.splitOn) ("."%string)) ((fun w_ => w_) (name))) in (((logic.ifElse) (((equality.equal) ((1)%Z)) ((lists.length) (parts)))) ((Build_QualifiedName) ((None) : (option) (Namespace)) ((fun w_ => w_) (name)))) ((Build_QualifiedName) ((Some) (((strings.intercalate) ("."%string)) ((lists.reverse) ((lists.tail) (parts))))) ((lists.head) (parts))).
Definition compactName : forall (_ : (list) ((prod) (Namespace) (string))) , forall (_ : Name) , string := fun (namespaces : (list) ((prod) (Namespace) (string))) => fun (name : Name) => let qualName := (qualifyName) (name) in let local := (fun r_ => (qualifiedName_local) (r_)) (qualName) in let mns := (fun r_ => (qualifiedName_namespace) (r_)) (qualName) in (((maybes.maybe) ((fun w_ => w_) (name))) (fun (ns : Namespace) => (((maybes.maybe) (local)) (fun (pre : string) => (strings.cat) ((cons) (pre) ((cons) (":"%string) ((cons) (local) (nil)))))) (((maps.lookup) (ns)) (namespaces)))) (mns).
...
Definition freshNames : forall (_ : Z) , forall (_ : Context_) , (prod) ((list) (Name)) (Context_) := fun (n : Z) => fun (cx : Context_) => let go := fun (acc : (prod) ((list) (Name)) (Context_)) => fun _ => let cx0 := (pairs.second) (acc) in let result := (freshName) (cx0) in let cx1 := (pairs.second) (result) in let name := (pairs.first) (result) in let names := (pairs.first) (acc) in (pair) (((lists.concat2) (names)) ((lists.pure) (name))) (cx1) in (((lists.foldl) (go)) ((pair) (nil) (cx))) (((lists.replicate) (n)) (tt)).
Definition localNameOf : forall (_ : Name) , string := fun (arg_ : Name) => (fun r_ => (qualifiedName_local) (r_)) ((qualifyName) (arg_)).
Definition nameToFilePath : forall (_ : CaseConvention) , forall (_ : CaseConvention) , forall (_ : FileExtension) , forall (_ : Name) , string := fun (nsConv : CaseConvention) => fun (localConv : CaseConvention) => fun (ext : FileExtension) => fun (name : Name) => let qualName := (qualifyName) (name) in let local := (fun r_ => (qualifiedName_local) (r_)) (qualName) in let ns := (fun r_ => (qualifiedName_namespace) (r_)) (qualName) in let nsToFilePath := fun (ns2 : Namespace) => ((strings.intercalate) ("/"%string)) (((lists.map) (fun (part : string) => (((convertCase) ((CaseConvention_Camel) (tt))) (nsConv)) (part))) (((strings.splitOn) ("."%string)) ((fun w_ => w_) (ns2)))) in let prefix := (((maybes.maybe) (""%string)) (fun (n : Namespace) => ((strings.cat2) ((nsToFilePath) (n))) ("/"%string))) (ns) in let suffix := (((convertCase) ((CaseConvention_Pascal) (tt))) (localConv)) (local) in (strings.cat) ((cons) (prefix) ((cons) (suffix) ((cons) ("."%string) ((cons) ((fun w_ => w_) (ext)) (nil))))).
