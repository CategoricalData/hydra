...
Definition maybe_bundle (t0 : Type) :=
  hydra_fix (fun (bundle_ : (t0 -> string) -> (option) (t0) -> string) =>
    let maybe := bundle_ in
    fun (f : t0 -> string) => fun (mx : (option) (t0)) => (((maybes.maybe) ("nothing"%string)) (fun (x : t0) => ((strings.cat2) ("just("%string)) (((strings.cat2) ((f) (x))) (")"%string)))) (mx)).
Arguments maybe_bundle {t0}.

Definition maybe (t0 : Type) : (t0 -> string) -> (option) (t0) -> string :=
  maybe_bundle.
Arguments maybe {t0}.
Definition map_bundle (t0 : Type) (t1 : Type) :=
  hydra_fix (fun (bundle_ : (t0 -> string) -> (t1 -> string) -> (list) ((prod) (t0) (t1)) -> string) =>
    let map := bundle_ in
    fun (showK : t0 -> string) => fun (showV : t1 -> string) => fun (m : (list) ((prod) (t0) (t1))) => let pairStrs := ((lists.map) (fun (p : (prod) (t0) (t1)) => (strings.cat) ((cons) ((showK) ((pairs.first) (p))) ((cons) (": "%string) ((cons) ((showV) ((pairs.second) (p))) (nil)))))) ((maps.toList) (m)) in (strings.cat) ((cons) ("{"%string) ((cons) (((strings.intercalate) (", "%string)) (pairStrs)) ((cons) ("}"%string) (nil))))).
Arguments map_bundle {t0} {t1}.

Definition map (t0 : Type) (t1 : Type) : (t0 -> string) -> (t1 -> string) -> (list) ((prod) (t0) (t1)) -> string :=
  map_bundle.
Arguments map {t0} {t1}.
Definition set (t0 : Type) : (t0 -> string) -> (list) (t0) -> string :=
  fun (f : t0 -> string) => fun (xs : (list) (t0)) => let elementStrs := ((lists.map) (f)) ((sets.toList) (xs)) in (strings.cat) ((cons) ("{"%string) ((cons) (((strings.intercalate) (", "%string)) (elementStrs)) ((cons) ("}"%string) (nil)))).
Arguments set {t0}.
Definition list_ (t0 : Type) : (t0 -> string) -> (list) (t0) -> string :=
  fun (f : t0 -> string) => fun (xs : (list) (t0)) => let elementStrs := ((lists.map) (f)) (xs) in (strings.cat) ((cons) ("["%string) ((cons) (((strings.intercalate) (", "%string)) (elementStrs)) ((cons) ("]"%string) (nil)))).
Arguments list_ {t0}.
Definition integerType : IntegerType -> string :=
  fun (it : IntegerType) => (fun x_ => match x_ with
...
Definition integer : IntegerValue -> string :=
...
Definition floatType : FloatType -> string :=
  fun (ft : FloatType) => (fun x_ => match x_ with
| FloatType_Bigfloat _ => "bigfloat"%string
| FloatType_Float32 _ => "float32"%string
| FloatType_Float64 _ => "float64"%string
end) (ft).
Definition literalType : LiteralType -> string :=
...
Definition float : FloatValue -> string :=
...
Definition literal : Literal -> string :=
  fun (l : Literal) => (fun x_ => match x_ with
...
Definition fieldType_type_bundle :=
  hydra_fix (fun (bundle_ : prod (FieldType -> string) (Type_ -> string)) =>
