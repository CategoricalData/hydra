...
(* Module dependencies *)
Require Import hydra.core hydra.lib.eithers hydra.lib.lists hydra.lib.literals hydra.lib.logic hydra.lib.maps hydra.lib.maybes hydra.lib.pairs hydra.lib.sets hydra.lib.strings.
...
Definition fieldType_type_bundle :=
  hydra_fix (fun (bundle_ : prod (forall (_ : FieldType) , string) (forall (_ : Type_) , string)) =>
    ...
    (pair (fun (ft : FieldType) => let fname := (fun w_ => w_) ((fun r_ => (fieldType_name) (r_)) (ft)) in let ftyp := (fun r_ => (fieldType_type) (r_)) (ft) in (strings.cat) ((cons) (fname) ((cons) (":"%string) ((cons) ((type) (ftyp)) (nil))))) (fun (typ : Type_) => let gatherFunctionTypes := (hydra_fix) (fun gatherFunctionTypes => fun (prev : (list) (Type_)) => fun (t : Type_) => (fun x_ => match x_ with
| Type__Function v_ => (fun (ft : FunctionType) => let cod := (fun r_ => (functionType_codomain) (r_)) (ft) in let dom := (fun r_ => (functionType_domain) (r_)) (ft) in ((gatherFunctionTypes) (((lists.cons) (dom)) (prev))) (cod)) (v_)
| _ => (lists.reverse) (((lists.cons) (t)) (prev))
end) (t)) in let gatherTypes := (hydra_fix) (fun gatherTypes => fun (prev : (list) (Type_)) => fun (app : ApplicationType) => let lhs := (fun r_ => (applicationType_function) (r_)) (app) in let rhs := (fun r_ => (applicationType_argument) (r_)) (app) in (fun x_ => match x_ with
...
end) (lhs)) in let showRowType := fun (flds : (list) (FieldType)) => let fieldStrs := ((lists.map) (fieldType)) (flds) in (strings.cat) ((cons) ("{"%string) ((cons) (((strings.intercalate) (", "%string)) (fieldStrs)) ((cons) ("}"%string) (nil)))) in (fun x_ => match x_ with
...
| Type__Either v_ => (fun (et : EitherType) => let leftTyp := (fun r_ => (eitherType_left) (r_)) (et) in let rightTyp := (fun r_ => (eitherType_right) (r_)) (et) in (strings.cat) ((cons) ("either<"%string) ((cons) ((type) (leftTyp)) ((cons) (", "%string) ((cons) ((type) (rightTyp)) ((cons) (">"%string) (nil))))))) (v_)
| Type__Forall v_ => (fun (ft : ForallType) => let body := (fun r_ => (forallType_body) (r_)) (ft) in let var := (fun w_ => w_) ((fun r_ => (forallType_parameter) (r_)) (ft)) in (strings.cat) ((cons) ("(∀"%string) ((cons) (var) ((cons) ("."%string) ((cons) ((type) (body)) ((cons) (")"%string) (nil))))))) (v_)
...
| Type__Map v_ => (fun (mt : MapType) => let keyTyp := (fun r_ => (mapType_keys) (r_)) (mt) in let valTyp := (fun r_ => (mapType_values) (r_)) (mt) in (strings.cat) ((cons) ("map<"%string) ((cons) ((type) (keyTyp)) ((cons) (", "%string) ((cons) ((type) (valTyp)) ((cons) (">"%string) (nil))))))) (v_)
| Type__Maybe v_ => (fun (etyp : Type_) => (strings.cat) ((cons) ("maybe<"%string) ((cons) ((type) (etyp)) ((cons) (">"%string) (nil))))) (v_)
| Type__Pair v_ => (fun (pt : PairType) => let firstTyp := (fun r_ => (pairType_first) (r_)) (pt) in let secondTyp := (fun r_ => (pairType_second) (r_)) (pt) in (strings.cat) ((cons) ("("%string) ((cons) ((type) (firstTyp)) ((cons) (", "%string) ((cons) ((type) (secondTyp)) ((cons) (")"%string) (nil))))))) (v_)
...
Definition projection : forall (_ : Projection) , string := fun (proj : Projection) => let fname := (fun w_ => w_) ((fun r_ => (projection_field) (r_)) (proj)) in let tname := (fun w_ => w_) ((fun r_ => (projection_typeName) (r_)) (proj)) in (strings.cat) ((cons) ("project("%string) ((cons) (tname) ((cons) ("){"%string) ((cons) (fname) ((cons) ("}"%string) (nil)))))).
Definition typeScheme : forall (_ : TypeScheme) , string := fun (ts : TypeScheme) => let body := (fun r_ => (typeScheme_type) (r_)) (ts) in let vars := (fun r_ => (typeScheme_variables) (r_)) (ts) in let varNames := ((lists.map) (fun w_ => w_)) (vars) in let fa := (((logic.ifElse) ((lists.null) (vars))) (""%string)) ((strings.cat) ((cons) ("forall "%string) ((cons) (((strings.intercalate) (","%string)) (varNames)) ((cons) (". "%string) (nil))))) in let toConstraintPair := fun (v : Name) => fun (c : Name) => (strings.cat) ((cons) ((fun w_ => w_) (c)) ((cons) (" "%string) ((cons) ((fun w_ => w_) (v)) (nil)))) in let toConstraintPairs := fun (p : (prod) (Name) (TypeVariableMetadata)) => ((lists.map) ((toConstraintPair) ((pairs.first) (p)))) ((sets.toList) ((fun r_ => (typeVariableMetadata_classes) (r_)) ((pairs.second) (p)))) in let tc := (((maybes.maybe) (nil)) (fun (m : (list) ((prod) (Name) (TypeVariableMetadata))) => (lists.concat) (((lists.map) (toConstraintPairs)) ((maps.toList) (m))))) ((fun r_ => (typeScheme_constraints) (r_)) (ts)) in (strings.cat) ((cons) ("("%string) ((cons) (fa) ((cons) ((((logic.ifElse) ((lists.null) (tc))) (""%string)) ((strings.cat) ((cons) ("("%string) ((cons) (((strings.intercalate) (", "%string)) (tc)) ((cons) (") => "%string) (nil)))))) ((cons) ((type) (body)) ((cons) (")"%string) (nil)))))).
Definition binding_caseStatement_bundle :=
  hydra_fix (fun (bundle_ : prod (forall (_ : Binding) , string) (prod (forall (_ : CaseStatement) , string) (prod (forall (_ : Field) , string) (prod (forall (_ : (list) (Field)) , string) (prod (forall (_ : Injection) , string) (prod (forall (_ : Lambda) , string) (prod (forall (_ : Let) , string) (forall (_ : Term) , string)))))))) =>
    let binding := (fst bundle_) in
    let caseStatement := (fst (snd bundle_)) in
    let field := (fst (snd (snd bundle_))) in
    let fields := (fst (snd (snd (snd bundle_)))) in
    let injection := (fst (snd (snd (snd (snd bundle_))))) in
    let lambda := (fst (snd (snd (snd (snd (snd bundle_)))))) in
    let let_ := (fst (snd (snd (snd (snd (snd (snd bundle_))))))) in
    let term := (snd (snd (snd (snd (snd (snd (snd bundle_))))))) in
    (pair (fun (el : Binding) => let name := (fun w_ => w_) ((fun r_ => (binding_name) (r_)) (el)) in let t := (fun r_ => (binding_term) (r_)) (el) in let typeStr := (((maybes.maybe) (""%string)) (fun (ts : TypeScheme) => (strings.cat) ((cons) (":("%string) ((cons) ((typeScheme) (ts)) ((cons) (")"%string) (nil)))))) ((fun r_ => (binding_type) (r_)) (el)) in (strings.cat) ((cons) (name) ((cons) (typeStr) ((cons) (" = "%string) ((cons) ((term) (t)) (nil)))))) ((pair (fun (cs : CaseStatement) => let csCases := (fun r_ => (caseStatement_cases) (r_)) (cs) in let mdef := (fun r_ => (caseStatement_default) (r_)) (cs) in let defaultField := (((maybes.maybe) (nil)) (fun (d : Term) => (cons) ((Build_Field) ("[default]"%string) (d)) (nil))) (mdef) in let allFields := (lists.concat) ((cons) (csCases) ((cons) (defaultField) (nil))) in let tname := (fun w_ => w_) ((fun r_ => (caseStatement_typeName) (r_)) (cs)) in (strings.cat) ((cons) ("case("%string) ((cons) (tname) ((cons) (")"%string) ((cons) ((fields) (allFields)) (nil)))))) ((pair (fun (field : Field) => let fname := (fun w_ => w_) ((fun r_ => (field_name) (r_)) (field)) in let fterm := (fun r_ => (field_term) (r_)) (field) in (strings.cat) ((cons) (fname) ((cons) ("="%string) ((cons) ((term) (fterm)) (nil))))) ((pair (fun (flds : (list) (Field)) => let fieldStrs := ((lists.map) (field)) (flds) in (strings.cat) ((cons) ("{"%string) ((cons) (((strings.intercalate) (", "%string)) (fieldStrs)) ((cons) ("}"%string) (nil))))) ((pair (fun (inj : Injection) => let f := (fun r_ => (injection_field) (r_)) (inj) in let tname := (fun r_ => (injection_typeName) (r_)) (inj) in (strings.cat) ((cons) ("inject("%string) ((cons) ((fun w_ => w_) (tname)) ((cons) (")"%string) ((cons) ((fields) ((cons) (f) (nil))) (nil)))))) ((pair (fun (l : Lambda) => let body := (fun r_ => (lambda_body) (r_)) (l) in let mt := (fun r_ => (lambda_domain) (r_)) (l) in let typeStr := (((maybes.maybe) (""%string)) (fun (t : Type_) => ((strings.cat2) (":"%string)) ((type) (t)))) (mt) in let v := (fun w_ => w_) ((fun r_ => (lambda_parameter) (r_)) (l)) in (strings.cat) ((cons) ("λ"%string) ((cons) (v) ((cons) (typeStr) ((cons) ("."%string) ((cons) ((term) (body)) (nil))))))) ((pair (fun (l : Let) => let bindings := (fun r_ => (let_bindings) (r_)) (l) in let bindingStrs := ((lists.map) (binding)) (bindings) in let env := (fun r_ => (let_body) (r_)) (l) in (strings.cat) ((cons) ("let "%string) ((cons) (((strings.intercalate) (", "%string)) (bindingStrs)) ((cons) (" in "%string) ((cons) ((term) (env)) (nil)))))) (fun (t : Term) => let gatherTerms := (hydra_fix) (fun gatherTerms => fun (prev : (list) (Term)) => fun (app : Application) => let lhs := (fun r_ => (application_function) (r_)) (app) in let rhs := (fun r_ => (application_argument) (r_)) (app) in (fun x_ => match x_ with
...
| Term_Record v_ => (fun (rec : Record_) => let flds := (fun r_ => (record__fields) (r_)) (rec) in let tname := (fun w_ => w_) ((fun r_ => (record__typeName) (r_)) (rec)) in (strings.cat) ((cons) ("record("%string) ((cons) (tname) ((cons) (")"%string) ((cons) ((fields) (flds)) (nil)))))) (v_)
...
| Term_TypeLambda v_ => (fun (ta : TypeLambda) => let body := (fun r_ => (typeLambda_body) (r_)) (ta) in let param := (fun w_ => w_) ((fun r_ => (typeLambda_parameter) (r_)) (ta)) in (strings.cat) ((cons) ("Λ"%string) ((cons) (param) ((cons) ("."%string) ((cons) ((term) (body)) (nil)))))) (v_)
| Term_TypeApplication v_ => (fun (tt : TypeApplicationTerm) => let t2 := (fun r_ => (typeApplicationTerm_body) (r_)) (tt) in let typ := (fun r_ => (typeApplicationTerm_type) (r_)) (tt) in (strings.cat) ((cons) ((term) (t2)) ((cons) ("⟨"%string) ((cons) ((type) (typ)) ((cons) ("⟩"%string) (nil)))))) (v_)
...
| Term_Wrap v_ => (fun (wt : WrappedTerm) => let term1 := (fun r_ => (wrappedTerm_body) (r_)) (wt) in let tname := (fun w_ => w_) ((fun r_ => (wrappedTerm_typeName) (r_)) (wt)) in (strings.cat) ((cons) ("wrap("%string) ((cons) (tname) ((cons) ("){"%string) ((cons) ((term) (term1)) ((cons) ("}"%string) (nil))))))) (v_)
end) (t)))))))))))))))).

Definition binding : forall (_ : Binding) , string :=
  (fst binding_caseStatement_bundle).
Definition caseStatement : forall (_ : CaseStatement) , string :=
  (fst (snd binding_caseStatement_bundle)).
Definition field : forall (_ : Field) , string :=
  (fst (snd (snd binding_caseStatement_bundle))).
Definition fields : forall (_ : (list) (Field)) , string :=
  (fst (snd (snd (snd binding_caseStatement_bundle)))).
Definition injection : forall (_ : Injection) , string :=
  (fst (snd (snd (snd (snd binding_caseStatement_bundle))))).
Definition lambda : forall (_ : Lambda) , string :=
  (fst (snd (snd (snd (snd (snd binding_caseStatement_bundle)))))).
Definition let_ : forall (_ : Let) , string :=
  (fst (snd (snd (snd (snd (snd (snd binding_caseStatement_bundle))))))).
Definition term : forall (_ : Term) , string :=
  (snd (snd (snd (snd (snd (snd (snd binding_caseStatement_bundle))))))).
