...
Definition universalTestCase : forall (_ : hydra.graph.Graph) , forall (_ : Term) , (sum) (DecodingError) (UniversalTestCase) := fun (cx : hydra.graph.Graph) => fun (raw : Term) => (((eithers.either) (fun (err : DecodingError) => (inl) (err))) (fun (stripped : Term) => (fun x_ => match x_ with
...
Definition testCase : forall (_ : hydra.graph.Graph) , forall (_ : Term) , (sum) (DecodingError) (TestCase) := fun (cx : hydra.graph.Graph) => fun (raw : Term) => (((eithers.either) (fun (err : DecodingError) => (inl) (err))) (fun (stripped : Term) => (fun x_ => match x_ with
| Term_Inject v_ => (fun (inj : Injection) => let variantMap := (maps.fromList) ((cons) ((pair) ("universal"%string) (fun (input : Term) => ((eithers.map) (fun (t : UniversalTestCase) => (TestCase_Universal) (t))) (((universalTestCase) (cx)) (input)))) (nil)) in let field := (fun r_ => (injection_field) (r_)) (inj) in let fname := (fun r_ => (field_name) (r_)) (field) in let fterm := (fun r_ => (field_term) (r_)) (field) in (((maybes.maybe) ((inl) ((strings.cat) ((cons) ("no such field "%string) ((cons) ((fun w_ => w_) (fname)) ((cons) (" in union"%string) (nil))))))) (fun (f : forall (_ : Term) , (sum) (DecodingError) (TestCase)) => (f) (fterm))) (((maps.lookup) (fname)) (variantMap))) (v_)
| _ => (inl) ("expected union"%string)
end) (stripped))) (((stripWithDecodingError) (cx)) (raw)).
Definition tag : forall (_ : hydra.graph.Graph) , forall (_ : Term) , (sum) (DecodingError) (Tag) := fun (cx : hydra.graph.Graph) => fun (raw : Term) => (((eithers.either) (fun (err : DecodingError) => (inl) (err))) (fun (stripped : Term) => (fun x_ => match x_ with
...
Definition testCaseWithMetadata : forall (_ : hydra.graph.Graph) , forall (_ : Term) , (sum) (DecodingError) (TestCaseWithMetadata) := fun (cx : hydra.graph.Graph) => fun (raw : Term) => (((eithers.either) (fun (err : DecodingError) => (inl) (err))) (fun (stripped : Term) => (fun x_ => match x_ with
