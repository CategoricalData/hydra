...
(* Module dependencies *)
Require Import hydra.graph hydra.core hydra.errors hydra.lib.eithers hydra.extract.core hydra.topology.

Definition vertex : hydra.graph.Graph -> Term -> (sum) (DecodingError) (Z) :=
...
Definition tarjanState : hydra.graph.Graph -> Term -> (sum) (DecodingError) (TarjanState) :=
  fun (cx : hydra.graph.Graph) => fun (raw : Term) => (((eithers.either) (fun (err : DecodingError) => (inl) (err))) (fun (stripped : Term) => (fun x_ => match x_ with
...
Definition graph_ : hydra.graph.Graph -> Term -> (sum) (DecodingError) ((list) ((prod) (Z) ((list) (Z)))) :=
  ((decodeMap) (vertex)) ((decodeList) (vertex)).
