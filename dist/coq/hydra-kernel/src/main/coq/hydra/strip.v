...
| Term_Annotated v_ => (fun (at_ : AnnotatedTerm) => let subj := (fun r_ => (annotatedTerm_body) (r_)) (at_) in let ann := (fun r_ => (annotatedTerm_annotation) (r_)) (at_) in (Term_Annotated) ((Build_AnnotatedTerm) ((stripTypeLambdas) (subj)) (ann))) (v_)
| Term_TypeLambda v_ => (fun (ta : TypeLambda) => (stripTypeLambdas) ((fun r_ => (typeLambda_body) (r_)) (ta))) (v_)
| _ => t
end) (t)).

Definition stripTypeLambdas : Term -> Term :=
...
Definition removeTypeAnnotationsFromTerm : Term -> Term :=
  fun (term_ : Term) => let strip := fun recurse => fun term2 => let stripBinding := fun (b : Binding) => (Build_Binding) ((fun r_ => (binding_name) (r_)) (b)) ((fun r_ => (binding_term) (r_)) (b)) (None) in let rewritten := (recurse) (term2) in (fun x_ => match x_ with
| Term_Let v_ => (fun (lt : Let) => (Term_Let) ((Build_Let) (((lists.map) (stripBinding)) ((fun r_ => (let_bindings) (r_)) (lt))) ((fun r_ => (let_body) (r_)) (lt)))) (v_)
| Term_TypeApplication v_ => (fun (tt : TypeApplicationTerm) => (fun r_ => (typeApplicationTerm_body) (r_)) (tt)) (v_)
| Term_TypeLambda v_ => (fun (ta : TypeLambda) => (fun r_ => (typeLambda_body) (r_)) (ta)) (v_)
...
Definition removeTypeAnnotations : Type_ -> Type_ :=
  fun (typ : Type_) => let remove := fun recurse => fun typ2 => let rewritten := (recurse) (typ2) in (fun x_ => match x_ with
| Type__Annotated v_ => (fun (at_ : AnnotatedType) => (fun r_ => (annotatedType_body) (r_)) (at_)) (v_)
| _ => rewritten
end) (rewritten) in ((rewriteType) (remove)) (typ).
Definition removeTermAnnotations : Term -> Term :=
  fun (term_ : Term) => let remove := fun (recurse : Term -> Term) => fun (term2 : Term) => let rewritten := (recurse) (term2) in (fun x_ => match x_ with
| Term_Annotated v_ => (fun (at_ : AnnotatedTerm) => (fun r_ => (annotatedTerm_body) (r_)) (at_)) (v_)
| _ => rewritten
end) (term2) in ((rewriteTerm) (remove)) (term_).
Definition deannotateTypeRecursive : Type_ -> Type_ :=
  fun (typ : Type_) => let strip := fun recurse => fun typ2 => let rewritten := (recurse) (typ2) in (fun x_ => match x_ with
| Type__Annotated v_ => (fun (at_ : AnnotatedType) => (fun r_ => (annotatedType_body) (r_)) (at_)) (v_)
| _ => rewritten
end) (rewritten) in ((rewriteType) (strip)) (typ).
Definition deannotateTypeSchemeRecursive : TypeScheme -> TypeScheme :=
  fun (ts : TypeScheme) => let vars := (fun r_ => (typeScheme_variables) (r_)) (ts) in let typ := (fun r_ => (typeScheme_type) (r_)) (ts) in let constraints := (fun r_ => (typeScheme_constraints) (r_)) (ts) in (Build_TypeScheme) (vars) ((deannotateTypeRecursive) (typ)) (constraints).
...
Definition deannotateType : Type_ -> Type_ :=
...
Definition deannotateTypeParameters : Type_ -> Type_ :=
...
Definition deannotateTerm : Term -> Term :=
...
Definition deannotateAndDetypeTerm : Term -> Term :=
  deannotateAndDetypeTerm_bundle.
Definition detypeTerm_bundle :=
  hydra_fix (fun (bundle_ : Term -> Term) =>
    let detypeTerm := bundle_ in
    fun (t : Term) => (fun x_ => match x_ with
| Term_Annotated v_ => (fun (at_ : AnnotatedTerm) => let subj := (fun r_ => (annotatedTerm_body) (r_)) (at_) in let ann := (fun r_ => (annotatedTerm_annotation) (r_)) (at_) in (Term_Annotated) ((Build_AnnotatedTerm) ((detypeTerm) (subj)) (ann))) (v_)
| Term_TypeApplication v_ => (fun (tt : TypeApplicationTerm) => (deannotateAndDetypeTerm) ((fun r_ => (typeApplicationTerm_body) (r_)) (tt))) (v_)
| Term_TypeLambda v_ => (fun (ta : TypeLambda) => (deannotateAndDetypeTerm) ((fun r_ => (typeLambda_body) (r_)) (ta))) (v_)
| _ => t
end) (t)).

Definition detypeTerm : Term -> Term :=
  detypeTerm_bundle.
