...
Definition removeTypesFromTerm : forall (_ : Term) , Term := fun (term_ : Term) => let strip := fun recurse => fun term2 => let stripBinding := fun (b : Binding) => (Build_Binding) ((fun r_ => (binding_name) (r_)) (b)) ((fun r_ => (binding_term) (r_)) (b)) (None) in let rewritten := (recurse) (term2) in (fun x_ => match x_ with
...
Definition removeTypeAnnotationsFromTerm : forall (_ : Term) , Term := fun (term_ : Term) => let strip := fun recurse => fun term2 => let stripBinding := fun (b : Binding) => (Build_Binding) ((fun r_ => (binding_name) (r_)) (b)) ((fun r_ => (binding_term) (r_)) (b)) (None) in let rewritten := (recurse) (term2) in (fun x_ => match x_ with
| Term_Let v_ => (fun (lt : Let) => (Term_Let) ((Build_Let) (((lists.map) (stripBinding)) ((fun r_ => (let_bindings) (r_)) (lt))) ((fun r_ => (let_body) (r_)) (lt)))) (v_)
| Term_TypeApplication v_ => (fun (tt : TypeApplicationTerm) => (fun r_ => (typeApplicationTerm_body) (r_)) (tt)) (v_)
| Term_TypeLambda v_ => (fun (ta : TypeLambda) => (fun r_ => (typeLambda_body) (r_)) (ta)) (v_)
| _ => rewritten
...
Definition deannotateTypeSchemeRecursive : forall (_ : TypeScheme) , TypeScheme := fun (ts : TypeScheme) => let vars := (fun r_ => (typeScheme_variables) (r_)) (ts) in let typ := (fun r_ => (typeScheme_type) (r_)) (ts) in let constraints := (fun r_ => (typeScheme_constraints) (r_)) (ts) in (Build_TypeScheme) (vars) ((deannotateTypeRecursive) (typ)) (constraints).
