...
(* Module dependencies *)
Require Import hydra.context hydra.core hydra.decode.core hydra.errors hydra.graph hydra.lexical hydra.lib.eithers hydra.lib.equality hydra.lib.lists hydra.lib.literals hydra.lib.logic hydra.lib.maps hydra.lib.math hydra.lib.maybes hydra.lib.pairs hydra.lib.strings hydra.names hydra.scoping hydra.show.core hydra.strip hydra.substitution hydra.variables.
...
| Type__Forall v_ => (fun (ft : ForallType) => let newVar := ((strings.cat2) ("_"%string)) ((literals.showInt32) (depth)) in let oldVar := (fun r_ => (forallType_parameter) (r_)) (ft) in (((go) (((math.add) (depth)) ((1)%Z))) ((((maps.insert) (oldVar)) (newVar)) (subst))) ((fun r_ => (forallType_body) (r_)) (ft))) (v_)
| _ => (pair) (subst) (t)
end) ((deannotateType) (t))) in let result := (((go) ((0)%Z)) (maps.empty)) (typ) in let body := (pairs.second) (result) in let subst := (pairs.first) (result) in ((substituteTypeVariables) (subst)) (body).
...
Definition instantiateTypeScheme : forall (_ : Context_) , forall (_ : TypeScheme) , (prod) (TypeScheme) (Context_) := fun (cx : Context_) => fun (scheme : TypeScheme) => let oldVars := (fun r_ => (typeScheme_variables) (r_)) (scheme) in let result := ((freshNames) ((lists.length) (oldVars))) (cx) in let cx2 := (pairs.second) (result) in let newVars := (pairs.first) (result) in let nameSubst := (maps.fromList) (((lists.zip) (oldVars)) (newVars)) in let renamedConstraints := ((maybes.map) (fun (oldConstraints : (list) ((prod) (Name) (TypeVariableMetadata))) => (maps.fromList) (((lists.map) (fun (kv : (prod) (Name) (TypeVariableMetadata)) => (pair) (((maybes.fromMaybe) ((pairs.first) (kv))) (((maps.lookup) ((pairs.first) (kv))) (nameSubst))) ((pairs.second) (kv)))) ((maps.toList) (oldConstraints))))) ((fun r_ => (typeScheme_constraints) (r_)) (scheme)) in let subst := (maps.fromList) (((lists.zip) (oldVars)) (((lists.map) (fun (x : Name) => (Type__Variable) (x))) (newVars))) in (pair) ((Build_TypeScheme) (newVars) (((substInType) (subst)) ((fun r_ => (typeScheme_type) (r_)) (scheme))) (renamedConstraints)) (cx2).
...
| _ => (Build_TypeScheme) ((lists.reverse) (vars)) (t) ((None) : (option) ((list) ((prod) (Name) (TypeVariableMetadata))))
