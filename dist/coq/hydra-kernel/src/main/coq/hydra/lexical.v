...
Definition matchUnion_bundle (t0 : Type) :=
  hydra_fix (fun (bundle_ : forall (_ : hydra.graph.Graph) , forall (_ : Name) , forall (_ : (list) ((prod) (Name) (forall (_ : Term) , (sum) (Error) (t0)))) , forall (_ : Term) , (sum) (Error) (t0)) =>
    ...
    fun (graph_ : hydra.graph.Graph) => fun (tname : Name) => fun (pairs : (list) ((prod) (Name) (forall (_ : Term) , (sum) (Error) (t0)))) => fun (term_ : Term) => let stripped := (deannotateAndDetypeTerm) (term_) in let mapping := (maps.fromList) (pairs) in (fun x_ => match x_ with
...
Definition graphToBindings : forall (_ : hydra.graph.Graph) , (list) (Binding) := fun (g : hydra.graph.Graph) => ((lists.map) (fun (p : (prod) (Name) (Term)) => let term_ := (pairs.second) (p) in let name := (pairs.first) (p) in (Build_Binding) (name) (term_) (((maps.lookup) (name)) ((fun r_ => (graph_boundTypes) (r_)) (g))))) ((maps.toList) ((fun r_ => (graph_boundTerms) (r_)) (g))).
