...
(* Module dependencies *)
Require Import hydra.core hydra.decode.core hydra.errors hydra.graph hydra.lib.eithers hydra.lib.logic hydra.lib.maps hydra.lib.maybes hydra.lib.sets hydra.lib.strings.
...
Definition instantiateTemplate_bundle (t0 : Type) :=
  hydra_fix (fun (bundle_ : forall (_ : t0) , forall (_ : bool) , forall (_ : (list) ((prod) (Name) (Type_))) , forall (_ : Name) , forall (_ : Type_) , (sum) (Error) (Term)) =>
    ...
    fun (cx : t0) => fun (minimal : bool) => fun (schema : (list) ((prod) (Name) (Type_))) => fun (tname : Name) => fun (t : Type_) => let forFloat := fun (ft : FloatType) => (fun x_ => match x_ with
...
end) (ft) in let forInteger := fun (it : IntegerType) => (fun x_ => match x_ with
...
end) (lt) in let inst := fun (tn : Name) => ((((instantiateTemplate) (cx)) (minimal)) (schema)) (tn) in let noPoly := fun (t1 : Type) => (inl) ((Error_Extraction) ((ExtractionError_UnexpectedShape) ((Build_UnexpectedShapeError) ("non-polymorphic type"%string) ("polymorphic or function type"%string)))) in (fun x_ => match x_ with
...
| Type__Map v_ => (fun (mt : MapType) => let kt := (fun r_ => (mapType_keys) (r_)) (mt) in let vt := (fun r_ => (mapType_values) (r_)) (mt) in (((logic.ifElse) (minimal)) ((inr) ((Term_Map) (maps.empty)))) (((eithers.bind) (((inst) (tname)) (kt))) (fun (ke : Term) => ((eithers.bind) (((inst) (tname)) (vt))) (fun (ve : Term) => (inr) ((Term_Map) (((maps.singleton) (ke)) (ve))))))) (v_)
