...
Definition keyValueToExpr_valueToExpr_bundle :=
  hydra_fix (fun (bundle_ : prod (forall (_ : (prod) (string) (Value)) , Expr) (forall (_ : Value) , Expr)) =>
    ...
    (pair (fun (pair_ : (prod) (string) (Value)) => let key := (pairs.first) (pair_) in let value := (pairs.second) (pair_) in (((ifx) (colonOp)) ((cst) ((jsonString) (key)))) ((valueToExpr) (value))) (fun (value : Value) => (fun x_ => match x_ with
...
| Value_Number v_ => (fun (n : Q) => let rounded := (literals.bigfloatToBigint) (n) in let shown := (literals.showBigfloat) (n) in (cst) ((((logic.ifElse) (((logic.and) (((equality.equal) (n)) ((literals.bigintToBigfloat) (rounded)))) ((logic.not) (((equality.equal) (shown)) ("-0.0"%string))))) ((literals.showBigint) (rounded))) (shown))) (v_)
