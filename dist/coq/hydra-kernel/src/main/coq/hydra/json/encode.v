...
Definition isSpecialFloatString : string -> bool :=
  fun (s : string) => ((logic.or) (((equality.equal) (s)) ("NaN"%string))) (((logic.or) (((equality.equal) (s)) ("Infinity"%string))) (((equality.equal) (s)) ("-Infinity"%string))).
...
| Term_Union v_ => (fun (inj : Injection) => let field := (fun r_ => (injection_field) (r_)) (inj) in let fname := (fun w_ => w_) ((fun r_ => (field_name) (r_)) (field)) in let fterm := (fun r_ => (field_term) (r_)) (field) in let encodedUnion := (toJsonUntyped) (fterm) in ((eithers.map) (fun (v : Value) => (Value_Object) ((maps.fromList) ((cons) ((pair) (fname) (v)) (nil))))) (encodedUnion)) (v_)
