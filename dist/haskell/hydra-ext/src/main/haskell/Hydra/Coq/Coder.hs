...
encodeAxiomDefinitionPair :: (String, Core.Type) -> Syntax.Sentence
encodeAxiomDefinitionPair nt =
    Syntax.Sentence {
      ...
      Syntax.sentenceContent = (Syntax.SentenceContentAxiom (Syntax.AxiomDeclaration {
        ...
        Syntax.axiomDeclarationType = (coqTypeTerm (encodeType (Pairs.second nt)))}))}

-- | Encode a Lambda into a Coq `fun` expression, sanitising the parameter name
encodeLambdaTerm :: M.Map String Int -> Core.Lambda -> Syntax.Term
encodeLambdaTerm cc lam =
      ...
      let paramName = sanitizeVar (Core.unName (Core.lambdaParameter lam)) 
          binder =
                  Maybes.maybe (Syntax.BinderName (coqName paramName)) (\domTy -> Syntax.BinderType (Syntax.TypeBinders {
                    Syntax.typeBindersNames = [
                      coqName paramName],
                    Syntax.typeBindersType = (coqTypeTerm (encodeType domTy))})) (Core.lambdaDomain lam)
      in (Syntax.TermForallOrFun (Syntax.ForallOrFunFun (Syntax.Fun {
        Syntax.funBinders = (Syntax.OpenBindersBinders [
          binder]),
        Syntax.funBody = (encodeTerm cc (Core.lambdaBody lam))})))
...
-- | Translate a Hydra Term into its Coq Term counterpart. The constructor-count map lets encodeUnionElim decide whether a match is exhaustive and either drop a redundant default or add `| _ => hydra_unreachable`.
encodeTerm :: M.Map String Int -> Core.Term -> Syntax.Term
encodeTerm cc tm =
    case tm of
      Core.TermAnnotated v0 -> encodeTerm cc (Core.annotatedTermBody v0)
      Core.TermApplication v0 -> coqTermApp (encodeTerm cc (Core.applicationFunction v0)) [
        encodeTerm cc (Core.applicationArgument v0)]
      Core.TermCases v0 -> encodeUnionElim cc v0
      Core.TermEither v0 -> Eithers.either (\l -> coqTermApp (coqTermQualid "inl") [
        encodeTerm cc l]) (\r -> coqTermApp (coqTermQualid "inr") [
        encodeTerm cc r]) v0
      Core.TermInject v0 ->  
        ...
        in (coqTermApp (coqTermQualid constrName) [
          encodeTerm cc fterm])
      Core.TermLambda v0 -> encodeLambdaTerm cc v0
      Core.TermLet v0 ->  
        ...
        in (Lists.foldr (\binding -> \acc ->  
              ...
              recBody =
                      coqTermApp (coqTermQualid "hydra_fix") [
                        Syntax.TermForallOrFun (Syntax.ForallOrFunFun (Syntax.Fun {
                          ...
                          Syntax.funBody = (encodeTerm cc bterm)}))]
              boundTerm = Logic.ifElse recursive recBody (encodeTerm cc bterm)
          in (Syntax.TermLet (Syntax.Let {
            ...
            Syntax.letIn = acc}))) (encodeTerm cc body) bindings)
      ...
      Core.TermPair v0 -> coqTermApp (coqTermQualid "pair") [
        encodeTerm cc (Pairs.first v0),
        (encodeTerm cc (Pairs.second v0))]
      Core.TermProject v0 -> encodeProjectionElim v0
      Core.TermRecord v0 ->  
        let rname = Core.recordTypeName v0 
            rfields = Core.recordFields v0
        in (Logic.ifElse (Lists.null rfields) (coqTermQualid "tt") (coqTermApp (coqTermQualid (Strings.cat2 "Build_" (Core.unName rname))) (Lists.map (\f -> encodeTerm cc (Core.fieldTerm f)) rfields)))
      Core.TermSet _ -> coqTermQualid "nil"
      Core.TermTypeApplication v0 -> encodeTerm cc (Core.typeApplicationTermBody v0)
      Core.TermTypeLambda v0 -> encodeTerm cc (Core.typeLambdaBody v0)
      ...
      Core.TermWrap v0 -> encodeTerm cc (Core.wrappedTermBody v0)

-- | Build a Coq `Definition name := body.` sentence from a Hydra term
encodeTermDefinition :: M.Map String Int -> String -> Core.Term -> Syntax.SentenceContent
encodeTermDefinition cc name body =
    Syntax.SentenceContentDefinition (Syntax.Definition {
      ...
      Syntax.definitionBody = (encodeTerm cc body)})

-- | Wrap encodeTermDefinition in a Coq Sentence with no leading comment
encodeTermDefinitionPair :: M.Map String Int -> (String, Core.Term) -> Syntax.Sentence
encodeTermDefinitionPair cc ed =
    Syntax.Sentence {
      Syntax.sentenceComment = Nothing,
      Syntax.sentenceContent = (encodeTermDefinition cc (Pairs.first ed) (Pairs.second ed))}

-- | Translate a Hydra Type into a Coq Term representing that type
encodeType :: Core.Type -> Syntax.Term
encodeType ty =
    case ty of
      Core.TypeAnnotated v0 -> encodeType (Core.annotatedTypeBody v0)
      Core.TypeApplication v0 -> coqTermApp (encodeType (Core.applicationTypeFunction v0)) [
        encodeType (Core.applicationTypeArgument v0)]
      Core.TypeEither v0 -> coqTermApp (coqTermQualid "sum") [
        encodeType (Core.eitherTypeLeft v0),
        (encodeType (Core.eitherTypeRight v0))]
      Core.TypeForall v0 -> Syntax.TermForallOrFun (Syntax.ForallOrFunForall (Syntax.Forall {
        ...
        Syntax.forallType = (coqTypeTerm (encodeType (Core.forallTypeBody v0)))}))
      Core.TypeFunction v0 -> coqArrow (encodeType (Core.functionTypeDomain v0)) (encodeType (Core.functionTypeCodomain v0))
      ...
      Core.TypeMap v0 -> coqTermApp (coqTermQualid "list") [
        coqTermApp (coqTermQualid "prod") [
          encodeType (Core.mapTypeKeys v0),
          (encodeType (Core.mapTypeValues v0))]]
      Core.TypeMaybe v0 -> coqTermApp (coqTermQualid "option") [
        encodeType v0]
      Core.TypePair v0 -> coqTermApp (coqTermQualid "prod") [
        encodeType (Core.pairTypeFirst v0),
        (encodeType (Core.pairTypeSecond v0))]
...
encodeTypeDefinitionPair td =
    Syntax.Sentence {
      Syntax.sentenceComment = Nothing,
      Syntax.sentenceContent = (encodeTypeDefinition (Pairs.first td) (Pairs.second td))}
...
-- | Build a Coq match expression from a Hydra union eliminator. Uses the constructor-count map to decide whether the match is exhaustive: if so, an explicit default is suppressed; if not and the kernel didn't provide one, inserts `| _ => hydra_unreachable`.
encodeUnionElim :: M.Map String Int -> Core.CaseStatement -> Syntax.Term
encodeUnionElim cc cs =
      ...
      let csName = Core.caseStatementTypeName cs 
          ...
          expectedCount = Maps.lookup csLocalName cc
          ...
          baseEqs =
                  Lists.map (\c ->  
                    ...
                    in (Logic.ifElse (isUnitLambda cft) (Syntax.Equation {
                      ...
                      Syntax.equationTerm = (encodeTerm cc (unitLambdaBody cft))}) (Syntax.Equation {
                      Syntax.equationPattern = [
                        [
                          Syntax.PatternPattern (Syntax.Pattern10Qualiid (Syntax.Pattern10_Qualid {
                            Syntax.pattern10_QualidQualid = (coqQualid constr),
                            Syntax.pattern10_QualidPatterns = [
                              Syntax.Pattern1 {
                                ...
                                Syntax.pattern1Scope = Nothing}]}))]],
                      Syntax.equationTerm = (coqTermApp (encodeTerm cc cft) [
          ...
          defaultEqs =
                  Maybes.maybe (Logic.ifElse (Maybes.maybe False (\n -> Logic.not (Equality.gte caseCount n)) expectedCount) [
                    ...
                    wildcardEq (encodeTerm cc defT)]) csDefault
...
extractLambdaBinders tm =
    case tm of
      Core.TermAnnotated v0 -> extractLambdaBinders (Core.annotatedTermBody v0)
      Core.TermLambda v0 ->  
        let param = Core.lambdaParameter v0 
            ...
            binder =
                    Maybes.maybe (Syntax.BinderName (coqName (Core.unName param))) (\domTy -> Syntax.BinderType (Syntax.TypeBinders {
                      ...
                      Syntax.typeBindersType = (coqTypeTerm (encodeType domTy))})) mDomain
        in (Lists.cons binder (extractLambdaBinders (Core.lambdaBody v0)))
      _ -> []
...
moduleToCoq :: M.Map String Int -> [(String, Core.Type)] -> [(String, Core.Term)] -> Syntax.Document
moduleToCoq cc typeDefs termDefs =
     
      let typesSentences = Lists.map (\td -> encodeTypeDefinitionPair td) typeDefs 
          termsSentences = Lists.map (\ed -> encodeTermDefinitionPair cc ed) termDefs
      in Syntax.Document {
