...
constructModule cx g mod typeDefs =
      ...
      let ns_ = Packaging.moduleName mod
          desc = Optionals.bind (Packaging.moduleMetadata mod) (\em -> Packaging.entityMetadataDescription em)
          toDef =
                  \td ->
          ...
          checkFields =
                  \checkType -> \checkFieldType -> \ts -> Lists.foldl (\b -> \t -> Logic.or b (Rewriting.foldOverType Coders.TraversalOrderPre (\b2 -> \t2 -> Logic.or b2 (
                    ...
                    in (Optionals.cases checkResult (case t2 of
                      ...
                      _ -> False) (\b3 -> b3)))) False t)) False ts
          wrapperImport =
      ...
      in (Eithers.bind (Analysis.moduleDependencyModuleNames (ProtobufEnvironment.encoderStateContext cx) g True False False False mod) (\schemaImports -> Eithers.bind (Eithers.mapList toDef typeDefs) (\definitions ->
...
encodeDefinition cx g localNs name typ =
      ...
      let cx1 =
              ProtobufEnvironment.EncoderState {
                ProtobufEnvironment.encoderStateContext = (ProtobufEnvironment.encoderStateContext cx),
                ProtobufEnvironment.encoderStateFieldIndex = 0}
          cx2 =
                  Pairs.second (
                    ProtobufEnvironment.encoderStateFieldIndex cx1,
                    ProtobufEnvironment.EncoderState {
                      ProtobufEnvironment.encoderStateContext = (ProtobufEnvironment.encoderStateContext cx1),
                      ProtobufEnvironment.encoderStateFieldIndex = (Math.add (ProtobufEnvironment.encoderStateFieldIndex cx1) 1)})
          wrapAsRecordType =
                  \t -> Core.TypeRecord [
                    Core.FieldType {
...
encodeFieldType cx g localNs ft =
      ...
      in (Eithers.bind (findOptions cx g ftype) (\options -> Eithers.bind (encodeType_ cx g localNs ftype) (\ft_ ->
        let idxPair =
                (
                  ProtobufEnvironment.encoderStateFieldIndex cx,
                  ProtobufEnvironment.EncoderState {
                    ProtobufEnvironment.encoderStateContext = (ProtobufEnvironment.encoderStateContext cx),
                    ProtobufEnvironment.encoderStateFieldIndex = (Math.add (ProtobufEnvironment.encoderStateFieldIndex cx) 1)})
...
findOptions cx g typ =
    Eithers.bind (Annotations.getTypeDescription (ProtobufEnvironment.encoderStateContext cx) g typ) (\mdesc -> Eithers.bind (readBooleanAnnotation cx g Constants.keyDeprecated typ) (\bdep ->
      let mdescAnn =
              Optionals.map (\desc_ -> Proto3.Option {
                Proto3.optionName = Serde.descriptionOptionName,
      ...
      let cx1 =
          ...
          makeField =
                  \cx0 -> \fname -> \ftyp -> Eithers.bind (encodeSimpleTypeForHelper cx0 localNs ftyp) (\ft ->
                    let idxPair =
                            (
                              ProtobufEnvironment.encoderStateFieldIndex cx0,
                              ProtobufEnvironment.EncoderState {
                                ProtobufEnvironment.encoderStateContext = (ProtobufEnvironment.encoderStateContext cx0),
                                ProtobufEnvironment.encoderStateFieldIndex = (Math.add (ProtobufEnvironment.encoderStateFieldIndex cx0) 1)})
                        idx = Pairs.first idxPair
        ...
        ProtobufEnvironment.StructuralTypeRefEither v0 ->
          ...
          in (Eithers.bind (makeField cx2 "left" lt) (\leftResult ->
            ...
            in (Eithers.bind (makeField cx3 "right" rt) (\rightResult ->
              ...
              in (Right (
                Proto3.DefinitionMessage (Proto3.MessageDefinition {
...
moduleToProtobuf mod defs cx g =
      ...
      let ns_ = Packaging.moduleName mod
          ...
          es =
                  ProtobufEnvironment.EncoderState {
                    ProtobufEnvironment.encoderStateContext = cx,
                    ProtobufEnvironment.encoderStateFieldIndex = 0}
      in (Eithers.bind (constructModule es g mod typeDefs) (\pfile ->
