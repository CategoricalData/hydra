...
structBodyToExpr body =
    case body of
      ...
      Syntax.StructBodyTuple v0 -> Serialization.cst (Strings.cat2 (Serialization.printExpr (Serialization.parenList False (Lists.map (\f ->
        let pub = Syntax.tupleFieldPublic f
            typ = Serialization.printExpr (typeToExpr (Syntax.tupleFieldType f))
        in (Logic.ifElse pub (Serialization.cst (Strings.cat2 "pub " typ)) (Serialization.cst typ))) v0))) ";")
...
enumVariantToExpr v =
      ...
      let name = Syntax.enumVariantName v
          ...
          docPart = Maybes.maybe [] (\d -> [
                Serialization.cst (toRustDocComment d)]) docC
          bodyExpr = enumVariantBodyToExpr body
      in (Serialization.newlineSep (Lists.concat [
        docPart,
        [
          Serialization.cst (Strings.cat2 name (Serialization.printExpr bodyExpr))]]))
...
fnDefToExpr f =
      ...
      let name = Syntax.fnDefName f
          ...
          nameAndParams =
                  Serialization.cst (Strings.cat2 (Serialization.printExpr (Serialization.spaceSep (Maybes.cat [
                    genericsExpr,
                    (Just nameExpr)]))) (Serialization.printExpr paramsExpr))
          header =
                  Serialization.spaceSep (Maybes.cat [
                    pubKw,
                    asyncKw,
...
pathSegmentToExpr seg =
      ...
      let name = Syntax.pathSegmentName seg
          ...
          argExpr = genericArgumentsToExpr args
      in (Maybes.maybe (Serialization.cst name) (\a -> Serialization.cst (Strings.cat2 name (Serialization.printExpr a))) argExpr)
...
callExprToExpr c =
      ...
      in (Serialization.cst (Strings.cat2 (Serialization.printExpr (expressionToExpr func)) (Serialization.printExpr (Serialization.parenList False (Lists.map expressionToExpr args)))))
...
blockToExpr b =
      ...
      in (Serialization.curlyBracesList (Just "") Serialization.halfBlockStyle allParts)
