...
buildStringOffsets strs =
      ...
      let step =
          ...
          aligned = Math.mul (Math.div (Math.add rawEnd 15) 16) 16
      ...
      in (Eithers.bind (Eithers.mapList (\a -> encodeTerm cx g stringOffsets fieldOffsets variantIndexes funcSigs a) args) (\realArgInstrs ->
        ...
        in case dfun of
          Core.TermVariable v0 ->
            ...
            in (Logic.ifElse (Lists.null (Lists.tail (Strings.splitOn "." rawName))) (Right (Lists.concat [
              droppedArgInstrs,
              [
          ...
          Core.TermProject v0 -> Logic.ifElse (Lists.null args) (Right [
            Syntax.InstructionConst (Syntax.ConstValueI32 0)]) (
            let firstArg = Lists.head args
            in (Eithers.bind (encodeTerm cx g stringOffsets fieldOffsets variantIndexes funcSigs firstArg) (\firstArgInstrs -> encodeProjection cx g fieldOffsets v0 firstArgInstrs)))
          Core.TermCases v0 -> Logic.ifElse (Lists.null args) (Right [
            Syntax.InstructionConst (Syntax.ConstValueI32 0)]) (
            let firstArg = Lists.head args
            in (Eithers.bind (encodeTerm cx g stringOffsets fieldOffsets variantIndexes funcSigs firstArg) (\firstArgInstrs -> encodeCases cx g stringOffsets fieldOffsets variantIndexes funcSigs v0 firstArgInstrs)))
...
encodeTermDefinition cx g stringOffsets fieldOffsets variantIndexes funcSigs tdef =
      ...
      let name = Packaging.termDefinitionName tdef
          ...
          scrutineeInstrs = Logic.ifElse (Lists.null paramNameStrs) [] [
                Syntax.InstructionLocalGet (Lists.head paramNameStrs)]
...
hexEscapeString b =
      ...
      in (Strings.cat [
        "\\",
        (digitToHex (Math.div byte 16)),
        (digitToHex (Math.mod byte 16))])
                  ...
                  \entry ->
                    let s = Pairs.first entry
                        ...
                        lenBytes =
                                [
                                  Math.mod len 256,
                                  (Math.mod (Math.div len 256) 256),
                                  (Math.mod (Math.div len 65536) 256),
                                  (Math.mod (Math.div len 16777216) 256)]
