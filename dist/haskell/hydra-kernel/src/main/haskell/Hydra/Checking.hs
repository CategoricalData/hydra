...
allEqual els =
    Maybes.cases (Lists.uncons els) True (\uc ->
...
applyTypeArgumentsToType cx tx typeArgs t =
    Maybes.cases (Lists.uncons typeArgs) (Right t) (\uc ->
...
checkSameType cx tx desc types =
      ...
      in (Logic.ifElse (typesAllEffectivelyEqual tx types) (Maybes.cases (Lists.maybeHead types) unequalErr (\t -> Right t)) unequalErr)
      ...
      let s = Typing.unTypeSubst subst
          ...
          isNominal =
                  \ts -> case (Strip.deannotateType (Core.typeSchemeBody ts)) of
                    ...
                    _ -> False
          badVars =
                  Sets.fromList (Lists.filter (\v -> Maybes.cases (Lexical.dereferenceSchemaType v (Graph.graphSchemaTypes tx)) False isNominal) (Sets.toList suspectVars))
...
typeOfCaseStatement cx tx typeArgs cs =
      ...
      in (Eithers.bind (Eithers.mapMaybe (\e -> typeOf cx tx [] e) dflt) (\dfltResult ->
        let tdflt = Maybes.map Pairs.first dfltResult
            cx2 = Maybes.cases dfltResult cx Pairs.second
            foldResult =
      ...
      let n = Lists.length typeArgs
          arityErr =
                  Left (Errors.ErrorChecking (Checking.CheckingErrorTypeArityMismatch (Checking.TypeArityMismatchError {
                    ...
                    Checking.typeArityMismatchErrorTypeArguments = typeArgs})))
      in (Maybes.cases (Lists.uncons typeArgs) arityErr (\uc0 ->
        let ta0 = Pairs.first uc0
        in (Maybes.cases (Lists.uncons (Pairs.second uc0)) arityErr (\uc1 ->
...
typeOfLambda cx tx typeArgs l =
      ...
      in (Eithers.bind (Maybes.cases mdom (Left (Errors.ErrorChecking (Checking.CheckingErrorUntypedLambda (Checking.UntypedLambdaError {
      })))) (\dom ->
...
typeOfLet cx tx typeArgs letTerm =
      ...
      let bs = Core.letBindings letTerm
          ...
          bindingType =
                  \b -> Maybes.cases (Core.bindingTypeScheme b) (Left (Errors.ErrorChecking (Checking.CheckingErrorUntypedLetBinding (Checking.UntypedLetBindingError {
...
typeOfList cx tx typeArgs els =
      ...
      let listArityErr =
              Left (Errors.ErrorChecking (Checking.CheckingErrorTypeArityMismatch (Checking.TypeArityMismatchError {
                ...
                Checking.typeArityMismatchErrorTypeArguments = typeArgs})))
      in (Logic.ifElse (Lists.null els) (Logic.ifElse (Equality.equal (Lists.length typeArgs) 1) (Maybes.cases (Lists.maybeHead typeArgs) listArityErr (\ta0 -> Right (Core.TypeList ta0, cx))) listArityErr) (
        let foldResult =
                Lists.foldl (\acc -> \term -> Eithers.bind acc (\accR ->
                  let types = Pairs.first accR
                      cxA = Pairs.second accR
...
typeOfMap cx tx typeArgs m =
      ...
      let mapArityErr =
              Left (Errors.ErrorChecking (Checking.CheckingErrorTypeArityMismatch (Checking.TypeArityMismatchError {
                ...
                Checking.typeArityMismatchErrorActualArity = (Lists.length typeArgs),
                Checking.typeArityMismatchErrorTypeArguments = typeArgs})))
      in (Logic.ifElse (Maps.null m) (Logic.ifElse (Equality.equal (Lists.length typeArgs) 2) (Maybes.cases (Lists.uncons typeArgs) mapArityErr (\uc0 ->
        let ta0 = Pairs.first uc0
        in (Maybes.cases (Lists.uncons (Pairs.second uc0)) mapArityErr (\uc1 ->
          let ta1 = Pairs.first uc1
          in (Right (
                ...
                let n = Lists.length typeArgs
                    maybeArityErr =
                            Left (Errors.ErrorChecking (Checking.CheckingErrorTypeArityMismatch (Checking.TypeArityMismatchError {
                              ...
                              Checking.typeArityMismatchErrorActualArity = n,
                              Checking.typeArityMismatchErrorTypeArguments = typeArgs})))
                in (Logic.ifElse (Equality.equal n 1) (Maybes.cases (Lists.maybeHead typeArgs) maybeArityErr (\ta0 -> Right (Core.TypeMaybe ta0, cx))) maybeArityErr)
          forJust =
                  \term -> Eithers.bind (typeOf cx tx [] term) (\tResult ->
                    ...
                    in (Eithers.bind (applyTypeArgumentsToType cx2 tx typeArgs t) (\applied -> Right (applied, cx2))))
      in (Maybes.cases mt forNothing forJust)
...
typeOfPrimitive cx tx typeArgs name =
      ...
      in (Maybes.cases rawTs (Left (Errors.ErrorUndefinedTermVariable (ErrorCore.UndefinedTermVariableError {
...
typeOfSet cx tx typeArgs els =
      ...
      let setArityErr =
              Left (Errors.ErrorChecking (Checking.CheckingErrorTypeArityMismatch (Checking.TypeArityMismatchError {
                ...
                Checking.typeArityMismatchErrorActualArity = (Lists.length typeArgs),
                Checking.typeArityMismatchErrorTypeArguments = typeArgs})))
      in (Logic.ifElse (Sets.null els) (Logic.ifElse (Equality.equal (Lists.length typeArgs) 1) (Maybes.cases (Lists.maybeHead typeArgs) setArityErr (\ta0 -> Right (Core.TypeSet ta0, cx))) setArityErr) (
        let foldResult =
                Lists.foldl (\acc -> \term -> Eithers.bind acc (\accR ->
                  let types = Pairs.first accR
                      cxA = Pairs.second accR
          ...
          forScheme =
                  \ts ->
                    let tResult =
                        ...
                        cx2 = Pairs.second tResult
                    in (Eithers.bind (applyTypeArgumentsToType cx2 tx typeArgs t) (\applied -> Right (applied, cx2)))
      in (Maybes.cases rawTypeScheme (Maybes.cases (Maybes.map (\_p -> Scoping.termSignatureToTypeScheme (Packaging.primitiveDefinitionSignature (Graph.primitiveDefinition _p))) (Maps.lookup name (Graph.graphPrimitives tx))) (Left (Errors.ErrorUntypedTermVariable (ErrorCore.UntypedTermVariableError {
