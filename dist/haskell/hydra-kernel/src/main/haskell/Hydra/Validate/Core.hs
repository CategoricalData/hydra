          ...
          case fun of
            Core.TermFunction v1 -> case v1 of
              Core.FunctionLambda v2 ->
                ...
                in case body of
                  Core.TermVariable v3 -> Logic.ifElse (Equality.equal param v3) (Just (ErrorCore.InvalidTermErrorUnnecessaryIdentityApplication (ErrorCore.UnnecessaryIdentityApplicationError {
              ...
              Core.FunctionElimination v2 -> case v2 of
                Core.EliminationWrap v3 -> case arg of
                  Core.TermWrap v4 ->
                    ...
                    in (Logic.ifElse (Equality.equal v3 wrapName) (Just (ErrorCore.InvalidTermErrorRedundantWrapUnwrap (ErrorCore.RedundantWrapUnwrapError {
                      ErrorCore.redundantWrapUnwrapErrorLocation = path,
                      ErrorCore.redundantWrapUnwrapErrorTypeName = v3}))) Nothing)
                  _ -> Nothing
                _ -> Nothing
              _ -> Nothing
            _ -> Nothing])
      ...
      Core.TermFunction v0 -> case v0 of
        Core.FunctionLambda v1 ->
          ...
          in (firstError [
            ...
            (Logic.ifElse typed (Maybes.cases (Core.lambdaDomain v1) Nothing (\dom -> checkUndefinedTypeVariablesInType path cx dom (\uvName -> Just (ErrorCore.InvalidTermErrorUndefinedTypeVariableInLambdaDomain (ErrorCore.UndefinedTypeVariableInLambdaDomainError {
