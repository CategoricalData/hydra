...
context cx raw =
    Eithers.either (\err -> Left err) (\stripped -> case stripped of
      Core.TermRecord v0 ->
        ...
        in (Eithers.bind (Core__.requireField "trace" (Core__.decodeList (\cx2 -> \raw2 -> Eithers.either (\err -> Left err) (\stripped2 -> case stripped2 of
          Core.TermLiteral v1 -> case v1 of
            Core.LiteralString v2 -> Right v2
            _ -> Left (Errors.DecodingError "expected string literal")
          _ -> Left (Errors.DecodingError "expected literal")) (Core__.stripWithDecodingError cx2 raw2))) fieldMap cx) (\field_trace -> Eithers.bind (Core__.requireField "messages" (Core__.decodeList (\cx2 -> \raw2 -> Eithers.either (\err -> Left err) (\stripped2 -> case stripped2 of
          Core.TermLiteral v1 -> case v1 of
            Core.LiteralString v2 -> Right v2
            _ -> Left (Errors.DecodingError "expected string literal")
          _ -> Left (Errors.DecodingError "expected literal")) (Core__.stripWithDecodingError cx2 raw2))) fieldMap cx) (\field_messages -> Eithers.bind (Core__.requireField "other" (Core__.decodeMap Core_.name Core_.term) fieldMap cx) (\field_other -> Right (Context.Context {
