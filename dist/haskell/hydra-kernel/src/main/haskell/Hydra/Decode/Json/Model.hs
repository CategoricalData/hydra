            ...
            variantMap =
                    Maps.fromList [
                      ...
                      (Core.Name "number", (\input -> Eithers.map (\t -> Model.ValueNumber t) (Eithers.either (\err -> Left err) (\stripped2 -> case stripped2 of
                        ...
                        _ -> Left (Errors.DecodingError "expected literal")) (Core_.stripWithDecodingError cx input)))),
                      (Core.Name "object", (\input -> Eithers.map (\t -> Model.ValueObject t) (Core_.decodeMap (\cx2 -> \raw2 -> Eithers.either (\err -> Left err) (\stripped2 -> case stripped2 of
                        Core.TermLiteral v1 -> case v1 of
                          Core.LiteralString v2 -> Right v2
                          _ -> Left (Errors.DecodingError "expected string literal")
                        _ -> Left (Errors.DecodingError "expected literal")) (Core_.stripWithDecodingError cx2 raw2)) value cx input))),
