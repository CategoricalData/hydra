...
decodeField decodeValue name m =
    Eithers.bind (decodeOptionalField decodeValue name m) (\mf -> Maybes.cases mf (Left (Strings.cat2 "missing field: " name)) (\f -> Right f))
...
decodeOptionalField decodeValue name m =
    Maybes.cases (Maps.lookup name m) (Right Nothing) (\v -> Eithers.map (\x -> Just x) (decodeValue v))
