...
evaluateEdge cx g edgeSpec record =
      ...
      in (Eithers.bind (Reduction.reduceTerm cx g True (Core.TermApplication (Core.Application {
        ...
        Core.applicationArgument = record}))) (\_term -> ExtractCore.maybeTerm (\t -> Right t) g _term)) (\mInId -> Eithers.bind (evaluateProperties cx g propSpecs record) (\props -> Right (Optionals.bind mOutId (\outId -> Optionals.map (\inId -> PgModel.Edge {
...
evaluateVertex cx g vertexSpec record =
      ...
      in (Eithers.bind (Eithers.bind (Reduction.reduceTerm cx g True (Core.TermApplication (Core.Application {
        Core.applicationFunction = idSpec,
        Core.applicationArgument = record}))) (\_term -> ExtractCore.maybeTerm (\t -> Right t) g _term)) (\mId -> Eithers.bind (evaluateProperties cx g propSpecs record) (\props -> Right (Optionals.map (\id -> PgModel.Vertex {
