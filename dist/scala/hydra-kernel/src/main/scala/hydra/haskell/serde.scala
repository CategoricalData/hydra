...
def alternativeToExpr(alt: hydra.haskell.syntax.Alternative): hydra.ast.Expr =
  hydra.serialization.structuralSpaceSep(Seq(hydra.haskell.serde.patternToExpr(alt.pattern),
     hydra.serialization.cst("->"), hydra.haskell.serde.caseRhsToExpr(alt.rhs)))
...
def caseExpressionToExpr(caseExpr: hydra.haskell.syntax.CaseExpression): hydra.ast.Expr =
  ...
  lazy val ofOp: hydra.ast.Op = hydra.ast.Op("of", hydra.ast.Padding(hydra.ast.Ws.space,
     hydra.ast.Ws.breakAndIndent("  ")), 0, hydra.ast.Associativity.none)
  lazy val lhs: hydra.ast.Expr = hydra.serialization.spaceSep(Seq(hydra.serialization.cst("case"),
...
def ifExpressionToExpr(ifExpr: hydra.haskell.syntax.IfExpression): hydra.ast.Expr =
  ...
  lazy val ifOp: hydra.ast.Op = hydra.ast.Op("", hydra.ast.Padding(hydra.ast.Ws.none,
     hydra.ast.Ws.breakAndIndent("  ")), 0, hydra.ast.Associativity.none)
...
def importToExpr(`import`: hydra.haskell.syntax.Import): hydra.ast.Expr =
  ...
  lazy val parts: Seq[hydra.ast.Expr] = hydra.lib.maybes.cat[hydra.ast.Expr](Seq(Some(hydra.serialization.cst("import")),
     ...
     Some(hydra.serialization.cst(name)), hydra.lib.maybes.map[hydra.haskell.syntax.ModuleName,
     hydra.ast.Expr]((m: hydra.haskell.syntax.ModuleName) => hydra.serialization.cst(hydra.lib.strings.cat2("as ")(m)))(mod),
...
def moduleToExpr(module: hydra.haskell.syntax.Module): hydra.ast.Expr =
  ...
  lazy val headerLine: Seq[hydra.ast.Expr] = hydra.lib.maybes.maybe[Seq[hydra.ast.Expr],
     hydra.haskell.syntax.ModuleHead](Seq())((h: hydra.haskell.syntax.ModuleHead) => Seq(hydra.haskell.serde.moduleHeadToExpr(h)))(mh)
  ...
  hydra.serialization.doubleNewlineSep(hydra.lib.lists.concat[hydra.ast.Expr](Seq(warning,
...
def nameToExpr(name: hydra.haskell.syntax.Name): hydra.ast.Expr =
  ...
  case hydra.haskell.syntax.Name.parens(v_Name_parens_qn) => hydra.lib.strings.cat(Seq("(",
     hydra.haskell.serde.writeQualifiedName(v_Name_parens_qn), ")")))
...
def typeSignatureToExpr(typeSig: hydra.haskell.syntax.TypeSignature): hydra.ast.Expr =
  ...
  lazy val inlineSig: hydra.ast.Expr = hydra.serialization.structuralSpaceSep(Seq(nameExpr,
     hydra.serialization.cst("::"), typeExpr))
...
def valueBindingToExpr(vb: hydra.haskell.syntax.ValueBinding): hydra.ast.Expr =
  ...
  case hydra.haskell.syntax.ValueBinding.simple(v_ValueBinding_simple_simpleVB) => {
    ...
    lazy val inlineBody: hydra.ast.Expr = hydra.serialization.structuralSpaceSep(Seq(lhsExpr,
       hydra.serialization.cst("="), rhsExpr))
