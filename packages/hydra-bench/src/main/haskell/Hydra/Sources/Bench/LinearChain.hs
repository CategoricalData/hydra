...
--   3. Lazy-alternative @Optionals.match@ (so eager evaluation in let-bindings is exposed)
  ...
  "t" ~>
    ...
    cases _Term (var "stripped") (Just $ just (var "stripped")) [
      _Term_application>>: "app" ~>
        ...
        -- Pattern 3 + 4: lazy-alternative Optionals.match + recursion via walkerRef
        Optionals.match (walkerRef (k-1) @@ var "fun") (walkerRef (k-1) @@ var "arg") ("_" ~> walkerRef (k-1) @@ var "fun"),
      _Term_lambda>>: "lam" ~>
        "body" <~ Core.lambdaBody (var "lam") $
        walkerRef (k-1) @@ var "body",
      _Term_let>>: "le" ~>
        "body" <~ Core.letBody (var "le") $
        Optionals.match (walkerRef (k-1) @@ var "body") nothing ("inner" ~> walkerRef (k-1) @@ var "inner"),
