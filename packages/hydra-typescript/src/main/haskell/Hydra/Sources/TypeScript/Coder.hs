...
collectInnerTypeImports :: TTermDefinition (Namespace -> Term -> S.Set Name)
  ...
  lambda "cx" $ lambda "g" $ lambda "currentNs" $ lambda "term" $ cases _Term (var "term")
    ...
    [_Term_annotated>>: lambda "at" $
     ...
     _Term_lambda>>: lambda "_lam" $
       ...
       "fsLE" <~ (Analysis.analyzeFunctionTerm @@ var "cx"
                  @@ (lambda "e" $ var "e")
       ...
       "paramPatterns" <~ (Lists.map
         (lambda "pn" $ tsTypedIdent
           @@ (Formatting.sanitizeWithUnderscores
                 @@ TypeScriptLanguageSource.typeScriptReservedWords
                 @@ (Names.localNameOf @@ var "pn"))
...
encodeBindingAsStatement :: TTermDefinition (Context -> Graph -> Namespace -> Binding -> TS.Statement)
...
functionDeclarationFromTerm :: TTermDefinition (Context -> Graph -> Namespace -> String -> Term -> Maybe Type -> TS.FunctionDeclaration)
functionDeclarationFromTerm = def "functionDeclarationFromTerm" $
  "cx" ~> "g" ~> "currentNs" ~> "lname" ~> "term" ~> "_mScheme" ~>
    "fsE" <~ (Analysis.analyzeFunctionTerm @@ var "cx"
              @@ (lambda "e" $ var "e")
              @@ (lambda "newG" $ lambda "_old" $ var "newG")
              @@ var "g" @@ var "term") $
    "fs" <~ Eithers.either_
...
encodeTermDefinition = def "encodeTermDefinition" $
  lambda "cx" $ lambda "g" $ lambda "currentNs" $ lambda "td" $
    ...
    "mScheme" <~ (Maybes.bind (Maybes.map Scoping.termSignatureToTypeScheme $ Packaging.termDefinitionSignature (var "td"))
      ("ts" ~> just (Core.typeSchemeBody (var "ts")))) $
...
moduleToTypeScript = def "moduleToTypeScript" $
  "mod" ~> "defs" ~> "cx" ~> "g" ~>
    ...
    "typeImportsFromTerms" <~ (Lists.foldl
      (lambda "acc" $ lambda "td" $
        Maybes.cases (Maybes.map Scoping.termSignatureToTypeScheme $ Packaging.termDefinitionSignature (var "td"))
          (var "acc")
          (lambda "ts" $ Sets.union (var "acc")
