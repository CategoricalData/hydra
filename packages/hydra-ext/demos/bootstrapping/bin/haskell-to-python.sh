...
#
# Usage: ./haskell-to-python.sh [--output DIR] [--types-only] [--kernel-only]
...
echo "=========================================="
echo "Haskell to Python bootstrapping demo"
echo "=========================================="
echo ""
...
echo "Step 3: Generating Python code (via Haskell host)..."
STEP_START=$(date +%s)
"$SCRIPT_DIR/invoke-haskell-host.sh" --target python --output "$OUTPUT_BASE" --include-tests $EXTRA_FLAGS 2>&1
STEP_END=$(date +%s)
...
if [ $TEST_EXIT -eq 0 ]; then
    echo "Bootstrap Haskell-to-Python: COMPLETE (tests PASSED)"
else
    echo "Bootstrap Haskell-to-Python: COMPLETE (tests FAILED)"
fi
echo "  Output:     $OUTPUT_DIR"
