...
constructModule = def "constructModule" $
  ...
  "cx" ~> "g" ~> "mod" ~> "typeDefs" ~> lets [
    ...
    "toDef">: "td" ~> lets [
      ...
      (match _Type (Strip.deannotateType @@ var "flatTyp")
    ...
    "descOption">: record P3._Option [
      ...
      P3._Option_value>>: inject P3._Value P3._Value_string $
        Strings.concat2
          (Optionals.match (var "desc") (string "") ("d" ~> Strings.concat2 (var "d") (string "\n\n")))
          (asTerm Constants.warningAutoGeneratedFile)],
    "checkFieldType_wrapper">: "typ" ~> match _Type (Strip.deannotateType @@ var "typ")
      (Just false) [
      _Type_optional>>: "ot" ~> match _Type (Strip.deannotateType @@ var "ot")
        (Just false) [
      ...
      Lists.foldl ("b" ~> "t" ~> Logic.or (var "b")
        (Rewriting.foldOverType @@ Coders.traversalOrderPre @@
          ("b2" ~> "t2" ~> Logic.or (var "b2") (lets [
            ...
            Optionals.match (var "checkResult") (match _Type (var "t2")
                (Just false) [
...
encodeDefinition = def "encodeDefinition" $
  ...
  "cx" ~> "g" ~> "localNs" ~> "name" ~> "typ" ~> lets [
    ...
    "encode">: "cx0" ~> "options" ~> "t" ~>
      match _Type (asTerm simplifyType @@ var "t")
...
encodeEnumValueName = def "encodeEnumValueName" $
  ...
  "tname" ~> "fname" ~> lets [
    ...
    wrap P3._EnumValueName (Strings.concat (list [var "prefix", string "_", var "suffix"]))
...
encodeFieldType = def "encodeFieldType" $
  ...
  "cx" ~> "g" ~> "localNs" ~> "ft" ~> lets [
    ...
    "encodeType_">: "cx0" ~> "g0" ~> "ns0" ~> "typ" ~>
      match _Type (asTerm simplifyType @@ var "typ")
        (Just $ Eithers.map
...
encodeTypeReference = def "encodeTypeReference" $
  ...
  "localNs" ~> "name" ~> lets [
    ...
    "localNsParts">: Optionals.withDefault (list ([] :: [TypedTerm String])) (Lists.init (Strings.splitOn (string ".") (unwrap _ModuleName @@ var "localNs")))] $
    wrap P3._TypeName $
      Optionals.match (var "ns_") (var "local") ("nsVal" ~> lets [
          "nsParts">: Optionals.withDefault (list ([] :: [TypedTerm String])) (Lists.init (Strings.splitOn (string ".") (unwrap _ModuleName @@ var "nsVal")))] $
          Logic.ifElse (Equality.equal (var "nsParts") (var "localNsParts"))
            (var "local")
            (Strings.join (string ".") (Lists.concat (list [var "nsParts", list [var "local"]]))))
    ...
    "bdep" <<~ (asTerm readBooleanAnnotation @@ var "cx" @@ var "g" @@ Constants.keyDeprecated @@ var "typ") $ lets [
      ...
      "mdepAnn">: Logic.ifElse (var "bdep")
        (just $ record P3._Option [
          ...
          P3._Option_value>>: inject P3._Value P3._Value_boolean true])
        nothing] $
      right $ Optionals.givens (list [var "mdescAnn", var "mdepAnn"])
    ...
    "makeField">: "cx0" ~> "fname" ~> "ftyp" ~>
      "ft" <<~ (asTerm encodeSimpleTypeForHelper @@ var "cx0" @@ var "localNs" @@ var "ftyp") $ lets [
        ...
        right $ pair
          ...
          (var "cx1_")] $
    cases _StructuralTypeRef Nothing [
      (Name "either")>>: "p" ~> lets [
    ...
    wrap P3._PackageName $
      Strings.join (string ".")
        (Lists.map
          ...
          (Optionals.withDefault (list ([] :: [TypedTerm String])) (Lists.init (Strings.splitOn (string ".") (unwrap _ModuleName @@ var "ns_")))))
...
readBooleanAnnotation = def "readBooleanAnnotation" $
  ...
  "cx" ~> "g" ~> "key" ~> "typ" ~>
    Optionals.match (Maps.lookup (var "key") (Annotations.typeAnnotationInternal @@ var "typ")) (right false) ("term" ~> ExtractCore.boolean @@ var "g" @@ var "term")
...
structuralTypeName = def "structuralTypeName" $
  ...
  "localNs" ~> "ref" ~> lets [
    ...
    wrap P3._TypeName $
      cases _StructuralTypeRef Nothing [
        (Name "either")>>: "p" ~>
          Strings.concat (list [string "Either_",
            var "typeSuffix" @@ (Pairs.first (var "p")),
            string "_",
            var "typeSuffix" @@ (Pairs.second (var "p"))]),
        (Name "pair")>>: "p" ~>
          Strings.concat (list [string "Pair_",
            var "typeSuffix" @@ (Pairs.first (var "p")),
            string "_",
...
unexpectedE = def "unexpectedE" $
  ...
  "cx" ~> "expected" ~> "found" ~>
  asTerm err @@ var "cx" @@ (Strings.concat (list [string "Expected ", var "expected", string ", found: ", var "found"]))
