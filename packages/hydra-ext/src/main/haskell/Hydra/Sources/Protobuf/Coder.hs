  ...
  "cx" ~> "g" ~> "mod" ~> "typeDefs" ~> lets [
    ...
    "descOption">: record P3._Option [
      ...
      P3._Option_value>>: inject P3._Value P3._Value_string $
        Strings.cat2
          (Maybes.cases (var "desc") (string "") ("d" ~> Strings.cat2 (var "d") (string "\n\n")))
      ...
      Lists.foldl ("b" ~> "t" ~> Logic.or (var "b")
        (Rewriting.foldOverType @@ Coders.traversalOrderPre @@
          ("b2" ~> "t2" ~> Logic.or (var "b2") (lets [
            ...
            Maybes.cases (var "checkResult") (cases _Type (var "t2")
                (Just false) [
...
encodeTypeReference = def "encodeTypeReference" $
  ...
  "localNs" ~> "name" ~> lets [
    ...
    wrap P3._TypeName $
      Maybes.cases (var "ns_") (var "local") ("nsVal" ~> lets [
...
readBooleanAnnotation = def "readBooleanAnnotation" $
  ...
  "cx" ~> "g" ~> "key" ~> "typ" ~>
    Maybes.cases (Maps.lookup (var "key") (Annotations.typeAnnotationInternal @@ var "typ")) (right false) ("term" ~> ExtractCore.boolean @@ var "g" @@ var "term")
