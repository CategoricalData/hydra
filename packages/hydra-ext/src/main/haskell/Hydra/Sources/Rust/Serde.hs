...
module_ = Module {
  ...
  where
    ...
    -- section (top-level serialization, use declarations, struct/enum/function definitions, type
    -- aliases, const/static/module, impl blocks, trait definitions, generics/where clauses, types,
    -- expressions, statements, patterns, literals, visibility/attributes, comments); see the
    -- section comments below for the grouping.
    definitions = [
...
crateToExpr = define "crateToExpr" $
  ...
  lambda "crate" $
    Serialization.doubleNewlineSep @@ (Lists.map (asTerm itemWithCommentsToExpr)
  ...
  lambda "derives" $
    Logic.ifElse (Lists.null $ var "derives")
      ...
      (just $ Serialization.cst @@ (Strings.concat (list [
        string "#[derive(",
        Strings.join (string ", ") (var "derives"),
        string ")]"])))
...
exprPathToExpr = define "exprPathToExpr" $
  ...
  lambda "ep" $ lets [
    ...
    Serialization.cst @@ (Strings.concat2 (var "prefix") (Strings.join (string "::") (var "segStrs")))
...
expressionToExpr = define "expressionToExpr" $
  ...
  lambda "expr" $
    cases R._Expression (var "expr") Nothing [
      ...
      R._Expression_tuple>>: lambda "es" $ Serialization.parenListAdaptive @@ (Lists.map (asTerm expressionToExpr) (var "es")),
      ...
      R._Expression_await>>: lambda "e" $ Serialization.cst @@ (Strings.concat2 (Serialization.printExpr @@ (expressionToExpr @@ var "e")) (string ".await")),
...
fnParamToExpr = define "fnParamToExpr" $
  ...
  lambda "param" $ lets [
    ...
    Serialization.spaceSep @@ list [
      Serialization.cst @@ (Strings.concat2 (Serialization.printExpr @@ (patternToExpr @@ var "pat")) (string ":")),
      typeToExpr @@ var "typ"]
  ...
  lambda "args" $
    cases R._GenericArguments (var "args") Nothing [
      ...
      R._GenericArguments_angleBracketed>>: lambda "ab" $ lets [
        ...
        just $ Serialization.angleBracesList @@ Serialization.inlineStyle @@ (Lists.map (asTerm genericArgToExpr) (var "args")),
...
implBlockToExpr = define "implBlockToExpr" $
  ...
  lambda "i" $ lets [
    ...
    "header">: Serialization.spaceSep @@ Optionals.givens (list [
      ...
      var "wherePart"]),
    "body">: Serialization.curlyBracesList @@ nothing @@ Serialization.halfBlockStyle @@
      (Lists.map (asTerm implItemToExpr) (var "items"))] $
...
itemWithCommentsToExpr = define "itemWithCommentsToExpr" $
  ...
  lambda "iwc" $ lets [
    ...
    Serialization.newlineSep @@ Lists.concat (list [var "docPart", list [
      Serialization.spaceSep @@ Optionals.givens (list [var "visPart", just (var "itemPart")])]])
...
macroInvocationToExpr = define "macroInvocationToExpr" $
  ...
  lambda "m" $ lets [
    ...
    "close">: cases R._MacroDelimiter (var "delim") Nothing [
      ...
      R._MacroDelimiter_bracket>>: constant $ string "]",
      R._MacroDelimiter_brace>>: constant $ string "}"]] $
    Serialization.cst @@ (Strings.concat (list [var "pathStr", string "!", var "open", var "tokens", var "close"]))
...
methodCallExprToExpr = define "methodCallExprToExpr" $
  ...
  lambda "m" $ lets [
    ...
    "turboPart">: Logic.ifElse (Lists.null $ var "turbo")
      ...
      (Strings.concat (list [
        string "::<",
        Strings.join (string ", ") (Lists.map (lambda "t" $ Serialization.printExpr @@ (typeToExpr @@ var "t")) (var "turbo")),
        string ">"]))] $
    Serialization.cst @@ (Strings.concat (list [
      ...
      var "turboPart",
      string "(",
      Strings.join (string ", ") (Lists.map (lambda "a" $ Serialization.printExpr @@ (expressionToExpr @@ var "a")) (var "args")),
      string ")"]))
...
modDefToExpr = define "modDefToExpr" $
  ...
  lambda "m" $ lets [
    ...
    Optionals.cases (var "body") (Serialization.spaceSep @@ list [
        ...
        Serialization.cst @@ var "name",
        Serialization.curlyBracesList @@ nothing @@ Serialization.halfBlockStyle @@
          (Lists.map (asTerm itemToExpr) (var "items"))])
...
patternToExpr = define "patternToExpr" $
  ...
  lambda "pat" $
    cases R._Pattern (var "pat") Nothing [
      ...
      R._Pattern_tuple>>: lambda "ps" $ Serialization.parenListAdaptive @@ (Lists.map (asTerm patternToExpr) (var "ps")),
      R._Pattern_slice>>: lambda "ps" $ Serialization.bracketList @@ Serialization.halfBlockStyle @@ (Lists.map (asTerm patternToExpr) (var "ps")),
      R._Pattern_or>>: lambda "ps" $ Serialization.cst @@ (Strings.join (string " | ") (Lists.map (lambda "p" $ Serialization.printExpr @@ (patternToExpr @@ var "p")) (var "ps"))),
...
refExprToExpr = define "refExprToExpr" $
  ...
  lambda "r" $ lets [
    ...
    Serialization.cst @@ (Strings.concat2 (var "prefix") (Serialization.printExpr @@ (expressionToExpr @@ var "expr")))
...
refPatternToExpr = define "refPatternToExpr" $
  ...
  lambda "rp" $ lets [
    ...
    "prefix">: Logic.ifElse (var "mut") (string "&mut ") (string "&")] $
    Serialization.cst @@ (Strings.concat2 (var "prefix") (Serialization.printExpr @@ (patternToExpr @@ var "pat")))
...
staticDefToExpr = define "staticDefToExpr" $
  ...
  lambda "s" $ lets [
    ...
    Serialization.spaceSep @@ Optionals.givens (list [
      ...
      var "mutKw",
      just $ Serialization.cst @@ (Strings.concat2 (var "name") (string ":")),
...
structPatternToExpr = define "structPatternToExpr" $
  ...
  lambda "sp" $ lets [
    ...
    "fieldExprs">: Lists.map (asTerm fieldPatternToExpr) (var "fields"),
...
traitDefToExpr = define "traitDefToExpr" $
  ...
  lambda "t" $ lets [
    ...
    "header">: Serialization.spaceSep @@ Optionals.givens (list [
      ...
      var "wherePart"]),
    "body">: Serialization.curlyBracesList @@ nothing @@ Serialization.halfBlockStyle @@
      (Lists.map (asTerm traitItemToExpr) (var "items"))] $
    Serialization.newlineSep @@ Lists.concat (list [
      var "docPart",
...
tupleStructPatternToExpr = define "tupleStructPatternToExpr" $
  ...
  lambda "tsp" $ lets [
    ...
    Serialization.spaceSep @@ list [
      exprPathToExpr @@ var "path",
      Serialization.parenListAdaptive @@ (Lists.map (asTerm patternToExpr) (var "elems"))]
    ...
    cases R._Type (var "typ") Nothing [
      ...
      R._Type_array>>: lambda "at" $ lets [
        ...
        Serialization.cst @@ (Strings.concat (list [
          ...
          Serialization.printExpr @@ (expressionToExpr @@ var "len"),
          string "]"])),
      R._Type_tuple>>: lambda "ts" $ Serialization.parenListAdaptive @@ (Lists.map (asTerm typeToExpr) (var "ts")),
      R._Type_fnPointer>>: lambda "fp" $ lets [
        ...
        Serialization.spaceSep @@ list [
          ...
          Serialization.parenListAdaptive @@ (Lists.map (asTerm typeToExpr) (var "params")),
          Serialization.cst @@ string "->",
          typeToExpr @@ var "ret"],
  ...
  lambda "tree" $
    cases R._UseTree (var "tree") Nothing [
      ...
      R._UseTree_rename>>: lambda "r" $ lets [
        ...
        Serialization.spaceSep @@ list [
          Serialization.cst @@ (Strings.join (string "::") (var "path")),
          Serialization.cst @@ string "as",
          Serialization.cst @@ var "alias"],
      R._UseTree_glob>>: lambda "segs" $
        Serialization.cst @@ (Strings.concat2 (Strings.join (string "::") (var "segs")) (string "::*")),
      R._UseTree_group>>: lambda "g" $ lets [
        ...
        Serialization.cst @@ (Strings.concat (list [
          var "prefixStr",
          string "{",
          Strings.join (string ", ") (Lists.map (lambda "t" $ Serialization.printExpr @@ (useTreeToExpr @@ var "t")) (var "trees")),
          string "}"]))]
  ...
  lambda "wc" $ lets [
    ...
    "predExprs">: Lists.map (lambda "p" $ lets [
      ...
      Serialization.spaceSep @@ list [
        typeToExpr @@ var "typ",
        Serialization.cst @@ string ":",
        Serialization.cst @@ (Strings.join (string " + ") (Lists.map (lambda "b" $ Serialization.printExpr @@ (typeParamBoundToExpr @@ var "b")) (var "bounds")))]) (var "preds")] $
    Serialization.spaceSep @@ list [
