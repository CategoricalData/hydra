...
pass (`bin/lib/inject-scaladoc-crosslinks.py`): it harvests scaladoc's own symbol index from a throwaway
unified build and rewrites each unresolved cross-reference to the owning dependency's published page,
linking only names that resolve unambiguously to a dependency symbol.
