...
Per-package Scala artifacts are ready to publish to Maven Central under `net.fortytwo.hydra`
(the same group ID as the Java artifacts), using sbt-sonatype + sbt-pgp.
The publish set mirrors the Java set — 8 packages: hydra-kernel, hydra-haskell, hydra-java,
...
```bash
heads/scala/bin/publish-sbt.sh       # publishLocal all 8 packages in dependency order
```
