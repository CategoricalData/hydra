...
Host-native DSL source. The Python analog of hydra.java.gradle.GradleBuildConfiguration
...
Unlike hydra.java.gradle (Gradle spans multiple languages, so it is keyed by build system),
...
shared with hydra.java.gradle via hydra.core.packaging; only the build-system-specific shell
...
PACKAGING_NS = ModuleName("hydra.core.packaging")
...
# The rich field docs live on the Java hydra.java.gradle analog; this module leaves the
...
def _packaging(local: str) -> Type:
    """Reference to a type in hydra.core.packaging: hydra.core.packaging.<local>."""
