...
def _decode_py_comparison_to_py_await_primary():
    body = lets(
        [
            ...
            field("shiftLhs", PySyn.shift_expression_lhs(var("andRhs"))),
            field("shiftRhs", PySyn.shift_expression_rhs(var("andRhs"))),
            field("sumLhs", PySyn.sum_lhs(var("shiftRhs"))),
            field("sumRhs", PySyn.sum_rhs(var("shiftRhs"))),
            field("termLhs", PySyn.term_lhs(var("sumRhs"))),
            field("termRhs", PySyn.term_rhs(var("sumRhs"))),
        ],
        _short_circuit_to_nothing([
            Logic.not_(Lists.is_empty(var("rhs"))),
...
def _indented_block():
    body = lets(
        [
            ...
            field("groups",
                Lists.filter(
                    lam("g", Logic.not_(Lists.is_empty(var("g")))),
                    Lists.cons(var("commentGroup"), var("stmts")),
                ),
            ),
        ],
        Logic.if_else(
            Lists.is_empty(var("groups")),
...
def _or_expression():
    ...
    build_body = Optionals.match(Lists.uncons(var("ps")), # Unreachable fallback
        PySyn.bitwise_or(
            ...
            "p",
            Logic.if_else(
                Lists.is_empty(Pairs.second(var("p"))),
                PySyn.bitwise_or(
                    var("prev"),
