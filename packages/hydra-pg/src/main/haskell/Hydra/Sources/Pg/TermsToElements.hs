...
decodeValueSpec = define "decodeValueSpec" $
  ...
  "cx" ~> "g" ~> "term" ~>
    ...
    match _Term (Strip.deannotateTerm @@ var "term") (Just $
      readInjection @@ var "cx" @@ var "g"
        @@ list [
...
parseElementSpec = define "parseElementSpec" $
  ...
  "cx" ~> "g" ~> "schema" ~> "spec" ~>
    cases PGM._ElementSpec Nothing [
...
termToString = define "termToString" $
  ...
  "term" ~>
    match _Term (Strip.deannotateTerm @@ var "term")
      ...
      _Term_literal>>: "lit" ~>
        match _Literal (var "lit") (Just $ PrintCore.term @@ var "term") [
