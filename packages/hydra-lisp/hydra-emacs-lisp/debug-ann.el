...
(let* ((cx (hydra-empty-context))
  ...
  (if (eq (car result) :left)
    ...
    (let ((actual (cadr result)))
      ...
      (message "show-term: %s" (funcall hydra_print_core_term actual))
      ...
      (let* ((suite hydra_test_test_suite_all_tests)
        ...
        (message "show-expected: %s" (funcall hydra_print_core_term expected))
        (message "equal? %s" (equal actual expected))
        (message "show-equal? %s" (equal (funcall hydra_print_core_term actual)
                                          (funcall hydra_print_core_term expected)))))))
