...
(def hydra_ext_org_json_decoding_decode_optional_field (fn [decode_value] (fn [name] (fn [m] (((hydra_lib_maybes_maybe (fn [] (list :right (list :nothing)))) (fn [v] ((hydra_lib_eithers_map (fn [x] (list :just x))) (decode_value v)))) ((hydra_lib_maps_lookup name) m))))))
