...
alternativeToExpr :: TypedTermDefinition (H.Alternative -> Expr)
...
applicationExpressionToExpr :: TypedTermDefinition (H.ApplicationExpression -> Expr)
...
applicationPatternToExpr :: TypedTermDefinition (H.ApplicationPattern -> Expr)
...
classConstraintToExpr :: TypedTermDefinition (H.ClassConstraint -> Expr)
...
constructorToExpr = haskellSerdeDefinition "constructorToExpr" $
  ...
  lambda "cons" $ lets [
    ...
    Optionals.match (var "mc") (var "body") (lambda "c" $ Serialization.newlineSep @@ (Lists.cons (Serialization.cst @@ (toHaskellComments @@ var "c")) (list [var "body"])))
...
declarationHeadToExpr :: TypedTermDefinition (H.DeclarationHead -> Expr)
...
declarationToExpr :: TypedTermDefinition (H.Declaration -> Expr)
  ...
  lambda "decl" $ lets [
    ...
    "body">: match H._Declaration (var "decl") Nothing [
      H._Declaration_data>>: lambda "dataDecl" $ lets [
        ...
        "derivingClause">: Logic.ifElse (Lists.isEmpty $ var "derivCat")
          (list ([] :: [TypedTerm Expr]))
...
ifExpressionToExpr :: TypedTermDefinition (H.IfExpression -> Expr)
...
importToExpr :: TypedTermDefinition (H.Import -> Expr)
...
--literalToExpr :: TypedTermDefinition (H.Literal -> Expr)
...
moduleHeadToExpr :: TypedTermDefinition (H.ModuleHead -> Expr)
moduleHeadToExpr = haskellSerdeDefinition "moduleHeadToExpr" $
  ...
  lambda "moduleHead" $ lets [
    ...
    Optionals.match (var "mc") (var "head") (lambda "c" $ Serialization.newlineSep @@ (Lists.cons
...
moduleToExpr = haskellSerdeDefinition "moduleToExpr" $
  ...
  lambda "module" $ lets [
    ...
    "headerLine">: Optionals.match (var "mh") (list ([] :: [TypedTerm Expr])) (lambda "h" $ list [moduleHeadToExpr @@ var "h"]),
    "declLines">: Lists.map (asTerm declarationToExpr) (var "decls"),
    "importLines">: Logic.ifElse (Lists.isEmpty $ var "imports")
      (list ([] :: [TypedTerm Expr]))
...
patternToExpr :: TypedTermDefinition (H.Pattern -> Expr)
...
rightHandSideToExpr :: TypedTermDefinition (H.RightHandSide -> Expr)
...
statementToExpr :: TypedTermDefinition (H.Statement -> Expr)
...
-- Name constants for hydra.core.packaging types used in Haskell-specific rendering
_EntityReference_haskell :: Name
_EntityReference_haskell = Name "hydra.core.packaging.EntityReference"
...
_DefinitionReference_haskell = Name "hydra.core.packaging.DefinitionReference"
...
haddockEntityRef = haskellSerdeDefinition "haddockEntityRef" $
  doc "Render a {@type hydra.core.packaging.EntityReference} as Haddock link syntax" $
...
typeToExpr :: TypedTermDefinition (H.Type -> Expr)
  ...
  lambda "vb" $
    match H._ValueBinding (var "vb") Nothing [
      H._ValueBinding_simple>>: lambda "simpleVB" $ lets [
        ...
        Optionals.match (var "local") (var "body") (lambda "localBindings" $ lets [
...
writeQualifiedName :: TypedTermDefinition (H.QualifiedName -> String)
