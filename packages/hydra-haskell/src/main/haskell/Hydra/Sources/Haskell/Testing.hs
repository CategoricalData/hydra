...
module_ = Module {
            ...
            moduleDependencies = unqualifiedDep <$> ([HaskellUtilsSource.ns, Formatting.ns, Names.ns, Constants.ns, Dependencies.ns, Predicates.ns, Rewriting.ns, Scoping.ns, PrintError.ns, Lexical.ns, Strip.ns, ModuleName "hydra.core.decode.model"] L.++ (HaskellSyntax.ns:KernelTypes.kernelTypesModuleNames)),
...
addNamespacesToNamespaces = define "addNamespacesToNamespaces" $
  ...
  lambda "ns0" $ lambda "names" $ lets [
    "newNamespaces">: Sets.fromList (Optionals.givens (Lists.map (asTerm Names.moduleNameOf) (Sets.toList (var "names")))),
    "toModuleName">: lambda "namespace" $
      wrap H._ModuleName (Formatting.capitalize @@ (Optionals.withDefault (unwrap _ModuleName @@ var "namespace") (Lists.last (Strings.splitOn (string ".") (unwrap _ModuleName @@ var "namespace"))))),
...
buildTestModule = define "buildTestModule" $
  ...
  lambda "testModule" $ lambda "testGroup" $ lambda "testBody" $ lambda "namespaces" $ lets [
    ...
    "specNs">: wrap _ModuleName (Strings.concat2 (unwrap _ModuleName @@ var "ns_") (string "Spec")),
    ...
    "header">: Strings.join (string "\n") (Lists.concat (list [
      list [
        Strings.concat2 (string "-- ") (asTerm Constants.warningAutoGeneratedFile),
        string ""],
      list [
        string "",
        Strings.concat (list [string "module ", var "moduleNameString", string " where"]),
        string ""],
      var "allImports",
      list [
        string "",
        string "spec :: H.Spec",
        Strings.concat (list [string "spec = H.describe ", Literals.printString (var "groupName_"), string " $ do"])]]))] $
    Strings.concat (list [var "header", string "\n", var "testBody", string "\n"])
    ...
    "filtered">: Maps.filterWithKey
      (lambda "ns_" $ lambda "_v" $
        Logic.not (Equality.equal
          (Optionals.withDefault (string "") (Lists.head (Strings.splitOn (string "hydra.core.test.") (unwrap _ModuleName @@ var "ns_"))))
          (string "")))
      (var "mapping_" :: TypedTerm (M.Map ModuleName H.ModuleName))] $
    Lists.map
      (lambda "entry" $ Strings.concat (list [
        string "import qualified ",
        Strings.join (string ".") (Lists.map (asTerm Formatting.capitalize) (Strings.splitOn (string ".") (unwrap _ModuleName @@ Pairs.first (var "entry")))),
        string " as ",
...
generateTestCase = define "generateTestCase" $
  ...
  lambda "depth" $ lambda "tcm" $ lets [
    ...
    right (list [
      Strings.concat (list [string "H.it ", Literals.printString (var "name_"), string " $ H.shouldBe"]),
      Strings.concat (list [string "  (", var "actual_", string ")"]),
      Strings.concat (list [string "  (", var "expected_", string ")"])])
    ...
    Eithers.bind
      ...
      (lambda "testCaseLinesRaw" $ lets [
        "testCaseLines">: Lists.map
          (lambda "lines_" $ Lists.map (lambda "line" $ Strings.concat2 (var "indent") (var "line")) (var "lines_"))
          (var "testCaseLinesRaw"),
        "testCasesStr">: Strings.join (string "\n") (Lists.concat (var "testCaseLines"))] $
        Eithers.map
          (lambda "subgroupsStr" $
            Strings.concat (list [
              var "testCasesStr",
              ...
              var "subgroupsStr"]))
          (Eithers.map
            (lambda "blocks" $ Strings.join (string "\n") (var "blocks"))
            (Eithers.mapList
                ...
                Eithers.map
                  (lambda "content" $
                    Strings.concat (list [
                      var "indent",
                      string "H.describe ",
                      Literals.printString (var "groupName_"),
                      string " $ do\n",
                      var "content"]))
...
namespaceToModuleName = define "namespaceToModuleName" $
  ...
  lambda "ns_" $
    Strings.join (string ".") (Lists.map (asTerm Formatting.capitalize) (Strings.splitOn (string ".") (unwrap _ModuleName @@ var "ns_")))
