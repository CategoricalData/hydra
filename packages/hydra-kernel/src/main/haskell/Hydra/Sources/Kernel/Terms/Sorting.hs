...
import qualified Hydra.Core.Topology as Topo


ns :: ModuleName
ns = ModuleName "hydra.core.sorting"
...
-- which is unusual for an authoring-DSL definition. The reason: the generated `Hydra.Core.Dsl.Lib.*`
-- DSLs (unlike the old hand-written `Hydra.Core.Dsl.Meta.Lib.*`) expose each primitive's class
