...
ns :: ModuleName
ns = ModuleName "hydra.core.predicates"
...
module_ = Module {
            ...
            moduleDependencies = Bootstrap.unqualifiedDep <$> ([Arity.ns, Dependencies.ns, ModuleName "hydra.core.decode.model", Lexical.ns, Reflect.ns, Rewriting.ns, Scoping.ns, Strip.ns] L.++ kernelTypesModuleNames),
...
isComplexBinding :: TypedTermDefinition (Graph -> Binding -> Bool)
...
isComplexTerm :: TypedTermDefinition (Graph -> Term -> Bool)
...
isComplexVariable :: TypedTermDefinition (Graph -> Name -> Bool)
...
isEnumRowType :: TypedTermDefinition ([FieldType] -> Bool)
...
isEnumType :: TypedTermDefinition (Type -> Bool)
...
isNominalType :: TypedTermDefinition (Type -> Bool)
...
isTrivialTerm = define "isTrivialTerm" $
  ...
  match _Term (Strip.deannotateTerm @@ var "t")
    ...
    -- like "hydra.core.lib.maps.empty") are calls, not just references, and are not trivial.
    _Term_variable>>: "nm" ~>
