...
ns :: ModuleName
ns = ModuleName "hydra.print.regex"
...
module_ = Module {
            ...
            moduleMetadata = Bootstrap.descriptionMetadata (Just $
              ...
              <> " (docs/specification/regex.md): hydra.regex AST -> canonical text. Escapes minimally"
              ...
              <> " (hydra.print.<dialect>.regex) build on this. See issue #567.")}
  where
   definitions = [
...
printRegex = define "printRegex" $
  doc ("Render a hydra.regex AST to canonical Hydra regex text. Escapes minimally, so that"
