...
module Hydra.Sources.Kernel.Terms.Print.Regex where
...
ns :: ModuleName
ns = ModuleName "hydra.core.print.regex"
...
-- Read/Docs.hs precedent, we reference the AST purely through Name constants + project/cases.
...
module_ = Module {
            ...
            moduleMetadata = Bootstrap.descriptionMetadata (Just $
              "Canonical printer for Hydra's translingual regular-expression syntax"
              <> " (docs/specification/regex.md): hydra.core.regex AST -> canonical text. Escapes minimally"
              <> " (a character is escaped only where it is special in its context), so that"
              <> " print.regex . parse.regex is idempotent on well-formed input. Per-dialect renderers"
              <> " (hydra.core.print.<dialect>.regex) build on this. See issue #567.")}
  where
   definitions = [
...
-- | Render a single codepoint as a one-character string.
...
-- | Escape a literal codepoint for the top-level context: backslash-escape it iff it is a metacharacter.
escapeLiteral :: TypedTermDefinition (Int -> String)
escapeLiteral = define "escapeLiteral" $
  doc ("Render a literal code point in top-level context, backslash-escaping it iff it is one of the"
    <> " top-level metacharacters . ^ $ * + ? ( ) [ ] { } | \\. A '-' is not special at top level and"
    <> " prints bare.") $
  "c" ~>
    lets [
      "isMeta">: Lists.foldl
...
-- | Escape a codepoint for inside a character class. Uniformly escape \ ] ^ - (option ii, no positional rules).
escapeClassChar :: TypedTermDefinition (Int -> String)
escapeClassChar = define "escapeClassChar" $
  doc ("Render a code point inside a character class, backslash-escaping it iff it is one of the class"
    <> " metacharacters \\ ] ^ -. Escaping is uniform, with no positional exceptions (a literal '-'"
    <> " always prints as \\-).") $
  "c" ~>
    lets [
      "isMeta">: Lists.foldl
...
-- | Render a class item: a single (escaped) character, or a range from-to.
...
-- | Render a quantifier suffix ("" for 'one').
...
-- | Render a sequence (concatenated quantified atoms).
...
-- | Render an alternation (branches joined by |).
...
-- | Render a complete regex to its canonical text form.
printRegex :: TypedTermDefinition ([Term] -> String)
printRegex = define "printRegex" $
  doc ("Render a hydra.core.regex AST to canonical Hydra regex text. Escapes minimally, so that"
    <> " printRegex . parseRegex is the identity on well-formed patterns.") $
  "r" ~> alternation @@ var "r"
