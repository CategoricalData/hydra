...
ns :: ModuleName
ns = ModuleName "hydra.core.print.pcre.regex"
...
module_ = Module {
            ...
            moduleMetadata = Bootstrap.descriptionMetadata (Just $
              "Per-dialect printer rendering the hydra.core.regex AST into the PCRE / java.util.regex / ECMA"
              <> " family syntax (Java, Scala, Clojure, Python, Common Lisp, TypeScript). Differs from the"
              <> " canonical hydra.core.print.regex in exactly one place: Hydra's . (any character INCLUDING"
...
-- literal alternatives (the only case the hydra.core.regex minimal core's own conformance suite exercises).
...
printRegex = define "printRegex" $
  doc ("Render a hydra.core.regex AST into PCRE / java.util.regex / ECMA syntax. Identical to the canonical"
