...
module Hydra.Sources.Kernel.Lib.Defaults.Equality where
...
module_ = Module {
            ...
            moduleMetadata = Bootstrap.descriptionMetadata (Just ("Default term-level implementations of Equality functions for the Hydra interpreter."))}
  where
    definitions = [
...
-- identity x = x
identity_ :: TypedTermDefinition (InferenceContext -> Graph -> Term -> Either Error Term)
...
max_ :: TypedTermDefinition (InferenceContext -> Graph -> Term -> Term -> Either Error Term)
max_ = define "max" $
...
min_ :: TypedTermDefinition (InferenceContext -> Graph -> Term -> Term -> Either Error Term)
min_ = define "min" $
