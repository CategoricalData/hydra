...
ns :: ModuleName
ns = ModuleName "hydra.core.print.emacs.regex"
...
module_ = Module {
            ...
            moduleMetadata = Bootstrap.descriptionMetadata (Just $
              "Per-dialect printer rendering the hydra.core.regex AST into GNU Emacs regexp syntax. Emacs"
...
-- hydra.core.regex minimal core's own conformance suite exercises). See print.pcre.regex.alternation, the same
...
printRegex = define "printRegex" $
  doc ("Render a hydra.core.regex AST into GNU Emacs regexp syntax (escaped \\| \\( \\) \\{..\\}, . as"
    <> " [^z-a]).") $
  "r" ~> alternation @@ var "r"
