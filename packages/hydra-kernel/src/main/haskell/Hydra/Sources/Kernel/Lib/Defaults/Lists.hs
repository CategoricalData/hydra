...
module Hydra.Sources.Kernel.Lib.Defaults.Lists where
...
module_ = Module {
            ...
            moduleDependencies = Bootstrap.unqualifiedDep <$> ([ExtractCore.ns, Reduction.ns, ShowCore.ns] L.++ kernelTypesModuleNames),
            moduleMetadata = Bootstrap.descriptionMetadata (Just ("Default term-level implementations of List functions for the Hydra interpreter."))}
  where
    definitions = [
      toDefinition apply_,
      toDefinition bind_,
