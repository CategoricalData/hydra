...
module_ = Module {
  ...
  where
    definitions = [
      primImpure "execute" "Run a program to completion and capture its result." executeSig [
      ...
      primImpure "exit" "Terminate the current process with a status code." exitSig [
        ...
        \ are not performed."],
      primImpure "getEnvironment" "Get the full set of environment variables." getEnvironmentSig [
      ...
      primImpure "getEnvironmentVariable" "Look up a single environment variable by name." getEnvironmentVariableSig [
      ...
      primImpure "getTime" "Get the current wall-clock time." getTimeSig [
      ...
      primImpure "getWorkingDirectory" "Get the current working directory." getWorkingDirectorySig [
...
executeSig :: TermSignature
executeSig = sig $ TypeScheme []
  (command Types.~> effect (Types.either_ systemError processResult)) Nothing

-- exit : StatusCode -> effect<unit>
exitSig :: TermSignature
exitSig = sig $ TypeScheme []
  (statusCode Types.~> effect Types.unit) Nothing

-- getEnvironment : effect<map<EnvironmentVariable, string>>
getEnvironmentSig :: TermSignature
getEnvironmentSig = sig $ TypeScheme []
  (effect (Types.map environmentVariable Types.string)) Nothing

-- getEnvironmentVariable : EnvironmentVariable -> effect<optional<string>>
getEnvironmentVariableSig :: TermSignature
getEnvironmentVariableSig = sig $ TypeScheme []
  (environmentVariable Types.~> effect (Types.optional Types.string)) Nothing

-- getTime : effect<Timespec>
getTimeSig :: TermSignature
getTimeSig = sig $ TypeScheme []
  (effect timespec) Nothing

-- getWorkingDirectory : effect<either<SystemError, FilePath>>
getWorkingDirectorySig :: TermSignature
getWorkingDirectorySig = sig $ TypeScheme []
  (effect (Types.either_ systemError filePath)) Nothing
