...
monomorphicRecordsTests :: TypedTermDefinition TestGroup
...
multiParameterPolymorphicRecordsTests :: TypedTermDefinition TestGroup
...
polymorphicRecordsTests :: TypedTermDefinition TestGroup
...
recordsInComplexContextsTests :: TypedTermDefinition TestGroup
...
recordsTests :: TypedTermDefinition TestGroup
...
higherOrderRecordProjectionsTests :: TypedTermDefinition TestGroup
higherOrderRecordProjectionsTests = define "higherOrderRecordProjectionsTests" $
  ...
  checkTest "map projection over list of records" []
    (primitive DefLists.map @@ (project (TestTypes.testTypePersonName) (name "firstName")) @@
     list [record (TestTypes.testTypePersonName) [
           ...
           record (TestTypes.testTypePersonName) [
             ...
             "lastName">: (string "Jones"),
             "age">: (int32 25)]])
    (tyapps (primitive DefLists.map) [Core.typeVariable $ TestTypes.testTypePersonName, T.string] @@ (project (TestTypes.testTypePersonName) (name "firstName")) @@
     list [record (TestTypes.testTypePersonName) [
             "firstName">: (string "Alice"),
             "lastName">: (string "Smith"),
             "age">: (int32 30)],
  ...
  checkTest "map polymorphic projection" []
    (primitive DefLists.map @@ (project (TestTypes.testTypeLatLonPolyName) (name "lat")) @@
     list [record (TestTypes.testTypeLatLonPolyName) [
           ...
           record (TestTypes.testTypeLatLonPolyName) [
             "lat">: (int32 34),
             "lon">: (int32 (-118))]])
    (tyapps (primitive DefLists.map) [T.apply (Core.typeVariable $ TestTypes.testTypeLatLonPolyName) T.int32, T.int32]
...
multiParameterPolymorphicProjectionsTests :: TypedTermDefinition TestGroup
  ...
  checkTest "project from Triple and use second field, which is another polymorphic record" []
    (lambda "triple" $ lambda "key" $
      match (TestTypes.testTypePersonOrSomethingName) nothing [
        ...
        "other">: lambda "m" $ primitive DefMaps.lookup @@ var "key" @@ var "m"] @@
    ...
    (tylams ["t0", "t1", "t2", "t3"] $
      ...
      tyapp (match (TestTypes.testTypePersonOrSomethingName) nothing [
        ...
        "other">: lambdaTyped "m" (T.map (T.var "t1") (T.var "t2")) $
          tyapps (primitive DefMaps.lookup) [T.var "t1", T.var "t2"] @@ var "key" @@ var "m"]) (T.map (T.var "t1") (T.var "t2")) @@
...
polymorphicRecordProjectionsAppliedTests :: TypedTermDefinition TestGroup
...
polymorphicRecordProjectionsTests :: TypedTermDefinition TestGroup
...
recordEliminationsTests :: TypedTermDefinition TestGroup
...
recordProjectionsAppliedToRecordsTests :: TypedTermDefinition TestGroup
...
recordProjectionsInComplexContextsTests :: TypedTermDefinition TestGroup
...
recordProjectionsWithMutualRecursionTests :: TypedTermDefinition TestGroup
...
recordProjectionsWithVariablesTests :: TypedTermDefinition TestGroup
...
recursiveRecordProjectionsTests :: TypedTermDefinition TestGroup
...
multiParameterPolymorphicInjectionsTests :: TypedTermDefinition TestGroup
...
polymorphicRecursiveUnionInjectionsTests :: TypedTermDefinition TestGroup
...
polymorphicUnionInjectionsTests :: TypedTermDefinition TestGroup
...
polymorphicUnionsFromLambdaTests :: TypedTermDefinition TestGroup
...
simpleUnionInjectionsTests :: TypedTermDefinition TestGroup
...
unionInjectionsWithDataTests :: TypedTermDefinition TestGroup
...
unionsInComplexContextsTests :: TypedTermDefinition TestGroup
...
unionsTests :: TypedTermDefinition TestGroup
...
higherOrderUnionEliminationsTests :: TypedTermDefinition TestGroup
  ...
  checkTest "map match over list" []
    (primitive DefLists.map @@
     ...
     list [injectUnit (TestTypes.testTypeComparisonName) "lessThan",
           injectUnit (TestTypes.testTypeComparisonName) "equalTo"])
    (tyapps (primitive DefLists.map) [Core.typeVariable $ TestTypes.testTypeComparisonName, T.string] @@
     (match (TestTypes.testTypeComparisonName) nothing [
...
multiParameterPolymorphicCaseStatementsTests = define "multiParameterPolymorphicCaseStatementsTests" $
  ...
  checkTest "case Either applied to injection" []
    (match (TestTypes.testTypeEitherName) nothing [
      "left">: lambda "n" $ primitive DefMath.add @@ var "n" @@ int32 1,
      "right">: lambda "s" $ primitive DefStrings.length @@ var "s"] @@
     inject (TestTypes.testTypeEitherName) "left" (int32 42))
    (tyapps (match (TestTypes.testTypeEitherName) nothing [
      "left">: lambdaTyped "n" T.int32 (primitive DefMath.add @@ var "n" @@ int32 1),
      "right">: lambdaTyped "s" T.string (primitive DefStrings.length @@ var "s")]) [T.int32, T.string] @@
  ...
  checkTest "case Either with polymorphic let bindings" []
    (lets ["makeLeft">: lambda "x" $ inject (TestTypes.testTypeEitherName) "left" (var "x"),
      ...
      lambda "flag" $
        match (TestTypes.testTypeEitherName) nothing [
          "left">: lambda "n" $ var "makeRight" @@ (primitive DefMath.add @@ var "n" @@ int32 10),
          "right">: lambda "s" $ var "makeLeft" @@ (primitive DefStrings.length @@ var "s")] @@
        var "flag")
    (letsTyped [("makeLeft", tylams ["t0", "t1"] $ lambdaTyped "x" (T.var "t0") $ tyapps (inject (TestTypes.testTypeEitherName) "left" (var "x")) [T.var "t0", T.var "t1"],
      ...
      lambdaTyped "flag" (T.applys (Core.typeVariable $ TestTypes.testTypeEitherName) [T.int32, T.string]) $
        tyapps (match (TestTypes.testTypeEitherName) nothing [
          "left">: lambdaTyped "n" T.int32 $ tyapps (var "makeRight") [T.int32, T.int32] @@ (primitive DefMath.add @@ var "n" @@ int32 10),
          "right">: lambdaTyped "s" T.string $ tyapps (var "makeLeft") [T.int32, T.int32] @@ (primitive DefStrings.length @@ var "s")]) [T.int32, T.string] @@
        var "flag")
...
nestedUnionEliminationsTests :: TypedTermDefinition TestGroup
...
polymorphicUnionEliminationsTests :: TypedTermDefinition TestGroup
...
recursiveUnionEliminationsTests :: TypedTermDefinition TestGroup
...
simplePolymorphicUnionTests :: TypedTermDefinition TestGroup
...
simpleUnitVariantEliminationsTests :: TypedTermDefinition TestGroup
...
unionEliminationsInComplexContextsTests :: TypedTermDefinition TestGroup
...
unionEliminationsTests :: TypedTermDefinition TestGroup
...
unionEliminationsWithDataTests :: TypedTermDefinition TestGroup
unionEliminationsWithDataTests = define "unionEliminationsWithDataTests" $
  ...
  checkTest "match Number applied to int variant" []
    (match (TestTypes.testTypeNumberName) nothing [
      "int">: lambda "i" (primitive DefMath.add @@ var "i" @@ int32 10),
      "float">: lambda "f" (int32 0)] @@
     inject (TestTypes.testTypeNumberName) "int" (int32 42))
    (match (TestTypes.testTypeNumberName) nothing [
      "int">: lambdaTyped "i" T.int32 (primitive DefMath.add @@ var "i" @@ int32 10),
      "float">: lambdaTyped "f" T.float32 (int32 0)] @@
     inject (TestTypes.testTypeNumberName) "int" (int32 42))
    T.int32,
...
unionEliminationsWithDefaultsTests :: TypedTermDefinition TestGroup
...
monomorphicWrappedTermsTests :: TypedTermDefinition TestGroup
...
multiParameterPolymorphicWrappersTests :: TypedTermDefinition TestGroup
multiParameterPolymorphicWrappersTests = define "multiParameterPolymorphicWrappersTests" $
  ...
  checkTest "symmetric triple with nested polymorphic types and foldl" []
    (lets ["sumList">: lambda "lst" $
            ...
            (lambda "acc" $ lambda "x" $ primitive DefMath.add @@ var "acc" @@ var "x") @@
            int32 0 @@
            var "lst"] $
    ...
    (letsTyped [("sumList",
                 lambdaTyped "lst" (T.list T.int32) $
                   tyapps (primitive DefLists.foldl) [T.int32, T.int32] @@
                   (lambdaTyped "acc" T.int32 $ lambdaTyped "x" T.int32 $ primitive DefMath.add @@ var "acc" @@ var "x") @@
                   int32 0 @@
                   var "lst",
...
multipleWrappingLevelsTests :: TypedTermDefinition TestGroup
multipleWrappingLevelsTests = define "multipleWrappingLevelsTests" $
  ...
  noChange "wrapped in optional"
    (Core.termOptional $ just $ wrap (TestTypes.testTypeStringAliasName) (string "wrapped"))
...
nestedWrappedTermsTests :: TypedTermDefinition TestGroup
nestedWrappedTermsTests = define "nestedWrappedTermsTests" $
  ...
  checkTest "wrapped optional" []
    (wrap (TestTypes.testTypePolymorphicWrapperName) (list [Core.termOptional $ just $ int32 42]))
    (tyapp (wrap (TestTypes.testTypePolymorphicWrapperName) (list [Core.termOptional $ just $ int32 42])) (T.optional T.int32))
...
polymorphicWrappedTermsTests :: TypedTermDefinition TestGroup
...
wrappedTermsInComplexContextsTests :: TypedTermDefinition TestGroup
...
wrappedTermsTests :: TypedTermDefinition TestGroup
...
chainedUnwrappingTests :: TypedTermDefinition TestGroup
chainedUnwrappingTests = define "chainedUnwrappingTests" $
  ...
  checkTest "unwrap polymorphic then map" []
    (lambda "wrappedList" $
      primitive DefLists.map @@ (primitive DefMath.add @@ int32 1) @@ (unwrap (TestTypes.testTypePolymorphicWrapperName) @@ var "wrappedList"))
    (lambdaTyped "wrappedList" (T.apply (Core.typeVariable $ TestTypes.testTypePolymorphicWrapperName) T.int32) $
      (tyapps (primitive DefLists.map) [T.int32, T.int32]) @@ (primitive DefMath.add @@ int32 1) @@ (tyapp (unwrap (TestTypes.testTypePolymorphicWrapperName)) T.int32 @@ var "wrappedList"))
...
multiParameterPolymorphicUnwrappersTests :: TypedTermDefinition TestGroup
multiParameterPolymorphicUnwrappersTests = define "multiParameterPolymorphicUnwrappersTests" $
  ...
  checkTest "unwrap and collect edges in set" []
    (lets ["getEdge" >: lambda "st" $
      ...
      lambda "triples" $
        primitive DefSets.map @@ var "getEdge" @@ var "triples")
    (tylams ["t0", "t1"] $
      letsTyped [("getEdge",
                 tylams ["t2", "t3"] $
      ...
      lambdaTyped "triples" (T.set $ T.applys (Core.typeVariable $ TestTypes.testTypeSymmetricTripleName) [T.var "t0", T.var "t1"]) $
        tyapps (primitive DefSets.map) [T.applys (Core.typeVariable $ TestTypes.testTypeSymmetricTripleName) [T.var "t0", T.var "t1"], T.var "t1"] @@
    ...
    (tylams ["t0", "t1"] $
      ...
      tyapps (primitive DefOptionals.cases)
        [T.applys (Core.typeVariable $ TestTypes.testTypeSymmetricTripleName) [T.var "t0", T.var "t1"],
         T.optional (T.var "t1")] @@
      var "mst" @@
      tyapp (Core.termOptional nothing) (T.var "t1") @@
      (lambdaTyped "st" (T.applys (Core.typeVariable $ TestTypes.testTypeSymmetricTripleName) [T.var "t0", T.var "t1"]) $
        Core.termOptional $ just $
...
multipleUnwrapOperationsTests :: TypedTermDefinition TestGroup
...
polymorphicUnwrappingTests :: TypedTermDefinition TestGroup
...
unwrapEliminationsInApplicationsTests :: TypedTermDefinition TestGroup
...
unwrapInComplexContextsTests :: TypedTermDefinition TestGroup
...
wrapEliminationsTests :: TypedTermDefinition TestGroup
...
eliminationsTests :: TypedTermDefinition TestGroup
...
projectionsWithVariablesTests :: TypedTermDefinition TestGroup
