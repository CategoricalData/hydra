...
individualEncoderTestCases = do
  H.describe "Individual encoder test cases" $ do
    ...
    H.it "string literal type" $ do
      encodeLiteralType LiteralTypeString `H.shouldBe` mterm (unitVariant _LiteralType_string)

    H.it "string type" $ do
      encodeType stringType `H.shouldBe` mterm (variant _Expression_literal (unitVariant _LiteralType_string))

    H.it "int32 type" $ do
      encodeType int32Type `H.shouldBe`
        mterm (variant _Expression_literal (variant _LiteralType_integer $ unitVariant _IntegerType_int32))
