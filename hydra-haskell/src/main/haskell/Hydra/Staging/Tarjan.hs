  ...
  where
    go acc = do
      ...
      case stack st of
        ...
        (x:xs) -> do
          CMS.modify $ \s -> s { stack = xs, onStack = S.delete x (onStack s) }
          let acc' = x : acc
          if x == v
  ...
  CM.forM_ neighbors $ \w -> do
    ...
    if not (M.member w (indices st'))
      then do
         ...
         CMS.modify $ \s -> s { lowlinks = M.insert v (min low_v low_w) (lowlinks s) }
        ...
        then do
         ...
         CMS.modify $ \s -> s { lowlinks = M.insert v (min low_v idx_w) (lowlinks s) }
        else return ()
  stFinal <- CMS.get
...
stronglyConnectedComponents graph = reverse $ fmap L.sort $ sccs finalState
  where
    ...
    finalState = CMS.execState (mapM_ (\v -> do
                      visited <- CMS.gets (M.member v . indices)
