...
yamlSerde :: (Eq m, Ord m, Read m, Show m) => Type m -> GraphFlow m (Coder (Graph m) (Graph m) (Term m) BS.ByteString)
yamlSerde typ = do
  coder <- yamlCoder typ
  return Coder {
...
yamlSerdeStr :: (Eq m, Ord m, Read m, Show m) => Type m -> GraphFlow m (Coder (Graph m) (Graph m) (Term m) String)
yamlSerdeStr typ = do
  serde <- yamlSerde typ
  return Coder {
