...
moduleToExprDef = haskellSerdeDefinition "moduleToExpr" $
  lambda "module" $ lets [
    ...
    "warning">: list [ref Serialization.cstDef @@ (ref toSimpleCommentsDef @@ ref Constants.warningAutoGeneratedFileDef)],
    ...
    ref Serialization.doubleNewlineSepDef @@ (Lists.concat $ list [var "warning", var "headerLine", var "importLines", var "declLines"])
...
toSimpleCommentsDef :: TBinding (String -> String)
toSimpleCommentsDef = haskellSerdeDefinition "toSimpleComments" $
  lambda "c" $ Strings.intercalate (string "\n") $ Lists.map (lambda "s" $ Strings.cat2 (string "-- ") (var "s")) (Strings.lines $ var "c")
