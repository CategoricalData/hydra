...
constructModule = haskellCoderDefinition "constructModule" $ lambdas ["namespaces", "mod", "defs"] $ lets [
  "h">: lambda "namespace" $
    unwrap _Namespace @@ var "namespace",
  "createDeclarations">: lambda "g" $ lambda "def" $
    cases _Definition (var "def") Nothing [
      _Definition_type>>: lambda "type" $ lets [
      ...
      _Definition_term>>: lambda "term" $
        bind "d" (toDataDeclaration @@ var "namespaces" @@ var "term") $
    ...
    bind "g" (Monads.getState) $
    bind "declLists" (Flows.mapList (var "createDeclarations" @@ var "g") (var "defs")) $ lets [
  ...
  lambda "namespaces" $ lambda "fun" $
    cases _Function (var "fun") Nothing [
      _Function_elimination>>: lambda "e" $
        cases _Elimination (var "e") Nothing [
          ...
          _Elimination_union>>: lambda "stmt" $ lets [
            ...
            "caseExpr">:
              bind "rt" (Lexical.withSchemaContext @@ (Schemas.requireUnionType @@ var "dn")) $ lets [
              ...
              bind "dcases" (Maybes.cases (var "def")
                ...
                (lambda "d" $
                  bind "cs" (Flows.map (unaryFunction $ wrap H._CaseRhs) $ encodeTerm @@ var "namespaces" @@ var "d") $ lets [
              ...
              "toAlt">: lambdas ["fieldMap", "field"] $ lets [
                ...
                Annotations.withDepth @@ keyHaskellVar @@ (
                  lambda "depth" $ lets [
                    ...
                    bind "rhs" (Flows.map (unaryFunction $ wrap H._CaseRhs) $ encodeTerm @@ var "namespaces" @@ var "rhsTerm") $
      ...
      _Function_lambda>>: lambda "lam" $ lets [
        ...
        Flows.bind (encodeTerm @@ var "namespaces" @@ var "body") $ lambda "hbody" $
...
encodeTerm = haskellCoderDefinition "encodeTerm" $
  lambda "namespaces" $ lambda "term" $ lets [
    ...
    "nonemptySet" <~ ("s" ~> lets [
      ...
      bind "rhs" (encodeTerm @@ var "namespaces" @@ (inject _Term _Term_list $ Sets.toList $ var "s")) $
      Flows.pure $ HaskellUtils.hsapp @@ var "lhs" @@ var "rhs") $
...
encodeTypeWithClassAssertions = haskellCoderDefinition "encodeTypeWithClassAssertions" $
  lambda "namespaces" $ lambda "explicitClasses" $ lambda "typ" $ lets [
    ...
    Monads.withTrace @@ (string "encode with assertions") @@
      (bind "htyp" (adaptTypeToHaskellAndEncode @@ var "namespaces" @@ var "typ") $
...
moduleToHaskellModule = haskellCoderDefinition "moduleToHaskellModule" $
  lambdas ["mod", "defs"] $
    bind "namespaces" (HaskellUtils.namespacesForModule @@ var "mod") $
      constructModule @@ var "namespaces" @@ var "mod" @@ var "defs"

moduleToHaskell :: TBinding (Module -> [Definition] -> Flow Graph (M.Map String String))
moduleToHaskell = haskellCoderDefinition "moduleToHaskell" $ lambdas ["mod", "defs"] $
  bind "hsmod" (moduleToHaskellModule @@ var "mod" @@ var "defs") $ lets [
...
toDataDeclaration = haskellCoderDefinition "toDataDeclaration" $
  lambdas ["namespaces", "def"] $ lets [
    ...
    "toDecl">: lambda "comments" $ lambda "hname'" $ lambda "term'" $ lambda "bindings" $
      cases _Term (Rewriting.deannotateTerm @@ var "term'")
        (Just $
          bind "hterm" (encodeTerm @@ var "namespaces" @@ var "term'") $ lets [
         "vb">: HaskellUtils.simpleValueBinding @@ var "hname'" @@ var "hterm" @@ var "bindings"] $
         bind "explicitClasses" (Annotations.getTypeClasses @@ (Rewriting.removeTypesFromTerm @@ var "term")) $
         bind "htype" (encodeTypeWithClassAssertions @@ var "namespaces" @@ var "explicitClasses" @@ var "typ") $ lets [
        ...
        _Term_let>>: lambda "letTerm" $ lets [
          ...
          "hnames">: Lists.map (lambda "binding" $ HaskellUtils.simpleName @@ (Core.unName $ Core.bindingName $ var "binding")) (var "lbindings"),
          "terms">: Lists.map (unaryFunction $ Core.bindingTerm) (var "lbindings")] $
          bind "hterms" (Flows.mapList (encodeTerm @@ var "namespaces") (var "terms")) $ lets [
    ...
    bind "comments" (Annotations.getTermDescription @@ var "term") $
...
toTypeDeclarations = haskellCoderDefinition "toTypeDeclarations" $
  lambda "namespaces" $ lambda "el" $ lambda "term" $ lets [
    ...
    "newtypeCons">: lambda "el'" $ lambda "typ'" $ lets [
      ...
      bind "htype" (adaptTypeToHaskellAndEncode @@ var "namespaces" @@ var "typ'") $ lets [
    ...
    "recordCons">: lambda "lname'" $ lambda "fields" $ lets [
      "toField">: lambda "fieldType" $ lets [
        ...
        bind "htype" (adaptTypeToHaskellAndEncode @@ var "namespaces" @@ var "ftype") $
        bind "comments" (Annotations.getTypeDescription @@ var "ftype") $
    ...
    "unionCons">: lambda "g'" $ lambda "lname'" $ lambda "fieldType" $ lets [
      ...
      "deconflict">: lambda "name" $ lets [
        ...
        Logic.ifElse (Maybes.isJust $ Maps.lookup (var "tname") (Graph.graphElements $ var "g'"))
          ...
          (var "name")] $
      bind "comments" (Annotations.getTypeDescription @@ var "ftype") $ lets [
      "nm">: var "deconflict" @@ Strings.cat2 (Formatting.capitalize @@ var "lname'") (Formatting.capitalize @@ (Core.unName $ var "fname"))] $
      bind "typeList" (Logic.ifElse (Equality.equal (Rewriting.deannotateType @@ var "ftype") MetaTypes.unit)
        (Flows.pure $ list ([] :: [TTerm H.CaseRhs])) $
        Flows.bind (adaptTypeToHaskellAndEncode @@ var "namespaces" @@ var "ftype") $ lambda "htype" $
    ...
    Monads.withTrace @@ (Strings.cat2 (string "type element ") (Core.unName $ var "elementName")) @@ (
      bind "g" (Monads.getState) $
      bind "t" (trace (string "to type declarations") $ DecodeCore.type_ @@ var "term") $
      bind "isSer" (Schemas.isSerializable @@ var "el") $ lets [
      ...
      bind "decl" (cases _Type (Rewriting.deannotateType @@ var "t'")
        (Just $ bind "htype" (adaptTypeToHaskellAndEncode @@ var "namespaces" @@ var "t") $
        ...
        _Type_wrap>>: lambda "wrapped" $ lets [
          ...
          bind "cons" (var "newtypeCons" @@ var "el" @@ var "wt") $
            Flows.pure $ inject H._Declaration H._Declaration_data $ record H._DataDeclaration [
              ...
              H._DataDeclaration_deriving>>: list [var "deriv"]]]) $
      bind "comments" (Annotations.getTermDescription @@ var "term") $
      bind "tdecls" (Logic.ifElse (includeTypeDefinitions)
        (Flows.bind (typeDecl @@ var "namespaces" @@ var "elementName" @@ var "t") $ lambda "decl'" $
...
toTypeDeclarationsFrom = haskellCoderDefinition "toTypeDeclarationsFrom" $
  lambda "namespaces" $ lambda "elementName" $ lambda "typ" $ lets [
    "lname">: Names.localNameOf @@ var "elementName",
    "hname">: HaskellUtils.simpleName @@ var "lname",
    "declHead">: lambdas ["name", "vars'"] $ Logic.ifElse (Lists.null $ var "vars'")
      (inject H._DeclarationHead H._DeclarationHead_simple $ var "name")
      (lets [
        ...
        inject H._DeclarationHead H._DeclarationHead_application $ record H._ApplicationDeclarationHead [
          H._ApplicationDeclarationHead_function>>: var "declHead" @@ var "name" @@ var "rest",
          H._ApplicationDeclarationHead_operand>>: var "hvar"]),
    "newtypeCons">: lambda "tname" $ lambda "typ'" $ lets [
    ...
    Monads.withTrace @@ (Strings.cat2 (string "type definition ") (Core.unName $ var "elementName")) @@ (
      bind "g" (Monads.getState) $
      bind "isSer" (Schemas.isSerializableByName @@ var "elementName") $ lets [
      "deriv">: wrap H._Deriving $ Logic.ifElse (var "isSer")
        (Lists.map (HaskellUtils.rawName) (list [string "Eq", string "Ord", string "Read", string "Show"]))
        (list ([] :: [TTerm H.Name])),
      ...
      "hd">: var "declHead" @@ var "hname" @@ (Lists.reverse $ var "vars")] $
      bind "decl" (cases _Type (Rewriting.deannotateType @@ var "t'")
        (Just $ bind "htype" (adaptTypeToHaskellAndEncode @@ var "namespaces" @@ var "typ") $
          Flows.pure $ inject H._Declaration H._Declaration_type $ record H._TypeDeclaration [
            H._TypeDeclaration_name>>: var "hd",
            H._TypeDeclaration_type>>: var "htype"]) [
        _Type_record>>: lambda "rt" $
        ...
        _Type_wrap>>: lambda "wrapped" $ lets [
          "wt">: Core.wrappedTypeBody $ var "wrapped"] $
          bind "cons" (var "newtypeCons" @@ var "elementName" @@ var "wt") $
            Flows.pure $ inject H._Declaration H._Declaration_data $ record H._DataDeclaration [
              H._DataDeclaration_keyword>>: injectUnit H._DataOrNewtype H._DataOrNewtype_newtype,
              H._DataDeclaration_context>>: list ([] :: [TTerm H.Assertion]),
              H._DataDeclaration_head>>: var "hd",
              H._DataDeclaration_constructors>>: list [var "cons"],
              H._DataDeclaration_deriving>>: list [var "deriv"]]]) $
      bind "comments" (Annotations.getTypeDescription @@ var "typ") $
      bind "tdecls" (Logic.ifElse (includeTypeDefinitions)
        (Flows.bind (typeDecl @@ var "namespaces" @@ var "elementName" @@ var "typ") $ lambda "decl'" $
          Flows.pure $ list [var "decl'"])
        (Flows.pure $ list ([] :: [TTerm H.DeclarationWithComments]))) $ lets [
      "mainDecl">: record H._DeclarationWithComments [
        H._DeclarationWithComments_body>>: var "decl",
...
typeDecl = haskellCoderDefinition "typeDecl" $
  lambda "namespaces" $ lambda "name" $ lambda "typ" $ lets [
    ...
    "rewrite">: lambda "recurse" $ lambda "term" $ lets [
      "variantResult">: cases _Term (Rewriting.deannotateTerm @@ var "term")
        ...
        _Term_union>>: "inj" ~> Logic.ifElse (Equality.equal (Core.injectionTypeName $ var "inj") (Core.nameLift _Type))
          ...
          nothing],
      "decodeString">: lambda "term" (cases _Term (Rewriting.deannotateTerm @@ var "term")
        (Just nothing) [
        _Term_literal>>: "lit" ~> cases _Literal (var "lit")
          (Just nothing) [
          _Literal_string>>: "s" ~> just (var "s")]]),
      "decodeName">: lambda "term" (cases _Term (Rewriting.deannotateTerm @@ var "term")
        (Just nothing) [
      ...
      "forVariableType">: lambda "vname" $ lets [
        ...
        Maybes.map (lambda "ns" $ Core.termVariable $ Names.qname @@ var "ns" @@ (Strings.cat $ list [string "_", var "local", string "_type_"])) (var "mns")] $
    ...
    bind "coder" (AdaptModules.constructCoder @@ (HaskellLanguage.haskellLanguage) @@ (encodeTerm @@ var "namespaces") @@ (Core.typeVariable $ Core.nameLift _Type)) $
