...
-- Standard Tier-2 imports, excluding a few symbols which conflict with Hydra.Grammar
...
import           Hydra.Sources.Tier1.All hiding (mapDef, Pattern, _Pattern)
...
hydraGrammarToModuleModule = Module (Namespace "hydra.grammarToModule") elements
    [hydraAnnotationsModule, hydraFormattingModule, hydraQnamesModule]
    [hydraGrammarModule, hydraComputeModule, hydraGraphModule, hydraMantleModule, hydraModuleModule] $
...
childNameDef = grammarToModuleDefinition "childName" $
  ...
  lambda "lname" $ lambda "n" $
    Strings.cat $ list [var "lname", string "_", ref capitalizeDef @@ var "n"]

findNamesDef :: TElement ([Pattern] -> [String])
...
grammarToModuleDef :: TElement (Namespace -> Grammar -> Maybe String -> Module)
grammarToModuleDef = grammarToModuleDefinition "grammarToModule" $
  ...
  lambda "ns" $ lambda "grammar" $ lambda "desc" $ lets [
    ...
    "capitalizedNames">: Lists.map (lambda "pair" $ ref capitalizeDef @@ (first $ var "pair")) (var "prodPairs"),
...
isNontrivialDef :: TElement (Bool -> [Pattern] -> Bool)
isNontrivialDef = grammarToModuleDefinition "isNontrivial" $
  ...
  lambda "isRecord" $ lambda "pats" $ lets [
    ...
    $ Logic.ifElse (Equality.equalInt32 (Lists.length $ var "minPats") (int32 1))
        (match _Pattern (Just false) [
          _Pattern_labeled>>: constant true] @@ Lists.head (var "minPats"))
        true

makeElementsDef :: TElement (Bool -> Namespace -> String -> Pattern -> [(String, Type)])
...
rawNameDef :: TElement (Pattern -> String)
...
simplifyDef = grammarToModuleDefinition "simplify" $
  ...
  lambda "isRecord" $ lambda "pats" $ lets [
    "isConstant">: lambda "p" $ match _Pattern (Just false) [
      _Pattern_constant>>: constant true] @@ var "p"]
