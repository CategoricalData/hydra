...
import           Hydra.Sources.Tier1.All hiding (Graph)
...
tarjanModule = Module (Namespace "hydra.tarjan") elements
    [hydraConstantsModule, hydraErrorsModule]
    [hydraCoreModule, hydraComputeModule, hydraMantleModule, hydraTopologyModule] $
...
adjacencyListsToGraphDef :: TElement ([(key, [key])] -> (Graph, Vertex -> key))
...
popStackUntilDef :: TElement (Vertex -> Flow TarjanState [Vertex])
        ...
        lambda "st" $
          Logic.ifElse (Lists.null $ Topology.tarjanStateStack $ var "st")
            ...
            (lets [
              ...
              $ Flows.bind (ref putStateDef @@ var "newSt2") $
                lambda "_" $
...
strongConnectDef :: TElement (Graph -> Vertex -> Flow TarjanState ())
              ...
              Logic.ifElse (Logic.not $ Maps.member (var "w") (Topology.tarjanStateIndices $ var "st'"))
                (Flows.bind (ref strongConnectDef @@ var "graph" @@ var "w") $
                  lambda "_" $
                    Flows.bind (ref getStateDef) $
                      lambda "stAfter" $ lets [
                        "low_v">: Maps.findWithDefault (ref maxInt32Def) (var "v") (Topology.tarjanStateLowLinks $ var "stAfter"),
                        "low_w">: Maps.findWithDefault (ref maxInt32Def) (var "w") (Topology.tarjanStateLowLinks $ var "stAfter")]
                        $ Flows.bind (ref modifyDef @@ (lambda "s" $
                ...
                (Logic.ifElse (Sets.member (var "w") (Topology.tarjanStateOnStack $ var "st'"))
                  (lets [
                    "low_v">: Maps.findWithDefault (ref maxInt32Def) (var "v") (Topology.tarjanStateLowLinks $ var "st'"),
                    "idx_w">: Maps.findWithDefault (ref maxInt32Def) (var "w") (Topology.tarjanStateIndices $ var "st'")]
                    $ Flows.bind (ref modifyDef @@ (lambda "s" $
                        Topology.tarjanStateWithLowLinks (var "s")
                          (Maps.insert (var "v") (Math.min (var "low_v") (var "idx_w")) (Topology.tarjanStateLowLinks $ var "s")))) $
                      lambda "_" $ Flows.pure unit)
                  (Flows.pure unit))]
        $ Flows.bind (ref putStateDef @@ var "newSt") $
          lambda "_" $
            Flows.bind (Flows.mapList (var "processNeighbor") (var "neighbors")) $
              lambda "_" $
                Flows.bind (ref getStateDef) $
                  lambda "stFinal" $ lets [
                    "low_v">: Maps.findWithDefault (ref maxInt32Def) (var "v") (Topology.tarjanStateLowLinks $ var "stFinal"),
                    "idx_v">: Maps.findWithDefault (ref maxInt32Def) (var "v") (Topology.tarjanStateIndices $ var "stFinal")]
...
stronglyConnectedComponentsDef :: TElement (Graph -> [[Vertex]])
stronglyConnectedComponentsDef = tarjanDefinition "stronglyConnectedComponents" $
  ...
  lambda "graph" $ lets [
    ...
    "processVertex">: lambda "v" $
      Flows.bind (Flows.map (lambda "st" $ Maps.member (var "v") (Topology.tarjanStateIndices $ var "st")) $ ref getStateDef) $
        lambda "visited" $
          Logic.ifElse (Logic.not $ var "visited")
            (ref strongConnectDef @@ var "graph" @@ var "v")
            (Flows.pure unit),
    "finalState">: ref execDef @@ (Flows.mapList (var "processVertex") (var "verts")) @@ ref initialStateDef]
