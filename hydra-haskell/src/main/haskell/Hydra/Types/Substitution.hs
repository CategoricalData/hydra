...
normalizeScheme (TypeScheme _ body) = TypeScheme (fmap snd ord) (normalizeType body)
  where
    ...
    normalizeType typ = case typeData typ of
      ...
      TypeExprUniversal (UniversalType v t) -> universal v $ normalizeType t
...
substituteInType s typ = case typeData typ of
    ...
    TypeExprUniversal (UniversalType v body) -> if Y.isNothing (M.lookup v s)
