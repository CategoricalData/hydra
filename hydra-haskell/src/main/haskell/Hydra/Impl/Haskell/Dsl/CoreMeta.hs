...
nominalProjection cx name fname ftype = withType cx (Types.function (Types.nominal name) ftype) $ projection name fname

nominalRecord :: Context m -> Name -> [Field m] -> Term m
nominalRecord cx name fields = named cx name $ record name fields

named :: Context m -> Name -> Term m -> Term m
named cx name = withType cx (Types.nominal name)

nominalUnion :: Context m -> Name -> Field m -> Term m
nominalUnion cx name field = withType cx (Types.nominal name) $ union name field
...
nominalVariant cx name fname term = named cx name $ variant name fname term
