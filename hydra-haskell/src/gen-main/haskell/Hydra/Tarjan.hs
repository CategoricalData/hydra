...
stronglyConnectedComponents graph =  
  let verts = (Maps.keys graph) 
      processVertex = (\v -> Flows.bind (Flows.map (\st -> Maps.member v (Topology.tarjanStateIndices st)) Errors.getState) (\visited -> Logic.ifElse (Logic.not visited) (strongConnect graph v) (Flows.pure ())))
      finalState = (Errors.exec (Flows.mapList processVertex verts) initialState)
...
popStackUntil v =  
  let go = (\acc -> Flows.bind Errors.getState (\st -> Logic.ifElse (Lists.null (Topology.tarjanStateStack st)) (Flows.fail "popStackUntil: empty stack") ( 
          ...
          in (Flows.bind (Errors.putState newSt2) (\_ -> Logic.ifElse (Equality.equalInt32 x v) (Flows.pure (Lists.reverse acc_)) (go acc_))))))
  in (go [])

-- | Visit a vertex and recursively explore its successors
strongConnect :: (M.Map Int [Int] -> Int -> Compute.Flow Topology.TarjanState ())
strongConnect graph v = (Flows.bind Errors.getState (\st ->  
  let i = (Topology.tarjanStateCounter st) 
      ...
      processNeighbor = (\w -> Flows.bind Errors.getState (\st_ -> Logic.ifElse (Logic.not (Maps.member w (Topology.tarjanStateIndices st_))) (Flows.bind (strongConnect graph w) (\_ -> Flows.bind Errors.getState (\stAfter ->  
              ...
              in (Flows.bind (Errors.modify (\s -> Topology.TarjanState {
  ...
  in (Flows.bind (Errors.putState newSt) (\_ -> Flows.bind (Flows.mapList processNeighbor neighbors) (\_ -> Flows.bind Errors.getState (\stFinal ->  
    ...
    in (Logic.ifElse (Equality.equalInt32 low_v idx_v) (Flows.bind (popStackUntil v) (\comp -> Flows.bind (Errors.modify (\s -> Topology.TarjanState {
      Topology.tarjanStateCounter = (Topology.tarjanStateCounter s),
      Topology.tarjanStateIndices = (Topology.tarjanStateIndices s),
