...
fileExtension cx raw = (Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermWrap v0 -> (Eithers.map (\b -> Module.FileExtension b) ((\raw -> Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
...
namespace cx raw = (Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermWrap v0 -> (Eithers.map (\b -> Module.Namespace b) ((\raw -> Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
    Core.TermLiteral v1 -> ((\x -> case x of
      Core.LiteralString v2 -> (Right v2)
      _ -> (Left (Error.DecodingError "expected string literal"))) v1)
    _ -> (Left (Error.DecodingError "expected literal"))) stripped) (Lexical.stripAndDereferenceTermEither cx raw)) (Core.wrappedTermBody v0)))
...
qualifiedName cx raw = (Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermRecord v0 ->  
    let fieldMap = (Helpers.toFieldMap v0)
    in (Eithers.bind (Helpers.requireField "namespace" (Helpers.decodeMaybe namespace) fieldMap cx) (\field_namespace -> Eithers.bind (Helpers.requireField "local" (\cx -> \raw -> Eithers.either (\err -> Left (Error.DecodingError err)) (\stripped -> (\x -> case x of
      Core.TermLiteral v1 -> ((\x -> case x of
        Core.LiteralString v2 -> (Right v2)
        _ -> (Left (Error.DecodingError "expected string literal"))) v1)
