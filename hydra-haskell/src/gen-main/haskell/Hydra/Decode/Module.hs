...
module_ cx raw = (Eithers.either (\err -> Left (Util.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermRecord v1 ->  
    ...
    in (Eithers.either (\err -> Left err) (\namespace -> Eithers.either (\err -> Left err) (\elements -> Eithers.either (\err -> Left err) (\termDependencies -> Eithers.either (\err -> Left err) (\typeDependencies -> Eithers.either (\err -> Left err) (\description -> Right (Module.Module {
      ...
      Module.moduleDescription = description})) (Maybes.maybe (Left (Util.DecodingError (Strings.cat [
      "missing field ",
      "description",
      ...
      "typeDependencies",
      " in record"]))) (\fieldTerm -> Eithers.either (\err -> Left (Util.DecodingError err)) (\stripped -> (\x -> case x of
      Core.TermList v2 -> (Eithers.mapList (Hydra.Decode.Module.namespace cx) v2)
...
namespaces n cx raw = (Eithers.either (\err -> Left (Util.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermRecord v1 ->  
    let fieldMap = (Maps.fromList (Lists.map (\f -> (Core.fieldName f, (Core.fieldTerm f))) (Core.recordFields v1)))
    in (Eithers.either (\err -> Left err) (\focus -> Eithers.either (\err -> Left err) (\mapping -> Right (Module.Namespaces {
      Module.namespacesFocus = focus,
      Module.namespacesMapping = mapping})) (Maybes.maybe (Left (Util.DecodingError (Strings.cat [
      "missing field ",
      "mapping",
      " in record"]))) (\fieldTerm -> Eithers.either (\err -> Left (Util.DecodingError err)) (\stripped -> (\x -> case x of
      Core.TermMap v2 ->  
        let pairs = (Maps.toList v2) 
            decodePair = (\kv ->  
                    let k = (Pairs.first kv) 
                        v = (Pairs.second kv)
                    in (Eithers.either (\err -> Left err) (\k2 -> Eithers.either (\err2 -> Left err2) (\v2 -> Right (k2, v2)) (n cx v)) (namespace cx k)))
      ...
      Core.TermPair v2 ->  
        let a = (Pairs.first v2) 
            b = (Pairs.second v2)
        in (Eithers.either (\err -> Left err) (\a2 -> Eithers.either (\err2 -> Left err2) (\b2 -> Right (a2, b2)) (n cx b)) (namespace cx a))
...
qualifiedName cx raw = (Eithers.either (\err -> Left (Util.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermRecord v1 ->  
    let fieldMap = (Maps.fromList (Lists.map (\f -> (Core.fieldName f, (Core.fieldTerm f))) (Core.recordFields v1)))
    in (Eithers.either (\err -> Left err) (\namespace -> Eithers.either (\err -> Left err) (\local -> Right (Module.QualifiedName {
      Module.qualifiedNameNamespace = namespace,
      Module.qualifiedNameLocal = local})) (Maybes.maybe (Left (Util.DecodingError (Strings.cat [
      "missing field ",
      "local",
      " in record"]))) (\fieldTerm -> Eithers.either (\err -> Left (Util.DecodingError err)) (\stripped -> (\x -> case x of
      Core.TermLiteral v2 -> ((\x -> case x of
...
termDefinition cx raw = (Eithers.either (\err -> Left (Util.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermRecord v1 ->  
    let fieldMap = (Maps.fromList (Lists.map (\f -> (Core.fieldName f, (Core.fieldTerm f))) (Core.recordFields v1)))
    in (Eithers.either (\err -> Left err) (\name -> Eithers.either (\err -> Left err) (\term -> Eithers.either (\err -> Left err) (\type_ -> Right (Module.TermDefinition {
      Module.termDefinitionName = name,
      Module.termDefinitionTerm = term,
      Module.termDefinitionType = type_})) (Maybes.maybe (Left (Util.DecodingError (Strings.cat [
      "missing field ",
      "type",
...
typeDefinition cx raw = (Eithers.either (\err -> Left (Util.DecodingError err)) (\stripped -> (\x -> case x of
  Core.TermRecord v1 ->  
    let fieldMap = (Maps.fromList (Lists.map (\f -> (Core.fieldName f, (Core.fieldTerm f))) (Core.recordFields v1)))
    in (Eithers.either (\err -> Left err) (\name -> Eithers.either (\err -> Left err) (\type_ -> Right (Module.TypeDefinition {
      Module.typeDefinitionName = name,
      Module.typeDefinitionType = type_})) (Maybes.maybe (Left (Util.DecodingError (Strings.cat [
      "missing field ",
      "type",
