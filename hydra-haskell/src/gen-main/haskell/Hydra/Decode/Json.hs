  ...
  Core.TermUnion v1 ->  
    let tname = (Core.injectionTypeName v1) 
        ...
        variantMap = (Maps.fromList [
                (Core.Name "array", (\input -> Eithers.map (\t -> Json.ValueArray t) (Helpers.decodeList value cx input))),
                ...
                (Core.Name "null", (\input -> Eithers.map (\t -> Json.ValueNull) (Helpers.decodeUnit cx input))),
                (Core.Name "number", (\input -> Eithers.map (\t -> Json.ValueNumber t) (Eithers.either (\err -> Left (Util.DecodingError err)) (\stripped -> (\x -> case x of
                  ...
                  _ -> (Left (Util.DecodingError "expected literal"))) stripped) (Lexical.stripAndDereferenceTermEither cx input)))),
                (Core.Name "object", (\input -> Eithers.map (\t -> Json.ValueObject t) (Helpers.decodeMap (\cx -> \raw -> Eithers.either (\err -> Left (Util.DecodingError err)) (\stripped -> (\x -> case x of
                  Core.TermLiteral v2 -> ((\x -> case x of
