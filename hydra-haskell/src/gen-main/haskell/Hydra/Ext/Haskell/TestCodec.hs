...
haskellTestCodec namespaces = Testing.TestCodec {
...
haskellTestCaseTemplate = (Strings.intercalate "\n" [
...
haskellModuleTemplate = (Strings.intercalate "\n" [
...
namespaceToModuleName ns_ = (Strings.intercalate "." (Lists.map Formatting.capitalize (Strings.splitOn "." (Module.unNamespace ns_))))
...
indentContinuationLines n s = (Strings.intercalate (Strings.cat2 "\n" (Strings.fromList (Lists.replicate n 32))) (Strings.splitOn "\n" s))
...
generateTypeAnnotationFor g namespaces inputTerm outputTerm = (Logic.ifElse (Logic.not (containsTriviallyPolymorphic outputTerm)) (Right Nothing) (Maybes.maybe (Right Nothing) (\result ->  
  let typeScheme = (Pairs.second result) 
      ...
      isEither = ((\x -> case x of
              Core.TermEither _ -> True
              _ -> False) (Rewriting.deannotateTerm outputTerm))
...
containsTriviallyPolymorphic term = ((\x -> case x of
...
generateTestFileWithCodec codec testModule testGroup namespaces g = (Eithers.map (\testBody ->  
...
extractEncodedTermVariableNames graf term = (Rewriting.foldOverTerm Coders.TraversalOrderPre (collectNames graf) Sets.empty term)
...
collectNames graf names t = (Logic.ifElse (Schemas.isEncodedTerm (Rewriting.deannotateTerm t)) (Eithers.either (\_ -> names) (\decodedTerm -> Sets.union names (Rewriting.termDependencyNames True True True decodedTerm)) (Eithers.bimap (\_e -> _e) (\_a -> _a) (Core_.term graf t))) names)
...
generateHaskellTestFile testModule testGroup g = (Eithers.bind (buildNamespacesForTestGroup testModule testGroup g) (\namespaces -> generateTestFileWithCodec (haskellTestCodec namespaces) testModule testGroup namespaces g))
...
buildNamespacesForTestGroup mod tgroup graph_ =  
  let testCases_ = (collectTestCases tgroup) 
      ...
      testBindings = (Lists.map (\term -> Core.Binding {
...
collectTestCases tg = (Lists.concat2 (Testing.testGroupCases tg) (Lists.concat (Lists.map collectTestCases (Testing.testGroupSubgroups tg))))

-- | Extract input and output terms from a test case
extractTestTerms :: (Testing.TestCaseWithMetadata -> [Core.Term])
extractTestTerms tcm = ((\x -> case x of
  Testing.TestCaseDelegatedEvaluation v0 -> [
    Testing.delegatedEvaluationTestCaseInput v0,
    (Testing.delegatedEvaluationTestCaseOutput v0)]
  _ -> []) (Testing.testCaseWithMetadataCase tcm))
