...
bindConstraints :: (Context.Context -> Graph.Graph -> [Typing_.TypeConstraint] -> Either (Context.InContext Error.OtherError) Typing_.TypeSubst)
bindConstraints flowCx cx constraints = (Eithers.bind (Eithers.bimap (\_ic -> Context.InContext {
  Context.inContextObject = (Error.OtherError (Error.unificationErrorMessage (Context.inContextObject _ic))),
...
finalizeInferredTerm :: (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.OtherError) Core.Term)
...
forInferredTerm :: (Context.Context -> Graph.Graph -> Core.Term -> String -> (Typing_.InferenceResult -> t0) -> Either (Context.InContext Error.OtherError) (t0, Context.Context))
...
inferGraphTypes :: (Context.Context -> [Core.Binding] -> Graph.Graph -> Either (Context.InContext Error.OtherError) ((Graph.Graph, [Core.Binding]), Context.Context))
        ...
        in  
          ...
          in (Eithers.bind (finalizeInferredTerm fcx2 g0 term) (\finalized -> (\x -> case x of
            ...
            Core.TermVariable _ -> (Left (Context.InContext {
              Context.inContextObject = (Error.OtherError "Expected inferred graph as let term"),
              Context.inContextContext = fcx2}))) finalized))))

-- | Infer the type of a term in a given inference context
inferInGraphContext :: (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
inferInGraphContext fcx cx term = (inferTypeOfTerm fcx cx term "single term")

-- | Infer types for multiple terms (Either version)
inferMany :: (Context.Context -> Graph.Graph -> [(Core.Term, String)] -> Either (Context.InContext Error.OtherError) (([Core.Term], ([Core.Type], Typing_.TypeSubst)), Context.Context))
                          ...
                          in  
                            ...
                            in  
                              ...
                              in  
                                ...
                                in (Right ((Lists.cons (Substitution.substTypesInTerm s2 e1) e2, (Lists.cons (Substitution.substInType s2 t1) t2, (Substitution.composeTypeSubst s1 s2))), fcx3))))))
  in (Logic.ifElse (Lists.null pairs) (Right (([], ([], Substitution.idTypeSubst)), fcx)) dflt)

-- | Map a possibly untyped term to a fully typed term and its type
inferTypeOf :: (Context.Context -> Graph.Graph -> Core.Term -> Either (Context.InContext Error.OtherError) ((Core.Term, Core.TypeScheme), Context.Context))
        ...
        in  
          ...
          in  
            ...
            in (Maybes.maybe (Left (Context.InContext {
              Context.inContextObject = (Error.OtherError "Expected a type scheme"),
...
inferTypeOfAnnotatedTerm :: (Context.Context -> Graph.Graph -> Core.AnnotatedTerm -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfApplication :: (Context.Context -> Graph.Graph -> Core.Application -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfCaseStatement :: (Context.Context -> Graph.Graph -> Core.CaseStatement -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
                                      ...
                                      in  
                                        ...
                                        in  
                                          ...
                                          in  
                                            ...
                                            in (Eithers.bind (mapConstraints fcx5 cx (\subst -> yield fcx5 (buildTypeApplicationTerm svars (Core.TermFunction (Core.FunctionElimination (Core.EliminationUnion (Core.CaseStatement {
...
-- | Infer the type of a collection (Either version)
inferTypeOfCollection :: (Context.Context -> Graph.Graph -> (Core.Type -> Core.Type) -> ([Core.Term] -> Core.Term) -> String -> [Core.Term] -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
inferTypeOfCollection fcx cx typCons trmCons desc els =  
  let varResult = (Schemas.freshName fcx)
  in  
    let var = (Pairs.first varResult)
    in  
      let fcx2 = (Pairs.second varResult)
      in (Logic.ifElse (Lists.null els) (Right (yield fcx2 (buildTypeApplicationTerm [
        var] (trmCons [])) (typCons (Core.TypeVariable var)) Substitution.idTypeSubst)) (Eithers.bind (inferMany fcx2 cx (Lists.zip els (Lists.map (\i -> Strings.cat [
        "#",
                  ...
                  in (Eithers.bind (mapConstraints fcx3 cx (\subst2 ->  
                    ...
                    in  
                      ...
                      in  
                        ...
                        in (yield fcx3 iterm itype isubst)) constraints) (\mcResult -> Right mcResult)))))

-- | Infer the type of an either value (Either version)
inferTypeOfEither :: (Context.Context -> Graph.Graph -> Either Core.Term Core.Term -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfElimination :: (Context.Context -> Graph.Graph -> Core.Elimination -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfFunction :: (Context.Context -> Graph.Graph -> Core.Function -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfInjection :: (Context.Context -> Graph.Graph -> Core.Injection -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfLambda :: (Context.Context -> Graph.Graph -> Core.Lambda -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfLet :: (Context.Context -> Graph.Graph -> Core.Let -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfLetNormalized :: (Context.Context -> Graph.Graph -> Core.Let -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfList :: (Context.Context -> Graph.Graph -> [Core.Term] -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
inferTypeOfList fcx cx = (inferTypeOfCollection fcx cx (\x -> Core.TypeList x) (\x -> Core.TermList x) "list element")
...
inferTypeOfMap :: (Context.Context -> Graph.Graph -> M.Map Core.Term Core.Term -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
      ...
      in  
        ...
        in  
          ...
          in  
            ...
            in (Logic.ifElse (Maps.null m) (Right (yield fcx3 (buildTypeApplicationTerm [
              kvar,
              vvar] (Core.TermMap Maps.empty)) (Core.TypeMap (Core.MapType {
              Core.mapTypeKeys = (Core.TypeVariable kvar),
              Core.mapTypeValues = (Core.TypeVariable vvar)})) Substitution.idTypeSubst)) (Eithers.bind (inferMany fcx3 cx (Lists.map (\k -> (k, "map key")) (Maps.keys m))) (\kRp ->  
                ...
                in  
                  ...
                  in  
                    ...
                    in  
                      ...
                      in (Eithers.bind (inferMany fcx4 cx (Lists.map (\v -> (v, "map value")) (Maps.elems m))) (\vRp ->  
                              ...
                              in  
                                ...
                                in  
                                  ...
                                  in  
                                    ...
                                    in (Eithers.bind (mapConstraints fcx5 cx (\subst -> yield fcx5 (Core.TermMap (Maps.fromList (Lists.zip kterms vterms))) (Core.TypeMap (Core.MapType {
                                      Core.mapTypeKeys = (Core.TypeVariable kvar),
...
inferTypeOfOptional :: (Context.Context -> Graph.Graph -> Maybe Core.Term -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
inferTypeOfOptional fcx cx m =  
  ...
  in (inferTypeOfCollection fcx cx (\x -> Core.TypeMaybe x) trmCons "optional element" (Maybes.maybe [] Lists.singleton m))

-- | Infer the type of a pair (Either version)
inferTypeOfPair :: (Context.Context -> Graph.Graph -> (Core.Term, Core.Term) -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
                ...
                in  
                  ...
                  in  
                    ...
                    in  
                      ...
                      in (Right (yield fcx2 termWithTypes (Core.TypePair (Core.PairType {
                        Core.pairTypeFirst = tyFst,
                        Core.pairTypeSecond = tySnd})) isubst))))

-- | Infer the type of a primitive function (Either version)
inferTypeOfPrimitive :: (Context.Context -> Graph.Graph -> Core.Name -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
inferTypeOfPrimitive fcx cx name = (Maybes.maybe (Left (Context.InContext {
  Context.inContextObject = (Error.OtherError (Strings.cat2 "No such primitive: " (Core.unName name))),
...
inferTypeOfProjection :: (Context.Context -> Graph.Graph -> Core.Projection -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfRecord :: (Context.Context -> Graph.Graph -> Core.Record -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
                    ...
                    in  
                      ...
                      in  
                        ...
                        in  
                          ...
                          in (Eithers.bind (mapConstraints fcx3 cx (\subst -> yield fcx3 (buildTypeApplicationTerm svars (Core.TermRecord (Core.Record {
...
inferTypeOfSet :: (Context.Context -> Graph.Graph -> S.Set Core.Term -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
inferTypeOfSet fcx cx s = (inferTypeOfCollection fcx cx (\x -> Core.TypeSet x) (\terms -> Core.TermSet (Sets.fromList terms)) "set element" (Sets.toList s))

-- | Infer the type of a given term (Either version)
inferTypeOfTerm :: (Context.Context -> Graph.Graph -> Core.Term -> String -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfTypeLambda :: (Context.Context -> Graph.Graph -> Core.TypeLambda -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfTypeApplication :: (Context.Context -> Graph.Graph -> Core.TypeApplicationTerm -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfUnwrap :: (Context.Context -> Graph.Graph -> Core.Name -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypeOfVariable :: (Context.Context -> Graph.Graph -> Core.Name -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
inferTypeOfVariable fcx cx name = (Maybes.maybe (Left (Context.InContext {
  Context.inContextObject = (Error.OtherError (Strings.cat2 "Variable not bound to type: " (Core.unName name))),
  Context.inContextContext = fcx})) (\scheme ->  
  let tsResult = (Schemas.instantiateTypeScheme fcx scheme)
  in  
    let ts = (Pairs.first tsResult)
...
inferTypeOfWrappedTerm :: (Context.Context -> Graph.Graph -> Core.WrappedTerm -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
...
inferTypesOfTemporaryBindings :: (Context.Context -> Graph.Graph -> [Core.Binding] -> Either (Context.InContext Error.OtherError) (([Core.Term], ([Core.Type], (Typing_.TypeSubst, (M.Map Core.Name Core.TypeVariableMetadata)))), Context.Context))
...
mapConstraints :: (Context.Context -> Graph.Graph -> (Typing_.TypeSubst -> t0) -> [Typing_.TypeConstraint] -> Either (Context.InContext Error.OtherError) t0)
...
yieldDebug :: (Context.Context -> t0 -> String -> Core.Term -> Core.Type -> Typing_.TypeSubst -> Either (Context.InContext Error.OtherError) Typing_.InferenceResult)
