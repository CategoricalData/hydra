...
buildConvertCaseCall fromConv toConv input_ = (Core.TermApplication (Core.Application {
...
encodeCaseConvention conv = (Core.TermUnion (Core.Injection {
  ...
  Core.injectionField = Core.Field {
    Core.fieldName = ((\x -> case x of
      ...
      Util.CaseConventionPascal -> (Core.Name "pascal")) conv),
...
transformModule m = Module.Module {
...
collectTestCases tg = (Lists.concat2 (Testing.testGroupCases tg) (Lists.concat (Lists.map (\sg -> collectTestCases sg) (Testing.testGroupSubgroups tg))))

-- | Build a Term representing a topologicalSort function call
buildTopologicalSortCall :: ([(Int, [Int])] -> Core.Term)
buildTopologicalSortCall adjList = (Core.TermApplication (Core.Application {
...
buildTopologicalSortSCCCall adjList = (Core.TermApplication (Core.Application {
...
encodeAdjacencyList pairs = (Core.TermList (Lists.map (\p -> Core.TermPair (encodeInt (Pairs.first p), (Core.TermList (Lists.map (\d -> encodeInt d) (Pairs.second p))))) pairs))
...
encodeEitherListList e = (Core.TermEither (Eithers.bimap (\cycles -> encodeListList cycles) (\sorted -> encodeIntList sorted) e))
