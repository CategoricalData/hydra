...
checkingError ce = ((\x -> case x of
  ...
  Error.CheckingErrorUntypedLetBinding v0 -> (untypedLetBindingError v0)) ce)
...
error e = ((\x -> case x of
  ...
  Error.ErrorUnification v0 -> (unificationError v0)) e)
...
notAForallTypeError e =  
  ...
  in  
    let args = (Error.notAForallTypeErrorTypeArguments e)
    in (Strings.cat [
      "not a forall type: ",
      (Core_.type_ typ),
      ". Trying to apply ",
...
typeArityMismatchError e =  
  ...
  in  
    let expected = (Error.typeArityMismatchErrorExpectedArity e)
    in  
      let actual = (Error.typeArityMismatchErrorActualArity e)
      in  
        let args = (Error.typeArityMismatchErrorTypeArguments e)
        in (Strings.cat [
          "type ",
          (Core_.type_ typ),
...
typeMismatchError e =  
  ...
  in  
    let actual = (Error.typeMismatchErrorActualType e)
    in (Strings.cat [
...
unboundTypeVariablesError e =  
  ...
  in  
    let typ = (Error.unboundTypeVariablesErrorType e)
    in (Strings.cat [
...
undefinedFieldError e =  
  ...
  in  
    let tname = (Error.undefinedFieldErrorTypeName e)
    in (Strings.cat [
      "no such field \"",
      (Core.unName fname),
      "\" in type \"",
...
unequalTypesError e =  
  ...
  in  
    let desc = (Error.unequalTypesErrorDescription e)
    in (Strings.cat [
      "unequal types ",
...
unexpectedTermVariantError e =  
  ...
  in  
    let actual = (Error.unexpectedTermVariantErrorActualTerm e)
    in (Strings.cat [
      "expected ",
...
unexpectedTypeVariantError e =  
  ...
  in  
    let actual = (Error.unexpectedTypeVariantErrorActualType e)
    in (Strings.cat [
      "expected ",
...
unificationError e =  
  ...
  in  
    let rt = (Error.unificationErrorRightType e)
    in  
      let msg = (Error.unificationErrorMessage e)
      in (Strings.cat [
...
unsupportedTermVariantError e = (Strings.cat2 "unsupported term variant: " (Meta.termVariant (Error.unsupportedTermVariantErrorTermVariant e)))
