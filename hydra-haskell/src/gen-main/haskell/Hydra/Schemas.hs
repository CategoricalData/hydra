...
fieldTypes cx graph t =
      ...
      in case (Rewriting.deannotateType t) of
        ...
        Core.TypeVariable v0 -> Maybes.maybe
          (Eithers.bind (Lexical.requireElement cx graph v0) (\el -> Eithers.bind (Eithers.bimap (\_wc_e -> Context.InContext {
            Context.inContextObject = _wc_e,
          ...
          (\ts -> fieldTypes cx graph (Core.typeSchemeType ts))
          (Maps.lookup v0 (Graph.graphSchemaTypes graph))
