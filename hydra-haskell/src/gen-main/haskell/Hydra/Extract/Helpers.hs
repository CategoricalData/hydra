...
decodeEither leftDecoder rightDecoder g term =
    Eithers.bind (Eithers.bimap (\x -> Error.DecodingError x) (\x -> x) (Lexical.stripAndDereferenceTermEither g term)) (\stripped -> case stripped of
      Core.TermEither v0 -> Eithers.either (\lv -> Eithers.map (\x -> Left x) (leftDecoder g lv)) (\rv -> Eithers.map (\x -> Right x) (rightDecoder g rv)) v0
      _ -> Left (Error.DecodingError "expected either value"))
...
decodeList elemDecoder g term =
    Eithers.bind (Eithers.bimap (\x -> Error.DecodingError x) (\x -> x) (Lexical.stripAndDereferenceTermEither g term)) (\stripped -> case stripped of
      ...
      _ -> Left (Error.DecodingError "expected list"))
...
decodeMap keyDecoder valDecoder g term =
    Eithers.bind (Eithers.bimap (\x -> Error.DecodingError x) (\x -> x) (Lexical.stripAndDereferenceTermEither g term)) (\stripped -> case stripped of
      ...
      _ -> Left (Error.DecodingError "expected map"))
...
decodeMaybe elemDecoder g term =
    Eithers.bind (Eithers.bimap (\x -> Error.DecodingError x) (\x -> x) (Lexical.stripAndDereferenceTermEither g term)) (\stripped -> case stripped of
      ...
      _ -> Left (Error.DecodingError "expected optional value"))
...
decodePair firstDecoder secondDecoder g term =
    Eithers.bind (Eithers.bimap (\x -> Error.DecodingError x) (\x -> x) (Lexical.stripAndDereferenceTermEither g term)) (\stripped -> case stripped of
      ...
      _ -> Left (Error.DecodingError "expected pair"))
...
decodeSet elemDecoder g term =
    Eithers.bind (Eithers.bimap (\x -> Error.DecodingError x) (\x -> x) (Lexical.stripAndDereferenceTermEither g term)) (\stripped -> case stripped of
      ...
      _ -> Left (Error.DecodingError "expected set"))
...
decodeUnit g term =
    Eithers.bind (Eithers.bimap (\x -> Error.DecodingError x) (\x -> x) (Lexical.stripAndDereferenceTermEither g term)) (\stripped -> case stripped of
      Core.TermUnit -> Right ()
      _ -> Left (Error.DecodingError "expected a unit value"))
...
decodeWrapped bodyDecoder g term =
    Eithers.bind (Eithers.bimap (\x -> Error.DecodingError x) (\x -> x) (Lexical.stripAndDereferenceTermEither g term)) (\stripped -> case stripped of
      ...
      _ -> Left (Error.DecodingError "expected wrapped value"))
...
requireField fieldName decoder fieldMap g =
    Maybes.maybe (Left (Error.DecodingError (Strings.cat [
      "missing field ",
      fieldName,
