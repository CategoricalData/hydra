...
public class ReadBoolean extends PrimitiveFunction {
    ...
    protected Function<List<Term>, Function<Context, Function<Graph, Either<Error_, Term>>>> implementation() {
        return args -> cx -> graph -> hydra.lib.eithers.Map.apply((Function<String, Term>) s -> Terms.optional(apply(s).map(Terms::boolean_)), hydra.extract.Core.string(graph, args.get(0)));
    }
