...
public class BloomFilterHeader implements Serializable {
  public static final hydra.core.Name TYPE_NAME = new hydra.core.Name("hydra/ext/parquet/format.BloomFilterHeader");
   ...
   */
  public final hydra.ext.parquet.format.BloomFilterAlgorithm algorithm;
  
  /**
   * The hash function used for Bloom filter.
   */
  public final hydra.ext.parquet.format.BloomFilterHash hash;
  
  /**
   * The compression used in the Bloom filter
   */
  public final hydra.ext.parquet.format.BloomFilterCompression compression;
  
  public BloomFilterHeader (Integer numBytes, hydra.ext.parquet.format.BloomFilterAlgorithm algorithm, hydra.ext.parquet.format.BloomFilterHash hash, hydra.ext.parquet.format.BloomFilterCompression compression) {
  ...
  public BloomFilterHeader withAlgorithm(hydra.ext.parquet.format.BloomFilterAlgorithm algorithm) {
    java.util.Objects.requireNonNull((algorithm));
    return new BloomFilterHeader(numBytes, algorithm, hash, compression);
  }
  
  public BloomFilterHeader withHash(hydra.ext.parquet.format.BloomFilterHash hash) {
    java.util.Objects.requireNonNull((hash));
    return new BloomFilterHeader(numBytes, algorithm, hash, compression);
  }
  
  public BloomFilterHeader withCompression(hydra.ext.parquet.format.BloomFilterCompression compression) {
    java.util.Objects.requireNonNull((compression));
    return new BloomFilterHeader(numBytes, algorithm, hash, compression);
  }
}
