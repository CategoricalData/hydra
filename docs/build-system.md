...
- **Target distributions** are generated source trees, one per (target language × package). Every
  `dist/<lang>/` tree is gitignored and regenerated on demand from `dist/json/`, not checked in.
  On a cold clone, `dist/haskell/` is seeded by a published-host driver — see below.
...
### The published host is oil and water with the working tree

One principle governs every published-host decision, and breaking it is the root of a class of confusing
build failures:

> **The published host is a separate, frozen entity used only for generation and validation.
> Its runtime code must never be linked into a build of the current Git revision of Hydra.**

The published host (the *last released* Hydra) and the working tree (the *latest* revision) are two
distinct programs. The published host is a complete, self-contained translator: it reads `dist/json/`
and *emits* target code, and it does this without any reference to the working tree's generated output.
The working tree's own host is compiled fresh from the current sources. These two never share a compile:
you use the published host to **produce** artifacts, you never **link against** it as a library
dependency of HEAD. Mixing the two — e.g. linking a published `hydra-haskell` coder (built against the
*released* kernel) into a HEAD build whose kernel has moved on — is the category error that surfaces as
version-skew compile failures. They are oil and water.

This is *why* the Haskell host compiles its coders from the co-generated `dist/haskell/` rather than from
Hackage (above): the co-generated tree is the current revision's output, so linking it is self-consistent;
linking the published coders would mix the two entities. `--local-host` is not a workaround to avoid the
...
repository (`dist/json/` is the tracked source of truth). A fresh sync bootstraps it in three steps, and
the oil-and-water principle holds throughout:

1. **`dist/haskell/` does not exist in the repository.** It is built locally on each sync (gitignored;
   `dist/json/` remains the tracked source of truth).

2. **A fresh sync uses the *last published host* to generate the bootstrap `dist/haskell/`.** The
   published host (run as a separate generation step, *not* linked into HEAD) reads the tracked
   `dist/json/` and emits:
   - `dist/haskell/hydra-kernel`, and
   - `dist/haskell/hydra-<langxxx>` for **every** language target `langxxx` that (a) is built *with the
     Haskell host* and (b) has **no published coder package** on Hackage.

   `hydra-java` does **not** qualify — Java is generated by the **Java host**, not the Haskell host, so the
   Haskell build never needs `dist/haskell/hydra-java`. `hydra-scala` is the canonical case that *does*
   qualify: there is no `hydra-scala` package on Hackage, so Scala can only be generated by a locally-built
   Scala coder, which means `dist/haskell/hydra-scala` and its runtime dependencies must be built locally.
   **If every required target had a published coder package, we would not need to build Hydra Haskell
   locally at all** — the published host alone could generate every target.

3. **Having built Hydra Haskell locally, we use *it* to generate the target code.** The locally-built host
   (compiled from the current revision's sources plus the bootstrap `dist/haskell/` from step 2) is what
