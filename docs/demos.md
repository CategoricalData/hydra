...
The `demos/bootstrapping/bin/bootstrap-all.sh` script runs all paths and diffs the output against the canonical baselines
...
Source-to-source automatic differentiation on Hydra terms: mathematical functions are built
as typed lambda calculus terms, differentiated symbolically (chain, product, and power rules,
transcendental derivatives), and evaluated numerically via `reduceTerm` — all within Hydra's
cross-platform term representation. AD results are verified against central finite differences.
...
See [demos/grad/README.md](../demos/grad/README.md).

## Pg-formats (property-graph wire formats)

Three Hydra-generated views of the same property graph (the TinkerPop "Modern" reference
graph): Hydra-PG JSON, GraphSON 3.0 (JSON Lines), and a JSON Schema for validating Hydra-PG
JSON documents. All three views describe the same data, so any one can be cross-checked
against the others. The committed `output/` artifacts are regenerated by:

```bash
demos/pg-formats/bin/generate.sh
```

See [demos/pg-formats/README.md](../demos/pg-formats/README.md).
