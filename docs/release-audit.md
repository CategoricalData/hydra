...
distinction. A formal per-dependency license attestation is still needed (see gaps), but no
incompatible license was found.
...
| CI release-artifact verification | ❌ Absent | `.github/workflows/ci.yml` runs per-language test matrices; `pages.yml` deploys JavaDocs. Neither assembles a source release nor checks `LICENSE`/`NOTICE`/signatures |
...
6. **Add a Scala `licenses` declaration** to `packages/hydra-scala/build.sbt` for parity with the
   ...
   Apache-2.0).
7. **Produce a per-dependency license attestation** for the publish-set hosts (the audit above
   found all Category-A-compatible, but Apache wants this recorded, not just asserted). A generated
   `DEPENDENCIES` manifest or SBOM (next item) satisfies this.
