...
7. **Hand-written heads.** Java `Generation.java`, Python `generation.py`,
   Scala `Generation.scala`, the Haskell `Hydra.Module.Compat` /
   `Hydra.Dsl.Bootstrap` / `verify-json-kernel/Main.hs`, the auto-generated
   `Hydra.Sources.Json.Bootstrap` source. Each language uses its native
   field-name convention (`type_scheme` in Python, `typeScheme` in Java/Scala,
   `:typeScheme` keyword in Common Lisp). Cross-host coders fanned out through
   `dist/<lang>/hydra-<other>/...` also need to be regenerated by Phase 4 of
   sync — see step 9.
