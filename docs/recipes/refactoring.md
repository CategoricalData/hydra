...
# Regenerate Python and Java (from packages/hydra-ext)
cd ../packages/hydra-ext
./bin/sync-python.sh --quick
./bin/sync-java.sh --quick
...
- **hydra-ext coders**: Language coders in `packages/hydra-ext` (Java, Python,
...
grep -rn 'OldTypeName' packages/hydra-haskell/ packages/hydra-java/ packages/hydra-python/ packages/hydra-ext/
```
...
Phase 2: sync-ext.sh (packages/hydra-ext Haskell generation)
...
| `stack build` (packages/hydra-ext) | Coders using old type names/fields | `packages/hydra-ext` source files referencing old types |
...
A Haskell compilation error in packages/hydra-ext will cascade into meaningless sync-java failures.
