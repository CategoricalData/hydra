...
The `hydra-ext` tree collects the long-tail coders (Avro, Protobuf, GraphQL, ...)
and is now shipped as-is (`targetLanguages: []` in its `package.json`);
it is not regenerated by the sync matrix.
...
dist/haskell/hydra-ext/src/main/haskell/Hydra/ # Ext-coder code (long-tail; frozen, shipped as-is)
...
tree is frozen (`targetLanguages: []`) and shipped as-is rather than regenerated.
...
| `sync-packages.sh` | Phase 1 only: DSL sources → JSON + Haskell kernel + `stack test` + lexicon. |
...
|---------------------|---------|
| `sync-haskell.sh` | **Phase 1 sync.** Regenerate DSL → JSON, the Haskell kernel, run `stack test`, and refresh the lexicon. |
