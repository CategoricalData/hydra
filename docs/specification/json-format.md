...
A `Literal.decimal` value encodes as a JSON number in the representation-faithful form
of the `printDecimal` primitive, and decodes with `parseDecimal`:
**the JSON printer and parser call these primitives directly** — the primitive is the
JSON number serializer, exactly as `printString`/`parseString` are the string serializer.

Decimal values are scale-distinct (an integer coefficient plus a scale, so `1.10` and
`1.1` are distinct values); the digit string is the value, exactly, with coefficient
digits — trailing zeros included — preserved, and zero printed per its scale
(`0`, `0.0`, `0.00`).
Layout follows the ECMAScript `Number::toString` (§6.1.6.1.20) / RFC 8785 (JCS)
thresholds and spellings; see the
[textual syntax specification](syntax.md#26-literals) for the full statement,
which is shared verbatim between the two encodings.
...
A `Literal.string` value encodes as a JSON string.
These escape rules are shared: the [textual syntax](syntax.md#25-strings) uses them
verbatim for string literals and backtick-quoted names, and the same kernel code
(`printString`/`parseString`) serves both encodings.
...
- `Type.map` / `Term.map` arrays appear in **canonical order**: ascending by the
  canonical total order on terms, as specified in
